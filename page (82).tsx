...
export default function Home() {
  ...
  const buildFullDocument = useCallback((data: AnalyzeResult): string => {
    ...
    // CRITIQUE: Le parseur HTML termine <script> au 1er </script> rencontré même dans
    // une string JS. React minifié contient souvent ce pattern → le reste s'affiche en texte.
    // Fix: échapper </script> en <\/script> (valide JS, invisible pour le parseur HTML)
          ...
          {showCookies && (
            <div className="cookies-box">
              <p className="cookies-hint">
                Pour cloner une page qui nécessite une connexion, copie ta valeur <code>Cookie</code> depuis&nbsp;
                <strong>DevTools → Network → (requête) → Request Headers → Cookie</strong>
              </p>
              <textarea
