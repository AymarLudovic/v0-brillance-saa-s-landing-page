...
export default function GitHubDeployModal({ currentProject, isOpen, onClose, sandboxId }: GitHubDeployModalProps) {
  ...
  const handlePushToGitHub = async () => {
    ...
    try {
      // ── Récupérer TOUS les fichiers du sandbox avant de pusher ──
      let allFiles: { filePath: string; content: string }[] = [];
      if (sandboxId) {
        try {
          addLog('📂 Récupération des fichiers sandbox...', 'info');
          ...
          if (sandboxData.success && sandboxData.files) {
            ...
            addLog(`✅ ${allFiles.length} fichiers récupérés depuis le sandbox`, 'success');
          }
        } catch {
          addLog('⚠️ Sandbox inaccessible, fallback fichiers locaux', 'warning');
        }
      }
      ...
      const response = await fetch('/api/deploy/github', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token, repoName, branch, commitMessage, files: allFiles })
      });
      ...
      // Lire jusqu'à la fermeture naturelle du stream — ne pas breaker prématurément
      while (true) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
          ...
          try {
            ...
            // Fin détectée → mettre à jour état SANS casser la boucle
            if (data.type === 'DONE') {
            ...
            // Succès détecté via message
            if (data.message && (data.message.includes('🚀') || data.message.toLowerCase().includes('successfully'))) {
              setIsDeploying(false);
              setPushFailed(false);
              if (data.url) setRepoUrl(data.url);
            }

            // Erreur fatale → arrêter le loading
            if (data.type === 'error' && data.message && !data.message.toLowerCase().includes('warn')) {
              setIsDeploying(false);
              setPushFailed(true);
            }

            // Logger tous les messages sauf DONE
