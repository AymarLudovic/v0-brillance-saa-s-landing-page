...
export default function DesignerPage() {
  ...
  const [loadingStep, setLoadingStep]   = useState<"js" | "ux" | "merger" | "web" | "">("");
    ...
    try {
      ...
      if (mode === "web") {
        ...
        while (true) {
          ...
          if (done) break;
          full += dec.decode(value);
          // Affiche le HTML brut en live pendant la génération UX
          const html = extractHtml(full);
          if (html) { setPreviewHtml(html); setActiveTab("preview"); }
          setStreamText(full.slice(-200));
        }

        // HTML brut issu de l'agent UX (sans JS)
        const rawUxHtml = extractHtml(full);
        if (rawUxHtml) { setPreviewHtml(rawUxHtml); setActiveTab("preview"); }

        // ── MERGER — Fusion intelligente JS + HTML ───────────────────────────
        // Seulement si on a les deux outputs à connecter
        if (jsOutput.trim() && rawUxHtml) {
          setLoadingStep("merger");
          setStreamText("🔗 Connexion JS ↔ HTML en cours...");

          // Extrait le code JS pur (sans le bloc ```js-features``` wrapper)
          const jsBlockMatch = jsOutput.match(/```js-features\n?([\s\S]*?)```/);
          const jsCodeRaw = jsBlockMatch ? jsBlockMatch[1] : jsOutput;

          const mergerForm = new FormData();
          mergerForm.append("message", text);
          mergerForm.append("history", JSON.stringify(histApi));
          mergerForm.append("htmlCode", rawUxHtml);
          mergerForm.append("jsCode", jsCodeRaw);

          try {
            const mergerResp = await fetch("/api/merger", { method: "POST", body: mergerForm });
            if (mergerResp.ok && mergerResp.body) {
              const mergerReader = mergerResp.body.getReader();
              const mergerDec = new TextDecoder();
              let mergerFull = "";

              while (true) {
                const { done, value } = await mergerReader.read();
                if (done) break;
                mergerFull += mergerDec.decode(value);
                const mergedHtml = extractHtml(mergerFull);
                if (mergedHtml) { setPreviewHtml(mergedHtml); }
                setStreamText("🔗 " + mergerFull.slice(-150));
              }

              const finalMerged = extractHtml(mergerFull);
              if (finalMerged) setPreviewHtml(finalMerged);
            }
          } catch { /* merger optionnel — l'UX brut reste affiché comme fallback */ }
        }

        setMessages(prev => [...prev, {
          role: "assistant",
          content: rawUxHtml
            ? `✅ Interface générée — ${mode === "clone" ? "⚡ Clone" : "✦ Création"}${jsOutput.trim() ? " · 🔗 JS intégré" : ""}`
            : full.trim(),
          mode,
        }]);
        if (rawUxHtml) setActiveTab("preview");
      }
              ...
              <div className="flex gap-2 items-start">
                ...
                <div className="bg-[#0f0f20] border border-white/[0.06] rounded-[4px_12px_12px_12px] px-3 py-2.5 max-w-[82%] min-w-[140px]">
                  <div className="flex items-center gap-2 text-xs text-slate-500 mb-1.5">
                    <span className={`w-3 h-3 rounded-full border-2 animate-spin shrink-0 ${loadingStep === "js" ? "border-amber-500/30 border-t-amber-400" : loadingStep === "merger" ? "border-cyan-500/30 border-t-cyan-400" : loadingStep === "web" ? "border-emerald-500/30 border-t-emerald-400" : "border-violet-500/30 border-t-violet-400"}`} />
                    <span>
                      {loadingStep === "js" ? "Construction JavaScript..." :
                       loadingStep === "merger" ? "Connexion JS ↔ HTML..." :
