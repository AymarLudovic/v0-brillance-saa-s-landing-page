...
export default function SiteInspector() {
  ...
  // ── Second iframe pour preview du résultat extrait ────────────────────────
  ...
  // ── Whitelist des propriétés CSS visuelles seulement ──────────────────────
  // On n'inkline QUE ces props — pas les 300+ computed props inutiles
  const VISUAL_PROPS = [
    // Texte
    "color","font-family","font-size","font-weight","font-style","font-variant",
    "line-height","letter-spacing","text-align","text-decoration","text-transform",
    "text-shadow","white-space","word-break","word-spacing","vertical-align",
    // Fond & bordures
    ...
    // Visuels
    "opacity","visibility","transform","clip-path","mask","filter",
  ...
  const VISUAL_SET = new Set(VISUAL_PROPS)
  ...
  const extractInlinedHTML = async () => {
    ...
    setExtracting(true)
    try {
      // ── 1. Mesurer le viewport de l'iframe ─────────────────────────────────
      const vpW = win.innerWidth || 1280
      const vpH = win.innerHeight || 800

      // ── 2. Identifier les éléments VISIBLES dans le viewport ───────────────
      //   (on garde aussi les éléments hors-viewport qui sont parents de visibles)
      ...
      allOrig.forEach((el) => {
        ...
        if (cs.visibility === "hidden" && cs.position !== "absolute") return
        if (parseFloat(cs.opacity) < 0.01) return

        const rect = el.getBoundingClientRect()
        // Garder si dans le viewport étendu (2x la hauteur pour capturer le fold)
        if (rect.width === 0 && rect.height === 0) return
        if (rect.bottom < -vpH || rect.top > vpH * 3) return
        if (rect.right < -vpW || rect.left > vpW * 2) return

        // Marquer l'élément et tous ses ancêtres
      ...
      const cloneAllEls = Array.from(clone.querySelectorAll("*")) as HTMLElement[]
      const origAllEls = Array.from(doc.querySelectorAll("*")) as HTMLElement[]

      // Map orig → clone par index
      cloneAllEls.forEach((cloneEl, i) => {
        ...
        // ── SVG : simplifier ou supprimer si décoratif ──────────────────────
        if (tag === "svg") {
          const rect = orig.getBoundingClientRect()
          const isDecorativeLargeSvg = (
            rect.width > 400 || rect.height > 400 ||
            (orig.innerHTML.length > 5000 && !orig.getAttribute("aria-label") && !orig.getAttribute("role"))
          )
          if (isDecorativeLargeSvg) {
            // Remplacer par un div de même taille avec la couleur de fond
            const placeholder = document.createElement("div")
            placeholder.style.cssText = `width:${rect.width}px;height:${rect.height}px;display:inline-block;`
            cloneEl.replaceWith(placeholder)
            return
          }
          // SVG petit/important : garder mais vider les <defs> lourds
          cloneEl.querySelectorAll("defs filter,defs mask,defs clipPath").forEach(d => d.remove())
          return
        }

        // ── Scripts, styles, liens ─────────────────────────────────────────
        if (["script","noscript","style","link","meta","template"].includes(tag)) {
          if (tag === "meta" && (orig.getAttribute("charset") || orig.getAttribute("name") === "viewport")) return
          if (tag !== "meta") cloneEl.remove()
          return
        }

        // ── Attributs événements JS ────────────────────────────────────────
        Array.from(cloneEl.attributes)
          .filter(a => a.name.startsWith("on") || a.name === "data-aos" || a.name === "data-scroll")
          .forEach(a => cloneEl.removeAttribute(a.name))

        // ── Inline uniquement les props visuelles qui diffèrent du défaut ──
        const cs = win.getComputedStyle(orig)
        const inlined: string[] = []

        VISUAL_PROPS.forEach(prop => {
          const val = cs.getPropertyValue(prop)
          if (!val || val === defaultMap[prop]) return
          if (val === "initial" || val === "unset" || val === "inherit") return
          // Skip les "none" inutiles sauf pour display et visibility
        ...
        // ── Nettoyer les classes et data-* (réduit taille) ────────────────
        // On garde class pour les quelques cas où du CSS externe reste
        // mais on vide data-* inutiles
        Array.from(cloneEl.attributes)
          .filter(a => a.name.startsWith("data-") && !["data-href","data-src"].includes(a.name))
          .forEach(a => cloneEl.removeAttribute(a.name))
      })

      // ── 5. Construire le document final minimal ────────────────────────────
      // Head : juste charset + viewport + éventuellement les polices Google
      const googleFontsLinks = Array.from(doc.querySelectorAll("link[href*='fonts.googleapis.com']"))
...
${googleFontsLinks}
<style>*{box-sizing:border-box;margin:0;padding:0}img{max-width:100%;display:block}</style>
</head>
<body>
${clone.querySelector("body")?.innerHTML || ""}
      ...
      // ── 6. Mettre à jour le 2ème iframe de preview ─────────────────────────
      setExtractedHTML(finalHtml)
      setExtractedSize(sizeKb)
      setShowExtracted(true)

      setCopyStatus({ id: "static", message: `✅ ${sizeKb}KB — vérifie la preview ci-dessous` })
      setTimeout(() => setCopyStatus(null), 5000)
    } catch (e) {
      setCopyStatus({ id: "static", message: `Extraction failed: ${e instanceof Error ? e.message : String(e)}` })
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="p-4 sm:p-8">
        ...
        <div className={`max-w-4xl mx-auto ${result ? "pb-24" : "pb-36"}`}>
          ...
          {/* ── 2ème iframe : preview du HTML extrait ── */}
          {showExtracted && extractedHTML && (
            <div className="mt-6">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  <h3 className="text-base font-bold text-black">Preview statique</h3>
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    extractedSize < 200 ? "bg-green-100 text-green-700" :
                    extractedSize < 500 ? "bg-yellow-100 text-yellow-700" :
                    "bg-red-100 text-red-700"
                  }`}>
                    {extractedSize}KB {extractedSize < 200 ? "✓ Claude-ready" : extractedSize < 500 ? "⚠ large" : "✗ too large"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={copyExtracted} className="flex items-center gap-1.5 h-[30px] px-3 rounded-[8px] border border-[#eee] bg-white text-xs font-medium text-black hover:bg-gray-50">
                    <Copy size={12} /> Copy
                  </button>
                  <button onClick={downloadExtracted} className="flex items-center gap-1.5 h-[30px] px-3 rounded-[8px] bg-black text-white text-xs font-medium hover:bg-gray-800">
                    <Download size={12} /> Download
                  </button>
                  <button onClick={() => setShowExtracted(false)} className="flex items-center h-[30px] w-[30px] justify-center rounded-[8px] border border-[#eee] text-gray-400 hover:text-black">
                    <X size={14} />
                  </button>
                </div>
              </div>
              <div
                className="w-full rounded-xl border-2 border-dashed border-black/10 overflow-hidden bg-white relative"
                style={{ aspectRatio: "1280 / 800" }}
                ref={(wrap) => {
                  if (!wrap) return
                  const apply = () => {
                    const s = wrap.getBoundingClientRect().width / 1280
                    const f = wrap.querySelector("iframe") as HTMLIFrameElement | null
              ...
              <p className="text-xs text-gray-400 mt-2 text-center">
                Compare avec la preview originale ci-dessus — ce que Claude/Lovable recevra
              </p>
            </div>
          )}
      ...
      {result && (
        <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-[#eee] px-4 py-3 z-30">
          <div className="max-w-4xl mx-auto flex items-center gap-2 overflow-x-auto no-scrollbar">
            ...
            {/* ── Static HTML (computed styles inlinés, sans JS) ── */}
            <button
              onClick={extractInlinedHTML}
              disabled={extracting}
              className="flex-shrink-0 flex items-center gap-1.5 h-[36px] px-3 rounded-[10px] border border-[#111] bg-[#111] text-white text-sm font-medium hover:bg-gray-800 disabled:opacity-60 disabled:cursor-not-allowed transition-colors whitespace-nowrap"
              title="Extrait le HTML avec les styles calculés inlinés directement sur chaque élément — sans JS, compatible Claude (<200KB)"
            >
              {extracting ? (
              ...
              <span className="hidden sm:inline">{extracting ? "Extracting…" : "Static HTML"}</span>
              <span className="sm:hidden">{extracting ? "…" : "Static"}</span>
            </button>
