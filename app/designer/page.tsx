...
export default function SiteInspector() {
  ...
  // Propriétés à TOUJOURS exclure (inutiles dans un snapshot statique)
  const SKIP_PROPS = new Set([
    ...
    "will-change","pointer-events","cursor","user-select",
    "-webkit-tap-highlight-color","touch-action",
  ])
  ...
  const extractInlinedHTML = async () => {
    ...
    setExtracting(true)
    try {
      // 1. Référence des styles par défaut du navigateur (via un div vierge dans l'iframe)
      const defaultEl = doc.createElement("div")
      ...
      for (let i = 0; i < defaultStyles.length; i++) {
        const p = defaultStyles[i]
        defaultMap[p] = defaultStyles.getPropertyValue(p)
      }
      doc.body.removeChild(defaultEl)

      // 2. Cloner le document complet
      const clone = doc.cloneNode(true) as Document

      // 3. Sur le DOCUMENT ORIGINAL (rendu), récupérer les computed styles
      //    et les appliquer sur le clone
      const origEls = doc.querySelectorAll("*")
      const cloneEls = (clone as Document).querySelectorAll("*")

      origEls.forEach((orig, i) => {
        const cloneEl = cloneEls[i] as HTMLElement
        if (!cloneEl || typeof (cloneEl as HTMLElement).style === "undefined") return
        const cs = iframe.contentWindow!.getComputedStyle(orig as Element)
        const inlined: string[] = []
        for (let j = 0; j < cs.length; j++) {
          const prop = cs[j]
          if (SKIP_PROPS.has(prop)) continue
          const val = cs.getPropertyValue(prop)
          // N'inclure que si différent du défaut
          if (val && val !== defaultMap[prop] && val !== "initial" && val !== "unset") {
            inlined.push(`${prop}:${val}`)
          }
        }
        if (inlined.length > 0) {
          // Fusionner avec le style inline existant (priorité au computed)
          cloneEl.setAttribute("style", inlined.join(";"))
        }
      })

      // 4. Nettoyer : supprimer JS, CSS externe, styles internes
      const cloneDoc = clone as Document
      cloneDoc.querySelectorAll("script").forEach(el => el.remove())
      cloneDoc.querySelectorAll("link[rel='stylesheet']").forEach(el => el.remove())
      cloneDoc.querySelectorAll("style").forEach(el => el.remove())
      // Supprimer les attributs d'événement JS
      ...
      const html = (cloneDoc.documentElement as HTMLElement).outerHTML
      const final = doctype + html

      // 6. Vérifier la taille
      const sizeKb = Math.round(new Blob([final]).size / 1024)
      createDownloadLink(final, "static-ui.html", "text/html")

      setCopyStatus({ id: "static", message: `Downloaded ✅ (~${sizeKb}KB)` })
