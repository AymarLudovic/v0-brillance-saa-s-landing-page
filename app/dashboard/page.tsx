...
function SiteFavicon({ url, name, size = 18 }: { url: string; name: string; size?: number }) {
  ...
  if (!domain || err) return (
    <div className="w-[18px] h-[18px] rounded bg-[#181818] flex items-center justify-center text-white text-[9px] font-bold shrink-0">
      {name[0]?.toUpperCase()}
    </div>
  );
  return (
...
function SiteCard({ site, onSelect, onShowScript }: {
  ...
  return (
    <div className="bg-transparent flex flex-col gap-4 border-b border-[#e0e0e0] py-4 last:border-0">
              ...
              <ResponsiveContainer width="99%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false}
                    tick={{ fontSize: 11, fill: '#989898', fontWeight: 500 }} dy={10}
...
export default function DashboardPage() {
  ...
  // ── Authenticated ──────────────────────────────────────────────────────────
  return (
    <div className={`${font.className} min-h-screen bg-[#fafafa] flex flex-col`}>
      ...
      {selectedSite ? (
        ...
        <div className="max-w-5xl mx-auto w-full px-4 py-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-xl font-bold text-[#171717]">My Sites</h1>
              <p className="text-sm text-[#989898] mt-0.5">
                {sites.length === 0 ? 'No sites yet.' : `${sites.length} site${sites.length > 1 ? 's' : ''} tracked`}
              </p>
            </div>
            <button onClick={() => setShowAdd(true)}
              className="flex items-center gap-2 px-3 h-[34px] bg-[#171717] text-white text-sm font-bold hover:bg-[#333] transition-colors rounded-lg shadow-sm">
          ...
          {loadingSites ? (
            ...
            <div onClick={() => setShowAdd(true)}
              className="border-2 border-dashed border-gray-200 rounded-2xl p-16 text-center cursor-pointer hover:border-gray-400 hover:bg-white transition-all">
              <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center mx-auto mb-3">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
              </div>
              <p className="text-[#686868] text-sm font-medium">Add your first site</p>
              <p className="text-[#989898] text-xs mt-1">Click to start tracking visitors</p>
            </div>
          ) : (
            <div className="bg-white border border-[#e0e0e0] rounded-xl px-4 divide-y divide-[#f0f0f0]">
              {sites.map(site => (
                <SiteCard
                  key={site.id}
                  site={site}
