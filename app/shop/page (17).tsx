...
/* ── Manager Agent — decides which pipeline to use ── */
const MANAGER_PROMPT = `You are a routing agent for a UI generation system. Given a feature request and the current project context, you decide which pipeline to use.

OUTPUT: Raw JSON only — { "route": "html" | "nextjs", "reasoning": "one sentence" }

ROUTE "html" — use when the feature is achievable purely client-side:
- New UI sections, components, modals, drawers, panels
- Animations, transitions, visual effects
- Client-side forms, validation, filters, search
- Charts, data visualizations with fake/local data
- localStorage state, tabs, toggles, carousels
- Any feature where JavaScript DOM manipulation is sufficient

ROUTE "nextjs" — use ONLY when the feature genuinely requires server infrastructure:
- API routes (fetching from external APIs, databases)
- Server actions / server components
- Authentication, session management, OAuth
- File system access, file uploads to server
- Server-sent events, WebSockets via server
- Environment variables that must stay secret
- Background jobs, cron tasks
- Email sending, push notifications via server
- Database queries (Prisma, Drizzle, Supabase, etc.)

When in doubt, prefer "html" — most UI features are achievable client-side.`;

interface ManagerDecision { route: 'html' | 'nextjs'; reasoning: string; }
...
async function callGeminiManager(
  ...
  onProgress('Manager analyzing request…');
  const userMsg = `Feature request: "${userRequest}"\n\nDecide which pipeline to use. Output raw JSON only: { "route": "html"|"nextjs", "reasoning": "..." }`;
  return withGeminiRetry(async () => {
    const ai = getAI(apiKey);
    const response = await ai.models.generateContent({
      model: GEMINI_MODEL,
      ...
      config: {
        ...
        thinkingConfig: { thinkingLevel: 'medium' as any },
        temperature: 0.1, maxOutputTokens: 256, topP: 0.9,
      },
    });
    const raw = extractText(response).replace(/```(?:json)?\s*/gi, '').replace(/```/g, '').trim();
    const m = raw.match(/\{[\s\S]*\}/);
    if (!m) return { route: 'nextjs', reasoning: 'Could not parse manager response' };
    return JSON.parse(m[0]) as ManagerDecision;
  }, onProgress, 2, 10000);
}
...
export default function PixelPerfectAI() {
  ...
  const smartPatch = async () => {
    if (!apiKey) { setShowModal(true); return; }
    if (!nextjsFiles.length) { setPatchError('Generate the Next.js project first.'); return; }
    if (!patchPrompt.trim()) { setPatchError('Describe what you want to add or change.'); return; }
    setGeneratingPatch(true); setPatchError(''); setPatchFiles([]);
    setManagerDecision(null); setPatchRoute(null);

    try {
      // ── Step 1: Manager decides route ──────────────────────────────────────
      const decision = await callGeminiManager(apiKey, patchPrompt, setPatchProgress);
      setManagerDecision(decision);
      setPatchRoute(decision.route);

      if (decision.route === 'html') {
        // ── HTML Pipeline: snippet → JS interactions → JS features → Next.js patch ──
        setPatchProgress('Route: HTML agents pipeline…');

        // Need the original HTML code to work from
        const baseHtml = code; // original generated HTML
        if (!baseHtml || !imgB64) {
          setPatchError('Original HTML and image required for HTML pipeline.');
          return;
        }

        // Step 2a: Generate HTML snippet matching the design
        const snippet = await callGeminiFeatureSnippet(
          apiKey, imgB64, imgMime, baseHtml, patchPrompt, setPatchProgress
        );

        // Step 2b: Inject snippet into the full HTML so JS agents see the full context
        const enrichedHtml = baseHtml.includes('</body>')
          ? baseHtml.replace('</body>', `\n<!-- NEW FEATURE: ${patchPrompt} -->\n${snippet}\n</body>`)
          : baseHtml + `\n${snippet}`;

        // Step 2c: JS Interactions agent — wire the new elements
        setPatchProgress('JS agent: wiring interactions…');
        const newJs1 = await callGeminiJS(apiKey, imgB64, imgMime, enrichedHtml, setPatchProgress);

        // Step 2d: JS Features agent — add real functionality
        setPatchProgress('Features agent: building logic…');
        const newJs2Scripts = await callGeminiFeatures(
          apiKey, imgB64, imgMime, enrichedHtml, newJs1, setPatchProgress
        );

        // Step 2e: Convert the enriched HTML + new scripts into a Next.js patch (page.tsx + globals.css only)
        setPatchProgress('Converting to Next.js patch…');
        const enrichedWithJs = injectScripts(
          injectScripts(enrichedHtml, newJs1),
          newJs2Scripts
        );
        const fullPatched = await callGeminiNextJS(
          apiKey, imgB64, imgMime, enrichedHtml, newJs1, enrichedWithJs, setPatchProgress
        );

        // Only keep page.tsx and globals.css from this re-generation (avoid overwriting unrelated files)
        const relevantPaths = ['app/page.tsx', 'app/globals.css'];
        const patchedRelevant = fullPatched.filter(f => relevantPaths.includes(f.path));

        setPatchFiles(patchedRelevant);
        const mergedMap = new Map(nextjsFiles.map(f => [f.path, f]));
        patchedRelevant.forEach(f => mergedMap.set(f.path, f));
        const merged = Array.from(mergedMap.values());
        setNextjsFiles(merged);
        if (activeProjectId) await dbPatchProjectNextjs(activeProjectId, merged);

      } else {
        // ── Next.js Pipeline: direct patch ──────────────────────────────────
        setPatchProgress('Route: Next.js patch agent…');
        const patched = await callGeminiNextJSPatch(apiKey, nextjsFiles, patchPrompt, setPatchProgress);
      ...
      setPatchError(err instanceof Error ? err.message : 'Unknown patch error.');
    } finally {
      setGeneratingPatch(false); setPatchProgress('');
    }
  };
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="w-[320px] shrink-0 border-r border-white/[0.06] bg-[#09091b] flex flex-col overflow-hidden">
          <div className="flex-1 overflow-y-auto p-2.5 flex flex-col gap-2">
            ...
            {/* ── Next.js Patch ── */}
            {nextjsFiles.length > 0 && (
              <div className="bg-[#0e0e1f] border border-sky-900/20 rounded-lg p-2.5 flex flex-col gap-2">
                <p className="text-[8.5px] font-semibold tracking-[0.1em] uppercase text-sky-400 flex items-center gap-1">
                  <IcZap/> Smart Patch
                </p>
                <p className="text-[8px] text-slate-600 leading-relaxed">
                  Describe a feature to add. A manager agent decides the best pipeline — HTML agents (design-perfect) or Next.js patch (server features).
                </p>
                <textarea
                  value={patchPrompt}
                  onChange={e => setPatchPrompt(e.target.value)}
                  rows={3}
                  placeholder="e.g. Add a dark mode toggle · Export table to CSV · Add a /settings page with auth · Add real-time chart from API…"
                  className="w-full bg-[#09091b] border border-white/[0.06] rounded text-slate-300 font-mono text-[9px] p-2 outline-none leading-relaxed resize-y focus:border-sky-500/40 transition-colors placeholder:text-slate-700"
                />

                {/* Manager decision badge */}
                {managerDecision && (
                  <div className={`flex items-start gap-1.5 rounded px-2 py-1.5 border text-[8px] leading-relaxed
                    ${patchRoute==='html'
                      ? 'bg-cyan-950/20 border-cyan-900/20 text-cyan-400'
                      : 'bg-emerald-950/20 border-emerald-900/20 text-emerald-400'}`}>
                    <span className="shrink-0 mt-px">{patchRoute==='html' ? '🎨' : '⚙️'}</span>
                    <div>
                      <span className="font-bold uppercase tracking-widest text-[7px]">
                        {patchRoute==='html' ? 'HTML Agents Pipeline' : 'Next.js Pipeline'}
                      </span>
                      <p className="text-slate-500 mt-0.5">{managerDecision.reasoning}</p>
                    </div>
                  </div>
                )}

                {patchError && (
                  <div className="bg-red-950/30 border border-red-900/30 rounded px-2 py-1.5 text-[8.5px] text-red-400">{patchError}</div>
                )}

                <button onClick={smartPatch} disabled={generatingPatch || !patchPrompt.trim()}
                  className={`w-full py-2 rounded-md font-bold text-[10.5px] tracking-wide flex items-center justify-center gap-1.5 transition-all duration-150
                    ${generatingPatch || !patchPrompt.trim()
                      ? 'bg-sky-950/20 border border-sky-900/20 text-sky-800 cursor-not-allowed'
                      : 'bg-gradient-to-r from-sky-500 to-blue-400 text-black shadow-[0_0_14px_rgba(14,165,233,0.15)] hover:shadow-[0_0_24px_rgba(14,165,233,0.3)] hover:-translate-y-px cursor-pointer'}`}>
                  {generatingPatch
                    ? <><Spinner/><span className="text-sky-400 text-[9px]">{patchProgress || 'Routing…'}</span></>
                    : <><IcZap/> Smart Patch</>}
                </button>
                ...
                {patchFiles.length > 0 && (
                  <div className="flex flex-col gap-1.5">
                    <div className="flex flex-col gap-0.5">
                      <p className="text-[7.5px] text-slate-600 uppercase tracking-widest">
                        {patchRoute==='html' ? '🎨 HTML pipeline —' : '⚙️ Next.js patch —'} {patchFiles.length} file(s):
                      </p>
                      {patchFiles.map(f => (
                        <div key={f.path} className="flex items-center gap-1.5 bg-sky-950/20 border border-sky-900/20 rounded px-2 py-1">
                          <span className="text-[7px] text-sky-400 font-mono truncate flex-1">{f.path}</span>
                          <span className="text-[6.5px] text-slate-700 shrink-0">{(f.content.length/1024).toFixed(1)}KB</span>
                        </div>
                      ))}
                    </div>
                    <button
                      onClick={applyPatchToSandbox}
                      disabled={patchAddingFiles || !sandbox.id}
                      className={`w-full py-1.5 rounded-md text-[9px] font-bold flex items-center justify-center gap-1.5 transition-all duration-150
                        ${patchAddingFiles || !sandbox.id
                          ? 'bg-white/[0.03] border border-white/[0.05] text-slate-700 cursor-not-allowed'
                          : 'bg-emerald-900/30 border border-emerald-800/40 text-emerald-400 hover:bg-emerald-900/50 cursor-pointer'}`}>
                      {patchAddingFiles
                        ? <><Spinner/><span className="text-emerald-500 text-[8px]">Writing files…</span></>
                        : <><IcPlay/> Add Files to Sandbox & Rebuild</>}
                    </button>
                    {!sandbox.id && (
                      <p className="text-[7.5px] text-slate-700 text-center">Create the sandbox first to write files</p>
                    )}
                  </div>
                )}
              </div>
                  ...
                  return (
                    <button key={f.path} onClick={()=>setSelectedNxFile(i)}
                      className={`flex items-center gap-1 px-2 py-1 rounded text-[8px] font-mono whitespace-nowrap transition-all cursor-pointer shrink-0
                        ${selectedNxFile===i
                          ? isPatched ? 'bg-sky-950/40 border border-sky-800/40 text-sky-400'
