...
const NEXTJS_SYSTEM_PROMPT = `You are a senior Next.js 15 TypeScript architect. Convert a complete HTML/CSS/JavaScript page into a Next.js 15 App Router project that is PIXEL-PERFECT and FUNCTIONALLY IDENTICAL to the original.

OUTPUT FORMAT — CRITICAL:
Output a single valid raw JSON object. No markdown, no prose:
{ "files": [ { "path": "app/page.tsx", "content": "..." }, ... ] }

Required files ALWAYS: app/page.tsx, app/globals.css, app/layout.tsx, next.config.mjs

HTML → JSX RULES:
- class → className (EVERY SINGLE ONE)
- for → htmlFor, tabindex → tabIndex
- All void elements self-close: <img />, <input />, <br />, <hr />, <link />
- Inline style="color:red;font-size:14px" → style={{color:'red',fontSize:'14px'}} (camelCase)
- Remove onclick/onchange HTML attributes — events handled in JS useEffect instead
- HTML comments → {/* comment */}
- <script> tags → removed from JSX (JS goes in useEffect)
- <style> tags → removed from JSX (CSS goes in globals.css)
- <link rel="stylesheet"> for CDNs → keep in layout.tsx <head>

CSS STRATEGY:
- Copy ALL CSS from <style> tags verbatim into globals.css
- Add at top: html,body{margin:0;padding:0;width:100%;min-height:100vh;}
- Keep ALL :root variables, @keyframes, @media queries exactly
- Do NOT use CSS Modules or Tailwind for original styles

JAVASCRIPT → TYPESCRIPT — "useEffect Island" pattern:
1. Extract JS from inside <script> tags (remove the tags)
2. Remove: document.addEventListener('DOMContentLoaded', ...) → unwrap the callback body
3. Remove: (function(){...})() IIFE wrappers → unwrap
4. Remove: window.onload = ... → unwrap
5. Combine Script#1 + Script#2 into ONE useEffect(() => { ... }, [])
6. CDN libs (SortableJS, Chart.js, GSAP, etc.) → load BEFORE main JS via dynamic script injection:
   const loadLib = () => new Promise<void>(r => { const s=document.createElement('script'); s.src='CDN_URL'; s.onload=()=>r(); document.head.appendChild(s); });
   Then await all loadLib() calls before running main JS.

CRITICAL RULES:
- document.write() → REMOVE entirely
- reactStrictMode: false in next.config.mjs (REQUIRED — prevents double useEffect in dev)
- Use 'as any' or 'as HTMLElement' type casts to fix TS errors quickly
- Do NOT use dangerouslySetInnerHTML for the page body
- localStorage, fetch, setTimeout, setInterval, IntersectionObserver → keep as-is inside useEffect
- All classList.add/remove/toggle, createElement, appendChild → keep as-is inside useEffect

next.config.mjs (ALWAYS use this exact content):
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (<html lang="en"><head>{/* CDN stylesheets here */}</head><body>{children}</body></html>);
}
...
export default function Page() {
  useEffect(() => {
    // ALL JS HERE (Script#1 + Script#2 combined, DOMContentLoaded unwrapped)
  }, []);
  return (<> {/* JSX from HTML body */} </>);
}

QUALITY CHECKLIST before outputting:
□ Every class= converted to className=
□ All self-closing tags have />
□ CSS 100% preserved in globals.css
□ DOMContentLoaded wrappers removed from JS
□ next.config.mjs has reactStrictMode: false
□ Output is valid parseable JSON (escape backslashes, quotes inside strings properly)
□ No TS errors that block compilation (use 'as any' if needed)`;
...
async function callGeminiNextJS(
  ...
  const extractJS = (src: string) =>
    [...src.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)].map(m=>m[1].trim()).filter(Boolean).join('\n\n');
  const js1 = extractJS(jsCode);
  // Features scripts are the ones injected on top of fullCode; extract from featuresCode minus what's in htmlCode
  const allFeatScripts = featuresCode ? [...featuresCode.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)].map(m=>m[1].trim()).filter(Boolean) : [];
  const htmlScripts = [...htmlCode.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)].map(m=>m[1].trim());
  const js2 = allFeatScripts.filter(s=>!htmlScripts.includes(s) && s !== js1).join('\n\n');

  const userMsg = `Convert to Next.js 15 TypeScript project.
...
Output raw JSON only. No markdown fences.`;
  ...
  return withGeminiRetry(async () => {
    ...
    const response = await ai.models.generateContent({
      model: GEMINI_MODEL,
      contents:[{role:'user',parts:[{inlineData:{mimeType,data:imageB64}},{text:userMsg}]}],
      config:{
...
async function fixNextJSBuildErrors(
  ...
  const userMsg = `Fix the build errors in this Next.js project WITHOUT changing any design or functionality.
...
Rules: fix ONLY the errors, use 'as any' to silence TS issues, ensure reactStrictMode:false, keep all CSS intact.
Output complete corrected JSON files array. Raw JSON only.`;
