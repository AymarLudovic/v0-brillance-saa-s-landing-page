...
interface Project { id: string; name: string; htmlCode: string; jsCode: string; fullCode: string; imgDataUrl: string; createdAt: number; }
...
async function callGeminiJS(
  ...
  // Fallback: wrap raw in script tag if it looks like JS
...
export default function PixelPerfectAI() {
  ...
  const [viewMode, setViewMode]           = useState<'html'|'full'>('html');
  ...
  const loadProject = (p: Project) => {
    ...
    setViewMode(p.fullCode ? 'full' : 'html');
    setTab('preview');
  ...
  const generate = async () => {
    ...
    setGenerating(true); setError(''); setCode(''); setJsCode(''); setFullCode(''); setActiveProjectId(null); setViewMode('html');
    try {
  ...
  const generateJs = async () => {
    ...
    setGeneratingJs(true); setJsError('');
    try {
                ...
                {jsCode && (
                  <div className="flex gap-2 text-[8px] text-slate-700 pl-0.5 items-center">
                    ...
                    <span className="ml-auto flex items-center gap-1">
                      <span className={`cursor-pointer px-1.5 py-0.5 rounded text-[7.5px] border transition-all ${viewMode==='html'?'border-cyan-700/50 text-cyan-400 bg-cyan-950/30':'border-white/[0.06] text-slate-600 hover:text-slate-400'}`}
                        onClick={()=>setViewMode('html')}>HTML</span>
                      <span className={`cursor-pointer px-1.5 py-0.5 rounded text-[7.5px] border transition-all ${viewMode==='full'?'border-yellow-700/50 text-yellow-400 bg-yellow-950/30':'border-white/[0.06] text-slate-600 hover:text-slate-400'}`}
                        onClick={()=>setViewMode('full')}>HTML+JS</span>
                    </span>
                  </div>
                )}
              </div>
          ...
          <div className="flex items-center h-9 px-3 gap-0.5 border-b border-white/[0.06] bg-[#09091b] shrink-0">
            ...
            {code && (
              <div className="ml-auto flex items-center gap-1">
                ...
                {/* OPEN IN NEW TAB */}
                <button onClick={()=>openInTab(viewMode==='full'&&fullCode?fullCode:code)}
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {fullCode && (
                        <>
                          ...
                          <button onClick={()=>setViewMode('full')} className={`text-[8px] px-1.5 py-0.5 rounded border cursor-pointer transition-all ${viewMode==='full'?'border-yellow-700/50 text-yellow-400 bg-yellow-950/30':'border-white/[0.06] text-slate-600'}`}>HTML+JS</button>
                        </>
                      )}
                    </div>
                    <pre className="text-[10px] leading-[1.8] text-[#c8d3f5] whitespace-pre-wrap break-words font-mono">{viewMode==='full'&&fullCode?fullCode:code}</pre>
                  </>
