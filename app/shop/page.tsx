...
You receive BOTH the screenshot image AND the HTML/CSS. The image is the ultimate design authority.
Use the image to verify what you implement visually matches the real app — the HTML alone may miss subtle
spacing, depths, or color nuances that are visible in the screenshot.
...
▸ Cross-check your injected elements against the screenshot — if they would visually break
  the design level shown in the image, redesign them to fit seamlessly

DO NOT inject any new visual elements that break the layout. Use position:fixed or position:absolute for overlays.

══════════════════════════════════════════════════════════════
SECTION 2B — VISUAL DESIGN FAILURE MODES (DO NOT COMMIT THESE)
══════════════════════════════════════════════════════════════

When injecting new DOM elements, avoid these systematic errors that destroy visual fidelity:

1. BADGE SYNDROME: Do not wrap plain text labels in filled pill/chip divs unless the screenshot
   clearly shows a filled shape around that text. A dot + label = dot + plain text, not a badge.

2. ICON SIZE INFLATION: Icons in dense UIs are 14–16px, matching text line-height.
   Do NOT default to 20–24px icons unless the screenshot clearly shows large icons.

3. BORDER-RADIUS CREEP: Professional UIs often use 0–4px radius on inputs and panels.
   Only round elements that visually appear round in the screenshot. Do not auto-round.

4. PADDING INFLATION: If text is tight against its container → padding is 4–8px.
   Do NOT inflate to 12–16px unless clearly visible in the screenshot.

5. COLOR GUESSING: Read colors from existing CSS variables or getComputedStyle on real elements.
   Do NOT invent colors. Do NOT use generic #f5f5f5, #007bff, or #ffffff if those clash.

6. INVENTED SHADOWS: Only add box-shadow if the screenshot shows a visible blurred elevation.
   Do not auto-add drop shadows to every new element.

7. GENERIC LAYOUT: Do NOT center new panels with margin:auto in a narrow max-width box
   if the original app is full-width. Match the layout density of the host page.

8. FOREIGN AESTHETICS: If you inject a modal, dropdown, or toast — it must look like it
   belongs in THIS app, not a generic UI kit. Match every visual property to the host design.
...
══════════════════════════════════════════════════════════════
SECTION 4 — NO SHORTCUTS — GO ALL THE WAY (CRITICAL)
══════════════════════════════════════════════════════════════

This is the most violated rule. Read it carefully before writing any code.

❌ ABSOLUTELY FORBIDDEN — these are signs of incomplete work:

  ❌ Showing a toast / alert / console.log as a substitute for real behavior
     → "Menu item clicked" toast is NOT an implementation. It is a stub. Build the real action.
  ❌ Adding a click listener that does nothing except log or notify
  ❌ Leaving nav links, sidebar items, menu entries, buttons, icons without functional behavior
  ❌ Implementing only the "main" feature and skipping secondary/tertiary elements
  ❌ Writing "// TODO: implement" or "// placeholder" comments in the output
  ❌ Simplifying a multi-step interaction to a single alert()
  ❌ Stopping code generation before ALL identified elements are covered

✅ REQUIRED behavior for EVERY interactive element:
  - If it is a nav/sidebar/menu link → it MUST navigate, show/hide a section, or trigger a real view change
  - If it is a button → it MUST execute its implied action (open a modal, submit data, trigger a filter, etc.)
  - If it is a dropdown option → it MUST apply the filter/sort/value it represents to the actual UI
  - If it is a form → it MUST validate, process the data, and update the UI visibly
  - If it is a toggle/switch → it MUST immediately apply its effect (theme, visibility, state, etc.)
  - If it is a tab → it MUST show the correct panel and hide all others
  - If it is a card/row click → it MUST open a detail view, highlight, or expand — not just log

✅ HOW TO HANDLE elements where the full behavior requires backend data:
  - Use localStorage to simulate persistence
  - Use realistic hardcoded seed data that matches the app type
  - Build the full UI flow (open → fill → submit → update list) with that data
  - NEVER skip the UI flow just because there is no real backend

✅ COMPLETENESS CHECK — before outputting your final code, verify:
  □ Every button in the HTML has a real event listener
  □ Every nav/menu item navigates or triggers a real state change
  □ Every form has validation + submission behavior
  □ Every interactive element identified in Step 1 has corresponding code
  □ Zero elements are left dead or stub-only
...
- Tabs MUST show the correct panel and hide all others.
- Before finishing, verify: is there any element from Step 2 that has no corresponding code? If yes, add it.
...
The screenshot image is your ultimate design authority — not just the HTML/CSS.
Use the image to visually verify that what you inject belongs in this app.
If an element you would inject looks foreign against the screenshot, redesign it.

▸ STEP 1 — EXTRACT THE DESIGN SYSTEM FROM THE HTML/CSS AND IMAGE:
  Before writing any DOM injection code, read the provided HTML and cross-reference the screenshot:
  - CSS custom properties from :root (e.g. --bg-primary, --accent, --text-muted, --radius, etc.)
  - The exact background colors used on cards/panels (read from style or class, verify against image)
  ...
  - The exact padding scale (are things tight/dense or spacious? cross-check with image)
...
▸ STEP 4 — VISUAL FAILURE MODES (avoid these for every element you inject):
  - BADGE SYNDROME: don't wrap plain text in filled pills unless the screenshot shows it
  - ICON SIZE INFLATION: match icon size to existing text line-height (usually 14–16px)
  - BORDER-RADIUS CREEP: don't auto-round — only round what looks round in the screenshot
  - PADDING INFLATION: if the app is dense, keep padding 4–8px, not 12–16px
  - INVENTED SHADOWS: only add box-shadow if elevation is visible in the screenshot
  - GENERIC AESTHETICS: your injected toasts/modals/drawers must match THIS app's visual language
...
══════════════════════════════════════════════════════════════
SECTION 3 — FEATURE SCOPE (OPEN-ENDED — BUILD EVERYTHING, NO SHORTCUTS)
══════════════════════════════════════════════════════════════

Look at the screenshot and the HTML. Identify EVERY element, button, input, zone, or implied workflow that suggests a real feature, then BUILD IT COMPLETELY — regardless of what type of application it is.

❌ FORBIDDEN SHORTCUTS — these are signs of incomplete work:
  - A toast/alert/log as a substitute for real behavior ("Item deleted" toast without removing the DOM element)
  - A feature stub that opens an empty modal without content or form fields
  - A button that only changes its own visual state without affecting data or other UI
  - Implementing 2–3 features and ignoring the rest visible in the screenshot
  - Any "// TODO" or placeholder in the output code

✅ FOR EVERY IDENTIFIED FEATURE, BUILD COMPLETELY:
...
✅ COMPLETENESS REQUIREMENT:
  Before finishing, go through every action button, form, list, and interactive zone in the HTML.
  If any element does not yet have a real feature behind it → add it before outputting.
  "Real" means: it changes data, updates the DOM meaningfully, and persists the result.
  A toast alone is NEVER sufficient as a feature implementation.
