...
async function callGeminiFeatures(
  ...
  const userMsg = `Here is the full HTML/CSS of the page (read the CSS variables, colors, fonts, border-radius, spacing — you MUST reuse them for any new DOM you inject):

\`\`\`html
${htmlCode}
\`\`\`

Here is Script #1 — the interaction JS already written (do NOT re-bind its events, do NOT overwrite what it does):

\`\`\`javascript
${jsScript1}
\`\`\`

REMINDER before you write a single line:
- NEVER use innerHTML/outerHTML to modify existing elements — it destroys their CSS classes.
- NEVER touch existing <style> or <link> tags.
- Inject new elements (modals, toasts, panels) ONLY via createElement + appendChild into document.body.
- Match the design system EXACTLY: extract CSS variables from :root, read colors/fonts from existing elements with getComputedStyle if no variables exist.

Now analyze the screenshot and the HTML to identify ALL implied features and build them. Output ONLY <script>...</script> tags.`;
