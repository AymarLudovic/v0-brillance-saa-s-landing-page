...
const SKILLS: Skill[] = [
  // IA skills
  { id: "resume",      icon: "✨", label: "Résumé",      description: "Résume un texte en quelques lignes",           placeholder: "Colle ton texte ici…",                    type: "ai" },
  { id: "reformuler",  icon: "✍️", label: "Reformuler",  description: "Reformule un devoir ou une rédaction",         placeholder: "Colle le texte à reformuler…",             type: "ai" },
  // Mécanique skills
  { id: "compter",     icon: "🔢", label: "Compter",     description: "Compte mots, caractères et paragraphes",       placeholder: "Colle ton texte ici…",                    type: "mechanical" },
  { id: "password",    icon: "🔐", label: "Password",    description: "Génère un mot de passe fort aléatoire",        placeholder: "Appuie sur Envoyer pour générer…",         type: "mechanical" },
]
...
\`\`\``
  ...
  const runAiSkill = async (skill: Skill, input: string) => {
    ...
    try {
      ...
      const prompts: Record<string, string> = {
        resume: `Fais un résumé clair et concis du texte suivant en 3-5 phrases maximum :\n\n${input}`,
        reformuler: `Reformule le texte suivant de manière plus claire, professionnelle et bien structurée, en conservant le sens original :\n\n${input}`,
      }
      ...
      setSkillError(`❌ Erreur Gemini : ${msg}`)
    } finally {
  ...
  const copySkillResult = () => {
    if (skillResult) navigator.clipboard.writeText(skillResult)
  }
      ...
      <div className="p-4 sm:p-8">
        ...
        <div className={`max-w-4xl mx-auto ${result ? "pb-24" : "pb-36"}`}>
          ...
          <div className="w-full max-w-[480px] mx-auto mb-4 px-2 relative">
            ...
            {/* Skill dropdown */}
            {showSkillDropdown && (
              <div className="absolute bottom-full left-2 right-2 mb-2 bg-white rounded-[14px] border border-[#eee] shadow-xl overflow-hidden z-20">
                ...
                {SKILLS.filter(s =>
                  ...
                  >
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full flex-shrink-0 font-medium ${
                      skill.type === "ai" ? "bg-purple-100 text-purple-600" : "bg-blue-100 text-blue-600"
                    }`}>
                      {skill.type === "ai" ? "✨ IA" : "⚙️ Auto"}
                    </span>
                  </button>
                ))}
                {SKILLS.filter(s =>
            ...
            <div className={`w-full ring-2 ring-[#eee] rounded-[14px] flex items-start p-1 ${activeSkill ? "min-h-[48px]" : "h-[48px]"}`}>
              <div className={`w-full bg-white ring-2 ring-[#FAFAFA] rounded-[12px] flex items-start px-2 gap-1.5 ${activeSkill ? "py-2 flex-wrap" : "h-full items-center"}`}>
                ...
                {/* Skill pill */}
                {activeSkill && (
                  <span className={`flex items-center gap-1.5 text-xs px-2.5 py-1 rounded-full flex-shrink-0 font-medium mt-0.5 ${
                    activeSkill.type === "ai" ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"
                  }`}>
                    {activeSkill.icon} {activeSkill.label}
                    <button
                      onClick={clearSkill}
                      className="ml-0.5 hover:opacity-60 transition-opacity text-base leading-none"
                    >×</button>
                  </span>
                )}
          ...
          {/* ── Skill Result ── */}
          {(skillResult || skillError || skillLoading) && activeSkill && (
                ...
                <div className="flex items-center justify-between px-4 py-3 border-b border-[#f5f5f5] bg-[#FAFAFA]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                      activeSkill.type === "ai" ? "bg-purple-100 text-purple-600" : "bg-blue-100 text-blue-600"
                    }`}>
                      {activeSkill.type === "ai" ? "✨ IA" : "⚙️ Auto"}
                    </span>
                  </div>
                  {skillResult && (
                    <button
                      onClick={copySkillResult}
                      className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-black transition-colors px-2 py-1 rounded-lg hover:bg-gray-100"
                    >
                      <Copy size={12} /> Copier
                    </button>
                  )}
