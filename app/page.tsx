...
\`\`\``
  ...
  const runSkill = async () => {
    ...
    if (activeSkill.type === "connector") {
      setSkillResult(`🔗 **${activeSkill.label}** — ${activeSubSkill?.label ?? "Connecteur"}\n\nCette intégration OAuth sera disponible prochainement. Pour l'activer, connecte ton compte ${activeSkill.label} dans les paramètres.`)
      return
    }
        ...
        <div className={`max-w-4xl mx-auto ${result ? "pb-24" : "pb-36"}`}>
          ...
          <div className="w-full max-w-[480px] mx-auto mb-4 px-2 relative">
            ...
            {showSubSkillDropdown && activeSkill?.subSkills && (
              <div className="absolute bottom-full left-2 right-2 mb-2 bg-white rounded-[14px] border border-orange-100 shadow-xl overflow-hidden z-20">
