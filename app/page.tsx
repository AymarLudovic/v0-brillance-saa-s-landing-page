...
export default function DashboardIndex() {
  ...
  useEffect(() => {
    if (user) {
      setShowAuth(false);
      if (sessionStorage.getItem('betaplus_just_signed_in')) {
        sessionStorage.removeItem('betaplus_just_signed_in');
        setShowAdd(true);
      }
    }
  }, [user]);
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4 pb-8 sm:pb-0">
          ...
          <div className="relative z-10 bg-white rounded-2xl border border-gray-200 shadow-2xl p-7 w-full max-w-xs text-center">
            ...
            <p className="text-gray-500 text-sm leading-relaxed mb-7">
              Sign in with Google to add your apps and start tracking visitors in real time.<br /><br />
              <span className="text-gray-400 text-xs">This helps us know who's using the platform so we can adapt it to your needs.</span>
            </p>
            <button
              onClick={() => {
                sessionStorage.setItem('betaplus_just_signed_in', '1');
