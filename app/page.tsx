...
export default function SiteInspector() {
  ...
  useEffect(() => {
    fetch("/api/connector").then(r => r.json()).then(setConnectorStatus).catch(() => {})
    // Handle OAuth callback redirect (?connected=google)
    const sp = new URLSearchParams(window.location.search)
    const connected = sp.get("connected")
    if (connected) {
      setConnectorStatus(prev => ({ ...prev, [connected]: true }))
      window.history.replaceState({}, "", window.location.pathname)
    }
    const error = sp.get("error")
    if (error) {
      setSkillError(`❌ Connexion échouée : ${error}`)
      window.history.replaceState({}, "", window.location.pathname)
    }
  }, [])
...
\`\`\``
  ...
  // ── MCP connector skill (Google Gmail/Drive + Notion via /api/connector) ───────────
  const runConnectorSkill = async (service: string, subSkillId: string, input: string) => {
    ...
    try {
      if (!connectorStatus[service]) {
        throw new Error(`Connecte ton compte ${service} d'abord dans ≡ → Connecteurs`)
      }
      if (!geminiApiKey) throw new Error("Clé Gemini requise pour interpréter l'instruction")

      const res = await fetch("/api/connector", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ service, intent: input, geminiKey: geminiApiKey, subSkillId }),
      })
      ...
      setSkillResult(`✅ **${service} — \`${data.tool}\`**\n\n${text}`)
    } catch (err: unknown) {
            ...
            <div className="mt-2 space-y-2">
              ...
              <div className="rounded-xl border border-[#eee] p-3 flex items-center justify-between gap-3">
                ...
                {connectorStatus.google ? (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    <a href="/api/connector?action=disconnect&service=google" className="text-[10px] text-red-400 hover:text-red-600">✕</a>
                  </div>
                ) : (
                  <a href="/api/connector?action=auth&provider=google" className="flex-shrink-0 text-[11px] px-3 py-1.5 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors font-medium whitespace-nowrap">
                    Connecter
                  </a>
                )}
              </div>
              ...
              <div className="rounded-xl border border-[#eee] p-3 flex items-center justify-between gap-3">
                ...
                {connectorStatus.notion ? (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-[10px] px-2 py-1 bg-green-100 text-green-600 rounded-full whitespace-nowrap">✓ Connecté</span>
                    <a href="/api/connector?action=disconnect&service=notion" className="text-[10px] text-red-400 hover:text-red-600">✕</a>
                  </div>
                ) : (
                  <a href="/api/connector?action=auth&provider=notion" className="flex-shrink-0 text-[11px] px-3 py-1.5 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors font-medium whitespace-nowrap">
                    Connecter
                  </a>
                )}
              </div>
              ...
              <div className="rounded-xl border border-[#eee] p-3 flex items-center justify-between gap-3">
                ...
                {connectorStatus.trello ? (
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-[10px] px-2 py-1 bg-green-100 text-green-600 rounded-full whitespace-nowrap">✓ Connecté</span>
                    <a href="/api/connector?action=disconnect&service=trello" className="text-[10px] text-red-400 hover:text-red-600">✕</a>
                  </div>
                ) : (
                  <a href="/api/connector?action=auth&provider=trello" className="flex-shrink-0 text-[11px] px-3 py-1.5 bg-black text-white rounded-lg hover:bg-gray-800 transition-colors font-medium whitespace-nowrap">
                    Connecter
                  </a>
                )}
              </div>
