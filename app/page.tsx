...
type ConnectorService = "github" | "slack" | "gmail" | "notion" | "linear" | "stripe" | "gemini"
...
type LiveResult = {
  ...
  message: string
  url?: string
  preview?: string   // short text to show in output panel
}
...
type SkillConnector = {
  ...
  serviceLabel: string
  serviceIcon: string
  liveLabel: string  // what the live version actually does
  fields: ConnectorField[]
  executeLive: (input: string, creds: ConnectorCredentials) => Promise<LiveResult>
}
...
type Skill = {
  ...
  inputLabel: string
  placeholder: string
  execute: (input: string) => string         // Sandbox — always available
  connector?: SkillConnector                 // Live — real service behind
}
...
function videoWorkflowSandbox(input: string): string {
  const url = input.trim().split("\n")[0] || "https://youtube.com/watch?v=..."
  return `// Connect your Gemini API key to analyze the video and generate a real workflow.\n// Video URL detected: ${url}\n\n// Example n8n workflow structure that will be generated:\n{\n  "name": "Automation Workflow",\n  "nodes": [\n    { "id": "1", "name": "Webhook", "type": "n8n-nodes-base.webhook", "position": [250, 300], "parameters": { "path": "trigger" } },\n    { "id": "2", "name": "HTTP Request", "type": "n8n-nodes-base.httpRequest", "position": [450, 300], "parameters": {} },\n    { "id": "3", "name": "Set", "type": "n8n-nodes-base.set", "position": [650, 300], "parameters": {} }\n  ],\n  "connections": {\n    "Webhook": { "main": [[{ "node": "HTTP Request", "type": "main", "index": 0 }]] },\n    "HTTP Request": { "main": [[{ "node": "Set", "type": "main", "index": 0 }]] }\n  }\n}\n\n// → Connect Gemini to generate the real workflow from the video`
}
...
const geminiVideoConnector: SkillConnector = {
  service: "gemini",
  serviceLabel: "Gemini",
  serviceIcon: "✨",
  liveLabel: "Watches the entire YouTube video and generates a real importable workflow",
  fields: [
    { key: "apiKey", label: "Google AI Studio Key", placeholder: "AIzaSy…", type: "password" },
  ],
  executeLive: async (input, creds) => {
    const { apiKey } = creds
    if (!apiKey) return { success: false, message: "Missing API key — get one at aistudio.google.com" }

    const lines    = input.trim().split("\n").filter(Boolean)
    const url      = lines[0]?.trim()
    const pLine    = lines.find(l => /n8n|make|zapier/i.test(l))
    const platform = pLine?.match(/n8n|make|zapier/i)?.[0]?.toLowerCase() ?? "n8n"

    if (!url || !url.startsWith("http")) return { success: false, message: "First line must be a valid YouTube URL" }

    const platformInstructions: Record<string, string> = {
      n8n: `Return a valid n8n workflow JSON:
{"name":"...","nodes":[{"id":"1","name":"...","type":"n8n-nodes-base.xxx","typeVersion":1,"position":[x,y],"parameters":{}}],"connections":{"NodeName":{"main":[[{"node":"NextNode","type":"main","index":0}]]}}}
Use real n8n node types: webhook, httpRequest, code, slack, gmail, googleSheets, scheduleTrigger, if, merge, set, splitInBatches, etc.`,
      make: `Return a valid Make scenario blueprint JSON with a modules array. Use real Make module identifiers.`,
      zapier: `Return a JSON with trigger and actions array using real Zapier app names and event names.`,
    }

    const prompt = `You are an expert automation engineer. Watch this entire video carefully from start to finish.

Your goal: understand exactly what automation workflow the video describes or demonstrates, then reproduce it completely as a ${platform} workflow.

${platformInstructions[platform] ?? platformInstructions.n8n}

Rules:
- Output ONLY valid JSON. Zero markdown, zero backticks, zero explanation.
- Map every step shown in the video to a real node/module with correct type names.
- Include realistic parameters based on what you observe in the video.
- If the video is not about automation, generate a workflow that automates the main process it covers.
- The result must be complete and directly importable into ${platform}.
...
Generate now:`
    ...
    try {
      ...
      JSON.parse(clean) // throws if invalid JSON
      return { success: true, message: `✅ ${platform} workflow generated`, preview: clean }
    } catch (e: any) {
      return { success: false, message: `Gemini: ${e.message}` }
    }
  },
}

const githubIssueConnector: SkillConnector = {
  service: "github",
  serviceLabel: "GitHub",
  serviceIcon: "🐙",
  liveLabel: "Creates a real issue on your GitHub repo",
  fields: [
    { key: "token",  label: "Personal Access Token", placeholder: "ghp_xxxxxxxxxxxx", type: "password" },
    { key: "owner",  label: "Owner (user or org)",   placeholder: "vercel" },
    { key: "repo",   label: "Repository",            placeholder: "next.js" },
  ],
  executeLive: async (input, creds) => {
    const { token, owner, repo } = creds
    if (!token || !owner || !repo) return { success: false, message: "Missing credentials — fill in all fields" }
    const isBug   = /bug|error|crash|fail|broken|wrong|not work/i.test(input)
    const title   = input.split("\n")[0].slice(0, 100)
    const body    = genGHIssue(input)
    try {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues`, {
        ...
        body: JSON.stringify({ title, body, labels: [isBug ? "bug" : "enhancement"] }),
      })
      if (!res.ok) { const err = await res.json(); throw new Error(err.message ?? res.statusText) }
      const data = await res.json()
      return { success: true, message: `✅ Issue #${data.number} created on ${owner}/${repo}`, url: data.html_url, preview: `#${data.number}: ${data.title}\n${data.html_url}` }
    } catch (e: any) {
      return { success: false, message: `GitHub error: ${e.message}` }
    }
  },
}

const githubPRConnector: SkillConnector = {
  service: "github",
  serviceLabel: "GitHub",
  serviceIcon: "🐙",
  liveLabel: "Opens a real PR on your GitHub repo",
  fields: [
    { key: "token",  label: "Personal Access Token", placeholder: "ghp_xxxxxxxxxxxx", type: "password" },
    { key: "owner",  label: "Owner",                 placeholder: "vercel" },
    { key: "repo",   label: "Repository",            placeholder: "next.js" },
    { key: "head",   label: "Head branch",           placeholder: "feat/my-feature" },
    { key: "base",   label: "Base branch",           placeholder: "main" },
  ],
  executeLive: async (input, creds) => {
    const { token, owner, repo, head, base } = creds
    if (!token || !owner || !repo || !head) return { success: false, message: "Missing credentials — fill in all fields" }
    const title = input.split("\n")[0].slice(0, 100) || "Update"
    const body  = genPRDesc(input)
    try {
      const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/pulls`, {
        ...
        body: JSON.stringify({ title, body, head, base: base || "main", draft: false }),
      })
      if (!res.ok) { const err = await res.json(); throw new Error(err.message ?? res.statusText) }
      const data = await res.json()
      return { success: true, message: `✅ PR #${data.number} opened on ${owner}/${repo}`, url: data.html_url, preview: `PR #${data.number}: ${data.title}\n${data.html_url}` }
    } catch (e: any) {
      return { success: false, message: `GitHub error: ${e.message}` }
    }
  },
...
const slackConnector: SkillConnector = {
  service: "slack",
  serviceLabel: "Slack",
  serviceIcon: "💬",
  liveLabel: "Posts the standup directly to your Slack channel",
  fields: [
    { key: "webhook", label: "Webhook URL", placeholder: "https://hooks.slack.com/services/…", type: "url" },
    { key: "channel", label: "Channel (optional)", placeholder: "#standup" },
  ],
  executeLive: async (input, creds) => {
    const { webhook, channel } = creds
    if (!webhook) return { success: false, message: "Missing Slack webhook URL" }
    const text = genStandup(input).replace(/\/\/ .*/g, "").trim()
    try {
      await fetch(webhook, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      return { success: true, message: `✅ Standup posted to Slack${channel ? ` ${channel}` : ""}`, preview: text }
    } catch (e: any) {
      return { success: false, message: `Slack error: ${e.message}` }
    }
  },
}

const gmailInvoiceConnector: SkillConnector = {
  service: "gmail",
  serviceLabel: "Gmail",
  serviceIcon: "📧",
  liveLabel: "Sends the invoice by email via your API route",
  fields: [
    { key: "to",      label: "Client email",    placeholder: "client@company.com" },
    { key: "from",    label: "Your email",       placeholder: "you@yourdomain.com" },
    { key: "apiKey",  label: "Resend API Key",   placeholder: "re_xxxxxxxx", type: "password" },
  ],
  executeLive: async (input, creds) => {
    const { to, from, apiKey } = creds
    if (!to || !from || !apiKey) return { success: false, message: "Missing email credentials" }
    const invoiceText = genInvoice(input)
    try {
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(err.message ?? res.statusText) }
      const data = await res.json()
      return { success: true, message: `✅ Invoice sent to ${to}`, preview: `Email ID: ${data.id}` }
    } catch (e: any) {
...
const SKILLS: Skill[] = [
  ...
  },
  {
    ...
    execute: genInvoice,
    connector: gmailInvoiceConnector,
  },
  {
  ...
  {
    id: "video-workflow", icon: "🎬", label: "Video → Workflow", category: "ops",
    description: "Paste a YouTube URL — Gemini watches the video and generates an importable n8n / Make / Zapier workflow",
    inputLabel: "YouTube URL  (+ platform on line 2)", placeholder: "https://youtube.com/watch?v=dQw4w9WgXcQ\n\nn8n",
    execute: videoWorkflowSandbox,
    connector: geminiVideoConnector,
  },
]
...
export default function SkillsPage() {
  ...
  // Connector state: service → { connected, creds }
  const [connectorCreds, setConnectorCreds] = useState<Partial<Record<ConnectorService, ConnectorCredentials>>>({})
  const [showConnectPanel, setShowConnectPanel] = useState(false)
  const [draftCreds, setDraftCreds] = useState<ConnectorCredentials>({})
  ...
  const isConnected = (skill: Skill) => {
    if (!skill.connector) return false
    const creds = connectorCreds[skill.connector.service]
    return !!(creds && skill.connector.fields.every(f => creds[f.key]?.trim()))
  }

  const runSkill = async (forceSandbox = false) => {
    if (!activeSkill || !input.trim()) return
    setResult(null)
    setLiveResult(null)

    const useLive = !forceSandbox && activeSkill.connector && isConnected(activeSkill)

    if (useLive && activeSkill.connector) {
      setIsRunning(true)
      try {
        const creds = connectorCreds[activeSkill.connector.service]!
        const res = await activeSkill.connector.executeLive(input, creds)
        setLiveResult(res)
        if (res.preview) setResult(res.preview)
      } finally {
        setIsRunning(false)
      }
    } else {
  ...
  const saveConnector = () => {
    if (!activeSkill?.connector) return
    setConnectorCreds(prev => ({ ...prev, [activeSkill.connector!.service]: { ...draftCreds } }))
    setShowConnectPanel(false)
    setDraftCreds({})
  }

  const disconnectService = (service: ConnectorService) => {
    setConnectorCreds(prev => { const n = { ...prev }; delete n[service]; return n })
  }
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-1.5">
                          ...
                          {skill.connector && (
                            isConnected(skill)
                              ? <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200">⚡ LIVE</span>
                              : <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500">{skill.connector.serviceIcon}</span>
                          )}
                        </div>
      ...
      {activeSkill && (
        <div className="w-full max-w-2xl mt-4 space-y-3">
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 bg-gray-50/50">
              ...
              <div className="flex items-center gap-2">
                {/* Connector badge */}
                {activeSkill.connector && (
                  isConnected(activeSkill) ? (
                    <button
                      onClick={() => disconnectService(activeSkill.connector!.service)}
                      className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-700 text-[10px] font-semibold hover:bg-emerald-100 transition-colors"
                      title="Click to disconnect"
                    >
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block" />
                      Live · {activeSkill.connector.serviceLabel}
                    </button>
                  ) : (
                    <button
                      onClick={() => { setShowConnectPanel(v => !v); setDraftCreds({}) }}
                      className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-gray-100 border border-gray-200 text-gray-500 text-[10px] font-semibold hover:bg-gray-200 transition-colors"
                    >
                      {activeSkill.connector.serviceIcon} Connect {activeSkill.connector.serviceLabel}
                    </button>
                  )
                )}
            ...
            {/* Connect panel */}
            {activeSkill.connector && showConnectPanel && !isConnected(activeSkill) && (
              <div className="border-b border-gray-100 bg-gray-50 px-4 py-3 space-y-2">
                <p className="text-xs font-semibold text-gray-700 mb-2">
                  {activeSkill.connector.serviceIcon} {activeSkill.connector.serviceLabel} · {activeSkill.connector.liveLabel}
                </p>
                {activeSkill.connector.fields.map(field => (
                  <div key={field.key} className="flex items-center gap-2">
                    <label className="text-[10px] text-gray-500 w-28 shrink-0 font-medium">{field.label}</label>
                    <input
                      type={field.type ?? "text"}
                      value={draftCreds[field.key] ?? ""}
                      onChange={e => setDraftCreds(prev => ({ ...prev, [field.key]: e.target.value }))}
                      placeholder={field.placeholder}
                      className="flex-1 bg-white border border-gray-200 rounded-lg px-3 py-1.5 text-xs text-gray-800 placeholder:text-gray-400 outline-none focus:border-gray-400 font-mono"
                    />
                  </div>
                ))}
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className="px-4 py-1.5 rounded-full bg-black text-white text-xs font-medium hover:bg-gray-800 transition-colors"
                  >
                    Save & connect
                  </button>
                  <button
                    onClick={() => { setShowConnectPanel(false); setDraftCreds({}) }}
                    className="px-4 py-1.5 rounded-full bg-gray-100 text-gray-600 text-xs font-medium hover:bg-gray-200 transition-colors"
                  >
                    Cancel
                  </button>
                </div>
            ...
            <textarea
              ref={skillInputRef}
              value={input}
              onChange={e => { setInput(e.target.value); setResult(null); setLiveResult(null) }}
          ...
          <div className="flex gap-2">
            {activeSkill.connector && isConnected(activeSkill) ? (
              <>
                <button
                  onClick={() => runSkill(false)}
                  disabled={!input.trim() || isRunning}
                  className="flex-1 h-11 rounded-full bg-black text-white text-sm font-medium hover:bg-gray-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex items-center justify-center gap-2"
                >
                  {isRunning ? (
                    <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Running…</>
                  ) : (
                    <>⚡ Run Live · {activeSkill.connector.serviceLabel}</>
                  )}
                </button>
                <button
                  onClick={() => runSkill(true)}
                  disabled={!input.trim() || isRunning}
                  className="h-11 px-5 rounded-full bg-gray-100 text-gray-600 text-sm font-medium hover:bg-gray-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                  title="Run in sandbox (no API call)"
                >
                  Sandbox
                </button>
              </>
              ...
              <button
                onClick={() => runSkill()}
                disabled={!input.trim()}
                className="flex-1 h-11 rounded-full bg-black text-white text-sm font-medium hover:bg-gray-800 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
              >
        ...
        <div className="w-full max-w-2xl mt-3 rounded-2xl border border-gray-200 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 bg-gray-50/50">
            <div className="flex items-center gap-2">
              ...
              {liveResult && (
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${liveResult.success ? "bg-emerald-50 text-emerald-700" : "bg-red-50 text-red-600"}`}>
                  {liveResult.message}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2">
              {liveResult?.url && (
                <a href={liveResult.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:text-blue-700 transition-colors">
                  Open ↗
                </a>
              )}
              <button
