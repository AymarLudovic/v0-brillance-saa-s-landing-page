...
export default function Dashboard({
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const q = query(
        collection(db, 'analytics', siteId, 'pageviews'),
        where('timestamp', '>=', startTs),
        orderBy('timestamp', 'asc')
      );

      const snap = await getDocs(q);
      const docs = snap.docs.map((d) => d.data() as PageViewDoc);
      setPageviews(docs);
    } catch (err) {
      console.error('[Poyne] Fetch error:', err);
      setError('Impossible de charger les données. Vérifiez votre siteId et les règles Firestore.');
    } finally {
      setLoading(false);
    }
  }, [siteId, period]);
  ...
  const stats = useMemo(() => {
    ...
    pageviews.forEach((p) => {
      const day = p.timestamp?.toDate?.().toISOString().slice(0, 10) ?? '';
