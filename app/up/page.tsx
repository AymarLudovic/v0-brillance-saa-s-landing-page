...
export default function SiteInspector() {
  ...
  const [spaFramework, setSpaFramework] = useState("")
  ...
  const analyzeSite = async (urlToAnalyze = url) => {
    ...
    try {
      ...
      // Enregistrer le flag SPA
      if (data.isSPA) {
        setIsSPA(true)
        setSpaFramework(data.framework || "")
      }
...
- If you encounter a compile/runtime error, fix only the minimal lines needed without changing the overall code.`
  ...
  const createOptimizedPreview = () => {
    if (!result) return ""
    // The route already returns a complete, self-contained HTML page with:
    // - all URLs absolutified
    // - <base href> injected
    // - iframe-busting patches
    // - all external CSS inlined as <style> blocks
    // So we use it directly without re-wrapping it.
