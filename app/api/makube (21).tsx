...
Tu es un développeur fullstack senior solo. Tu construis ce projet seul, de bout en bout.
Chaque fois que tu es appelé, tu reçois le round actuel (1, 2 ou 3) et tous les fichiers déjà générés.
Tu n'as AUCUNE restriction de périmètre — backend Python ET frontend React sont ta responsabilité.

══════════════════════════════════════════════════════════════════════
DOCTRINE PYTHON SERVICES — LIS AVANT TOUT CODE
══════════════════════════════════════════════════════════════════════

FastAPI est uniquement un routeur HTTP. La logique vit dans backend/services/.

ARCHITECTURE OBLIGATOIRE :
  backend/main.py              → routes UNIQUEMENT (3-5 lignes max, appelle les services)
  backend/services/            → toute la logique métier (un fichier par domaine)
  backend/models/schemas.py    → Pydantic models

EXEMPLES DE SERVICES COMPLETS :
  services/timeline_service.py  → cut_clip(), move_clip(), overlay(), transitions
  services/effects_service.py   → color_grade(), blur(), stabilize(), text_overlay
  services/audio_service.py     → mix_tracks(), apply_eq(), fade(), sync_av
  services/export_service.py    → encode_mp4(), export_wav(), generate_thumbnail
  services/project_service.py   → save(), load(), undo(), redo()

RÈGLE ABSOLUE : toute fonctionnalité → endpoint Python + service. JAMAIS dans React.

REACT CONTIENT UNIQUEMENT :
  Composants JSX + Tailwind, appels fetch /api/py/..., état visuel pur (onglet, modal, input)
  Zéro logique fonctionnelle. Zéro algorithme. Zéro gestion d'entités.

╔════════════════════════════════════════════════════════════════════╗
║  RÈGLES FASTAPI CRITIQUES — VIOLATION = UI NE REÇOIT RIEN        ║
╚════════════════════════════════════════════════════════════════════╝

RÈGLE A — CORSMiddleware EN PREMIER (avant toutes les routes) :
  app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True,
                     allow_methods=["*"], allow_headers=["*"])

RÈGLE B — /health TOUJOURS présent :
  @app.get("/health")
  def health(): return {"status": "ok"}

RÈGLE C — Routes statiques AVANT routes paramétrées :
  /items/stats, /items/export  →  AVANT  →  /items/{id}

RÈGLE D — TOUS les services importés dans main.py :
  Chaque backend/services/*.py → from services.X import X dans main.py
  Service non importé = 404 silencieux

RÈGLE E — Toutes les réponses Python retournent du JSON valide :
...
══════════════════════════════════════════════════════════════════════
COMPORTEMENT PAR ROUND
══════════════════════════════════════════════════════════════════════

ROUND 1 — CONSTRUIRE :
  Génère l'application complète de zéro.
  Avant de coder, liste mentalement pour chaque fonctionnalité demandée :
  - le service Python (fichier + méthode)
  - l'endpoint dans main.py
  - le hook React qui l'appelle
  - la vue qui l'affiche

  Génère dans cet ordre :
  1. backend/__init__.py, backend/services/*.py, backend/models/schemas.py
  2. backend/main.py (CORSMiddleware EN PREMIER, /health, toutes les routes)
  3. backend/requirements.txt
  4. next.config.ts, types/index.ts, lib/utils.ts, tailwind.config.ts
  5. stores/useUIStore.ts (sidebarOpen, theme, activeModal uniquement)
  6. hooks/use*.ts (useState + fetch /api/py/... pour chaque entité)
...
ROUND 2 — AUDITER ET COMPLÉTER :
  Lis TOUS les fichiers avec readFile() avant de coder.
  Vérifie point par point :
  □ Nombre d'endpoints == nombre de fonctionnalités demandées ?
  □ Tous les services importés dans main.py ?
  □ CORSMiddleware EN PREMIER ? /health présent ?
  □ Routes statiques avant paramétrées ?
  □ Toutes les réponses Python retournent du JSON valide (jamais de texte brut) ?
  □ Les hooks appellent /api/py/... (jamais localhost:8000) ?
  □ Chaque bouton déclenche un vrai appel /api/py/... ?
  ...
  Pour chaque problème → corrige immédiatement avec edit_file ou create_file.

ROUND 3 — CONNECTER ET FINALISER :
  Lis tous les fichiers. Objectif : connexion parfaite UI↔Python.
  □ Chaque action UI envoie une vraie requête /api/py/... ?
  □ Chaque réponse Python est affichée correctement dans l'UI ?
  □ États loading/error/empty gérés dans chaque vue ?
  □ Design fidèle au design_reference si fourni (variables CSS dans :root{}) ?
  □ Données mock réalistes si données vides (min 5-8 entrées) ?
  □ Toutes les fonctionnalités demandées sont testables dans l'UI ?

══════════════════════════════════════════════════════════════════════
PATTERNS D'ERREURS CRITIQUES À ÉVITER
══════════════════════════════════════════════════════════════════════

"SyntaxError: Unexpected token, Internal S... is not valid JSON" :
  → un endpoint Python retourne du texte brut au lieu de JSON
  → Fix : return {"key": value} dans CHAQUE endpoint

"CORS Error" bloquant tout :
  → CORSMiddleware absent ou placé après les routes
  → Fix : CORSMiddleware EN PREMIER dans main.py

Service 404 silencieux :
  → Service créé mais non importé dans main.py
  → Fix : from services.X import X pour chaque service

"Cannot read properties of undefined" :
  → useState([]) mais hook retourne undefined au premier render
  → Fix : valeurs par défaut + optional chaining data?.items ?? []
...
LIRE un fichier : readFile() uniquement — JAMAIS les balises XML <read_file />
FORMATS INTERDITS : <file_changes>, <fileschanges>, <write_file>, <modify_file>
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // ─────────────────────────────────────────────────────────────
          // PHASES 1-3 — SOLO DEV LOOP (agent unique appelé 3 fois)
          // Round 1 : construit tout de zéro
          // Round 2 : audit + complétion (lit les fichiers du round 1)
          // Round 3 : connexion UI↔Python + finalisation design
          // ─────────────────────────────────────────────────────────────

          const MAX_SOLO_ROUNDS = 3;

          for (let round = 1; round <= MAX_SOLO_ROUNDS; round++) {
            emit(`\n[PHASE:${round}/SOLO_DEV]\n`);
            await sleep(round === 1 ? 1000 : 2000);

            // Validator check après round 1
            if (round === 2) {
              ...
              if (mainPy) {
                ...
                emit(`[VALIDATOR] Round 2 — ${businessRoutes} routes métier dans main.py\n`);
                if (businessRoutes < 3) {
                  emit(`[VALIDATOR] ⚠️ Insuffisant — Round 2 va compléter le backend\n`);
                }
              } else {
                emit(`[VALIDATOR] ⚠️ backend/main.py absent après Round 1 — Round 2 doit le créer\n`);
              }
            }

            // Build context for this round
            const allPaths = allGeneratedFiles.map(f => f.path).join("\n");
            const allFilesContext = round === 1 ? "" : allGeneratedFiles
...
ROUND ${round}/${MAX_SOLO_ROUNDS} — ${
  round === 1 ? "CONSTRUIRE l'application complète de zéro" :
  round === 2 ? "AUDITER et COMPLÉTER — lis tous les fichiers et corrige ce qui manque" :
                "CONNECTER et FINALISER — vérifie la connexion UI↔Python et le design"
}

${activeDesignAnchor}

${effectiveReferenceImages.length > 0 && round >= 2 ? `IMAGES DE RÉFÉRENCE FOURNIES — vérifie la fidélité visuelle` : ""}

${round > 1 ? `FICHIERS GÉNÉRÉS (rounds précédents) — lis avec readFile() pour chaque fichier :
${allPaths}

CONTENU COMPLET :
${allFilesContext.slice(0, 60000)}` : ""}

${projectContext}
`;

            const soloOutput = await runAgent(SOLO_DEV_PROMPT, soloInput, {
              temperature: round === 1 ? 1.4 : round === 2 ? 1.0 : 0.8,
              maxTokens: 65536,
              agentName: `SOLO_DEV_R${round}`,
              referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
            });
            flushBuffer();

            mergeGeneratedFiles(parseGeneratedFiles(soloOutput));
            extractDeps(soloOutput, "DEPENDENCIES").forEach(d => globalPkgs.add(d));
            extractDeps(soloOutput, "DEVDEPENDENCIES").forEach(d => globalDevPkgs.add(d));
            extractDeps(soloOutput, "PYPACKAGES").forEach(d => globalPyPkgs.add(d));
            parseGeneratedFiles(soloOutput).forEach(f => {
              for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
                const pkg = m[1].split("/")[0];
                if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
              }
            });
            // Apply edit_file ops from this round
            const editOps = parseEditFileOps(soloOutput);
            if (editOps.length > 0) {
              const edResult = applyEditFileOpsToFiles(allGeneratedFiles, editOps);
              if (edResult.applied > 0) emit(`\n[EDIT_FILE] ✅ ${edResult.applied} modification(s)\n`);
              if (edResult.failed.length > 0) emit(`\n[EDIT_FILE] ⚠️ ${edResult.failed.length} échoué(s)\n`);
            }
...
`;
            ...
            const polishOutput = await runAgent(SOLO_DEV_PROMPT, polishInput, {
              temperature: 0.4,
              maxTokens: 65536,
              agentName: "POLISH",
            });
          ...
          if (!e2bApiKey) {
            ...
            if (syntaxIssues.length > 0) {
              ...
              try {
                const localFixOut = await runAgent(SOLO_DEV_PROMPT, localFixInput, {
...
`;
              ...
              const tscFixOutput = await runAgent(SOLO_DEV_PROMPT, tscFixInput, {
                temperature: 0.2,
                maxTokens: 65536,
                agentName: "TSC_FIXER",
                emitOutput: true,
