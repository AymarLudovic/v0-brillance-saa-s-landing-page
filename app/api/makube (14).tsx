...
BACKEND PYTHON (priorité absolue) :
  ...
  backend/main.py :
    ...
    ✅ from services.xxx_service import xxx_service en haut
    ✅ Une route par fonctionnalité — délègue au service
...
// =============================================================================
// PHASE 3 — AGENT 3 (Finalisation design + vues manquantes + câblage)_AGENT
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 1 — AGENT 1 (Architecture complète : services Python + hooks + types)
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:1/AGENT_1]\n");
          await sleep(1000);
...
`;
          ...
          const agent1Output = await runAgent(AGENT_1_PROMPT, agent1Input, {
            temperature: 1.4,
            maxTokens: 65536,
            agentName: "AGENT_1",
          ...
          mergeGeneratedFiles(parseGeneratedFiles(agent1Output));
          extractDeps(agent1Output, "DEPENDENCIES").forEach((d) => globalPkgs.add(d));
          extractDeps(agent1Output, "DEVDEPENDENCIES").forEach((d) => globalDevPkgs.add(d));

          // Scan des imports pour capturer les packages non déclarés
          parseGeneratedFiles(agent1Output).forEach((f) => {
            for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
              const pkg = m[1].split("/")[0];
              if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
            }
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 2 — AGENT 2 (Audit + complétion services Python + vues câblées)
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:2/AGENT_2]\n");

          const agent1Summary = allGeneratedFiles
            .map((f) => `=== ${f.path} ===\n${f.content}`)
            .join("\n---\n");
...
`;
          ...
          const agent2Output = await runAgent(AGENT_2_PROMPT, agent2Input, {
            temperature: 1.4,
            maxTokens: 65536,
            agentName: "AGENT_2",
            referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
          ...
          mergeGeneratedFiles(parseGeneratedFiles(agent2Output));
          extractDeps(agent2Output, "DEPENDENCIES").forEach((d) => globalPkgs.add(d));
          extractDeps(agent2Output, "DEVDEPENDENCIES").forEach((d) => globalDevPkgs.add(d));

          parseGeneratedFiles(agent2Output).forEach((f) => {
            for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
              const pkg = m[1].split("/")[0];
              if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
            }
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 3 — AGENT 3 (Finalisation design + vues manquantes + câblage)
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:3/AGENT_3]\n");
...
`;
          ...
          const agent3Output = await runAgent(AGENT_3_PROMPT, agent3Input, {
            temperature: 1.4,
            maxTokens: 65536,
            agentName: "AGENT_3",
          ...
          mergeGeneratedFiles(parseGeneratedFiles(agent3Output));
          extractDeps(agent3Output, "DEPENDENCIES").forEach((d) => globalPkgs.add(d));
          extractDeps(agent3Output, "DEVDEPENDENCIES").forEach((d) => globalDevPkgs.add(d));

          parseGeneratedFiles(agent3Output).forEach((f) => {
            for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
              const pkg = m[1].split("/")[0];
              if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
            }
