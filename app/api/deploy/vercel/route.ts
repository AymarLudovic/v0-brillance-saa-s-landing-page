...
const createProjectRes = await fetch('https://api.vercel.com/v13/projects', {
    ...
    const deployFiles: VercelFile[] = (files as any[]).map((f: any) => {
      ...
      !f.file.toLowerCase().startsWith('backend/') &&
      !f.file.toLowerCase().startsWith('venv/') &&
      !f.file.toLowerCase().startsWith('.venv/') &&
      !f.file.toLowerCase().endsWith('.py') &&
      !f.file.toLowerCase().endsWith('.pyc') &&
      !f.file.toLowerCase().includes('__pycache__') &&
      !f.file.toLowerCase().includes('node_modules/') &&
      !f.file.toLowerCase().includes('.next/') &&
      !f.file.toLowerCase().endsWith('.log') &&
      f.file !== 'package-lock.json' &&
      f.file !== 'yarn.lock' &&
      f.data.length < 500_000
    );
