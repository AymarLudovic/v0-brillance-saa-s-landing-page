...
export async function POST(req: Request) {
  try {
    ...
    if (!isAnthropic) {
      ...
      const functionalityReminder = `[BEFORE YOU WRITE A SINGLE LINE OF CODE — ask yourself: will every feature actually work, or just look like it works? Every button must trigger real logic. Every form must save real data. Every chart must reflect real changing state. Every "generate" action must produce real output. No simulated states, no fake timers, no hardcoded arrays as live data, no handlers that only toggle a boolean. If you are about to write something "simulated" — stop and build the real thing instead.]\n\n`;
