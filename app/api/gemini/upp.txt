...
function runTypeSafetyFixer(files: GeneratedFile[]): GeneratedFile[] {
  ...
  return files.map(f => {
    ...
    // ── 2. Monaco Editor → remplacement par CodeMirror ou textarea ─────────────
    if (content.includes("@monaco-editor/react") || content.includes("monaco-editor")) {
      // Remplace l'import monaco par une implémentation textarea simple
      content = content
        .replace(/import\s+(?:Editor|MonacoEditor|\w+)\s+from\s+['"]@monaco-editor\/react['"]/g,
          "// Monaco remplacé par CodeMirror — voir implémentation ci-dessous")
        .replace(/import\s+.*?from\s+['"]monaco-editor.*?['"]/g,
          "// Monaco remplacé")
        // Remplace le composant <Editor ... /> par un textarea stylisé
        .replace(/<(?:Editor|MonacoEditor)\s[^>]*\/>/g,
          "<textarea className=\"w-full h-full bg-[#1e1e1e] text-[#d4d4d4] font-mono text-sm p-4 resize-none outline-none border border-[#3c3c3c] rounded\" spellCheck={false} />")
        .replace(/<(?:Editor|MonacoEditor)\s[^>]*>[\s\S]*?<\/(?:Editor|MonacoEditor)>/g,
          "<textarea className=\"w-full h-full bg-[#1e1e1e] text-[#d4d4d4] font-mono text-sm p-4 resize-none outline-none border border-[#3c3c3c] rounded\" spellCheck={false} />");
      changed = true;
    }

    // ── 3. Modules inconnus problématiques ────────────────────────────────────
    const PROBLEMATIC_MODULES: Record<string, string> = {
      "@monaco-editor/react": "",
      "monaco-editor": "",
    ...
    for (const [mod, replacement] of Object.entries(PROBLEMATIC_MODULES)) {
      if (content.includes(`from '${mod}'`) || content.includes(`from "${mod}"`)) {
        if (replacement) {
          content = content
            .replace(new RegExp(`import\\s+.*?from\\s+['"]${mod.replace(/[/\\@]/g, "\\$&")}['"]`, "g"), replacement);
        } else {
          content = content
            .replace(new RegExp(`import\\s+.*?from\\s+['"]${mod.replace(/[/\\@]/g, "\\$&")}['"]\\n?`, "g"), "");
        }
        changed = true;
      }
    }
...
════════════════════════════════════════════════════════════
PACKAGES INTERDITS — NE JAMAIS UTILISER
════════════════════════════════════════════════════════════

INTERDIT : @monaco-editor/react — instable, cause des erreurs de build systematiques
  Remplacant : utilise un textarea avec styles dark ou n'importe quel editeur simple
INTERDIT : ace-builds, react-ace — meme probleme
  Remplacant : textarea stylise ou CodeMirror (@codemirror/view si editeur avance)
INTERDIT : tout package dont l'existence dans npm n'est pas certaine
  Regle : si tu n'es pas sur a 100% qu'un package existe sur npm, ne l'utilise pas
