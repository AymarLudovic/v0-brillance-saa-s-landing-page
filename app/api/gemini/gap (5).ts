...
async function callWithRetry(
  ...
  const { maxAttempts = 4, baseDelay = 12000, onThought, onUsage } = opts;
  let lastErr: any;
...
// =============================================================================
// IMAGE INTENT AGENT — Détermine si l'image uploadée est une référence de design
// =============================================================================

const IMAGE_INTENT_PROMPT = `Tu es un agent de classification ultra-rapide.

Analyse l'image uploadée par l'utilisateur ET son message texte, et réponds UNIQUEMENT par un JSON valide :
...
isDesignReference = true SI :
- L'image montre une maquette UI/UX, un écran d'application, un dashboard, un site web, une interface logicielle
- L'image ressemble à un wireframe, mockup, screenshot d'app, ou design Figma/Sketch
- Le message évoque "génère", "crée", "reproduis", "clone", "fait comme", "style", "design", ou toute intention de construire quelque chose
- L'utilisateur ne précise pas explicitement ce qu'il veut mais l'image est clairement une interface
- Le contexte suggère que l'utilisateur veut que l'app ressemble à cette image

isDesignReference = false SI :
- L'image est une photo, un logo seul, un schéma, un diagramme technique, un document
- Le message indique clairement que l'image est un exemple de données ou de contenu à afficher
- L'utilisateur veut analyser le contenu de l'image, pas son design
...
▸ PATCH_ACTION  — l'utilisateur veut une PETITE modification sur son projet existant (ajout d'un composant, correction d'un style, ajout d'une fonctionnalité ciblée)
▸ FIX_ACTION    — l'utilisateur signale une erreur de build, un crash, un bug TypeScript
▸ CHAT_ONLY     — l'utilisateur pose une question, discute, demande des conseils

RÈGLE CRITIQUE : Choisis PATCH_ACTION si ET SEULEMENT SI :
- Il y a déjà des fichiers de projet existants
- La demande est clairement ciblée (un composant, une page, un style, une feature)
- La demande ne demande PAS de reconstruire toute l'app
...
Exemples valides :
CODE_ACTION
Super, je comprends ce que tu veux construire...

PATCH_ACTION
Parfait, je vais ajouter cette fonctionnalité directement...
...
Tu es un expert en UI/UX et reproduction pixel-perfect d'interfaces.
Ton rôle : analyser les images de référence fournies et produire un HTML/CSS de haute fidélité.

══════════════════════════════════════════════════════════════════════
MISSION
══════════════════════════════════════════════════════════════════════

Analyse les images de style reçues et produis un HTML/CSS complet qui capture :
1. La palette de couleurs exacte (extraire les hex de chaque zone visuelle)
2. La typographie (familles, tailles, weights)
3. La densité et l'espacement (padding, gap, border-radius)
4. L'ambiance générale (dark/light, corporate/créatif, minimaliste/dense)
5. Les composants UI récurrents (cards, boutons, tableaux, sidebars)

RÈGLES D'OR :
- Déclare TOUT dans :root {} — aucune valeur hardcodée dans les composants
- Police : charge depuis Google Fonts via <link>
- Icônes : Tabler Icons CDN exclusivement (<i class="ti ti-[nom]">)
- Images : DiceBear pour avatars, Google Favicon API pour logos, Picsum pour contenu
- Densité professionnelle : texte 12-13px, padding compact, gap serré
- JAMAIS de Lorem ipsum — données réalistes inventées

OUTPUT FORMAT :
Émets UNIQUEMENT un bloc HTML dans cette balise :
<design_reference>
<!DOCTYPE html>
<html lang="fr">
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=[NOM]&display=swap" rel="stylesheet">
  <style>
    :root {
      /* couleurs extraites des images */
      --bg: #...;
      --sidebar-bg: #...;
      /* etc */
    }
...
<body>
  <!-- interface complète avec données réalistes -->
</body>
</html>
</design_reference>

AUCUN texte avant ou après la balise design_reference.
`;
...
Option A — Réécriture complète du fichier (si plus de 30% du fichier change) :
<create_file path="chemin/fichier.tsx">
... fichier entier corrigé ...
</create_file>

Option B — Édition chirurgicale str_replace (PRÉFÉRER si moins de 30% change) :
<str_replace path="chemin/fichier.tsx">
<old_str>code exact à remplacer (doit être unique dans le fichier)</old_str>
<new_str>code corrigé</new_str>
</str_replace>

RÈGLE : Pour le TSC fixer, préfère TOUJOURS Option B (str_replace) pour des corrections précises.
Cela évite de réécrire accidentellement des parties correctes du fichier.
Si tu utilises Option B, old_str doit être exactement identique au code dans le fichier (copie littérale).
      ...
      async start(controller) {
        ...
        async function runAgent(
          ...
          opts: {
            ...
            emitOutput?: boolean;
            noTools?: boolean;
            agentName?: string; // nom pour l'émission des pensées
          } = {}
        ): Promise<string> {
          const { temperature = 1.0, maxTokens = 65536, useChatHistory = false, emitOutput = true, noTools = false, agentName = "" } = opts;
            ...
            for await (const chunk of response) {
              ...
              if (chunk.functionCalls && chunk.functionCalls.length > 0) {
                ...
                for await (const cont of continueResponse) {
                  const parts = cont.candidates?.[0]?.content?.parts;
                  if (parts) {
                    for (const part of parts) {
                      if (!part.text) continue;
                      if (part.thought) { emitThought(part.text); }
                      else { fullOutput += part.text; if (emitOutput) onChunk(part.text); }
                    }
                  } else {
                    const txt = cont.text;
        ...
        try {
          // ═══════════════════════════════════════════════════════════════
          // ÉTAPE 0-BIS — IMAGE INTENT : détecte si l'image est un design de référence
          // Si oui, elle est traitée comme allReferenceImages pour le Design Agent
          // ═══════════════════════════════════════════════════════════════
          let effectiveReferenceImages = allReferenceImages ?? [];
          
          if (uploadedImages && uploadedImages.length > 0) {
            try {
              // Quick classification: is this image a UI design reference?
              const intentParts: any[] = uploadedImages.map(img => ({
                inlineData: { data: cleanBase64(img), mimeType: getMimeType(img) }
              }));
              intentParts.push({ text: `Message utilisateur: "${lastUserMsg}"\n\nAnalyse si cette image est une référence de design UI.` });
              ...
              const intentResult = await callWithRetry(
                () => ai.models.generateContentStream({
                  ...
                  config: {
                    systemInstruction: IMAGE_INTENT_PROMPT,
                    temperature: 0.1,
                    maxOutputTokens: 256,
                    thinkingConfig: { thinkingLevel: "NONE" as any },
                  },
                }),
                () => {},
              ...
              const cleanResult = intentResult.replace(/```json|```/g, "").trim();
              const intent = JSON.parse(cleanResult);
              
              if (intent.isDesignReference && intent.confidence >= 0.6) {
                // Treat uploaded images as design references
                effectiveReferenceImages = [...uploadedImages, ...effectiveReferenceImages];
                emit(`\n[DESIGN:INTENT] 🎨 Image détectée comme référence de design (confiance: ${Math.round(intent.confidence * 100)}%)\n`);
              }
            } catch (intentErr: any) {
              // Silent fail — continue without image intent classification
            }
          }
          ...
          // ═══════════════════════════════════════════════════════════════
          // MODE PATCH — modification ciblée (un seul agent, str_replace)
          // ═══════════════════════════════════════════════════════════════
          if (decision === "PATCH_ACTION") {
            emit("\n[PHASE:1/PATCH]\n");

            const patchInput = `
DEMANDE DE MODIFICATION : "${lastUserMsg}"

${activeDesignAnchor}

${projectContext}

Applique cette modification de façon chirurgicale.
Utilise readFile() pour lire n'importe quel fichier avant de le modifier.
Préfère str_replace pour les modifications < 30% d'un fichier.
`;

            const patchOutput = await runAgent(PATCH_AGENT_PROMPT, patchInput, {
              temperature: 1.0,
              maxTokens: 65536,
              agentName: "PATCH",
            });
            ...
            const patchAllFiles: { path: string; content: string }[] = [...(currentProjectFiles ?? []).map(f => ({ path: f.path, content: f.content }))];
            const patchFiles = parseGeneratedFiles(patchOutput);
            const patchOps = parseStrReplaceOps(patchOutput);
            for (const f of patchFiles) {
              const idx2 = patchAllFiles.findIndex(g => g.path === f.path);
              if (idx2 >= 0) patchAllFiles[idx2] = f; else patchAllFiles.push(f);
            }
            if (patchOps.length > 0) {
              const srResult = applyStrReplaceToFiles(patchAllFiles, patchOps);
              emit(`\n[STR_REPLACE] ✅ ${srResult.applied} édition(s) appliquée(s)\n`);
            }
            // Emit modified files with --- separator so frontend can stream them to Monaco
            for (const f of patchAllFiles) {
              const wasModified = patchFiles.some(p => p.path === f.path) || patchOps.some(op => op.path === f.path);
              if (wasModified) emit(`\n---\n<create_file path="${f.path}">\n${f.content}\n</create_file>`);
            }

            emit("\n[PHASE:2/TSC_CHECK]\n");
            if (e2bApiKey) {
              const tscResult2 = await runTscCheck(patchAllFiles, e2bApiKey, emit);
              if (tscResult2.hasErrors) {
                await sleep(5000);
                const fixOutput2 = await runAgent(FIXER_PROMPT, `ERREURS TSC après patch:\n${tscResult2.errors}\n\n${projectContext}`, { temperature: 0.4, maxTokens: 32768, agentName: "TSC_FIXER" });
                flushBuffer();
                parseGeneratedFiles(fixOutput2).forEach(f => {
                  const idx3 = patchAllFiles.findIndex(g => g.path === f.path);
                  if (idx3 >= 0) patchAllFiles[idx3] = f;
                  emit(`\n---\n<create_file path="${f.path}">\n${f.content}\n</create_file>`);
                });
                const ops3 = parseStrReplaceOps(fixOutput2);
                if (ops3.length > 0) applyStrReplaceToFiles(patchAllFiles, ops3);
              }
            }
...
`;
            ...
            const fixOutput = await runAgent(FIXER_PROMPT, fixInput, { temperature: 0.4, maxTokens: 32768, agentName: "FIXER" });
            flushBuffer();
          ...
          function mergeAgentOutput(agentOutput: string) {
...
`;
            ...
            try {
              ...
              // Extract the design_reference block
