...
async function callWithRetry(
  ...
  opts: { maxAttempts?: number; baseDelay?: number; onThought?: (txt: string) => void } = {}
): Promise<string> {
  const { maxAttempts = 4, baseDelay = 12000, onThought } = opts;
  let lastErr: any;
...
══════════════════════════════════════════════════════════════════════
STRATÉGIE INTELLIGENTE — DESIGN PREMIUM OBLIGATOIRE
══════════════════════════════════════════════════════════════════════
1. Analyse le design_reference HTML/CSS reçu — il contient la palette, la typographie, l'ambiance
2. REPRODUIS ce design avec FIDÉLITÉ — couleurs, espacements, border-radius, font
3. AMÉLIORE CRÉATIVEMENT — si le design de référence est bon, va encore plus loin
4. ⛔ JAMAIS downgrader le design — chaque version doit être plus belle que la précédente
5. Identifie toutes les fonctionnalités de la demande et implémente-les TOUTES
        ...
        async function runAgent(
          ...
          try {
            const result = await callWithRetry(
              ...
              emitOutput ? onChunk : () => {},
              { maxAttempts: 4, baseDelay: 12000, onThought: emitThought }
            );
            flushThoughts();
            return result;
          } catch (e: any) {
...
`;
            ...
            for (const f of patchAllFiles) {
              ...
              if (wasModified) emit(`<create_file path="${f.path}">\n${f.content}\n</create_file>`);
            }
...
`;
          ...
          if (allReferenceImages && allReferenceImages.length > 0) {
            ...
            const colorExtractions = await Promise.all(
              allReferenceImages.slice(0, 3).map(img => extractDominantColorsFromBase64(img))
            );
...
`;
            ...
            const refParts = allReferenceImages.map(img => ({
              inlineData: { data: cleanBase64(img), mimeType: getMimeType(img) }
            }));
