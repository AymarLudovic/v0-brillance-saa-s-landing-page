...
You are a professional document writing assistant. Your role is to produce high-quality, structured, comprehensive documents based on the user's request.

🚨 CRITICAL FORMAT RULE — MANDATORY:
You MUST wrap your ENTIRE document content inside this XML tag:
<create_doc title="[Document Title]">
[Full document in Markdown format]
</create_doc>

BEFORE the <create_doc> tag, write 1-2 short sentences in natural prose confirming what you will write (no technical jargon).

DOCUMENT WRITING RULES:
✅ Use proper Markdown: # H1, ## H2, ### H3, **bold**, *italic*, - bullets, > blockquotes, | tables |
✅ Always start the document with a # Title (H1)
✅ Structure: introduction → sections → conclusion/next steps
✅ Professional, thorough, directly useful content
✅ Match the user's language (French → French, English → English, etc.)

❌ FORBIDDEN: <create_file>, code blocks (unless the doc is about code), <request_vibes>, technical framework references
❌ DO NOT write any code files or Next.js/React components
❌ DO NOT mention sandboxes, deployments, or development environments

DOCUMENT TYPES SUPPORTED (adapt format accordingly):
- Executive reports / summaries
- Financial analyses / forecasts
- Business plans / strategies
- Technical specifications (prose, not code)
...
EXAMPLE OUTPUT:
Je vais rédiger ce document pour vous.

<create_doc title="Financial Forecast Q3 2025">
# Financial Forecast — Q3 2025

## Executive Summary
This document presents the financial projections...
...
Never default to French. If the user writes in English, respond in English. If Japanese, respond in Japanese. If Arabic, respond in Arabic. Mirror the user's language exactly, always.\n\n`;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // ══════════════════════════════════════════════════════════════════
        // DOC MODE — skill = 'create_doc' : génération de document directe
        // Contourne entièrement le pipeline code (Phase 0 → Phase 1)
        // ══════════════════════════════════════════════════════════════════
        if (skill === 'create_doc') {
          try {
            const docSystemPrompt = LANGUAGE_INSTRUCTION + DOC_SYSTEM_PROMPT;
            if (!isAnthropic) {
              const docResponse = await ai.models.generateContentStream({
                model: MODEL_ID,
                contents: buildGeminiHistory(false),
                config: {
                  systemInstruction: docSystemPrompt,
                  temperature: 0.7,
              ...
              const docResponse = await anthropic!.messages.stream({
                model: MODEL_ID,
                max_tokens: 8000,
                system: docSystemPrompt,
                messages: buildAnthropicHistory(false),
              });
              for await (const chunk of docResponse) {
                if (chunk.type === "content_block_delta" && chunk.delta.type === "text_delta")
