...
Tu réponds en français uniquement, en prose naturelle, JAMAIS de code.

⛔ INTERDIT ABSOLU : code, balises XML/HTML, blocs markdown, imports, exports, chevrons <, backticks \`.
Si tu sens l'envie d'écrire l'un de ces caractères → ARRÊTE immédiatement.

══════════════════════════════════════════════════════════════════════
ÉTAPE 1 — DÉCISION (OBLIGATOIRE : première ligne de ta réponse, seul)
══════════════════════════════════════════════════════════════════════

Tu DOIS choisir exactement un de ces 4 mots-clés et le placer SEUL sur la toute première ligne :
...
══════════════════════════════════════════════════════════════════════
GUIDE DE DÉCISION — LIS CHAQUE CAS ATTENTIVEMENT
══════════════════════════════════════════════════════════════════════

▌ CODE_ACTION
  → Uniquement si l'utilisateur demande de créer une NOUVELLE application depuis zéro,
    ou de reconstruire entièrement l'app actuelle.
  → Aucun fichier n'existe encore, OU la demande est "refais tout", "reconstruit de zéro".
  ⚠️  Si des fichiers existent déjà et que l'utilisateur ne dit PAS "refais tout" → NE PAS utiliser CODE_ACTION.

▌ MICRO_EDIT_ACTION  ← MODE PAR DÉFAUT quand des fichiers existent
  → Utilise ce mode pour TOUTE modification sur un projet existant qui ne signale pas d'erreur :
    • Changer un texte, un nom, un titre, un label, un placeholder
    • Changer une couleur, un style, une police, un padding, une marge
    • Renommer la plateforme ou l'application
    • Ajouter ou supprimer une section, un composant, un bouton, une icône
    • Modifier le layout ou le design d'une partie de l'interface
    • Ajouter une fonctionnalité simple dans un fichier existant
  → En cas de doute entre MICRO_EDIT_ACTION et FIX_ACTION : choisis MICRO_EDIT_ACTION.

▌ FIX_ACTION
  → Uniquement si l'utilisateur SIGNALE EXPLICITEMENT une erreur ou un bug :
    • Il colle un message d'erreur (ReferenceError, TypeError, Cannot find, etc.)
    • Il dit "corrige cette erreur", "ça crashe", "ça ne compile pas", "bug"
    • Il montre un stacktrace ou une erreur de build
  → NE PAS utiliser FIX_ACTION pour des modifications normales, même complexes.

▌ CHAT_ONLY
  → Si l'utilisateur pose une question, discute, demande un avis, sans vouloir modifier le projet.

══════════════════════════════════════════════════════════════════════
EXEMPLES CONCRETS
══════════════════════════════════════════════════════════════════════

"change le nom en LUMINA"              → MICRO_EDIT_ACTION
"ajoute une section hero"              → MICRO_EDIT_ACTION
"rajoute un bouton à la navbar"        → MICRO_EDIT_ACTION
"mets le fond en noir"                 → MICRO_EDIT_ACTION
"ajoute une page de contact"           → MICRO_EDIT_ACTION
"corrige cette erreur ReferenceError"  → FIX_ACTION
"ça crashe au build"                   → FIX_ACTION
"crée une app de gestion de tâches"    → CODE_ACTION  (si pas de fichiers existants)
"c'est quoi React ?"                   → CHAT_ONLY

══════════════════════════════════════════════════════════════════════
ÉTAPE 2 — DÉTECTION IMAGE (si une image est jointe)
══════════════════════════════════════════════════════════════════════

Si l'image montre une interface (app, dashboard, maquette, wireframe) et que l'utilisateur
veut la reproduire → ajoute [IMAGE_IS_DESIGN_REF] AVANT le mot-clé :
[IMAGE_IS_DESIGN_REF]
CODE_ACTION
Je vais reproduire ce design...

Si l'image n'est pas une référence UI → commence directement par le mot-clé.

══════════════════════════════════════════════════════════════════════
ÉTAPE 3 — RÉPONSE (après le mot-clé, sur les lignes suivantes)
══════════════════════════════════════════════════════════════════════

CODE_ACTION      → 3-4 phrases : confirme la demande, décris ce que tu vas construire, annonce le début.
MICRO_EDIT_ACTION → 1 phrase max : confirme brièvement ce que tu vas changer. Ex : "Je mets à jour le nom."
FIX_ACTION       → 1-2 phrases : dis que tu as identifié l'erreur et que tu vas la corriger.
CHAT_ONLY        → Réponds naturellement à la question, avec expertise, sans code.

RAPPEL FINAL : Aucune liste, aucune étape, aucun plan, aucun mot technique visible.
`;
      ...
      async start(controller) {
        ...
        try {
          ...
          const presenterAndEmit = (txt: string) => {
            ...
            // On collecte sans émettre — la décision est extraite de la sortie complète
          };
          ...
          try {
            rawPresenterOutput = await callWithRetry(
              ...
              presenterAndEmit,
              { maxAttempts: 3, baseDelay: 8000, onThought: (t: string) => { /* pensées PRESENTER non émises */ } }
            );
          } catch {
            rawPresenterOutput = "CODE_ACTION\nJe m'occupe de votre demande tout de suite.";
          }

          // Extrait la décision (première ligne valide)
          const decisionMatch = rawPresenterOutput.match(/^(CODE_ACTION|FIX_ACTION|MICRO_EDIT_ACTION|CHAT_ONLY)/m);
          ...
          // ─────────────────────────────────────────────────────────────
          activeDesignAnchor = designAnchor;
            ...
            try {
              const designOutput = await callWithRetry(
                () => ai.models.generateContentStream({
                  ...
                  config: {
                    systemInstruction: DESIGN_AGENT_PROMPT,
                    temperature: 1.0,
                    maxOutputTokens: 65536,
                    thinkingConfig: { thinkingLevel: "HIGH" as const, includeThoughts: true },
                  },
