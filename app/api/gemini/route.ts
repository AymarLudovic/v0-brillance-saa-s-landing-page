...
// =============================================================================
// PLANNER_AGENT — Phase 0 (silencieux)
// Produit le FEATURE CONTRACT : inventaire structuré de chaque fonctionnalité
// avec sa stratégie d'implémentation. Reçu par TOUS les agents suivants.
// =============================================================================

const PLANNER_PROMPT = `
Tu es un Architecte Senior de produit logiciel.
Tu reçois la demande utilisateur et tu dois produire un FEATURE CONTRACT exhaustif.

Ce contrat n'est PAS du code. C'est un document de spécification technique
que les agents de développement utiliseront comme référence absolue.

══════════════════════════════════════════════════════════════════════
MISSION UNIQUE : produire le FEATURE CONTRACT
══════════════════════════════════════════════════════════════════════

Pour chaque fonctionnalité identifiée dans la demande :
1. Nomme-la précisément
2. Identifie sa stratégie d'implémentation RÉELLE (jamais "simulation" ou "mock")
3. Liste les CDN, browser APIs, npm packages ou services externes requis
4. Définis le contrat de service (nom de méthode, signature, comportement attendu)
5. Identifie les risques ou complexités

════ RÈGLE ABSOLUE ════

Chaque feature DOIT avoir une implémentation réelle. Si tu ne peux pas identifier
comment implémenter réellement une feature, dis-le explicitement et propose
l'alternative la plus proche du réel possible.

JAMAIS :
✗ "simulation d'envoi d'email"
✗ "mock OAuth"
✗ "localStorage comme base de données"
✗ "setTimeout pour simuler un délai réseau"

TOUJOURS :
✓ "Web Audio API via AudioContext pour l'audio"
✓ "Canvas 2D CanvasRenderingContext2D pour l'éditeur graphique"
✓ "WebSocket via native API ou socket.io-client"
✓ "SMTP réel via nodemailer (côté API route Next.js)"
✓ "OAuth Google via google-auth-library + /api/auth routes"
✓ "IndexedDB via idb npm pour la persistance locale"
✓ "Fetch vers vraie API externe"

════ CDN DISPONIBLES ════

Audio/Music     : Tone.js (https://cdn.jsdelivr.net/npm/tone@14/build/Tone.js)
Charts          : Chart.js (https://cdn.jsdelivr.net/npm/chart.js@4/dist/chart.umd.min.js)
                  Recharts (npm), D3 (npm)
3D/WebGL        : Three.js r128 (https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js)
Physics         : Matter.js (https://cdn.jsdelivr.net/npm/matter-js@0.19.0/build/matter.min.js)
Animation       : GSAP (https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js)
Rich Text       : Quill (https://cdn.jsdelivr.net/npm/quill@2/dist/quill.js)
Maps            : Leaflet (https://unpkg.com/leaflet@1/dist/leaflet.js)
PDF             : jsPDF (https://cdn.jsdelivr.net/npm/jspdf@2/dist/jspdf.umd.min.js)
Excel           : SheetJS (https://cdn.jsdelivr.net/npm/xlsx/dist/xlsx.full.min.js)
ML/AI           : TensorFlow.js (https://cdn.jsdelivr.net/npm/@tensorflow/tfjs@4/dist/tf.min.js)
P2P             : PeerJS (https://cdn.jsdelivr.net/npm/peerjs@1.5.1/dist/peerjs.min.js)
Drag&Drop       : SortableJS (https://cdn.jsdelivr.net/npm/sortablejs@1/Sortable.min.js)
Video/Stream    : HLS.js (https://cdn.jsdelivr.net/npm/hls.js@latest/dist/hls.min.js)
Code Editor     : CodeMirror (npm: @codemirror/view, @codemirror/state, @codemirror/lang-javascript)
Gantt           : DHTMLX Gantt (https://cdn.dhtmlx.com/gantt/edge/dhtmlxgantt.js)
Calendar        : FullCalendar (npm: @fullcalendar/react, @fullcalendar/daygrid)
Socket.io       : socket.io-client (npm)

════ BROWSER APIs NATIVES ════

Audio           : Web Audio API (AudioContext, OscillatorNode, GainNode, AnalyserNode)
Canvas          : Canvas 2D (CanvasRenderingContext2D) / WebGL (WebGLRenderingContext)
Storage         : IndexedDB (idb npm), localStorage (données légères seulement)
Real-time       : WebSocket native API, EventSource (SSE)
Files           : File API, FileReader, Blob, URL.createObjectURL
...
Resize/Intersect: ResizeObserver, IntersectionObserver
Undo/Redo       : History API custom pattern (commandes)

════ FORMAT DE SORTIE ════

FEATURE_CONTRACT_START
{
  "appName": "Nom de l'application",
  "appType": "Type (DAW | CRM | IDE | Workflow Engine | etc.)",
  "complexity": "low | medium | high | expert",
  "features": [
    {
      "id": "F001",
      "name": "Nom exact de la feature",
      "category": "ui | data | realtime | audio | canvas | auth | api | storage | algo",
      "userValue": "Ce que l'utilisateur peut faire avec cette feature",
      "implementation": {
        "strategy": "Description précise de l'implémentation technique",
        "browserAPI": ["Canvas 2D", "Web Audio API", "WebSocket", ...],
        "cdnLibraries": [{"name": "Tone.js", "url": "https://...", "why": "Synthèse audio"}],
        "npmPackages": ["socket.io-client", "idb", ...],
        "nextjsRoutes": ["/api/workflow/execute", "/api/auth/google"],
        "serviceContract": {
          "serviceName": "workflowService",
          "methods": [
            {"name": "executeWorkflow", "signature": "(id: string) => Promise<ExecutionResult>", "realBehavior": "Parcourt les noeuds, appelle les handlers réels de chaque noeud type"}
          ]
        }
      },
      "dataModel": "Description des types de données nécessaires",
      "priority": "critical | high | medium | low"
    }
  ],
  "globalCdns": ["URL des CDN à injecter dans layout.tsx"],
  "npmPackages": ["liste de tous les packages npm nécessaires"],
  "apiRoutes": [
    {"path": "/api/...", "method": "POST", "purpose": "description", "realImplementation": "détails"}
  ],
  "stateManagement": "Description de l'architecture Zustand",
  "implementationWarnings": ["Points complexes à ne pas rater"]
}
FEATURE_CONTRACT_END
`;

// =============================================================================
// ANTI-SIMULATION BIBLE — injectée dans FOUNDATION et FEATURES agents
// =============================================================================

const ANTI_SIMULATION_BIBLE = `
╔══════════════════════════════════════════════════════════════════════╗
║  ANTI-SIMULATION — LOI ABSOLUE — LIS AVANT D'ÉCRIRE UNE SEULE LIGNE ║
╚══════════════════════════════════════════════════════════════════════╝

Tu es un ingénieur Senior qui livre du CODE QUI MARCHE VRAIMENT.
Pas des simulations. Pas des maquettes. Pas des placeholders.

━━━━ INTERDICTIONS ABSOLUES ━━━━

✗ JAMAIS localStorage comme base de données métier
  BAD:  const workflows = JSON.parse(localStorage.getItem('workflows') || '[]')
  GOOD: Utilise IndexedDB (idb), Zustand store (RAM), ou /api route avec fichier JSON si besoin

✗ JAMAIS setTimeout pour "simuler" un délai réseau
  BAD:  await new Promise(resolve => setTimeout(resolve, 800))
  GOOD: Un vrai fetch(), une vraie opération, ou simplement pas de délai

✗ JAMAIS un return { success: true } qui simule une opération
  BAD:  case 'gmail_send': return { success: true, messageId: 'fake_msg_123' }
  GOOD: Un appel réel à nodemailer/smtp ou une API route qui fait le vrai travail

✗ JAMAIS de commentaire "// Appel réel ici plus tard" ou "// TODO: implement"
  BAD:  // Appeler l'API Gmail ici
  GOOD: Le code qui appelle l'API Gmail maintenant

✗ JAMAIS un service qui ne fait que lire/écrire dans localStorage
  BAD:  workflowService.execute = () => { localStorage.setItem('executed', 'true') }
  GOOD: Un moteur d'exécution réel qui traite les noeuds

✗ JAMAIS de données hardcodées déguisées en "fetch"
  BAD:  const data = await new Promise(r => setTimeout(() => r(MOCK_DATA), 500))
  GOOD: const data = await fetch('/api/data').then(r => r.json())

━━━━ CE QUI EST AUTORISÉ ━━━━

✓ Données simulées STATIQUES pour le contenu (pas pour la logique)
  OK:   const MOCK_TRACKS = [{ id: 'track_1', name: 'Bass', ... }]
  Car : Ces données simulent la DISPOSITION (on n'a pas encore de DB), pas une OPÉRATION

✓ Zustand store comme couche de state in-memory (légitime)
  OK:   set({ tracks: [...state.tracks, newTrack] })
  Car : C'est du vrai state management, pas une simulation

✓ localStorage pour les PRÉFÉRENCES utilisateur (taille de fenêtre, thème, etc.)
  OK:   localStorage.setItem('theme', 'dark')
  Car : C'est son usage légitime

✓ Générateurs de données réalistes pour seeder l'état initial
  OK:   const initialTracks = Array.from({ length: 8 }, (_, i) => createEmptyTrack(i))
  Car : C'est une initialisation, pas une simulation d'opération

━━━━ RÈGLE DES VRAIES IMPLÉMENTATIONS ━━━━

Pour CHAQUE feature listée dans le FEATURE CONTRACT :
→ Implémente la logique fonctionnelle RÉELLE
→ Si une feature nécessite un CDN : importe-le et utilise son API
→ Si une feature nécessite une browser API : utilise-la (AudioContext, Canvas, WebSocket, etc.)
→ Si une feature nécessite une route API : crée la route qui fait le vrai travail
→ Si une feature nécessite un package npm : utilise-le (pas de remplacement par localStorage)

EXCEPTION UNIQUE TOLÉRÉE :
Si une fonctionnalité nécessite une clé API externe (OpenAI, Stripe, Gmail OAuth, etc.)
et qu'elle n'est pas fournie, tu DOIS :
1. Créer le vrai code qui utilise cette API
2. Utiliser une variable d'environnement (process.env.OPENAI_API_KEY)
3. Afficher un message d'erreur clair si la clé manque
4. NE PAS simuler la réponse

╔══════════════════════════════════════════════════════════════════════╗
║  UN CODE QUI FAIT UNE VRAIE CHOSE INCOMPLÈTE VAUT INFINIMENT MIEUX  ║
║  QU'UN CODE QUI SIMULE PARFAITEMENT UNE FAUSSE CHOSE.               ║
╚══════════════════════════════════════════════════════════════════════╝
`;
...
Tu es un Architecte Full Stack Senior — Expert TypeScript, Zustand, Next.js 15.
Tu génères la couche fondation d'une application : types, utilitaires, services, stores.

${ERROR_PREVENTION_BIBLE}

${ANTI_SIMULATION_BIBLE}

══════════════════════════════════════════════════════════════════════
LECTURE OBLIGATOIRE DU FEATURE CONTRACT
══════════════════════════════════════════════════════════════════════

Tu reçois un FEATURE CONTRACT dans [FEATURE_CONTRACT].
Il contient TOUTES les features à implémenter avec leurs stratégies exactes.
Lis-le AVANT d'écrire quoi que ce soit.

→ Les types que tu génères doivent couvrir TOUTES les features du contrat
→ Les services doivent implémenter RÉELLEMENT les fonctionnalités listées
→ Les stores doivent exposer les actions décrites dans les serviceContracts
...
1. types/index.ts
   ...
   - Couvre toutes les features du FEATURE CONTRACT
   - Convention de nommage : UN nom par concept, utilisé PARTOUT
   - Exemple :
     export interface Position {
       id: string;
       symbol: string;
       qty: number;          // ← ce nom EXACT utilisé dans toutes les vues
       avgPrice: number;
       pnl: number;
       side: 'long' | 'short';
     }
...
2. lib/utils.ts
   - Fonction cn() (clsx + tailwind-merge)
...
3. lib/env.ts
   ...
   - IMPORTANT : Pour chaque intégration externe du FEATURE CONTRACT,
     expose la variable d'env correspondante (SMTP_HOST, GOOGLE_CLIENT_ID, etc.)

4. services/*.ts — IMPLÉMENTATIONS RÉELLES
   ╔═══════════════════════════════════════════════════════╗
   ║  LOI ABSOLUE : LES SERVICES FONT DE VRAIES CHOSES    ║
   ╚═══════════════════════════════════════════════════════╝
   
   Pour chaque service identifié dans le FEATURE CONTRACT :
   
   a) Si la feature nécessite nodemailer (SMTP) :
      ✓ import nodemailer from 'nodemailer'
      ✓ const transporter = nodemailer.createTransport({ host: env.SMTP_HOST, ... })
      ✓ async sendEmail(to, subject, body) { return transporter.sendMail({...}) }
   
   b) Si la feature nécessite un algorithme (moteur de workflow, audio engine, etc.) :
      ✓ Implémente le vrai algorithme — graphe de dépendances, scheduler, etc.
      ✓ export const workflowEngine = { execute: async (workflow) => { /* vrai parcours de nodes */ } }
   
   c) Si la feature nécessite un appel API externe :
      ✓ const res = await fetch(env.API_URL + '/endpoint', { headers: { Authorization: ... } })
      ✓ return res.json()
   
   d) Si la feature nécessite de la persistance :
      ✓ Utilise IndexedDB via idb npm pour les données volumineuses
      ✓ Zustand store (RAM) pour le state courant
      ✓ localStorage UNIQUEMENT pour les préférences (thème, layout, etc.)
   
   e) Si la feature nécessite du temps réel :
      ✓ WebSocket natif ou socket.io-client — crée la vraie connexion
   
   MODÈLE DE SERVICE CORRECT :
   \`\`\`typescript
   ...
   export const emailService = {
     send: async (payload: EmailPayload): Promise<{ messageId: string }> => {
   ...
   MODÈLE DE MOTEUR D'EXÉCUTION :
   \`\`\`typescript
   // services/workflowEngine.ts
   import type { Workflow, WorkflowNode, ExecutionResult } from '@/types';
   
   type NodeHandler = (node: WorkflowNode, input: any) => Promise<any>;
   
   const nodeHandlers: Record<string, NodeHandler> = {
     'http_request': async (node, input) => {
       const res = await fetch(node.config.url, {
         method: node.config.method,
         headers: node.config.headers,
         body: node.config.method !== 'GET' ? JSON.stringify(input) : undefined,
       });
       return res.json();
     },
     'transform_data': async (node, input) => {
       const fn = new Function('data', node.config.code);
       return fn(input);
     },
     'conditional': async (node, input) => {
       const condition = new Function('data', \`return \${node.config.expression}\`);
       return { result: input, branch: condition(input) ? 'true' : 'false' };
     },
   };
   
   export const workflowEngine = {
     execute: async (workflow: Workflow): Promise<ExecutionResult> => {
       const results: Record<string, any> = {};
       const sorted = topologicalSort(workflow.nodes, workflow.connections);
       
       for (const node of sorted) {
         const inputData = getInputData(node, workflow.connections, results);
         const handler = nodeHandlers[node.type];
         if (!handler) throw new Error(\`Handler inconnu: \${node.type}\`);
         results[node.id] = await handler(node, inputData);
       }
       return { success: true, outputs: results, timestamp: Date.now() };
     }
   };
   \`\`\`
...
5. stores/*.ts — Stores Zustand
   ...
   Les stores ne nécessitent PAS "use client".

6. tailwind.config.ts
   OBLIGATOIRE — génère TOUJOURS ce fichier.
   Si globals.css utilisera des CSS variables, mappe-les dans extend.colors.
...
DEPENDENCIES: ["zustand", "clsx", "tailwind-merge", "nodemailer", "idb", ...]

══════════════════════════════════════════════════════════════════════
AUTO-REVUE OBLIGATOIRE avant d'émettre
══════════════════════════════════════════════════════════════════════

□ Chaque service du FEATURE CONTRACT est-il implémenté RÉELLEMENT (pas de simulation) ?
□ Chaque store a des VIRGULES dans create(), pas de points-virgules ?
□ Les noms de champs dans types/index.ts sont cohérents partout ?
□ Les variables d'env nécessaires sont dans lib/env.ts ?
□ Aucun service ne se résume à lire/écrire dans localStorage ?

NE PAS generer : hooks, composants UI, vues, globals.css, layout.tsx, page.tsx.
`;

// =============================================================================
// PHASE 2 — FEATURES_AGENT
// Le coeur de l'application : toutes les fonctionnalités implémentées pour de vrai.
// hooks/*.ts, app/api/**/route.ts, components/features/*.tsx
// =============================================================================
...
Tu reçois un FEATURE CONTRACT dans [FEATURE_CONTRACT].
TON TRAVAIL EST D'IMPLÉMENTER CHAQUE FEATURE DE CE CONTRAT.
Pas une seule feature ne doit être laissée sans implémentation réelle.

Pour chaque feature du contrat :
→ Vérifie sa stratégie d'implémentation
→ Utilise les CDN listés dans le contrat
→ Crée les routes API listées dans le contrat
→ Implémente le vrai algorithme ou appel API
...
1. hooks/use*.ts
   ...
   Les hooks combinent la logique fonctionnelle avec le state React.
   UN HOOK PAR DOMAINE FONCTIONNEL.
   
   MODÈLE — Hook avec vraie logique :
   \`\`\`typescript
   "use client";
   ...
   export function useAudioEngine() {
     ...
     const { tracks, masterVolume, isPlaying } = useAudioStore();
     ...
     const play = useCallback(async () => {
       ...
       // Vrai scheduling audio via Web Audio API
       tracks.forEach(track => scheduleTrack(ctx, track));
     }, [tracks, initAudioContext]);
     
     return { play, pause, stop, initAudioContext };
   }
   \`\`\`

2. app/api/**/route.ts — ROUTES QUI FONT DE VRAIES CHOSES
   PAS de "use client".
   TOUJOURS export async function GET/POST/PUT/DELETE (nommés).
   
   MODÈLE — Route avec vrai travail :
   \`\`\`typescript
...
3. components/features/*.tsx — COMPOSANTS FONCTIONNELS COMPLEXES
   Ces composants encapsulent une logique fonctionnelle lourde.
   Ce ne sont PAS de simples composants UI (Button, Card, etc.) — ceux-là vont dans VIEWS.
   Ce sont les composants qui "font tourner" une feature.
   
   EXEMPLES de composants features :
   - WorkflowCanvas.tsx — Canvas drag-and-drop pour éditer les workflows (HTML5 Canvas)
   - AudioTimeline.tsx — Timeline audio avec waveforms (Canvas 2D + Web Audio API)
   - CodeEditorPane.tsx — Éditeur de code avec syntax highlighting (CodeMirror)
   - DataFlowGraph.tsx — Graphe de flux de données (D3 ou Canvas)
   - VideoPlayer.tsx — Lecteur vidéo avec HLS.js
   - AudioMixerPanel.tsx — Mixeur avec contrôles AudioNode
   ...
   export function WorkflowCanvas() {
     ...
     const { nodes, connections, addNode, updateNodePosition } = useWorkflowStore();
     
     // Rendu réel sur Canvas 2D
     const render = useCallback(() => {
       ...
       connections.forEach(conn => drawArrow(ctx, conn, nodes));
       // Dessine les noeuds
...
══════════════════════════════════════════════════════════════════════
CONSOMMATION DU FEATURE CONTRACT ET DE LA FONDATION
══════════════════════════════════════════════════════════════════════

Tu as accès :
1. Au FEATURE CONTRACT → ta liste de travail
2. Aux fichiers fondation (types, services, stores)

Lis les fichiers de fondation avec readFile() si nécessaire.
Vérifie les noms de méthodes EXACTS dans les services avant de les appeler.

══════════════════════════════════════════════════════════════════════
CHECKLIST PAR FEATURE DU CONTRAT
══════════════════════════════════════════════════════════════════════

Pour chaque feature dans [FEATURE_CONTRACT]["features"] :

□ La feature a une implémentation RÉELLE (pas de simulation) ?
□ Si CDN requis → hook ou composant charge et utilise ce CDN ?
□ Si browser API → le code utilise vraiment cette API ?
□ Si route API requise → la route fait le vrai travail ?
□ Le hook correspondant gère le state complet de la feature ?
□ Le composant feature (si nécessaire) est fonctionnel ?

══════════════════════════════════════════════════════════════════════
CDN DANS LES COMPOSANTS REACT
══════════════════════════════════════════════════════════════════════

Pour les CDN qui doivent être chargés dans layout.tsx, déclare-les en DEPENDENCIES_CDN :
DEPENDENCIES_CDN: ["https://cdn.jsdelivr.net/npm/tone@14/build/Tone.js"]

Pour les CDN utilisables via npm (chart.js, d3, leaflet), utilise npm directement.

Pour les CDN chargés globalement, accède-les via window dans les hooks :
\`\`\`typescript
// Hook Tone.js (CDN global)
declare global { interface Window { Tone: any } }

export function useAudioSynth() {
  const [Tone, setTone] = useState<any>(null);
  useEffect(() => {
    if (window.Tone) { setTone(window.Tone); return; }
    const script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/npm/tone@14/build/Tone.js';
    script.onload = () => setTone(window.Tone);
...
<create_file path="hooks/useWorkflowEngine.ts">
"use client";
... code complet fonctionnel ...
</create_file>

À la fin :
DEPENDENCIES: ["socket.io-client", "idb", "leaflet"]
DEPENDENCIES_CDN: ["https://cdn.jsdelivr.net/npm/tone@14/build/Tone.js"]
...
□ Chaque feature du FEATURE CONTRACT a une implémentation réelle ?
□ Aucun hook/route ne simule avec setTimeout ou retourne des données hardcodées ?
□ "use client" ligne 1 sur tous les hooks et composants features ?
...
□ Tous les CDN nécessaires déclarés ?

NE PAS generer : views, globals.css, layout.tsx, page.tsx, stores, types.
NE PAS generer : composants UI simples (Button, Card, Badge) — ils vont dans VIEWS.
`;

// Alias pour la compatibilité interne
const UI_AGENT_PROMPT = FEATURES_AGENT_PROMPT;
...
Les features sont déjà implémentées dans components/features/ et hooks/.
Tu utilises ces implémentations dans tes vues — tu ne les réécris pas.
      ...
      async start(controller) {
        ...
        async function runAgent(
          ...
          opts: {
            ...
            emitOutput?: boolean;
            noTools?: boolean; // ← si true, n'envoie PAS le tool readFile (évite les interruptions mid-stream)
          } = {}
        ): Promise<string> {
          const { temperature = 1.0, maxTokens = 65536, useChatHistory = false, emitOutput = true, noTools = false } = opts;
            ...
            return await callWithRetry(
              async () => {
                const r = await ai.models.generateContentStream({
                  model: MODEL_ID,
                  contents,
                  // ← CRITIQUE : si noTools=true (ex: fixer), on n'envoie PAS readFile.
                  // Quand le modèle appelle un tool mid-stream, chunk.text devient vide,
        ...
        try {
          ...
          // Extrait la décision (première ligne)
          ...
          // Texte visible = tout sauf la ligne de décision et le code éventuel
          // On garde le filtre anti-code mais SANS couper le reste du texte
...
`;
          ...
          // ═══════════════════════════════════════════════════════════════
          // MODE CODE — pipeline 4 agents (PLANNER → FOUNDATION → FEATURES → VIEWS)
          // ═══════════════════════════════════════════════════════════════
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 0 — PLANNER (silencieux — produit le FEATURE CONTRACT)
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:0/PLANNER]\n");
...
${designAnchor ? `CONTEXTE DE DESIGN EXISTANT :\n${designAnchor}\n` : ""}

${projectContext}

Analyse cette demande et produis le FEATURE CONTRACT complet.
Identifie CHAQUE fonctionnalité demandée, sa stratégie d'implémentation réelle,
les CDN et browser APIs nécessaires, et les contrats de service.
`;

          const plannerCollector = makeSilentCollector();
          await callWithRetry(
            () => ai.models.generateContentStream({
              model: MODEL_ID,
            ...
            (txt) => plannerCollector.collect(txt),
            {}
          );

          const featureContract = plannerCollector.getOutput();
          emit(`\n[PLANNER] ✅ Feature Contract généré (${featureContract.length} chars)\n`);

          // Helper pour injecter le Feature Contract dans les prompts des agents suivants
          const withFeatureContract = (agentInput: string) => `
${agentInput}
...
`;
          ...
          const foundationInput = withFeatureContract(`
DEMANDE UTILISATEUR : "${lastUserMsg}"
...
Le FEATURE CONTRACT ci-dessous liste toutes les features — crée des types exhaustifs
et des services qui les implémentent RÉELLEMENT.
...
`);
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 2 — FEATURES (hooks, routes API, composants fonctionnels)
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:2/CODE_FEATURES]\n");

          const foundationSummary = allGeneratedFiles
            .map((f) => `=== ${f.path} ===\n${f.content}`)
            .join("\n---\n");

          const featuresInput = withFeatureContract(`
DEMANDE UTILISATEUR : "${lastUserMsg}"

${designAnchor}

FICHIERS FONDATION GÉNÉRÉS :
${foundationSummary}

${projectContext}

Implémente TOUTES les features du FEATURE CONTRACT.
Chaque feature doit avoir son hook, ses routes API, et son composant feature si nécessaire.
Utilise readFile() pour consulter les fichiers de fondation si besoin.
Vérifie les noms exacts des méthodes dans les services avant de les appeler.
AUCUNE simulation — TOUT doit être réel.
`);

          const featuresOutput = await runAgent(FEATURES_AGENT_PROMPT, featuresInput, {
            temperature: 1.4,
            maxTokens: 65536,
          });
          flushBuffer();

          mergeGeneratedFiles(parseGeneratedFiles(featuresOutput));
          extractDeps(featuresOutput, "DEPENDENCIES").forEach((d) => globalPkgs.add(d));
          extractDeps(featuresOutput, "DEVDEPENDENCIES").forEach((d) => globalDevPkgs.add(d));

          parseGeneratedFiles(featuresOutput).forEach((f) => {
            for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
              const pkg = m[1].split("/")[0];
              if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
            }
          ...
          const keyFilesSummary = allGeneratedFiles
            .filter((f) =>
              f.path === "types/index.ts" ||
              f.path.includes("store") ||
              f.path.includes("services/") ||
              f.path.includes("hooks/") ||
              f.path.includes("components/features/")
            )
            .map((f) => `=== ${f.path} ===\n${f.content}`)
            .join("\n---\n");

          const viewsInput = withFeatureContract(`
DEMANDE UTILISATEUR : "${lastUserMsg}"

${designAnchor}

FICHIERS CLÉS DÉJÀ GÉNÉRÉS (types, stores, services, hooks, composants features) :
${keyFilesSummary}
...
Les features sont déjà implémentées dans components/features/ et hooks/.
Ton rôle : assembler ces features dans des vues cohérentes et élégantes.
Importe et utilise les composants features déjà créés — ne les réécris pas.
...
`;
              ...
              if (fixedFiles.length > 0) {
                ...
                emit(`\n[TSC:FIXER] ⚠️ Aucun fichier émis par le fixer ce round.\n`);
              }
