...
════════════════════════════════════════════════════════════════════
LOI 2BIS — TAILWIND CSS v4 (configuration obligatoire)
════════════════════════════════════════════════════════════════════

RÈGLE TW-1 — postcss.config.mjs TOUJOURS en premier avec le bon plugin :
  // postcss.config.mjs
  const config = { plugins: { "@tailwindcss/postcss": {} } };
  export default config;
  ❌ JAMAIS : plugins: { tailwindcss: {}, autoprefixer: {} }

RÈGLE TW-2 — app/globals.css syntaxe v4 OBLIGATOIRE :
  @import "tailwindcss";
  ❌ JAMAIS : @tailwind base; @tailwind components; @tailwind utilities;

RÈGLE TW-3 — Si tu utilises @apply dans globals.css :
  @reference "tailwindcss";
  .ma-classe { @apply bg-white/80 backdrop-blur-xl; }
  ❌ JAMAIS @apply sans @reference préalable

RÈGLE TW-4 — Customisations via @theme dans globals.css (pas de tailwind.config.ts) :
  @theme { --color-primary: #1a1a2e; --font-sans: "DM Sans", sans-serif; }
  ❌ NE PAS créer tailwind.config.ts/js

RÈGLE TW-5 — Versions packages :
  dependencies    : "tailwindcss": "^4.0.0"
  devDependencies : "@tailwindcss/postcss": "^4.0.0"
  PAS autoprefixer (natif dans Tailwind v4)
...
DEVDEPENDENCIES: ["@types/howler", "@types/leaflet", ...]
  ...
  FIXER: {
    ...
    icon: "🔧",
    prompt: `
Tu es un expert en débogage Next.js / TypeScript / Tailwind CSS v4.
...
ERREURS TAILWIND CSS v4 CLASSIQUES :
  "trying to use tailwindcss directly as a PostCSS plugin"
    → Réécrire postcss.config.mjs :
      const config = { plugins: { "@tailwindcss/postcss": {} } };
      export default config;
    → Ajouter "@tailwindcss/postcss": "^4.0.0" en devDependency
    → "tailwindcss": "^4.0.0" en dependency (pas ^3.x)

  "Cannot apply unknown utility class bg-white/80"
    → Dans globals.css, ajouter @reference "tailwindcss"; avant le bloc @apply
    → Ou remplacer @apply par les classes directement dans le JSX

  "Missing field negated on ScannerOptions.sources"
    → Conflit de version Tailwind v3 vs v4 — forcer tailwindcss: "^4.0.0"
    → Supprimer tailwind.config.ts/js (inutile en v4)
    → globals.css doit utiliser @import "tailwindcss"; et non les directives @tailwind

  "@tailwind base/components/utilities deprecated"
    → Remplacer par @import "tailwindcss"; dans globals.css

  TEMPLATE globals.css CORRECT pour Tailwind v4 :
    @import "tailwindcss";
    @reference "tailwindcss";
    :root { /* variables CSS custom */ }
    @theme { /* personnalisations Tailwind */ }

  TEMPLATE postcss.config.mjs CORRECT :
    const config = { plugins: { "@tailwindcss/postcss": {} } };
    export default config;
...
DEVDEPENDENCIES: ["@types/X", "@tailwindcss/postcss"] si manquants
    `,
  },
};
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ── Règles Tailwind CSS v4 (injectées dans chaque agent) ──────────────
    // Ces règles évitent les erreurs de build PostCSS/Tailwind les plus fréquentes.
    const TAILWIND_V4_RULES = `
=== 🎨 TAILWIND CSS v4 — RÈGLES OBLIGATOIRES (Next.js 15) ===

RÈGLE TW-1 — postcss.config.js/mjs TOUJOURS avec @tailwindcss/postcss :
  // postcss.config.mjs ← TOUJOURS ce format exact
  const config = {
    plugins: {
      "@tailwindcss/postcss": {},  // ← PAS "tailwindcss": {} qui cause l'erreur PostCSS
    },
  };
  export default config;

  ❌ INTERDIT : plugins: { tailwindcss: {}, autoprefixer: {} }
  ✅ CORRECT  : plugins: { "@tailwindcss/postcss": {} }

RÈGLE TW-2 — app/globals.css avec syntaxe v4 :
  /* app/globals.css ← TOUJOURS ce format exact */
  @import "tailwindcss";

  /* Variables CSS custom ici si besoin */
  :root { --color-primary: #1a1a2e; }

  ❌ INTERDIT : @tailwind base; @tailwind components; @tailwind utilities;
  ✅ CORRECT  : @import "tailwindcss";

RÈGLE TW-3 — @apply dans globals.css avec @reference :
  Si tu utilises @apply dans globals.css avec des classes Tailwind :
  @reference "tailwindcss";  /* ← obligatoire avant tout @apply */
  .mon-composant { @apply bg-white/80 backdrop-blur-xl; }

  ❌ INTERDIT : @apply bg-white/80 sans @reference préalable
  ✅ CORRECT  : @reference "tailwindcss"; puis @apply

RÈGLE TW-4 — tailwind.config.ts ABSENT en v4 :
  Tailwind v4 ne nécessite PAS de tailwind.config.ts.
  Les customisations se font via CSS dans globals.css :
  @theme { --color-primary: #1a1a2e; --font-sans: "DM Sans", sans-serif; }
  ❌ NE PAS créer tailwind.config.ts ou tailwind.config.js

RÈGLE TW-5 — packages requis pour Tailwind v4 :
  dependencies     : tailwindcss (^4.0.0)
  devDependencies  : @tailwindcss/postcss (^4.0.0)
  PAS autoprefixer (intégré nativement dans Tailwind v4)

RÈGLE TW-6 — Version Tailwind dans package.json :
  "tailwindcss": "^4.0.0"      ← v4 (pas ^3.x)
  "@tailwindcss/postcss": "^4.0.0" en devDependencies
`;
...
(JAMAIS de markdown, JAMAIS de backticks)
        ...
        try {
          ...
          // ── PHASE 3 : FICHIERS D'INFRASTRUCTURE TAILWIND v4 (toujours générés) ──
          // Ces fichiers sont émis SYSTÉMATIQUEMENT pour éviter les erreurs PostCSS/Tailwind.
          const tailwindInfraFiles = `<create_file path="postcss.config.mjs">
// Tailwind CSS v4 — @tailwindcss/postcss obligatoire (pas "tailwindcss" directement)
const config = {
  plugins: {
    "@tailwindcss/postcss": {},
  },
...
/* Tailwind CSS v4 — @import obligatoire (pas @tailwind base/components/utilities) */
@import "tailwindcss";

/* Si tu utilises @apply, déclare @reference en premier */
@reference "tailwindcss";

/* Thème et variables CSS custom */
@theme {
...
* {
  ...
  padding: 0;
          ...
          send(tailwindInfraFiles);

          // ── PHASE 4 : RÉSOLUTION AUTOMATIQUE DES PACKAGES ────────────────
          ...
          // @tailwindcss/postcss est requis en devDep pour Tailwind v4
          globalDevPackages.add("@tailwindcss/postcss");
          ...
          const finalDevDeps: Record<string, string> = {
            ...
            "@types/react-dom": "^19",
            postcss: "^8",
            "@tailwindcss/postcss": "^4.0.0",  // requis par Tailwind v4
            eslint: "^8",
