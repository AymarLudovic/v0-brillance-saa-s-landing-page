...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // ─────────────────────────────────────────────────────────────
          // PHASES 1-3 — 3 appels directs à ai.models.generateContentStream
          // Chaque appel reçoit le contexte cumulatif des précédents
          // ─────────────────────────────────────────────────────────────

          const mergeRoundOutput = (output: string) => {
            mergeGeneratedFiles(parseGeneratedFiles(output));
            const editOps = parseEditFileOps(output);
            if (editOps.length > 0) {
              const r = applyEditFileOpsToFiles(allGeneratedFiles, editOps);
              if (r.applied > 0) emit(`\n[EDIT_FILE] ✅ ${r.applied} op(s)\n`);
            }
            extractDeps(output, "DEPENDENCIES").forEach(d => globalPkgs.add(d));
            extractDeps(output, "DEVDEPENDENCIES").forEach(d => globalDevPkgs.add(d));
            extractDeps(output, "PYPACKAGES").forEach(d => globalPyPkgs.add(d));
            parseGeneratedFiles(output).forEach(f => {
              for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
                const pkg = m[1].split("/")[0];
                if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
              }
          ...
          const ROUND_LABELS = [
            "ROUND 1 — CONSTRUIRE : génère l'application complète de zéro",
            "ROUND 2 — AUDITER ET COMPLÉTER : lis les fichiers, corrige ce qui manque, ajoute ce qui est absent",
            "ROUND 3 — FINALISER : vérifie la connexion UI↔Python, les états, le design",
          ];

          for (let round = 1; round <= 3; round++) {
            emit(`\n[PHASE:${round}/SOLO_DEV]\n`);
            await sleep(round === 1 ? 1000 : 3000);

            // Context cumulatif : liste des fichiers générés + contenu des fichiers clés
            const roundAllPaths = allGeneratedFiles.map(f => f.path).join("\n");
            const roundKeyFiles = round > 1 ? allGeneratedFiles
              .filter(f =>
                f.path === "backend/main.py" ||
                f.path.startsWith("backend/services/") ||
...
${round > 1 ? `FICHIERS DÉJÀ GÉNÉRÉS (${allGeneratedFiles.length} fichiers) :
${roundAllPaths}

FICHIERS CLÉS (backend + hooks + page) :
${roundKeyFiles}` : ""}

${projectContext}
`;

            // Parts pour le stream
            const roundParts: any[] = [];
            if (effectiveReferenceImages.length > 0) {
              effectiveReferenceImages.slice(0, 2).forEach(img =>
                roundParts.push({ inlineData: { data: cleanBase64(img), mimeType: getMimeType(img) } })
              );
            }
            roundParts.push({ text: roundUserContent });

            const roundOutput = await callWithRetry(
              () => ai.models.generateContentStream({
                model: MODEL_ID,
                contents: [{ role: "user", parts: roundParts }],
                tools: round > 1 ? [{ functionDeclarations: [readFileDecl] }] : undefined,
                config: {
                  systemInstruction: `${BASE_SYSTEM_PROMPT}\n\n${SOLO_DEV_PROMPT}`,
              ...
              onChunk,
              { maxAttempts: 4, baseDelay: 12000, onThought: (t) => emit(`[THOUGHT:SOLO_R${round}]${t}[/THOUGHT:SOLO_R${round}]`), onUsage }
            );
            flushBuffer();
            mergeRoundOutput(roundOutput);

            // Validator après round 1
            if (round === 1) {
              const mainPy = allGeneratedFiles.find(f => f.path === "backend/main.py");
              if (mainPy) {
                const bRoutes = [...mainPy.content.matchAll(/@app\.(get|post|put|delete|patch|websocket)\s*\(/gi)]
                  .filter(m => !mainPy.content.slice(m.index, m.index + 60).includes('/health')).length;
                emit(`[VALIDATOR] Round 1 → ${bRoutes} route(s) Python métier dans main.py\n`);
                if (bRoutes < 3) emit(`[VALIDATOR] ⚠️ Backend insuffisant — Round 2 va compléter\n`);
              } else {
                emit(`[VALIDATOR] ⚠️ backend/main.py ABSENT après Round 1\n`);
              }
            }
          }
