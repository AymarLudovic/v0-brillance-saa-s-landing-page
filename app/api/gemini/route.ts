...
□ Vues générées avec "use client" ligne 1 ?
□ Vues appellent les hooks et stores que tu as générés (pas de fetch inventé) ?
□ Aucun onClick vide dans les vues ?

══════════════════════════════════════════════════════════════════════
ÉTAPE 5 — GÉNÈRE LES VUES FONCTIONNELLES (components/views/*.tsx)
══════════════════════════════════════════════════════════════════════

⚡ MISSION CRITIQUE : Génère TOUTES les vues de l'application, câblées aux hooks,
stores et endpoints Python que tu viens de construire. Ces vues seront importées
directement dans app/page.tsx par l'agent VIEWS — elles doivent être COMPLÈTES.

Pour chaque section/écran de l'application :

components/views/*View.tsx
  ✅ "use client"; LIGNE 1 ABSOLUMENT
  ✅ NAMED export : export function DashboardView() {...}  JAMAIS export default
  ✅ Importe les hooks que tu as générés : import { useXxx } from '@/hooks/useXxx';
  ✅ Importe les stores : import { useXxxStore } from '@/stores/useXxxStore';
  ✅ Importe les composants UI : import { Button } from '@/components/ui/Button';
  ✅ Importe les services si besoin : import { xxxService } from '@/services/xxxService';
  ✅ CHAQUE bouton déclenche une vraie action (store action ou appel hook — jamais onClick vide)
  ✅ CHAQUE formulaire : useState par field + onSubmit qui appelle l'action store
  ✅ Filtres/Recherche : onChange en temps réel avec filtre useMemo
  ✅ Listes : données depuis store/hook — états loading/error/empty gérés avec call-to-action
  ✅ Données mock réalistes si données réelles non chargées (min 5-8 entrées)
  ✅ ZÉRO placeholder, ZÉRO "Bientôt disponible", ZÉRO TODO dans le rendu

  Si un design_reference est fourni dans ta demande → respecte sa palette et ses tokens.
  Utilise bg-[#hex] pour les couleurs exactes extraites du design.

  Pattern à suivre (câblage fonctionnel complet) :
  \`\`\`tsx
  "use client";
  ...
  export function XxxView() {
    const { items, loading, error, addItem, deleteItem } = useXxxStore();
...
⚠️ RÈGLE ABSOLUE :
- Ne crée JAMAIS un appel fetch qui n'a pas d'endpoint correspondant dans backend/main.py
- Ne crée JAMAIS un bouton sans handler fonctionnel
- Utilise UNIQUEMENT les hooks et stores que tu as toi-même générés à l'étape 4

TU GÉNÈRES : hooks/, components/ui/, components/views/*.tsx
NE PAS générer : globals.css, layout.tsx, app/page.tsx.
Ces fichiers sont la responsabilité de l'agent VIEWS (qui assemblera tes vues dans page.tsx).
`;
...
⚡ RÈGLE FONDAMENTALE : Tu interviens EN DERNIER. Deux agents ont déjà travaillé avant toi.
  ...
  • Checker a généré : hooks React /api/py/..., composants UI ET les vues fonctionnelles (components/views/*.tsx)
Tu NE réécris PAS leur travail de zéro — tu améliores le design des vues existantes et tu assembles tout.
...
⚡ LES VUES FONCTIONNELLES EXISTENT DÉJÀ (générées par Checker).
Ton rôle : les améliorer visuellement si nécessaire + générer les fichiers d'assemblage.

1. components/views/*View.tsx — DÉJÀ GÉNÉRÉES PAR CHECKER
   - Lis-les avec readFile() et identifie les améliorations visuelles à apporter
   - Si une vue manque une section présente dans les images de référence → ajoute-la (edit_file)
   - Si le design d'une vue n'est pas fidèle au design_reference → améliore-le (edit_file)
   - NE réécris PAS complètement une vue si elle est déjà fonctionnellement câblée
   - NE crée PAS une nouvelle vue si une équivalente existe déjà dans Checker
   - EXCEPTION : si une vue nécessaire est absente de Checker → génère-la complètement (avec câblage)

   Critères pour une vue à améliorer visuellement :
   - Design générique ou non-fidèle au design_reference → edit_file pour appliquer la palette
   - Layout ne correspondant pas aux images de référence → edit_file pour corriger le layout
   - Micro-interactions absentes (hover, transition, animation) → edit_file pour les ajouter
...
□ app/page.tsx existe et importe TOUTES les vues (celles de Checker + celles que tu as ajoutées) ?
□ Chaque vue est bien importée dans page.tsx ?
□ Pas de view orpheline non référencée dans page.tsx ?
□ La navigation entre les vues fonctionne (activeTab ou router) ?
□ Les vues améliorées utilisent toujours les mêmes hooks/stores qu'avant (pas cassés) ?
...
□ Les vues modifiées via edit_file conservent leur câblage fonctionnel (hooks/stores) ?
□ page.tsx importe TOUTES les vues Checker + celles ajoutées ici ?
`;
...
Tu es l'Agent d'Intégration Fonctionnelle. Tu interviens APRÈS les agents Foundation/Checker/Views.
Ton seul but : t'assurer que chaque élément visible dans l'app fonctionne RÉELLEMENT.

⚡ RÈGLE DESIGN INVIOLABLE :
Si un DESIGN CONTRACT est fourni dans ta demande, tu le respectes ABSOLUMENT.
Tu NE changes JAMAIS les couleurs, classes Tailwind, ou le layout d'une vue qui fonctionne déjà.
Tu corriges UNIQUEMENT les interactions manquantes (onClick vide, formulaire non câblé...).
Le design appartient aux agents Design/Views — pas à toi.
...
5. Ne touche PAS au design (classes Tailwind, couleurs, layout) — sauf si elles empêchent le fonctionnement
...
6. GÉNÈRE TOUTES LES VUES (components/views/*.tsx) qui appellent les hooks et endpoints que tu viens de créer — câblage fonctionnel COMPLET requis
...
`;
          ...
          // UI layer: hooks, components AND views generated by CHECKER (for VIEWS to improve/assemble)
          ...
          // Views already generated by CHECKER — VIEWS agent must read them and assemble page.tsx
...
2. VÉRIFIE que chaque section est présente dans les vues Checker — si une manque, ajoute-la
...
⚡ RÈGLE ABSOLUE POUR TON RÔLE :
Checker a déjà généré les vues fonctionnelles câblées aux hooks et endpoints Python.
- IMPORTE les vues Checker telles quelles dans page.tsx — ne les recrée PAS de zéro
- Si une vue a besoin d'améliorations visuelles → utilise edit_file (jamais create_file complet)
...
══════════════════════════════════════════════════════════════════════
VUES DÉJÀ GÉNÉRÉES PAR CHECKER (à importer dans page.tsx) :
${checkerViewsSummary || "(aucune vue générée par Checker — génère-les toi-même avec câblage complet)"}
...
══════════════════════════════════════════════════════════════════════
INSTRUCTIONS :
- Génère globals.css, layout.tsx et app/page.tsx (qui importe et assemble les vues Checker)
- Améliore visuellement les vues existantes via edit_file si le design ne correspond pas au design_reference
- Si une vue manque entièrement → génère-la avec câblage complet (hooks + stores)
...
`;
...

          {
            ...
            const integratorInput = [
              ...
              "IMPORTANT : Si tu modifies une vue, respecte le DESIGN CONTRACT ci-dessus — ne change pas les couleurs ni les classes CSS.",
              "",
