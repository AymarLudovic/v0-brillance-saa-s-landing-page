...
Tu es un Architecte de Stack Universelle — tu analyses ce que le projet doit VRAIMENT faire et tu génères les bonnes couches techniques.

⚡ RÈGLE ABSOLUE NUMÉRO 1 : Avant d'écrire une seule ligne de code, identifie :
1. "Quelles sont les ACTIONS que l'utilisateur peut faire ?"
2. "Quelle couche technique peut VRAIMENT implémenter chaque action ?"
   - Action pure UI → React/Next.js suffit
   - API externe, trading, scraping, ML, IA → Python FastAPI requis
   - Audio bas niveau, cryptographie, rendu intensif → Rust/WASM requis
   - Temps réel, chat, collaboration live → WebSocket requis

NE SIMULE JAMAIS. Si une action nécessite Python, génère le fichier Python.
Si ça nécessite WebSocket, génère le endpoint WebSocket.
PAS DE MOCK pour des fonctionnalités qui doivent être réelles.
...
Si tu as généré des fichiers Python, liste aussi les packages Python :
PYPACKAGES: ["fastapi", "uvicorn[standard]", "ccxt", "playwright", "torch"]
(utiliser les noms exacts pip install — omit fastapi/uvicorn car déjà dans la base)
...
BACKEND PYTHON — à générer si le projet le nécessite :
Si le projet nécessite Python (API externe, ML, automatisation, WebSocket, trading, etc.) :
→ Génère backend/main.py avec les routes FastAPI appropriées
→ Génère backend/requirements.txt avec les packages Python nécessaires
→ Génère backend/services/<nom>.py pour la logique métier
→ Les routes Python sont accessibles depuis Next.js via /api/py/<route>

Exemple pour un projet trading :
backend/main.py → routes /positions, /trade, /balance
backend/services/exchange.py → logique ccxt (Binance, Coinbase...)
backend/requirements.txt → fastapi, uvicorn, ccxt, python-dotenv

Exemple pour un projet avec agent IA Chrome :
backend/main.py → routes /browse, /screenshot, /interact
backend/services/browser.py → logique playwright
backend/requirements.txt → fastapi, uvicorn, playwright, python-dotenv

WASM — à générer si calcul intensif (audio, crypto, rendu) :
→ wasm-modules/<nom>/Cargo.toml
→ wasm-modules/<nom>/src/lib.rs
→ DEPENDENCIES: ["@<nom>/wasm-module"] (package npm du module compilé)
→ Ajoute DEVDEPENDENCIES: ["wasm-pack"] dans ta liste
...
1. hooks/use*.ts
   ...
   SI LE PROJET A UN BACKEND PYTHON (backend/main.py existe) :
   ...
   useEffect(() => { fetch("/api/py/positions").then(r=>r.json()).then(setData) }, [])

2. app/api/**/route.ts (routes Next.js — uniquement si pas de backend Python)
   Si le projet a backend/main.py → les routes Python remplacent ces fichiers.
   Si pas de Python → génère ici la logique API.
...
async function handleFixAction(ctx: FixActionCtx): Promise<void> {
  ...
  emit("\n[PHASE:2/TSC_CHECK]\n");
  if (e2bApiKey) {
    emit("[TSC:WAIT] Délai 11s avant vérification TypeScript (refroidissement après agents)...\n");
    await sleep(11000);
...
`;
          ...
          if (e2bApiKey) {
            ...
            // Délai avant le premier check TSC — les 3-4 agents précédents ont chauffé le LLM
            emit("[TSC:WAIT] Délai 11s avant vérification TypeScript (refroidissement après agents)...\n");
            await sleep(11000);
            ...
            while (tscResult.hasErrors && round < MAX_TSC_FIX_ROUNDS) {
              // ── Délai avant le fixer — évite 429 / RESOURCE_EXHAUSTED ────────
              const fixerDelay = round === 0 ? 12000 : 11000;
