...
// =============================================================================
// AGENT 1 — ARCHITECT PROMPT (temperature: 0.7)
// Génère l'application complète avec haute créativité
// =============================================================================

const ARCHITECT_PROMPT = `
Tu es un Ingénieur Full Stack Senior + Product Designer.
Tu construis de vraies applications ambitieuses, pas des maquettes.
CRÉATIVITÉ MAXIMALE sur le design et les fonctionnalités.
...
Liste TOUTES les fonctionnalités d'un vrai produit professionnel.

PHASE 2 — FEATURE INVENTORY COMPLET
Pour chaque feature : service → store → hook → composant → wire complet.
Aucune fonctionnalité ne reste déconnectée.
...
PHASE 4 — WIRE VERIFICATION
  ...
  □ Chaque nav item → View distincte avec vrai contenu
  □ Chaque service → importé ET appelé dans un composant
  □ Aucun "TODO" ou "Coming soon"

PHASE 5 — SELF-AUDIT
  □ "use client" dans tout tsx avec hooks/Zustand/window
  □ Zustand create() = VRAIES implémentations avec VIRGULES
  □ Aucun import vers fichier inexistant
  □ ZÉRO placeholder, ZÉRO "Under development"
  □ Imports internes → alias @/ uniquement

════════════════════════════════════════════════════════════
LES 7 CRIMES (violations = refaire le fichier)
════════════════════════════════════════════════════════════

CRIME 1 — SANDBOX : Math.random() pour des données réelles
...
CRIME 5 — ATOMIC NEGLECT : topbar avec boutons sans handler
...
services/[domain].ts → logique pure, API calls réels
stores/[name]Store.ts → Zustand typé, vraies implémentations
...
&apos; UNIQUEMENT dans texte JSX, JAMAIS dans string TS (case 'home': pas case 'home&apos;':)
...
// =============================================================================
// AGENT 2 — CRITIC PROMPT (temperature: 0.3)
// Reçoit tous les fichiers générés, détecte les problèmes, corrige sans casser
// =============================================================================

const CRITIC_PROMPT = `
Tu es un Architecte Senior TypeScript/Next.js avec un œil de faucon pour les bugs.
Tu as reçu une application complète générée par l'Agent 1.
TON SEUL RÔLE : détecter et corriger les erreurs — SANS ajouter de nouvelles features.

════════════════════════════════════════════════════════════
RÈGLE N°1 — NE PAS CASSER CE QUI FONCTIONNE
════════════════════════════════════════════════════════════

Tu reçois l'intégralité du code source de l'application.
DROIT ABSOLU : corriger les erreurs de syntaxe, imports, types.
INTERDIT ABSOLU : 
  - Ajouter de nouvelles fonctionnalités
  - Supprimer des fonctionnalités existantes
  - Changer la structure/design/CSS
  - Remplacer du code fonctionnel par des placeholders
  - Émettre un fichier si tu n'as PAS de correction à y apporter

════════════════════════════════════════════════════════════
CHECKLIST D'AUDIT (vérifie CHAQUE fichier)
════════════════════════════════════════════════════════════

□ ZUSTAND STORES (.ts avec create<>):
  - Dans create(): chaque méthode = vraie implémentation + virgule finale
  - INTERDIT: setX: () => void; dans le corps du create()
  - Interface séparée: les signatures peuvent avoir void;
  - Semicolons dans l'objet create() → remplace par virgules

□ "use client" MANQUANT (.tsx):
  - Présence de useState/useEffect/useRef/useCallback → "use client" requis
  - Présence de useRouter/usePathname → "use client" requis  
  - Présence de window/document → "use client" requis
  - Présence d'un store Zustand → "use client" requis

□ IMPORTS CASSÉS:
  - Imports relatifs ../ → remplace par @/
  - Identifiants importés deux fois → déduplique
  - Import de fichier inexistant → crée le fichier minimal ou corrige l'import

□ JSX vs TS STRING LITERALS:
  - &apos; dans case 'home': → remplace par apostrophe droite
  - &apos; dans useState('val') → remplace par apostrophe droite
  - ' non échappée dans texte JSX >L'app< → remplace par &apos;

□ NEXT.JS 15:
  - params: {id: string} → params: Promise<{id: string}>
  - Accès params.id sans await → (await params).id
  - Route handlers sans export nommé → ajoute export GET/POST

□ SERVICES DÉCONNECTÉS:
  - Service créé mais jamais importé → ajoute l'import dans le composant concerné
  - Store créé mais jamais utilisé → wire dans le composant approprié

□ FONCTIONNALITÉS VIDES:
  - onClick={() => {}} sur boutons principaux → implémentation réelle
  - "TODO" ou "Coming soon" → contenu réel
  - Views identiques → différencie leur contenu

════════════════════════════════════════════════════════════
PROTOCOLE DE RÉPONSE
════════════════════════════════════════════════════════════

1. Analyse mentalement TOUS les fichiers reçus.
2. Identifie les problèmes avec leur localisation exacte.
3. Émets UNIQUEMENT les fichiers qui ont des corrections réelles.
4. Chaque fichier émis doit être COMPLET (pas de diff, pas de "...").
5. Préserve intégralement le CSS/design/structure des fichiers non cassés.

FORMAT:
<create_file path="stores/useMarketStore.ts">
... fichier entier corrigé ...
</create_file>

Puis 2-3 lignes en français listant ce qui a été corrigé.
NE PAS réécrire les fichiers qui n'ont pas d'erreur.
`;

// =============================================================================
// AGENT 3 — FIXER PROMPT (temperature: 0.1)  
// Correction ultra-précise sur erreurs spécifiques du CRITIC
// =============================================================================

const FIXER_PROMPT = `
Tu es un compilateur TypeScript/Next.js de précision chirurgicale.
Tu reçois : (1) les erreurs identifiées par l'Agent 2, (2) les fichiers concernés.
Tu CORRIGES uniquement ce qui est listé. Tu ne touches à RIEN d'autre.

════════════════════════════════════════════════════════════
RÈGLES ABSOLUES
════════════════════════════════════════════════════════════

1. UNIQUEMENT les fichiers avec des erreurs confirmées.
2. Fichier complet corrigé — jamais de diff ou placeholder.
3. PRÉSERVE intégralement : CSS, classes Tailwind, noms de variables, structure.
4. INTERDIT : ajouter des features, changer le design, modifier les fichiers sans erreur.

════════════════════════════════════════════════════════════
CORRECTIONS PRIORITAIRES
════════════════════════════════════════════════════════════

ZUSTAND "Expression expected" / "Expected ',' got ';'":
  AVANT: setSelectedSymbol: () => void;
  APRÈS: setSelectedSymbol: (symbol) => set({ selectedSymbol: symbol }),

ZUSTAND semicolons → virgules dans create():
  AVANT: selectedSymbol: 'BTC';
  APRÈS: selectedSymbol: 'BTC',

"use client" manquant:
  → Ajoute "use client"; en toute première ligne
...
1 phrase expliquant la correction. RIEN D'AUTRE.
`;
...
`;
          ...
          // ════════════════════════════════════════════════════════════════
          // MODE BUILD — PIPELINE MULTI-AGENTS (3 AGENTS EN CASCADE)
          // ════════════════════════════════════════════════════════════════
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // AGENT 1 — ARCHITECT (température haute pour la créativité)
          // ─────────────────────────────────────────────────────────────
          emit("\n[AGENT:1/ARCHITECT]\n");
          ...
          const agent1Response = await ai.models.generateContentStream({
            model: MODEL_ID,
            contents,
            tools: [{ functionDeclarations: [readFileDecl] }],
            config: {
          ...
          for await (const chunk of agent1Response) {
            const txt = chunk.text;
            if (txt) {
              agent1Output += txt;
              buffer += txt;
          ...
          // Capture fichiers Agent 1
          for (const f of parseGeneratedFiles(agent1Output)) {
          ...
          // ── Correcteur programmatique après Agent 1 ───────────────────
          const zustandFixed1 = allGeneratedFiles.map(f => ({
            ...f,
            content: (f.path.includes("store") || f.path.includes("Store"))
              ? fixZustandStore(f.content) : f.content,
          }));
          for (let i = 0; i < zustandFixed1.length; i++) {
            if (zustandFixed1[i].content !== allGeneratedFiles[i].content) {
              allGeneratedFiles[i] = zustandFixed1[i];
            }
          }
          const { files: postAgent1Fixed } = runFixer(allGeneratedFiles);
          for (let i = 0; i < postAgent1Fixed.length; i++) {
            allGeneratedFiles[i] = postAgent1Fixed[i];
          }

          // ─────────────────────────────────────────────────────────────
          // AGENT 2 — CRITIC (analyse TOUS les fichiers, corrige sans casser)
          // ─────────────────────────────────────────────────────────────
          emit("\n[AGENT:2/CRITIC]\n");

          // Construit le contexte complet pour le CRITIC (tous les fichiers)
          const allFilesContext = allGeneratedFiles
            .map(f => `\n=== FILE: ${f.path} ===\n${f.content}`)
            .join("\n");

          const criticContext = `
Tu reçois l'intégralité du code source généré par l'Agent 1.
Analyse CHAQUE fichier. Corrige UNIQUEMENT les erreurs réelles.
NE PAS modifier les fichiers sans erreurs.
...
1. Stores Zustand : corps du create() avec vraies implémentations + virgules
2. "use client" manquant dans les tsx avec hooks/Zustand
3. Imports relatifs → @/...
4. &apos; dans strings TS → apostrophe droite
5. Services créés mais jamais importés dans leurs composants
6. Boutons et nav items sans handler réel
7. Views sans contenu réel (Coming soon, TODO)
`;

          const agent2Contents: { role: "user" | "model"; parts: any[] }[] = [
            { role: "user", parts: [{ text: criticContext }] },
          ];

          let agent2Output = "";

          const agent2Response = await ai.models.generateContentStream({
            model: MODEL_ID,
            contents: agent2Contents,
            config: {
              systemInstruction: `\n\n${CRITIC_PROMPT}`,
              temperature: 0.3,
          ...
          for await (const chunk of agent2Response) {
            const txt = chunk.text;
            if (txt) {
              agent2Output += txt;
              buffer += txt;
          ...
          // Merge corrections Agent 2 dans allGeneratedFiles
          for (const f of parseGeneratedFiles(agent2Output)) {
            const idx = allGeneratedFiles.findIndex(g => g.path === f.path);
            if (idx >= 0) allGeneratedFiles[idx] = f; else allGeneratedFiles.push(f);
            createdPaths.add(f.path);
          }
          ...
          // ── Correcteur programmatique après Agent 2 ───────────────────
          const zustandFixed2 = allGeneratedFiles.map(f => ({
            ...f,
            content: (f.path.includes("store") || f.path.includes("Store"))
              ? fixZustandStore(f.content) : f.content,
          }));
          for (let i = 0; i < zustandFixed2.length; i++) {
            if (zustandFixed2[i].content !== allGeneratedFiles[i].content) {
              allGeneratedFiles[i] = zustandFixed2[i];
            }
          }
          const { files: postAgent2Fixed } = runFixer(allGeneratedFiles);
          for (let i = 0; i < postAgent2Fixed.length; i++) {
            allGeneratedFiles[i] = postAgent2Fixed[i];
          }

          // ─────────────────────────────────────────────────────────────
          // AGENT 3 — FIXER (correction chirurgicale des erreurs résiduelles)
          // ─────────────────────────────────────────────────────────────
          emit("\n[AGENT:3/FIXER]\n");

          // Détecte les fichiers potentiellement encore problématiques
          const potentialIssues = allGeneratedFiles.filter(f => {
            const c = f.content;
            return (
              // Zustand : void dans create()
              (f.path.endsWith(".ts") && c.includes("create<") &&
                /create<[^>]+>[\s\S]*?\(\s*\([^)]*\)\s*=>\s*\(?\s*\{[\s\S]{0,5000}:\s*\(\s*\)\s*=>\s*void\s*;/.test(c)) ||
              // "use client" manquant
          ...
          if (potentialIssues.length > 0) {
            const issueContext = potentialIssues
              .map(f => `\n=== FILE: ${f.path} ===\n${f.content}`)
              .join("\n");

            const fixerContext = `
L'Agent 2 a fait une review. Voici les fichiers qui ont encore potentiellement des erreurs :

${issueContext}

Corrige UNIQUEMENT les erreurs réelles listées dans ton prompt système.
Ne touche à RIEN d'autre. Fichier complet pour chaque correction.
`;

            const agent3Contents: { role: "user" | "model"; parts: any[] }[] = [
              { role: "user", parts: [{ text: fixerContext }] },
            ];

            let agent3Output = "";

            const agent3Response = await ai.models.generateContentStream({
              model: MODEL_ID,
              contents: agent3Contents,
              config: {
                systemInstruction: `\n\n${FIXER_PROMPT}`,
                temperature: 0.1,
                maxOutputTokens: 16384,
            ...
            for await (const chunk of agent3Response) {
              const txt = chunk.text;
              if (txt) {
                agent3Output += txt;
                buffer += txt;
            ...
            // Merge corrections Agent 3
            for (const f of parseGeneratedFiles(agent3Output)) {
              const idx = allGeneratedFiles.findIndex(g => g.path === f.path);
              if (idx >= 0) allGeneratedFiles[idx] = f; else allGeneratedFiles.push(f);
            }
          }

          // ── Passe programmatique finale (2 passes) ────────────────────
          const zustandFinal = allGeneratedFiles.map(f => ({
            ...f,
            content: (f.path.includes("store") || f.path.includes("Store"))
              ? fixZustandStore(f.content) : f.content,
          }));
          for (let i = 0; i < zustandFinal.length; i++) {
            if (zustandFinal[i].content !== allGeneratedFiles[i].content) {
              allGeneratedFiles[i] = zustandFinal[i];
              emit(`<create_file path="${zustandFinal[i].path}">\n${zustandFinal[i].content}\n</create_file>`);
            }
          }
