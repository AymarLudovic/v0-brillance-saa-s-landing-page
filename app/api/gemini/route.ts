...
- User asks to fix a bug → edit_file on the affected lines only. Do NOT regenerate other files. Do NOT re-explain the whole project.
...
export async function POST(req: Request) {
  try {
    ...
    if (!isAnthropic) {
      ...
      const functionalityReminder = `[INSTRUCTION CRITIQUE — À APPLIQUER IMMÉDIATEMENT AVANT TOUT CODE]

⚠️ TEST OBLIGATOIRE avant d'écrire la moindre ligne : Pour chaque fonctionnalité que tu vas construire, pose-toi cette question brutale : "Si l'utilisateur ouvre l'app maintenant et clique dessus, est-ce que ça fait vraiment quelque chose de réel ?" Si la réponse est non, ou "ça simule", ou "ça affiche juste" — tu n'as pas encore construit la fonctionnalité. Tu as construit une image de la fonctionnalité.

VOICI CE QUI N'EST PAS UNE FONCTIONNALITÉ :
- Un lecteur vidéo dont le bouton play change juste une icône → ce n'est pas un lecteur
- Un bouton "Generate" qui affiche un spinner puis du texte hardcodé → ce n'est pas une génération
- Une barre de progression qui est juste un div stylé → ce n'est pas une timeline
- Des onglets qui switchent → ce n'est pas une fonctionnalité, c'est de la navigation UI basique
- Une transcription hardcodée dans un tableau → ce n'est pas de la transcription
- Un formulaire qui ne soumet rien nulle part → ce n'est pas un formulaire

VOICI CE QU'EST UNE VRAIE FONCTIONNALITÉ :
- Un lecteur vidéo : élément <video> HTML5 réel avec src, event timeupdate toutes les 250ms qui met à jour currentTime dans le state, scrubbing calculé via getBoundingClientRect sur click, onEnded, onLoadedMetadata pour la durée totale
- Un bouton Generate : appel fetch vers /api/generate avec le vrai contenu en body, await de la réponse, affichage du résultat retourné par l'API — pas du texte inventé
- Une transcription synchronisée : chaque segment a startTime/endTime, pendant la lecture le segment dont startTime <= currentTime <= endTime est highlighted, clic sur segment appelle videoRef.current.currentTime = segment.startTime
- Un formulaire : onSubmit qui fait fetch POST vers une API route, données stockées dans une vraie base ou localStorage, liste qui se met à jour après soumission

RÈGLE ABSOLUE : Chaque bouton, chaque action, chaque feature doit avoir une conséquence réelle et persistante. L'utilisateur doit pouvoir utiliser ce que tu construis maintenant, pas "quand on connectera une vraie API plus tard."

QUAND TU AS FINI, explique ce que l'utilisateur peut FAIRE — pas ce que tu as codé techniquement. Exemple correct : "Appuyez sur play : la timeline avance en temps réel. Cliquez n'importe où sur la barre de progression pour sauter à ce moment. Cliquez sur une ligne de transcription pour que la vidéo saute à ce timestamp exact." Exemple interdit : "J'ai utilisé useState, useRef et useEffect pour gérer l'état du lecteur."

---
DÉTECTION D'INTENTION (applique ceci en plus) :

→ Si l'utilisateur salue, pose une question ou discute : réponds en texte, aucun fichier.
→ Si l'utilisateur demande une correction ou petite modif : edit_file chirurgical sur les lignes concernées uniquement, rien d'autre. Confirme sobrement ce que tu as changé.
→ Si l'utilisateur demande un nouveau build : applique tout ce qui précède et construis vraiment.
