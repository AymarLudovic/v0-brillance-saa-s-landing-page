...
╔══════════════════════════════════════════════════════════════════════╗
║  FONCTIONNALITÉS RÉELLES — ZÉRO MOCK, ZÉRO SIMULATION              ║
╚══════════════════════════════════════════════════════════════════════╝
...
CODE_ACTION : Confirme la demande en 3-4 phrases. Décris ce que l'utilisateur va VIVRE (jamais technique). Tu peux et dois aussi écrire du texte pour expliquer brièvement ta démarche.
...
export async function POST(req: Request) {
  try {
    ...
    const {
      ...
      designOnly        = false,
      requestedVibes    = [] as { category: string; count: number }[],
    } = body;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          // ══ MODE DESIGN ONLY — lancé par sendChat après détection <request_vibes> ══
          if (designOnly) {
            emit("[PHASE:0/DESIGN]\n");
            emit("[DESIGN:THINKING] Chargement des vibes demandées...\n");
            const vibeImgs: string[] = [];
            for (const req of (requestedVibes as { category: string; count: number }[])) {
              const catImgs = (vibesByCategory[req.category] ?? []) as string[];
              vibeImgs.push(...catImgs.slice(0, req.count));
            }
            if (vibeImgs.length === 0) {
              for (const imgs of Object.values(vibesByCategory)) {
                vibeImgs.push(...(imgs as string[]).slice(0, 2));
                if (vibeImgs.length >= 6) break;
              }
            }
            emit(`[DESIGN:THINKING] ${vibeImgs.length} vibes chargés\n`);
            const designContentsOnly: any[] = [];
            const refPartsOnly = vibeImgs.map((img: string) => ({
              inlineData: { data: cleanBase64(img), mimeType: getMimeType(img) }
            }));
            const designInputOnly = `Demande : "${lastUserText}"\nMISSION : Génère le HTML/CSS pixel-perfect avec variables :root{} depuis ces images de référence de design.`;
            designContentsOnly.push({ role: "user", parts: [...refPartsOnly, { text: designInputOnly }] });
            try {
              const designOutputOnly = await callAISilent(isAnthropic, anthropic, ai, MODEL_ID, DESIGN_AGENT_PROMPT, designContentsOnly, { temperature: 1.0, maxTokens: 65536 });
              const designMatchOnly = designOutputOnly.match(/<design_reference>([\s\S]*?)<\/design_reference>/);
              if (designMatchOnly && designMatchOnly[1].length > 200) {
                const htmlRef = designMatchOnly[1].trim();
                const anchor = buildDesignAnchor(htmlRef);
                emit(`\n---\n<create_file path="${DESIGN_ANCHOR_FILE}">\n${anchor}\n</create_file>\n`);
                emit(`\n[DESIGN:READY] ✅ Design anchor généré depuis les vibes (${htmlRef.length} chars)\n`);
              } else {
                emit("[DESIGN:SKIP] Balise design_reference absente.\n");
              }
            } catch (e: any) {
              emit(`[DESIGN:SKIP] Design agent erreur: ${String(e?.message ?? "").slice(0, 60)}\n`);
            }
            emit("\n[PAGE_DONE]\n");
            controller.close();
            return;
          ...
          // Déclenché UNIQUEMENT si le Presenter le demande ET l'utilisateur a uploadé une image UI
          const hasVibes = vibesByCategory && Object.keys(vibesByCategory).length > 0;
          const isDesignRef = rawPresenterOutput.includes("[IMAGE_IS_DESIGN_REF]") &&
          ...
          if (isDesignRef) {
            ...
            // ── Parse <request_vibes> from the presenter raw output ────────────
            // L'agent single a pu émettre des <request_vibes> dans sa réponse initiale
            const presenterRequestedVibes: { category: string; count: number }[] = [];
            const rvRegex = /<request_vibes\s+category="([^"]+)"\s+count="?(\d+)"?\s*\/>/g;
            let rvMatch;
            while ((rvMatch = rvRegex.exec(rawPresenterOutput)) !== null) {
              presenterRequestedVibes.push({ category: rvMatch[1], count: parseInt(rvMatch[2], 10) });
            }

            // Collect design images: uploaded + vibes sélectionnées par l'agent
            let designImages = [...(uploadedImages || []), ...(allReferenceImages || [])].slice(0, 3);

            if (vibesByCategory && Object.keys(vibesByCategory).length > 0) {
              const vibeImgs: string[] = [];
              if (presenterRequestedVibes.length > 0) {
                // Utiliser exactement ce que l'agent a demandé
                for (const req of presenterRequestedVibes) {
                  const catImgs = (vibesByCategory[req.category] ?? []) as string[];
                  vibeImgs.push(...catImgs.slice(0, req.count));
                }
                emit(`[DESIGN:THINKING] Agent a demandé: ${presenterRequestedVibes.map(r => `${r.count}x ${r.category}`).join(", ")}\n`);
              } else {
                // Fallback : 2 images par catégorie
                for (const imgs of Object.values(vibesByCategory)) {
                  vibeImgs.push(...(imgs as string[]).slice(0, 2));
                  if (vibeImgs.length >= 6) break;
                }
              }
              designImages = [...designImages, ...vibeImgs].slice(0, 10);
              if (vibeCategoryNames.length > 0)
                emit(`[DESIGN:THINKING] Vibes injectés: ${vibeCategoryNames.join(", ")}\n`);
            }
...
`;
          ...
          if (vibeCategoryNames && vibeCategoryNames.length > 0) {
            systemPrompt += `\n\n` +
              ...
              `Quand tu crées ou redesignes une app et que tu veux t'inspirer des références visuelles\n` +
              `disponibles, émets ce XML sur une ligne seule dans ta réponse :\n` +
              `<request_vibes category="Background" count="3"/>\n` +
              `Tu peux en émettre plusieurs pour des catégories différentes. Le système chargera ces images\n` +
              `et lancera le Design Agent pour générer un contrat de design précis pour ta prochaine génération.\n` +
              `⚠️ N'émets <request_vibes> QUE si l'utilisateur demande explicitement un nouveau design ou\n` +
              `une refonte visuelle. Pour les corrections fonctionnelles, ne pas l'émettre.\n`;
          }
