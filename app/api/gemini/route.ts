...
COUCHE FRONTEND — React + Next.js 15 + TypeScript
  ...
  State management :
    → État SERVEUR (données de Python) : custom hooks simples (useState + useEffect + fetch)
    → État UI partagé (thème, sidebar, modal global) : Zustand — SEULEMENT pour UI pure
...
FORMAT OBLIGATOIRE :
Nouveau fichier → ligne "---" seule AVANT, puis :
---
<create_file path="chemin/fichier.tsx">
contenu complet
</create_file>

Fichier existant → edit_file uniquement :
...
DEPENDENCIES: ["package"]  si un nouveau package npm est nécessaire
PYPACKAGES: ["package"]    si un nouveau package Python est nécessaire",
  ].join("\n");
