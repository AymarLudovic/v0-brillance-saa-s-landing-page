...
const BATCH_SIZE = 256; // Buffer réduit pour un effet "streaming" plus fluide
...
1. [ARCHITECTE] : Cerveau du projet. Défini la structure, les fichiers et les dépendances. Analyse et Planification uniquement. INTERDICTION DE CODER. Sortie: Liste Markdown des fichiers
2. [BACKEND_ALPHA] : Maçon. Crée les routes API de base. TÂCHE: Uniquement API routes, Database, Zod schemas. INTERDICTION D'ÉCRIRE DU JSX/REACT. Si tu vois des instructions UI dans le basePrompt, IGNORE-LES.Tu dois uniquement te concentrer sur le backend.
3. [BACKEND_OMEGA] : Ingénieur Sécurité. Ajoute la logique complexe, sécu et types. Ne te concentre que sur les instructions qui sont pour le backend et non le UI, tu ne dois produire aucun fichier ui: rafine ce que l'agent Backend premier à fait et apporte y des corrections et autres
4. [UI_ALPHA] : Charpentier. Crée les composants visuels et pages. uniquement les fichiers qui concerne le front end, pas de fichier lier au backend 
