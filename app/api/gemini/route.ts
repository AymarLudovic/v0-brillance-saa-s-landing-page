...
▸ MICRO_EDIT_ACTION — l'utilisateur veut un changement COSMÉTIQUE ou TRÈS CIBLÉ sur 1-3 fichiers existants
                      (exemples : changer une couleur, un texte, une taille de police, supprimer un élément,
                       repositionner un bouton, corriger une faute, changer une icône, ajuster un padding)
▸ FIX_ACTION       — l'utilisateur veut une modification fonctionnelle OU signale une erreur/bug
                      (exemples : ajouter une fonctionnalité, corriger un bug, ajouter une page, remanier un composant)
▸ CHAT_ONLY        — l'utilisateur pose une question, discute, demande des conseils

RÈGLE CRITIQUE — HIÉRARCHIE DES DÉCISIONS :
1. Si la demande est purement cosmétique / 1-3 lignes de code max → MICRO_EDIT_ACTION
   Exemples MICRO_EDIT : "change la couleur du bouton", "mets le titre en gras", "supprime ce texte",
   "remplace 'Connexion' par 'Login'", "met le padding à 8px", "change l'icône de X par Y"
2. Si les fichiers existent ET la demande est ciblée mais substantielle → FIX_ACTION
3. Si l'utilisateur veut créer / reconstruire → CODE_ACTION
4. Sinon → CHAT_ONLY
...
async function runFixAction(ctx: {
  ...
  if (strOps.length > 0) {
    ...
    emit("\n[STR_REPLACE] ⚠️ Modification non prise en charge — aucune opération générée\n");
  }

  // Emit modified files
  const modifiedSet = new Set([...newFiles.map(f => f.path), ...strOps.map(op => op.path)]);
      ...
      async start(controller) {
        ...
        try {
          ...
          const projectStatusSignal = fileCount > 0
            ? `\n\n[SIGNAL SYSTÈME] Ce projet contient DÉJÀ ${fileCount} fichier(s). Si la demande est une modification/ajout/correction → FIX_ACTION OBLIGATOIRE.`
            ...
            rawPresenterOutput = await callWithRetry(
              () => ai.models.generateContentStream({
                ...
                config: {
                  systemInstruction: `${basePrompt}\n\n${PRESENTER_PROMPT}`,
                  temperature: 0.7,
                  maxOutputTokens: 800, // enough for decision keyword + 3-4 sentences
                },
              }),
              presenterAndEmit,
