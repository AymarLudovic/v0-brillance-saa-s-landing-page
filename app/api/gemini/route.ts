...
function buildDesignAnchor(htmlRef?: string, analysisBlocks?: string): string {
  ...
  // Extract :root{} CSS variables for quick reference
  const rootMatch = htmlRef.match(/:root\s*\{([^}]+)\}/);
  const cssVars: string[] = [];
  if (rootMatch) {
    (rootMatch[1].match(/--[\w-]+\s*:\s*[^;]+;/g) ?? []).forEach(v => cssVars.push(v.trim()));
  }
  // Extract font name
  const fontMatch = htmlRef.match(/--font:\s*'([^']+)'/);
  const fontName = fontMatch ? fontMatch[1] : null;

  // Build quick-ref: top 8 vars for agents
  const quickVars = cssVars.slice(0, 8).map(v => {
    const name = v.match(/--([\w-]+)/)?.[1] ?? "";
    const val = v.match(/:\s*([^;]+)/)?.[1]?.trim() ?? "";
    return `  --${name}: ${val}  →  bg-[${val}] / text-[${val}]`;
  }).join("\n");
...
✅ Utilise bg-[#hex] text-[#hex] border-[#hex] avec les hex des variables ci-dessous
✅ Nav items h-[34px] MAX — jamais gonfler les menus

VARIABLES CSS DU DESIGN (hex exacts à utiliser) :
${quickVars || "  (variables dans le HTML/CSS ci-dessous)"}
${fontName ? `\n  --font: '${fontName}'  →  font-['${fontName}'] ou dans layout.tsx` : ""}

LOIS :
1. bg-[#hex] EXACT depuis les variables — jamais bg-gray-*, text-blue-*
2. sidebar / topbar / nav : NO shadow — uniquement border
3. cards / modals / dropdowns : shadow autorisée si dans le design
4. Nav items : h-[34px] compact — max 36px
5. Icônes Tabler : <i className="ti ti-[name]" /> — CDN dans layout.tsx
6. Font depuis Google Fonts dans app/layout.tsx
${analysisBlocks ? `\n══ ANALYSE VISUELLE DU DESIGN AGENT (utilise ces observations pour coder) ══\n${analysisBlocks}══ FIN ANALYSE ══` : ""}
...
`;
  ...
  // Build quick-ref: top 6 vars for agents
  const quickVars = cssVars.slice(0, 8).map(v => {
    const name = v.match(/--([\w-]+)/)?.[1] ?? "";
    const val = v.match(/:\s*([^;]+)/)?.[1]?.trim() ?? "";
    return `  --${name}: ${val}  →  bg-[${val}] / text-[${val}]`;
