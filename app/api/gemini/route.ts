...
// =============================================================================
// FIX_ACTION HANDLER — Extracted to top-level to avoid SWC TDZ bug with const
// after await inside nested if blocks inside async start() callbacks.
// =============================================================================
async function runFixAction(ctx: {
  ...
  const { emit, flushBuffer, runAgent, lastUserMsg, activeDesignAnchor, projectContext, currentProjectFiles, e2bApiKey, controller } = ctx;
  ...
  let fixOutput = "";
  try {
    fixOutput = await runAgent(FIXER_PROMPT, fixInput, { temperature: 0.7, maxTokens: 65536, agentName: "FIXER" });
  } catch (e: any) {
    emit("\n[Erreur FIXER: " + e.message + "]\n");
  }
  flushBuffer();

  // ── Appliquer str_replace et nouveaux fichiers ──────────────────────────────
  const workingFiles: { path: string; content: string }[] = (currentProjectFiles ?? []).map(f => ({ path: f.path, content: f.content }));

  const newFiles = parseGeneratedFiles(fixOutput);
  newFiles.forEach(f => {
    const i = workingFiles.findIndex(g => g.path === f.path);
    if (i >= 0) workingFiles[i] = f; else workingFiles.push(f);
  });
  ...
  if (strOps.length > 0) {
    ...
    if (srResult.applied > 0) {
      ...
      emit("\n[STR_REPLACE] ⚠️ Modification non prise en charge — aucun remplacement valide trouvé\n");
    }
    if (srResult.failed && srResult.failed.length > 0) {
      emit("\n[STR_REPLACE] ⚠️ " + srResult.failed.length + " échoué(s): " + srResult.failed.map((f: any) => f.path + "(" + f.reason + ")").join(", ") + "\n");
    }
  }

  // Applique aussi les edit_file ops du FIXER (format moderne)
  const fixEditOps = parseEditFileOps(fixOutput);
  if (fixEditOps.length > 0) {
    const edResult = applyEditFileOpsToFiles(workingFiles, fixEditOps);
    if (edResult.applied > 0) {
      emit("\n[EDIT_FILE] ✅ " + edResult.applied + " edit_file op(s) appliquée(s)\n");
    }
    if (edResult.failed.length > 0) {
      emit("\n[EDIT_FILE] ⚠️ " + edResult.failed.length + " échoué(s): " + edResult.failed.map(f => f.path + "(" + f.reason + ")").join(", ") + "\n");
    }
  }

  if (strOps.length === 0 && fixEditOps.length === 0 && newFiles.length === 0) {
    emit("\n[FIX] ⚠️ Aucune opération de modification générée par le FIXER\n");
  }

  // Emit modified files
  const modifiedSet = new Set([...newFiles.map(f => f.path), ...strOps.map(op => op.path), ...fixEditOps.map(op => op.path)]);
  ...
  // ── Package.json si nouvelles deps ────────────────────────────────────────
  const depNames = extractDeps(fixOutput);
  if (depNames.length > 0) {
    const pkgFile = (currentProjectFiles ?? []).find(f => f.path === "package.json");
    let pkg: any = { name: "app", version: "1.0.0", private: true,
      scripts: { dev: "next dev", build: "next build", start: "next start", lint: "next lint" },
      dependencies: {}, devDependencies: {} };
    if (pkgFile) { try { pkg = JSON.parse(pkgFile.content); } catch {} }
    const baseDeps: Record<string,string> = { next: "15.1.0", react: "19.0.0", "react-dom": "19.0.0", "lucide-react": "0.475.0", ...(pkg.dependencies ?? {}) };
    const newDeps: Record<string,string> = {};
    await Promise.all(depNames.map(async (pkgName) => {
      if (!pkgName || baseDeps[pkgName]) return;
      try {
        const resolved = await import("package-json").then(m => m.default(pkgName));
        newDeps[pkgName] = (resolved as any).version ?? "latest";
      } catch { newDeps[pkgName] = "latest"; }
    }));
    if (Object.keys(newDeps).length > 0) {
      pkg.dependencies = { ...baseDeps, ...newDeps };
  ...
  emit("\n[PHASE:2/TSC_CHECK]\n");
  if (e2bApiKey) {
    const tscFiles = workingFiles.slice(); // shallow copy
    const tscResult = await runTscCheck(tscFiles, e2bApiKey, emit);
    if (tscResult.hasErrors) {
      await sleep(3000);
      let tscFixOut = "";
      try {
        tscFixOut = await runAgent(FIXER_PROMPT, "ERREURS TSC restantes:\n" + tscResult.errors + "\n\n" + projectContext, {
  ...
  emit("\n[TOKEN_USAGE]" + JSON.stringify({ total: ctx.totalTokensUsed, prompt: ctx.totalPromptTokens, candidates: ctx.totalCandidatesTokens }) + "[/TOKEN_USAGE]\n");
  emit("\n[PAGE_DONE]\n");
  controller.close();
}
...
Tu es un chirurgien du code ultra-précis. Tu reçois une demande de modification MINIME sur un projet existant.
Ta mission : identifier exactement les lignes à changer et utiliser UNIQUEMENT le format edit_file.
...
══════════════════════════════════════════════════════════════════════
✗ JAMAIS utiliser <create_file> sauf si on te demande de créer un nouveau fichier
✗ JAMAIS réécrire un fichier complet
✗ JAMAIS utiliser str_replace
✗ JAMAIS modifier autre chose que ce qui est explicitement demandé
✗ JAMAIS changer la logique, le layout, ou le design global
...
async function handleFixAction(ctx: FixActionCtx): Promise<void> {
  ...
  const fixInput = [
    ...
    "",
    "Utilise readFile() pour lire TOUS les fichiers concernés avant de les modifier.",
    "PRÉFÈRE edit_file (par numéros de ligne) pour modifier les fichiers existants. Les snapshots de fichiers ci-dessus incluent des numéros de ligne.",
  ].join("\n");
  ...
  // Package.json update if new deps declared
  const depNames = extractDeps(fixOutput);
  if (depNames.length > 0) {
    const pkgFile = (currentProjectFiles ?? []).find(f => f.path === "package.json");
    let pkg: any = {
  ...
  emit("\n[PHASE:2/TSC_CHECK]\n");
  if (e2bApiKey) {
    const tscFiles = workingFiles.slice();
    const tscResult = await runTscCheck(tscFiles, e2bApiKey, emit);
    if (tscResult.hasErrors) {
      await sleep(3000);
      let tscFixOut = "";
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ── ÉTAPE 1A : appel décision SÉPARÉ (non-streamé, ultra-rapide) ──────
          // Le LLM ne génère QUE le mot-clé. Aucune prose. 100% fiable.
          const errorKeywords = /\b(erreur|error|bug|crash|undefined|cannot|not defined|failed|exception|stacktrace|ReferenceError|TypeError|SyntaxError|Module not found|corrige|fix|répare|réparer|résous|résoudre)\b/i;
          const looksLikeError = errorKeywords.test(lastUserMsg) && fileCount > 0;

          let decision: string;

          // Fast-path déterministe : si aucun fichier → CODE_ACTION
          if (fileCount === 0) {
            decision = "CODE_ACTION";
          } else {
            // Appel décision minimal — demande UNIQUEMENT le mot-clé, sans prose
            const decisionContents = buildHistoryParts();
            const decisionLastPart = decisionContents[decisionContents.length - 1];
            if (decisionLastPart?.role === "user") {
              decisionLastPart.parts.push({ text: `\n\n[CONTEXTE PROJET — ${fileCount} fichier(s) existant(s)]\n${projectContext.slice(0, 800)}` });
              decisionLastPart.parts.push({ text: `\n\n[INSTRUCTION CRITIQUE] Réponds avec UN SEUL MOT sur une ligne, rien d'autre :\n${
                looksLikeError
                  ? "FIX_ACTION — le message signale une erreur/bug"
                  : "- MICRO_EDIT_ACTION si changement visuel/contenu (texte, couleur, nom, section, style, icône)\n- FIX_ACTION si erreur/bug explicite\n- CODE_ACTION si reconstruction complète\n- CHAT_ONLY si question"
              }\nRéponds UNIQUEMENT avec le mot-clé exact, rien d'autre.` });
            }
            let decisionRaw = looksLikeError ? "FIX_ACTION" : "MICRO_EDIT_ACTION"; // fallback intelligent
            try {
              const decisionResult = await ai.models.generateContent({
                model: MODEL_ID,
                contents: decisionContents,
                config: {
                  systemInstruction: "Tu es un router. Tu réponds UNIQUEMENT avec un mot-clé parmi : CODE_ACTION, FIX_ACTION, MICRO_EDIT_ACTION, CHAT_ONLY. Rien d'autre. Pas de phrase, pas de ponctuation.",
                  temperature: 0.1,
                  maxOutputTokens: 20,
                },
              });
              const rawDecision = decisionResult.text?.trim() ?? "";
              const dm = rawDecision.match(/(CODE_ACTION|FIX_ACTION|MICRO_EDIT_ACTION|CHAT_ONLY)/);
              if (dm) decisionRaw = dm[1];
            } catch {
              // garde le fallback intelligent
            }
            decision = decisionRaw;
          }

          // ── ÉTAPE 1B : stream de l'intro PRESENTER (texte visible uniquement) ─
          let rawPresenterOutput = "";
          const presenterAndEmit = (txt: string) => { rawPresenterOutput += txt; };

          const presenterIntroContents = buildHistoryParts();
          const introLastPart = presenterIntroContents[presenterIntroContents.length - 1];
          if (introLastPart?.role === "user") {
            introLastPart.parts.push({ text: `\n\n[CONTEXTE] Projet avec ${fileCount} fichier(s). Décision prise : ${decision}.\nÉcris UNIQUEMENT le texte d'intro (1-2 phrases), sans mot-clé de décision.` });
          }
          try {
            rawPresenterOutput = await callWithRetry(
              () => ai.models.generateContentStream({
                model: MODEL_ID,
                contents: presenterIntroContents,
                config: {
                  systemInstruction: `${basePrompt}\n\n${PRESENTER_PROMPT}`,
                  temperature: 0.7,
            ...
            rawPresenterOutput = decision === "FIX_ACTION"
              ? "Je corrige ça immédiatement."
              : decision === "MICRO_EDIT_ACTION"
              ? "Je mets à jour ça tout de suite."
              : "Je m'en occupe.";
          }
          ...
          if (decision === "MICRO_EDIT_ACTION") {
            ...
            const microInput = [
              "DEMANDE DE MODIFICATION MINIME :",
              lastUserMsg,
              "",
              projectContext,
              "",
              "INSTRUCTION : Utilise UNIQUEMENT des balises <edit_file> pour appliquer la modification.",
              "Les snapshots de fichiers ci-dessus incluent des numéros de ligne. Cible exactement les lignes à modifier.",
            ].join("\n");
            ...
            emit("\n[PHASE:2/TSC_CHECK]\n");
            if (e2bApiKey) {
              const microTscResult = await runTscCheck(workingFilesMicro.slice(), e2bApiKey, emit);
...
══════════════════════════════════════════════════════════════════════
COUCHE FONDATION (types, stores, services) — lis avec readFile() si besoin du détail :
${keyFilesSummary}

══════════════════════════════════════════════════════════════════════
COUCHE UI (hooks et composants disponibles à IMPORTER — générés par l'agent précédent) :
${uiLayerSummary}
...
- Importe les composants UI ci-dessus dans tes vues (ne les recrée pas)
