...
CURRENT LLM MODELS (use these exact versions without trying to replace them with ones you know — just consider your training stopped when they were released)
...
❌ NEVER use @apply with arbitrary values like border-[var(--x)] in globals.css.
   ...
   - CORRECT: Use inline style={{ borderColor: 'var(--border-color)' }} or a CSS property directly.
...
✅ app/layout.tsx must always have "use client" if it uses React hooks,
   or no directive if it is a pure Server Component.
