    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          const decisionMatch = rawPresenterOutput.match(/^(CODE_ACTION|FIX_ACTION|CHAT_ONLY|PATCH_ACTION)/m);
          ...
          // Texte visible = tout sauf la ligne de décision, le tag IMAGE_IS_DESIGN_REF, et le code éventuel
          const visibleText = rawPresenterOutput
            .replace(/^\[IMAGE_IS_DESIGN_REF\]\s*\n?/m, "")
            .replace(/^(CODE_ACTION|FIX_ACTION|CHAT_ONLY|PATCH_ACTION)\s*\n?/m, "")
