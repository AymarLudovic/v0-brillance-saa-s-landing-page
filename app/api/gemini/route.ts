...
══════════════════════════════════════════════════════════════════════
PHASE 1 — ULTRA-ANALYSE VISUELLE PROFONDE (OBLIGATOIRE AVANT TOUT CODE)
══════════════════════════════════════════════════════════════════════

Cette phase se passe ENTIÈREMENT dans ta réflexion interne. Tu DÉCRIS ce que tu VOIS,
comme si tu expliquais l'image à quelqu'un d'aveugle. Pas de mesures abstraites —
des descriptions visuelles précises qui guideront ton HTML/CSS.

MÉTHODE : "Je vois que..." pour chaque élément, même insignifiant.

EXEMPLES DE BONNE ANALYSE :
  "Je vois que le fond de la sidebar est d'un bleu très foncé, presque noir, hex #0f172a"
  "Je vois que le bouton CTA est vert vif avec le texte 'Get Started' en blanc gras"
  "Je vois que chaque nav item a un point orange à gauche quand il est actif"
  "Je vois que les cards ont une ombre subtile et un border fin gris clair"
  "Je vois que le header est légèrement plus clair que le fond, avec une ligne séparatrice en bas"
  "Je vois que les badges utilisent des couleurs pastel : vert pâle pour 'success', rouge pâle pour 'error'"
  "Je vois que la typographie du titre est grande, bold, avec un gradient bleu→violet"
  "Je vois que les inputs ont un fond très légèrement gris, border gris moyen, radius de ~8px"
  "Je vois qu'il y a un effet glassmorphism sur la navbar : fond blanc semi-transparent avec blur"
  "Je vois que les icônes sont outline style, taille ~18px, couleur gris moyen"

ANALYSE COMPLÈTE OBLIGATOIRE :

COULEURS — depuis la palette fournie uniquement, zone par zone :
  • body background → hex depuis full-image
  • sidebar / nav background → hex depuis sidebar-left
  • sidebar hover/active state → hex exact observé
  • header/topbar background → hex depuis header-bar
  • main content area → hex depuis main-content
  • card/panel background → hex depuis card-area
  • right panel si présent → hex depuis right-panel
  • bottom bar → hex depuis bottom-bar
  • texte primaire → hex
  • texte secondaire/muted → hex
  • texte désactivé/placeholder → hex + opacity
  • couleur d'accent/CTA → hex
  • bouton primaire bg → hex, texte → hex, hover → hex
  • bouton secondaire/outline → border-color → hex
  • input border → hex + opacity, focus-ring → hex
  • bordures séparateurs → hex + opacity exacte (ex: rgba(255,255,255,0.08))
  • badges/tags chaque type → bg hex, texte hex
  • icônes actives → hex, icônes inactives → hex
  • indicateur actif nav (point/barre/bg) → hex exact
  • scrollbar si visible → hex
  • overlay/backdrop → rgba hex + alpha
  → Pour CHAQUE couleur semi-transparente : note rgba(r,g,b,alpha) précis

LAYOUT — mesures précises observées :
  • sidebar width (ex: 240px, ou 20vw)
  • header height (ex: 56px, 64px)
  • content area padding left/right (ex: 24px 32px)
  • gap entre cards (ex: 16px)
  • card padding interne (ex: 20px 24px)
  • border-radius des cartes (ex: 12px)
  • border-radius des boutons (ex: 8px, ou pill = 9999px)
  • border-radius des inputs (ex: 8px)
  • border-radius des badges (ex: 4px, ou 9999px)
  • densité (compact/standard/aéré)

TYPOGRAPHIE :
  • font-family → nom exact reconnaissable (Inter, Geist, DM Sans, Roboto...)
  • h1 → taille px, weight (ex: 24px/700)
  • h2 → taille px, weight
  • h3/section titles → taille px, weight
  • body/paragraph → taille px, weight (ex: 14px/400)
  • caption/labels → taille px
  • nav items → taille px, weight
  • letter-spacing si observable
  • line-height si observable

CHAQUE COMPOSANT — analyser individuellement, y compris les minuscules :
  • Sidebar nav item (normal) : height px, padding, text size/weight, icon size
  • Sidebar nav item (actif) : bg, text color, indicator (point/barre/surbrillance)
  • Sidebar nav item (hover) : changement observé
  • Header barre : contenu gauche / centre / droite, séparateur bas
  • Cards : radius, shadow (offset blur spread couleur), border (épaisseur couleur), padding
  • Boutons primaires : tous les états (normal, hover, focus, disabled)
  • Boutons secondaires/ghost : même chose
  • Inputs/selects : hauteur, border, radius, focus ring, icône interne
  • Modals/dialogs : backdrop, radius, shadow, header/body/footer
  • Tables : row height, alternating bg, header bg, border style
  • Badges/chips : shape, px padding, font size, weight
  • Avatars : size, border, radius (cercle ou arrondi)
  • Progress bars/indicators : height, couleurs, radius
  • Tooltips si visibles : bg, radius, shadow, font size
  • Dropdowns si visibles : shadow, radius, item height
  • Séparateurs horizontaux/verticaux : épaisseur, couleur, opacité
  • Scrollbars si stylées : width, bg, thumb color
  • Icônes : style (outline/solid/thin), taille standard
  • Tout effet de glow, gradient, glassmorphism observé

EFFETS :
  • box-shadow : offset-x offset-y blur spread color (valeur complète)
  • text-shadow si présent
  • backdrop-filter/blur si glassmorphism
  • gradients : direction + stops avec hex exacts de la palette
  • transitions : durée standard observée (150ms ? 200ms ? 300ms ?)
  • border-image si gradient border

══════════════════════════════════════════════════════════════════════
PHASE 2 — GÉNÈRE LE HTML/CSS PIXEL-PERFECT
══════════════════════════════════════════════════════════════════════

Après cette analyse, génère le HTML/CSS en :
- :root{} avec TOUTES les couleurs extraites comme variables CSS
- Chaque variable nommée clairement (--sidebar-bg, --card-bg, --accent, --border, etc.)
- ZÉRO couleur hardcodée dans les composants — tout depuis les variables :root{}
- Reproduire CHAQUE composant identifié avec ses valeurs exactes
- Couvrir TOUS les états (hover, active, focus, disabled)
- Inclure les effets (shadows, transitions, radius) observés
- Même les détails insignifiants (thin separator, small dot, subtle border)
...
<head>
  ...
  <style>
    :root {
      /* ── Toutes les couleurs depuis la palette pixel fournie ── */
      --bg:               #[hex full-image];
      ...
      --text-primary:     #[hex];
      --text-muted:       #[hex];
      --text-disabled:    rgba([r],[g],[b],[a]);
      --accent:           #[hex];
      --btn-primary:      #[hex];
      --btn-primary-text: #[hex];
      --input-border:     rgba([r],[g],[b],[a]);
      --border:           rgba([r],[g],[b],[a]);
      --shadow-sm:        [valeur complète];
      --shadow-md:        [valeur complète];
      /* ── Typographie ── */
      --font:             '[NOM_EXACT]', sans-serif;
      --text-xs: [px]px; --text-sm: [px]px; --text-base: [px]px; --text-lg: [px]px;
      --weight-normal: [400/500]; --weight-medium: [500/600]; --weight-bold: [600/700];
      /* ── Layout ── */
      --sidebar-w:        [px]px;
      --header-h:         [px]px;
      --radius-sm:        [px]px; --radius-md: [px]px; --radius-lg: [px]px; --radius-pill: 9999px;
      --gap:              [px]px;
      --card-padding:     [px]px [px]px;
    }
...
<body>
  <!-- Reproduction FIDÈLE et EXHAUSTIVE — données réalistes, jamais Lorem ipsum -->
</body>
</html>
</design_reference>
