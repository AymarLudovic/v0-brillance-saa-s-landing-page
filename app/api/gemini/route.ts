...
// Reproduit exactement ce que Lovable/v0 font en coulisses :
// écriture des fichiers → stubs dynamiques → tsc --noEmit → parse des erreurs
// =============================================================================
...
// Packages qui ont leur propre déclaration précise (utile pour la validation Zustand)
const SPECIFIC_STUBS: Record<string, string> = {
  ...
  export function create<T>(): (fn: StateCreator<T>) => (() => T) & { getState: () => T; setState: SetState<T>; subscribe: (listener: (s: T) => void) => () => void };
  export function create<T>(fn: StateCreator<T>): (() => T) & { getState: () => T };
}`,
  ...
  "next/image": `declare module "next/image" { const Image: (props: { src: any; alt: string; width?: number; height?: number; fill?: boolean; className?: string; priority?: boolean; [k: string]: any }) => any; export default Image; }`,
  "next/link": `declare module "next/link" { const Link: (props: { href: string | any; children?: any; className?: string; [k: string]: any }) => any; export default Link; }`,
  "next/navigation": `declare module "next/navigation" {
  export function useRouter(): { push: (p: string) => void; replace: (p: string) => void; back: () => void; forward: () => void; refresh: () => void; prefetch: (p: string) => void };
  export function usePathname(): string;
  export function useSearchParams(): { get: (k: string) => string | null; getAll: (k: string) => string[] };
  ...
  export class NextResponse {
    ...
    static next(init?: any): Response;
    static rewrite(url: string | URL): Response;
  }
  export type NextRequest = Request & { cookies: any; nextUrl: URL & { pathname: string; searchParams: URLSearchParams }; ip?: string };
}`,
  "next/headers": `declare module "next/headers" { export function cookies(): { get: (k: string) => { value: string } | undefined; set: (k: string, v: string) => void; delete: (k: string) => void; getAll: () => any[] }; export function headers(): Headers; }`,
  "next/font/google": `declare module "next/font/google" { export function Inter(o: any): { className: string; style: any }; export function Geist(o: any): { className: string; style: any }; export function [key: string]: any; }`,
  "next/font/local": `declare module "next/font/local" { const fn: (o: any) => { className: string; style: any }; export default fn; }`,
};

// Packages à ignorer (ont leurs propres @types ou déclarations intégrées)
const ALWAYS_SKIP = new Set(["react", "react-dom", "next", "typescript"]);

/**
 * Génère dynamiquement les stubs TypeScript pour TOUS les packages importés
 * dans les fichiers générés — plus de liste statique, plus de faux positifs.
 */
function buildDynamicStubs(files: { path: string; content: string }[]): string {
  const detectedPackages = new Set<string>();

  for (const f of files) {
    // Capture tous les imports externes : "package", "package/sub", "@scope/pkg", "@scope/pkg/sub"
    const rx = /from\s+['"](@?[^./'"@][^'"]*)['"]/g;
    let m;
    while ((m = rx.exec(f.content)) !== null) {
      const raw = m[1];
      // Racine du package (gère @scope/name ET regular/name)
      const root = raw.startsWith("@")
        ? raw.split("/").slice(0, 2).join("/")
        : raw.split("/")[0];

      if (ALWAYS_SKIP.has(root)) continue;
      if (SPECIFIC_STUBS[raw] || SPECIFIC_STUBS[root]) continue;

      detectedPackages.add(root);
      // Ajoute aussi le sous-chemin complet si différent (ex: "react-hook-form/resolvers")
      if (raw !== root) detectedPackages.add(raw);
    }
  }

  const lines: string[] = [
    "// ============================================================",
    "// STUBS AUTO-GÉNÉRÉS — évite 'Cannot find module' pour tous les",
    "// packages utilisés dans le projet, sans npm install.",
    "// ============================================================",
    "",
  ];

  // Stubs précis pour les packages qui le méritent
  for (const stub of Object.values(SPECIFIC_STUBS)) {
    lines.push(stub, "");
  }

  // Stub générique universel pour chaque package détecté
  for (const pkg of Array.from(detectedPackages).sort()) {
    lines.push(
      `declare module "${pkg}" {`,
      `  const _default: any;`,
  ...
  lines.push(
    `declare module "*.css" {}`,
    `declare module "*.svg" { const c: any; export default c; }`,
    `declare module "*.png" { const c: any; export default c; }`,
    `declare module "*.jpg" { const c: any; export default c; }`,
  );
...
interface TscCheckResult {
  errors: string;
  hasErrors: boolean;
  errorsByFile: Record<string, string[]>;   // fichier → liste d'erreurs
  errorCount: number;
}
...
async function runTscCheck(
  ...
  try {
    ...
    // ── PREUVE SANDBOX : prouve que le code tourne vraiment dans E2B ──────────
    const proof = await sbx.commands.run(
      `echo "ID=$(hostname) | OS=$(uname -r | cut -d- -f1) | Node=$(node -v 2>/dev/null || echo n/a)"`,
      { timeoutMs: 5_000 }
    );
    onProgress(`[TSC:SANDBOX] 🔒 Sandbox E2B actif → ${proof.stdout.trim()}\n`);
    // ─────────────────────────────────────────────────────────────────────────

    // Génère les stubs DYNAMIQUEMENT à partir des imports réels des fichiers
    const dynamicStubs = buildDynamicStubs(files);
    onProgress(`[TSC:STUBS] ${(dynamicStubs.match(/^declare module/gm) ?? []).length} stubs de packages générés dynamiquement.\n`);

    await sbx.files.write("tsconfig.json", TSC_CONFIG);
    await sbx.files.write("__stubs__.d.ts", dynamicStubs);

    // Installe TypeScript uniquement
    onProgress("[TSC:INSTALL] Installation de TypeScript...\n");
    await sbx.commands.run(
      "npm install --save-dev typescript --no-package-lock --prefer-offline 2>&1 | tail -2",
      { timeoutMs: 45_000 }
    );

    // Écrit tous les fichiers .ts / .tsx
    const tsFiles = files.filter(f => f.path.endsWith(".ts") || f.path.endsWith(".tsx"));
    onProgress(`[TSC:FILES] Écriture de ${tsFiles.length} fichiers TypeScript dans le sandbox...\n`);

    await Promise.all(tsFiles.map(f => sbx!.files.write(f.path, f.content)));

    // Lance tsc --noEmit
    onProgress("[TSC:RUN] Vérification TypeScript (tsc --noEmit)...\n");
    const tscResult = await sbx.commands.run("npx tsc --noEmit 2>&1 || true", { timeoutMs: 60_000 });

    const rawOutput = tscResult.stdout ?? "";

    // Filtre les erreurs "Cannot find module" résiduelles (stubs manquants non bloquants)
    // et les erreurs provenant des stubs eux-mêmes
    const realErrorLines = rawOutput
      .split("\n")
      .filter(l =>
        (l.includes("error TS") || l.includes(": error")) &&
        !l.includes("Cannot find module") &&
        !l.includes("__stubs__") &&
        !l.includes("Could not find a declaration file")
      );
    ...
    if (!hasErrors) {
      ...
      return { errors: "", hasErrors: false, errorsByFile: {}, errorCount: 0 };
    }

    // ── Groupe les erreurs par fichier avec numéro de ligne ───────────────────
    const errorsByFile: Record<string, string[]> = {};
    for (const line of realErrorLines) {
      // Format tsc : path/to/File.tsx(12,5): error TS2345: ...
      const m = line.match(/^([^\s(]+\.tsx?)\((\d+),(\d+)\):\s*(.+)$/);
      if (m) {
        ...
        // Erreur sans position (ex: erreur globale de config)
    ...
    // Affiche le rapport détaillé dans le stream
    const fileReport = Object.entries(errorsByFile)
      .map(([f, errs]) => `    📄 ${f} (${errs.length} erreur${errs.length > 1 ? "s" : ""}):\n${errs.slice(0, 5).join("\n")}${errs.length > 5 ? `\n      ... +${errs.length - 5} autres` : ""}`)
      .join("\n");

    onProgress(
      `[TSC:ERRORS] ⚠️ ${errorCount} erreur(s) TypeScript dans ${fileCount} fichier(s) :\n${fileReport}\n` +
      `[TSC:FIXER] Correction automatique en cours...\n`
    );

    return {
      errors: realErrorLines.join("\n"),
      hasErrors: true,
      errorsByFile,
      errorCount,
    };
    ...
    onProgress(`[TSC:SKIP] Sandbox E2B indisponible (${err.message?.slice(0, 80) ?? "?"}) — on continue sans vérification.\n`);
    return { errors: "", hasErrors: false, errorsByFile: {}, errorCount: 0 };
  } finally {
...
`;
          ...
          if (e2bApiKey) {
            const MAX_TSC_FIX_ROUNDS = 2;

            for (let round = 0; round < MAX_TSC_FIX_ROUNDS; round++) {
              const tscResult = await runTscCheck(allGeneratedFiles, e2bApiKey, emit);

              if (!tscResult.hasErrors) break;

              emit(`\n[TSC:FIXER] Round ${round + 1}/${MAX_TSC_FIX_ROUNDS} — Correction en cours...\n`);
              ...
              // Fichier types toujours inclus (souvent la source des erreurs de type)
              const typesFile = allGeneratedFiles.find(f => f.path === "types/index.ts");

              // ── Construit le contexte des fichiers AVEC NUMÉROS DE LIGNE ────────────
              // Les numéros de ligne permettent au fixer de naviguer directement à l'erreur
              // ex: "L45:12 — Argument of type 'string' is not assignable to type 'number'"
              ...
              const brokenFilesContext = Array.from(brokenPaths)
                .map(p => {
                  ...
                  return (
                    `\n${"=".repeat(60)}\n` +
                    `FICHIER : ${f.path} (${lineCount} lignes)\n` +
...
- Le numéro de ligne correspond EXACTEMENT à la ligne marquée dans le fichier ci-dessous
- Les fichiers sont affichés avec leurs numéros de ligne (format : "  42 | code ici")
...
2. Repère la ligne 45 dans le fichier ci-dessus (marquée "  45 | ...")
3. Corrige UNIQUEMENT cette ligne ou ce bloc
4. Émet le fichier COMPLET corrigé (sans les numéros de ligne — code propre)
5. Ne corrige QUE les erreurs listées, rien d'autre
...
- "() => void" dans Zustand create() → remplace par l'implémentation réelle
...
`;
              ...
              mergeGeneratedFiles(parseGeneratedFiles(tscFixOutput));
          ...
          // 1. Scan final de TOUS les fichiers générés pour détecter les imports
          for (const f of allGeneratedFiles) {
            for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
              const pkg = m[1].split("/")[0];
              if (pkg && !pkg.startsWith(".") && pkg !== "next" && pkg !== "react" && pkg !== "react-dom") {
