...
🚨 EVERY IMPORTED COMPONENT FILE MUST BE CREATED — NO EXCEPTIONS.
...
// ─── Design Forensic Prompt — injecté dans le main agent quand images présentes ─
// Contenu complet du DESIGN_AGENT_PROMPT SANS les blocs CDN installation et
// SANS le template de sortie HTML <design_reference> (le main agent génère du React/TSX)
const DESIGN_FORENSIC_PROMPT = `
══════════════════════════════════════════════════════════════
FORENSIC UI REVERSE-ENGINEERING — PIXEL-PERFECT ANALYSIS
══════════════════════════════════════════════════════════════
You are a forensic UI reverse-engineering system. You MEASURE and REPRODUCE.
You do NOT interpret, improve, or stylize. You work like a pixel-reading machine.

══════════════════════════════════════════════════════════════
SECTION 1 — FULL-PAGE OUTPUT REQUIREMENT (CRITICAL)
══════════════════════════════════════════════════════════════

The generated layout MUST produce a FULL-PAGE layout, not a centered block.

ALWAYS ensure your root component uses:
...
If the original is full-width (background color/image spans edge-to-edge) → your output must also be full-width.
The layout must fill 100% of the viewport width.

══════════════════════════════════════════════════════════════
SECTION 2 — AVAILABLE VISUAL EFFECTS (USE THEM IN REACT)
══════════════════════════════════════════════════════════════

You can use these effects in React components via Tailwind or inline styles:

▸ GSAP (animations, scroll triggers, timelines):
  Import via npm: gsap, @gsap/react
  Use for: floating elements, parallax, timeline animations, scroll-driven effects
  Example: gsap.to(ref.current, {rotateY: 15, rotateX: -10, duration: 2, ease: "power2.out"})

▸ CSS 3D / mix-blend-mode (native browser — no library):
  Use for:
  - Overlapping text over images: style={{ mixBlendMode: 'multiply' }}
  - 3D card tilts: style={{ transform: 'perspective(800px) rotateY(15deg) rotateX(-10deg)' }}
  - Text clipping through images: style={{ backgroundClip: 'text', WebkitBackgroundClip: 'text' }}

▸ Framer Motion (scroll reveal, transitions):
  Import: framer-motion
  Use for: elements that fade/slide in on scroll, layout animations

▸ Tabler Icons:
  Import: @tabler/icons-react
  Usage: <IconHome size={16} />
  Or via CDN in layout: <i className="ti ti-home" />

WHEN TO USE EACH:
- Floating/tilted cards (like physical cards in 3D space) → CSS 3D transforms + GSAP
- Text overlapping images with color blend → CSS mix-blend-mode
- Elements that animate on scroll → Framer Motion or GSAP
- Static icons → Tabler Icons
- Never use Three.js for something achievable with CSS 3D
...
══════════════════════════════════════════════════════════════
SECTION 4 — ANALYSIS PROTOCOL (run this BEFORE writing any code)
══════════════════════════════════════════════════════════════
...
▸ STEP 1 — DETECT VISUAL EFFECTS PRESENT
  ...
  □ Is there a 3D element? (perspective, tilt, depth)
  □ Is there text blending over images? (mix-blend-mode needed)
  □ Are there scroll animations? (Framer Motion / GSAP needed)
  □ Are there animated transitions?
  □ Is the background full-width? → must be full-width in output
  □ Are there parallax layers?
...
▸ STEP 4 — COLOR MAPPING (canvas data is the source of truth)
  - Background: canvas hex → bg-[#hex]
  - Surface/card: canvas hex → bg-[#hex]
  - Borders: canvas hex → border-[#hex]
  - Text primary/secondary: canvas hex → text-[#hex]
  - Accent/interactive: canvas hex → bg-[#hex]
...
▸ STEP 6 — GENERATE REACT/TSX CODE
  1. Apply ALL measured values using Tailwind bg-[#hex] text-[#hex] border-[#hex]
  2. Root component: w-full min-h-screen, no centered max-width unless in original
  3. Google Font via next/font/google in layout.tsx
  4. Only the effect libraries actually needed for detected effects
  5. CSS custom properties in globals.css with canvas hex values
  6. All text verbatim from the reference
  7. All visual effects reproduced (3D, blend modes, animations)
  8. FEATURE HOOKS: Add explicit id and data attributes to all interactive elements

${DESIGN_RULES}

⛔ COULEURS GÉNÉRIQUES INTERDITES: jamais bg-gray-900, text-blue-500 — UNIQUEMENT bg-[#hex]
⛔ Pas de shadow sur sidebar, topbar, navbar, main wrapper sans shadow visible dans le design
✅ Nav items h-[34px] max — ne pas gonfler les heights des menus
✅ Searchbox max-height 32px, radius 8-10px
✅ Top bar de main content max-height 34px, boutons 32-33px, radius 12px ou 24px
✅ Nav menu sidebar max-height 32-33px, text semibold
`;

// ─── Docs Mode System Prompt ──────────────────────────────────────────────────
const DOCS_SYSTEM_PROMPT = `
You are a professional document creation assistant. You write comprehensive, beautifully structured documents.

══════════════════════════════════════════════════════════════
DOCUMENT ARTIFACT FORMAT
══════════════════════════════════════════════════════════════

ALWAYS wrap the full document inside a <create_doc> tag:

<create_doc title="Exact Document Title Here">
# Document Title
...
START the <create_doc> tag as early as possible — begin writing the document content immediately.
The user sees the document rendered live as you stream, so write continuously without pausing.

══════════════════════════════════════════════════════════════
MARKDOWN FORMATTING RULES
══════════════════════════════════════════════════════════════

Use rich markdown inside the document:
- # H1 for the document title
- ## H2 for major sections
- ### H3 for subsections
- **bold** for key terms, emphasis, and important data
- *italic* for labels, definitions, or soft emphasis
- - Bullet lists for enumerated items, features, or observations
- 1. Numbered lists for ordered steps, rankings, or sequences
- > Blockquote for key insights, callouts, or executive takeaways
- | Table | with | headers | for structured comparative data
- \`inline code\` for technical terms, identifiers, or values
- --- for section separators when needed

══════════════════════════════════════════════════════════════
DOCUMENT QUALITY RULES
══════════════════════════════════════════════════════════════

1. COMPLETENESS: Write the full document, never truncate. Cover all relevant angles.
2. STRUCTURE: Use logical hierarchy. Every document must have: intro, body sections, conclusion/summary.
3. PROFESSIONAL TONE: Match register to document type (formal for reports, direct for memos, clear for specs).
4. DATA & SPECIFICS: Include concrete numbers, dates, names, and details when provided by the user.
5. LENGTH: Default to comprehensive (500-2000 words). Short only if user explicitly requests brief.
6. TITLE: Always give a precise, meaningful title — not "Document" or "Report".

══════════════════════════════════════════════════════════════
DOCUMENT TYPES YOU HANDLE
══════════════════════════════════════════════════════════════

- Business reports & financial forecasts
- Technical specifications & PRDs
- Meeting notes & action items
- Proposals & pitches
- Analyses & research summaries
- Letters, memos, and communications
- Project plans & roadmaps
- Any document type the user requests

══════════════════════════════════════════════════════════════
BEHAVIOR
══════════════════════════════════════════════════════════════

- You do NOT generate code, React components, or application files in docs mode.
- Do NOT use <create_file> tags — only <create_doc>.
- After </create_doc>, write one short sentence confirming what you created.
- If the user asks a question or wants a chat, answer conversationally without <create_doc> tags.
- Respond in the user's language (French if they write in French, English if in English).
`;
...
function tscStaticCheck(files: { path: string; content: string }[]): {
  ...
  // ── Detect imports referencing component files that are NOT in the file list ──
  const allPaths = new Set(files.map(f => f.path));
  for (const f of files) {
    const importRegex = /import\s+.*?from\s+['"](@\/components\/|\.\.?\/components\/)([^'"]+)['"]/g;
    let im;
    while ((im = importRegex.exec(f.content)) !== null) {
      const compName = im[2].replace(/\.(tsx?|jsx?)$/, "");
      const expectedPath = `components/${compName}.tsx`;
      const expectedPathJs = `components/${compName}.ts`;
      if (!allPaths.has(expectedPath) && !allPaths.has(expectedPathJs) && !allPaths.has(`components/${compName}/index.tsx`)) {
        issues.push(`CRITICAL [${f.path}]: imports '@/components/${compName}' but that file was never created → webpack Module not found`);
      }
    }
  }
...
async function buildPackageJson(
  ...
  let pkg: any = existFile ? JSON.parse(existFile.content) : {
    ...
    dependencies: { next: "15.1.9", react: "19.0.0", "react-dom": "19.0.0", "lucide-react": "0.475.0", clsx: "2.1.1", "tailwind-merge": "2.3.0" },
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ═════════════════════════════════════════════════════════════════
          // AGENT PLAN — décomposition visible des tâches en temps réel
          // Émis uniquement pour CODE_ACTION et FIX_ACTION (pas micro-edit)
          // ═════════════════════════════════════════════════════════════════
          if (decision === "CODE_ACTION" || decision === "FIX_ACTION") {
            try {
              const planPrompt = `Décompose la tâche suivante en 3 à 6 étapes courtes et précises (max 55 caractères chacune).
Réponds UNIQUEMENT avec du JSON valide, sans markdown ni explication : {"tasks":["étape 1","étape 2",...]}
Tâche : "${lastUserText.slice(0, 400)}"`;

              const planHistory = isAnthropic
                ? [{ role: "user" as const, content: planPrompt }]
                : [{ role: "user" as const, parts: [{ text: planPrompt }] }];

              const planOutput = await callAISilent(
                isAnthropic, anthropic, ai, MODEL_ID, planPrompt,
                planHistory,
              ...
              if (jsonMatch) {
                ...
                if (Array.isArray(parsed.tasks) && parsed.tasks.length >= 2) {
                  emit(`[AGENT:PLAN]${JSON.stringify({ tasks: parsed.tasks })}[/AGENT:PLAN]\n`);
                }
              }
            } catch { /* silencieux — le plan est optionnel, ne bloque pas */ }
          }
          ...
          // ═════════════════════════════════════════════════════════════════
          // PHASE 0.25 — RESEARCH AGENT
          // Lit les fichiers pertinents + cherche sur le web si besoin
          // Visible dans l'UI comme panel "N étapes" (à la Claude.ai)
          // ═════════════════════════════════════════════════════════════════
          let researchContext = "";

          if ((decision === "CODE_ACTION" || decision === "FIX_ACTION") && currentProjectFiles.length > 0) {
            try {
              const fileList = currentProjectFiles.map(f => `- ${f.path} (${f.content.split("\n").length} lignes)`).join("\n");

              const researchPrompt = `Tu es un agent de recherche. Analyse cette demande et décide quels fichiers lire et si une recherche web est nécessaire.

Demande utilisateur : "${lastUserText.slice(0, 500)}"
...
{
  "read_files": ["chemin/fichier1", "chemin/fichier2"],
  "web_search": "query de recherche ou null",
  "thought": "En une phrase : ce que tu vas faire et pourquoi"
}

Règles :
- read_files : max 4 fichiers, seulement ceux DIRECTEMENT liés à la demande
- web_search : seulement si bibliothèque inconnue, erreur à déboguer, ou info récente nécessaire. Sinon null
- Si la demande est simple et les fichiers suffisent, web_search = null`;

              const researchHistory = isAnthropic
                ? [{ role: "user" as const, content: researchPrompt }]
                : [{ role: "user" as const, parts: [{ text: researchPrompt }] }];

              const researchOutput = await callAISilent(
                isAnthropic, anthropic, ai, MODEL_ID, researchPrompt,
                researchHistory,
                { temperature: 0.2, maxTokens: 400 }
              );

              const jsonMatch = researchOutput.match(/\{[\s\S]*?"read_files"[\s\S]*?\}/);
              if (jsonMatch) {
                const plan = JSON.parse(jsonMatch[0]);
                const filesToRead: string[] = Array.isArray(plan.read_files) ? plan.read_files.slice(0, 4) : [];
                const searchQuery: string | null = plan.web_search && plan.web_search !== "null" ? plan.web_search : null;
                const thought: string = plan.thought || "";

                // Émettre la pensée initiale
                if (thought) {
                  emit(`[AGENT:THOUGHT]${JSON.stringify({ text: thought })}[/AGENT:THOUGHT]\n`);
                }

                // ── Lire les fichiers pertinents ──
                for (const filePath of filesToRead) {
                  const file = currentProjectFiles.find(f =>
                    f.path === filePath || f.path.endsWith(filePath) || filePath.endsWith(f.path)
                  );
                  if (!file) continue;

                  emit(`[AGENT:READ]${JSON.stringify({ file: file.path, lines: file.content.split("\n").length })}[/AGENT:READ]\n`);

                  // Injecter dans le contexte de recherche (extrait pertinent)
                  const snippet = file.content.slice(0, 8000);
                  researchContext += `\n\n=== FICHIER LU: ${file.path} ===\n${snippet}${file.content.length > 8000 ? "\n[...tronqué]" : ""}`;
                }

                // ── Recherche web via Tavily ──
                if (searchQuery && tavilyKey) {
                  emit(`[AGENT:SEARCH]${JSON.stringify({ query: searchQuery })}[/AGENT:SEARCH]\n`);
                  try {
                    ...
                    if (tavilyRes.ok) {
                      ...
                      const results = (tavilyData.results || []).slice(0, 3).map((r: any) =>
                        `[${r.title}](${r.url})\n${r.content?.slice(0, 400) || ""}`
                      ).join("\n\n");
                      researchContext += `\n\n=== RECHERCHE WEB: "${searchQuery}" ===\n${answer}\n\n${results}`;
                      emit(`[AGENT:SEARCH_DONE]${JSON.stringify({ query: searchQuery, hasResults: true })}[/AGENT:SEARCH_DONE]\n`);
                    }
                  } catch { /* recherche échouée — silencieux */ }
                }

                // Pensée finale après recherche
                if (filesToRead.length > 0 || searchQuery) {
                  const summary = filesToRead.length > 0 && searchQuery
                    ? `J'ai lu ${filesToRead.length} fichier(s) et cherché sur le web. Contexte complet, je commence à coder.`
                    : filesToRead.length > 0
                    ? `J'ai une compréhension claire de ${filesToRead.length} fichier(s). Je commence.`
                    : `Recherche web effectuée. Je procède.`;
                  emit(`[AGENT:THOUGHT]${JSON.stringify({ text: summary })}[/AGENT:THOUGHT]\n`);
                }
              }
            } catch { /* recherche échouée — ne bloque pas le reste */ }
          }
          ...
          // ── Inject forensic design analysis when reference images are present ──
          // Le main agent fait maintenant lui-même l'analyse pixel-perfect + le code React
          const hasDesignImages = allImages.length > 0;
          if (hasDesignImages) {
            systemPrompt += `\n\n` +
              `╔═══════════════════════════════════════════════════════════╗\n` +
              `║  FORENSIC DESIGN ANALYSIS — ACTIVE (images detected)      ║\n` +
              `╚═══════════════════════════════════════════════════════════╝\n` +
              `Des images de référence sont présentes. Tu DOIS appliquer le protocole\n` +
              `d'analyse forensique complet ci-dessous AVANT d'écrire la moindre ligne de code.\n\n` +
              DESIGN_FORENSIC_PROMPT;

            // Inject canvas color maps if available
            if (uploadedColorMaps && uploadedColorMaps.length > 0) {
              systemPrompt +=
                `\n\n╔══════════════════════════════════════════════════════════════════╗\n` +
                `║  CANVAS PIXEL-EXTRACTED COLOR DATA — SOURCE DE VÉRITÉ ABSOLUE   ║\n` +
                `╚══════════════════════════════════════════════════════════════════╝\n` +
                `Ces couleurs ont été extraites pixel par pixel via l'API Canvas du navigateur.\n` +
                `Tu DOIS utiliser UNIQUEMENT ces valeurs hex exactes. Zéro bg-gray-*. Zéro text-blue-*.\n\n` +
                uploadedColorMaps.join("\n\n") + "\n";
            }
          }

          // ── Inject document creation capability (always active — AI auto-detects intent) ──
          systemPrompt += `

══════════════════════════════════════════════════════════════════
DOCUMENT CREATION CAPABILITY — AUTO-DETECT FROM USER INTENT
══════════════════════════════════════════════════════════════════

You have TWO output modes. Choose automatically based on what the user asks:

▸ MODE 1 — VIBE CODING (default)
  User asks to build, create, or code an app/website/component/UI
  → Use <create_file path="..."> tags as you always do

▸ MODE 2 — DOCUMENT CREATION
  User asks to write, draft, create, or generate a document:
  reports, analyses, proposals, letters, specs, plans, summaries, essays, memos, etc.
  → Use <create_doc title="Exact Title"> tags instead of <create_file>

DOCUMENT FORMAT (when MODE 2 is detected):
  <create_doc title="Document Title Here">
  # Document Title

  ## Section
  Content with **bold**, *italic*, - bullets, 1. numbered lists, > blockquotes
...
RULES:
- Start writing <create_doc> immediately, stream the content continuously
- Use rich markdown inside: # ## ### **bold** *italic* - bullets > quotes
- Never mix <create_file> and <create_doc> in the same response
- After </create_doc>, write ONE short confirmation sentence, nothing more
- If ambiguous, default to MODE 1 (coding)
`;
          ...
          // Inject research context (files read + web search results from Phase 0.25)
          if (researchContext) {
            systemPrompt += `\n\n╔══════════════════════════════════════════════════════════╗\n║  RESEARCH CONTEXT — lu avant de coder (Phase 0.25)       ║\n╚══════════════════════════════════════════════════════════╝\n${researchContext}`;
          }
          ...
          // ── 2a-bis. Agent file summary markers ───────────────────────────
          // Émis après le stream Phase 1 pour l'UI agent (task list + file cards)
          if (newFiles.length > 0 && (decision === "CODE_ACTION" || decision === "FIX_ACTION")) {
            for (const f of newFiles) {
              const lines = f.content.split("\n").length;
              const ext = f.path.split(".").pop() ?? "";
              const isReact = ["tsx", "jsx"].includes(ext);
              const isTs = ["ts"].includes(ext);
              const icon = isReact ? "react" : isTs ? "ts" : ext;
              emit(`\n[AGENT:FILE]${JSON.stringify({ path: f.path, lines, op: "create", icon })}[/AGENT:FILE]`);
            }
          }
