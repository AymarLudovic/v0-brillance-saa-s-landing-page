  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          // ═════════════════════════════════════════════════════════════════
          // PHASE 0 — INTENT DETECTION (buffered, silent — only intro emitted)
          // Détecte CHAT_ONLY / ACTION + [IMAGE_IS_DESIGN_REF]
          // ═════════════════════════════════════════════════════════════════
          emit("\n[PRESENTER:INTRO]\n");
          ...
          try {
            ...
            rawPresenterOutput = (isErr || isFix) ? "FIX_ACTION\nJe m'en occupe immédiatement." : (fc === 0 ? "CODE_ACTION\nJe construis ça de suite." : "MICRO_EDIT_ACTION\nModification en cours.");
          }
          ...
          // ── Clean and emit presenter text ──
          let presenterText = rawPresenterOutput
            .replace(/^\[IMAGE_IS_DESIGN_REF\]\s*\n?/gm, "")
            .replace(/^(CODE_ACTION|FIX_ACTION|MICRO_EDIT_ACTION|CHAT_ONLY)\s*\n?/gm, "");
          // Strip any code that leaked
          const codeStartIdx = presenterText.search(/\[\[START\]\]|<create_file|<str_replace|<edit_file|```[a-z]/);
          if (codeStartIdx >= 0) presenterText = presenterText.slice(0, codeStartIdx);
          ...
          if (presenterText) emit(presenterText);
          emit("\n[/PRESENTER:INTRO]\n");
