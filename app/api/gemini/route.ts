    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Inject explicit project-status signal for better decision
          const fileCount = (currentProjectFiles ?? []).length;
          // Détecte si le message ressemble à un rapport d'erreur (erreur runtime, stacktrace, etc.)
          ...
          const projectStatusSignal = fileCount > 0
            ? `\n\n[SIGNAL SYSTÈME] Ce projet contient DÉJÀ ${fileCount} fichier(s).
${looksLikeError
  ? "⚠️ Le message semble signaler une ERREUR ou un BUG → utilise FIX_ACTION pour que le FIXER corrige le problème."
  : "Rappel des modes :\n- MICRO_EDIT_ACTION → tout changement VISUEL ou de CONTENU (texte, nom, couleur, section, style, icône)\n- FIX_ACTION → UNIQUEMENT si l'utilisateur signale une erreur/bug explicite\n- En cas de doute : MICRO_EDIT_ACTION."
}`
            : `\n\n[SIGNAL SYSTÈME] Aucun fichier de projet existant → CODE_ACTION si l'utilisateur veut créer une app.`;
          const lastPresenterPart = presenterContents[presenterContents.length - 1];
          if (lastPresenterPart && lastPresenterPart.role === "user") {
            lastPresenterPart.parts.push({ text: projectStatusSignal });
          }
          ...
          try {
            ...
            const fallback = fileCount > 0 ? "FIX_ACTION" : "CODE_ACTION";
            rawPresenterOutput = fallback + "\nJe m'occupe de votre demande tout de suite.";
          }
