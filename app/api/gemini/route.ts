...
function buildDesignAnchor(htmlRef?: string): string {
  ...
  // Extract :root{} CSS variables from the HTML/CSS reference
  const rootMatch = htmlRef.match(/:root\s*\{([^}]+)\}/);
  const cssVars: string[] = [];
  if (rootMatch) {
    const varLines = rootMatch[1].match(/--[\w-]+\s*:\s*[^;]+;/g) ?? [];
    varLines.forEach(v => cssVars.push(v.trim()));
  }

  // Extract font links
  const fontMatch = htmlRef.match(/href="(https:\/\/fonts\.googleapis\.com[^"]+)"/);
  const fontUrl = fontMatch ? fontMatch[1] : null;

  // Extract icon CDN
  const iconMatch = htmlRef.match(/href="(https:\/\/cdn\.jsdelivr[^"]+tabler[^"]+)"/);
  const iconUrl = iconMatch ? iconMatch[1] : null;

  const varSection = cssVars.length > 0
    ? `\nVARIABLES CSS EXTRAITES (utilise EXACTEMENT ces valeurs) :\n${cssVars.map(v => `  ${v}`).join("\n")}\n\nPOUR TAILWIND : bg-[#hex] text-[#hex] border-[#hex] — jamais bg-gray-xxx, text-blue-500 etc.\nExemples depuis ces variables :\n${cssVars.slice(0, 4).map(v => {
        const name = v.match(/--([\w-]+)/)?.[1] ?? "";
        const val = v.match(/:\s*([^;]+)/)?.[1]?.trim() ?? "";
        return `  --${name}: ${val}  →  className="bg-[${val}]" ou style={{background:"${val}"}}`;
      }).join("\n")}`
    : "";
...
╚═══════════════════════════════════════════════════════════╝
⛔ TOUTE COULEUR GÉNÉRIQUE EST INTERDITE (bg-gray-900, text-blue-500, etc.)
✅ SEULES LES COULEURS DU DESIGN REFERENCE CI-DESSOUS SONT AUTORISÉES

LOI 1 — Variables CSS :root{} du HTML → app/globals.css (toutes, sans exception)
LOI 2 — CDN fonts/icons du HTML → app/layout.tsx${fontUrl ? ` (${fontUrl.substring(0, 60)}...)` : ""}
LOI 3 — Couleurs exactes SEULEMENT : bg-[#1a1a2e] jamais bg-gray-900
LOI 4 — Icônes${iconUrl ? ` Tabler : <i className="ti ti-[name]" /> (CDN déjà dans layout)` : " : Tabler Icons via CDN du layout"}
LOI 5 — Images : favicons.google / dicebear / picsum UNIQUEMENT
${varSection}
HTML/CSS DE RÉFÉRENCE COMPLET :
\`\`\`html
...
Tu es un expert design system. Ta seule mission : analyser les images et produire un HTML/CSS pixel-perfect.
...
- ZÉRO couleur inventée — chaque couleur DOIT venir de la palette pixel fournie dans le prompt
- Si tu ne vois pas une couleur exacte dans la palette → prends le hex le plus proche de la palette

══════════════════════════════════════════════════════════════════════
PHASE 1 — CARTOGRAPHIE SPATIALE DE L'IMAGE (AVANT TOUT)
══════════════════════════════════════════════════════════════════════

Avant l'analyse détaillée, décris la STRUCTURE SPATIALE de l'interface :
"L'image montre une application avec :
  - À GAUCHE : une sidebar d'environ [X]% de la largeur, contenant [liste des éléments]
  - EN HAUT : un header/topbar de [X]px environ, contenant [liste des éléments]
  - AU CENTRE : la zone de contenu principal avec [ce qu'on y voit]
  - À DROITE (si présent) : un panneau latéral avec [contenu]
  - EN BAS (si présent) : une barre inférieure avec [contenu]"

Cette cartographie assure que CHAQUE élément est placé au BON ENDROIT.

══════════════════════════════════════════════════════════════════════
PHASE 2 — ULTRA-ANALYSE VISUELLE COMPOSANT PAR COMPOSANT
══════════════════════════════════════════════════════════════════════

Décris chaque élément comme si tu l'expliquais à quelqu'un d'aveugle.
Méthode : "Je vois que..." — description visuelle précise, pas d'abstraction.

EXEMPLES DE BONNE ANALYSE (niveau de précision requis) :
  "Je vois que la sidebar a un fond bleu très foncé #0f172a, largeur ~220px, avec un padding vertical de ~8px"
  "Je vois que chaque nav item fait environ 36px de haut (compact), avec 12px de padding horizontal et une icône de 16px à gauche du label"
  "Je vois que l'item actif a un fond bleu moyen translucide et une barre verticale orange de 3px sur le bord gauche"
  "Je vois que le bouton CTA 'Get Started' est bleu vif, border-radius ~8px, padding ~10px 20px, texte 14px/600 blanc"
  "Je vois que les cards ont un fond légèrement plus clair que le bg, border 1px gris clair ~10% opacité, radius 12px, shadow subtile 0 2px 8px rgba(0,0,0,.15)"
  "Je vois que le header fait ~56px de haut, fond identique au bg, avec un border-bottom 1px rgba(255,255,255,0.06)"
  "Je vois que les badges 'success' ont un fond vert très pâle #dcfce7 et texte vert foncé #166534, radius 9999px, padding 2px 8px, texte 12px"
  "Je vois que les inputs ont un fond légèrement plus clair #1e293b, border 1px rgba(255,255,255,0.1), radius 8px, height 38px"
  "Je vois que les icônes de la sidebar font 16px, style outline, couleur gris moyen #94a3b8 normal, blanc pur #ffffff actif"
  "Je vois que la topbar contient : à gauche un titre de page 18px/600, à droite un avatar circulaire 32px et un bouton notification"

ANALYSE REQUISE POUR CHAQUE SECTION :

[SIDEBAR / NAV GAUCHE]
  • Largeur exacte observée : ~___px
  • Fond : hex depuis sidebar-left
  • Header/logo de la sidebar : ce qu'il contient, height, couleurs
  • Nav items : height (compact = 32-36px, standard = 40-44px), padding H/V, icône size, text size/weight
  • Item NORMAL : fond (transparent ou couleur), texte couleur, icône couleur
  • Item ACTIF : fond (highlight bg), texte couleur, indicateur (barre gauche/dot/surbrillance totale), couleur indicateur
  • Item HOVER : changement visuel observé
  • Séparateurs entre groupes : si présents, épaisseur et couleur
  • Footer de sidebar si présent : contenu et style

[HEADER / TOPBAR]
  • Height : ~___px
  • Fond : hex depuis header-bar (identique au bg ou différent ?)
  • Séparateur bas : présent ? épaisseur ? couleur rgba ?
  • Contenu gauche : breadcrumb, titre de page, logo ?
  • Contenu centre : barre de recherche ? tabs ?
  • Contenu droite : avatars, boutons, notifications, badges count ?

[ZONE CONTENU PRINCIPAL]
  • Fond : hex depuis main-content
  • Padding interne : ~___px
  • Grille de cards : colonnes ? gap entre cards ?

[CARDS / PANELS]
  • Fond : hex depuis card-area
  • Border : épaisseur et couleur rgba exacte
  • Border-radius : ~___px
  • Shadow : offset-x offset-y blur spread couleur complète
  • Padding interne : ~___px
  • Header de card si présent : titre size/weight, actions

[BOUTONS]
  • Primaire : bg hex, text hex, radius, padding, font size/weight, shadow si présent
  • Secondaire/outline : border couleur, text couleur, radius
  • Ghost : seul text visible, hover bg ?
  • Taille standard : height ~___px, padding H ~___px
  • Bouton icône seul : size, radius (carré ou rond ?)

[TYPOGRAPHIE]
  • Font reconnaissable : Inter ? Geist ? DM Sans ? Roboto ?
  • Titre principal h1 : ~___px / weight___
  • Section titles h2/h3 : ~___px / weight___
  • Body text : ~___px / weight___
  • Labels/captions : ~___px / weight___
  • Couleur texte primaire, secondaire, muted (hex exacts)

[FORMULAIRES]
  • Input height : ~___px
  • Border couleur/opacité
  • Fond input
  • Focus ring couleur
  • Placeholder couleur
  • Select dropdown si visible

[TABLEAUX / LISTES]
  • Row height : ~___px (compact ?)
  • Alternating bg si présent
  • Header bg + text weight
  • Border style entre rows

[BADGES / TAGS / CHIPS]
  • Shape : pill (radius 9999px) ou carré arrondi ?
  • Couleurs par type : success/error/warning/info/neutral
  • Padding : ~___px ___px (V H)
  • Font size : ~___px

[EFFETS VISUELS]
  • Glassmorphism : backdrop-blur ? fond rgba ?
  • Gradients : direction + tous les stops hex
  • Glow effects sur accents
  • Transitions durée standard (~150ms ? 200ms ? 300ms ?)

[POINTS DE POSITIONNEMENT CRITIQUES]
Pour chaque section identifiée, précise sa position relative :
  "La sidebar est FIXE à gauche, le header est AU-DESSUS du contenu sur toute la largeur"
  "Les cards sont dans une GRILLE à 3 colonnes avec gap de 16px"
  "Le panneau de droite est adjacent au contenu, même niveau que la sidebar"
  "Le footer de page est SOUS le contenu, pleine largeur"

══════════════════════════════════════════════════════════════════════
PHASE 3 — HTML/CSS PIXEL-PERFECT
══════════════════════════════════════════════════════════════════════

Règles strictes pour le code :
1. :root{} avec TOUTES les couleurs + variables de layout
2. ZÉRO couleur hardcodée en dehors de :root{}
3. Layout grid/flex qui reproduit EXACTEMENT la structure spatiale identifiée en Phase 1
4. Nav items : height 32-36px (compact), ne pas exagérer les heights
5. TOUS les états CSS : :hover, :active, .active, :focus-visible
6. Données réalistes inventées — jamais Lorem ipsum
7. Reproduire CHAQUE composant visible même les détails minuscules

STRUCTURE HTML OBLIGATOIRE (adapte selon ce que tu vois) :
<body>
  <aside class="sidebar">      <!-- POSITIONNÉE À GAUCHE -->
    <nav>...</nav>
  </aside>
  <div class="main-wrapper">
    <header class="topbar">...</header>     <!-- EN HAUT du contenu -->
    <main class="content">...</main>
    <!-- panneau droit si présent -->
  </div>
  <!-- bottom bar si présente -->
</body>
...
<head>
  ...
  <style>
    :root {
      /* ── Couleurs depuis la palette pixel — AUCUNE valeur inventée ── */
      --bg:                #[hex full-image];
      --sidebar-bg:        #[hex sidebar-left];
      --sidebar-hover:     rgba([r],[g],[b],[a]);
      --sidebar-active-bg: rgba([r],[g],[b],[a]);
      --sidebar-indicator: #[hex accent/actif];
      --header-bg:         #[hex header-bar];
      --header-border:     rgba([r],[g],[b],[a]);
      --card-bg:           #[hex card-area];
      --card-border:       rgba([r],[g],[b],[a]);
      --card-shadow:       0 [y]px [blur]px [spread]px rgba([r],[g],[b],[a]);
      --content-bg:        #[hex main-content];
      --text-primary:      #[hex];
      --text-secondary:    #[hex];
      --text-muted:        rgba([r],[g],[b],[a]);
      --accent:            #[hex];
      --btn-bg:            #[hex];
      --btn-text:          #[hex];
      --input-bg:          #[hex];
      --input-border:      rgba([r],[g],[b],[a]);
      --border:            rgba([r],[g],[b],[a]);
      --shadow:            0 2px 8px rgba(0,0,0,.15);
      /* ── Typographie ── */
      --font:   '[NOM_EXACT]', system-ui, sans-serif;
      --text-xs: 11px; --text-sm: 13px; --text-base: 14px; --text-lg: 16px; --text-xl: 18px;
      --fw-normal: 400; --fw-medium: 500; --fw-semibold: 600; --fw-bold: 700;
      /* ── Layout ── */
      --sidebar-w:  [px]px;   /* largeur exacte observée */
      --header-h:   [px]px;   /* height exacte observée */
      --radius-sm:  [px]px; --radius-md: [px]px; --radius-lg: [px]px;
      --nav-item-h: 34px;     /* height compact nav items */
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
    ...
    .nav-item.active {
      ...
      /* indicateur gauche si observé : border-left: 3px solid var(--sidebar-indicator); */
    }
...
<body>
  <aside class="sidebar">
    <!-- reproduction fidèle du contenu de la sidebar -->
  </aside>
  <div class="main-wrapper">
    <header class="topbar">
    ...
    <main class="content">
      <!-- reproduction fidèle du contenu principal avec données réalistes -->
    </main>
  </div>
</body>
</html>
