...
══════════════════════════════════════════════════════════════════════
LOI ABSOLUE — RÉPARTITION FRONTEND / BACKEND
══════════════════════════════════════════════════════════════════════

⚠️ VIOLATION LA PLUS FRÉQUENTE : mettre la logique métier dans React au lieu de Python.
Cette loi prime sur TOUT le reste.

━━ PYTHON FASTAPI — TOUT CE QUI TRAITE, CALCULE OU STOCKE ━━━━━━━━━━

OBLIGATOIREMENT dans backend/main.py ou backend/services/ :
  ✅ Toute logique métier (algorithmes, calculs, transformations de données)
  ✅ Gestion de l'état applicatif persistant (projets, sessions, fichiers)
  ✅ Traitement audio/vidéo (mixage, effets, enregistrement, export)
  ✅ Manipulation de fichiers (lecture, écriture, ZIP, MP3, WAV, PDF...)
  ✅ Connexion à des services externes (APIs, bases de données, mails)
  ✅ IA / ML / traitement NLP
  ✅ Automatisation (Playwright, scraping)
  ✅ Export de données (CSV, PDF, audio, vidéo)

━━ REACT/NEXT.JS — UNIQUEMENT L'INTERFACE ET LES CAS ZÉRO-LATENCE ━━

Le frontend ne contient QUE :
  ✅ Rendu UI (composants, layout, état visuel local)
  ✅ Appels vers /api/py/... et affichage des résultats
  ✅ Routing et navigation
  ✅ État UI local (quel onglet, si modal ouvert, valeur d'un input)

  EXCEPTION ZÉRO-LATENCE (< 20ms requis, Web API native) :
  ✅ Sons instantanés au clic (Web Audio API — synthèse directe, pas de fichier)
  ✅ Rendu canvas/WebGL frame-by-frame
  ✅ Animations et transitions CSS/JS
  → MAIS : même dans ce cas, le stockage, l'export et le traitement lourd restent Python

━━ RÈGLE DE DÉCISION — APPLIQUE-LA POUR CHAQUE FONCTIONNALITÉ ━━━━━

  QUESTION : "Cette fonctionnalité produit-elle, traite-t-elle ou stocke-t-elle des données ?"
  → OUI  : Python FastAPI. Toujours. Sans exception.
  → NON  : C'est une interaction visuelle pure → peut rester React

  QUESTION : "Cette action a-t-elle besoin de moins de 20ms pour ne pas sembler lente ?"
  → OUI  : Web API native côté client (Web Audio, Canvas, requestAnimationFrame)
            MAIS les données manipulées/sauvegardées passent quand même par Python
  → NON  : Python FastAPI

━━ EXEMPLES CONCRETS — ANTI-PATTERNS VS PATTERNS CORRECTS ━━━━━━━━━

  ❌ FAUX — DAW / logiciel audio :
    "Je génère les pistes avec useState([]) dans React"
    "J'enregistre avec MediaRecorder et stocke en base64 dans le state React"
    "Je mixe les pistes avec une fonction JS dans le composant"

  ✅ CORRECT — DAW / logiciel audio :
    Python : POST /tracks/create → crée une piste, retourne son ID
    Python : POST /tracks/{id}/record → reçoit le blob audio, stocke en session RAM
    Python : POST /project/export → mixe avec pydub, retourne FileResponse WAV/MP3
    React  : Web Audio API pour le son instantané des touches → python pour sauvegarder
    React  : affiche l'état reçu de Python, appelle les endpoints pour toute action

  ❌ FAUX — Instrument virtuel avec enregistrement :
    "L'enregistrement se fait avec MediaRecorder côté React, stocké dans useState"
    "Je génère le fichier audio avec une lib npm côté client"

  ✅ CORRECT — Instrument virtuel avec enregistrement :
    React  : Web Audio API pour les sons immédiats des touches (zéro latence)
    Python : POST /record/start → initialise la session d'enregistrement
    Python : POST /record/chunk → reçoit les chunks audio du client
    Python : POST /record/stop → finalise, retourne FileResponse

  ❌ FAUX — Application de montage vidéo :
    "Je gère la timeline avec un tableau JS dans le composant"
    "J'applique les effets avec canvas côté client"

  ✅ CORRECT — Application de montage vidéo :
    Python : POST /project/timeline → gère la timeline (clips, ordre, durée)
    Python : POST /clip/effect → applique l'effet avec moviepy/ffmpeg
    Python : GET /project/preview → retourne la préview encodée
    Python : POST /project/export → export final FileResponse
...
"DAW/audio pro" → Python pydub/librosa + WebSocket + Web Audio API côté React pour son immédiat
...
"Instrument virtuel" → Web Audio API React pour son immédiat + Python pour enregistrement/export
"Éditeur de musique (FL Studio)" → Python pydub/librosa pour TOUTE la logique audio + React UI
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ERREUR #20 — LOGIQUE MÉTIER DANS REACT (l'erreur la plus coûteuse)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SYMPTÔME : L'app ressemble à ce qui est demandé MAIS ne fait rien de réel.
Boutons présents, fonctionnalités absentes. UI correcte, logique manquante.

Cette erreur survient quand un agent place dans React ce qui DOIT être dans Python.

❌ PATTERNS INTERDITS (crash fonctionnel, pas de build) :

  // FAUX — gestion de pistes audio dans un composant React
  const [tracks, setTracks] = useState<Track[]>([]);
  const addTrack = () => setTracks(prev => [...prev, { id: Date.now(), clips: [] }]);
  // ← Cette piste n'existe nulle part. Elle disparaît au rechargement. Export impossible.

  // FAUX — enregistrement audio géré uniquement côté React
  const mediaRecorder = useRef<MediaRecorder | null>(null);
  const chunks = useRef<Blob[]>([]);
  // ← Les chunks ne sont jamais traités, pas d'export, pas de mixage possible.

  // FAUX — effet audio appliqué côté client avec une lib npm
  import { applyReverb } from 'some-audio-lib';
  const processed = applyReverb(audioBuffer, 0.5);
  // ← Impossible d'exporter, de sauvegarder, de chaîner avec d'autres effets.

  // FAUX — logique de timeline vidéo dans le state React
  const [timeline, setTimeline] = useState<Clip[]>([]);
  const moveClip = (id: string, position: number) => setTimeline(...);
  // ← La timeline meurt au rechargement. Export impossible sans Python.

✅ PATTERNS CORRECTS :

  // CORRECT — La piste existe dans Python, React affiche l'état
  const { data: tracks } = useTracks();  // hook qui appelle GET /api/py/tracks
  const addTrack = async () => {
    await fetch("/api/py/tracks", { method: "POST", body: JSON.stringify({ name: "Piste 1" }) });
    mutate();  // re-fetch
  };

  // CORRECT — Enregistrement : React capture, Python traite
  // React : MediaRecorder → envoie chunks → POST /api/py/record/chunk
  // Python : stocke en session RAM, traite avec pydub, retourne FileResponse

  // CORRECT — Effet audio : Python l'applique
  const applyEffect = async (trackId: string, effect: string, params: object) => {
    await fetch("/api/py/tracks/" + trackId + "/effect", {
      method: "POST", body: JSON.stringify({ effect, params })
    });
  };

RÈGLE RÉSUMÉE :
  Si tu veux écrire const [données, setDonnées] = useState([]) pour stocker AUTRE CHOSE
  qu'un état UI local (onglet actif, modal ouvert, valeur de champ) → STOP.
  Ces données vont dans Python. React ne fait qu'appeler l'endpoint et afficher le résultat.

EXCEPTION LÉGALE — useState est autorisé pour :
  ✅ activeTab, isModalOpen, searchQuery, inputValue
  ✅ données de mock temporaires en attendant l'endpoint
  ✅ état de chargement (isLoading, error)
  ❌ JAMAIS pour des données métier (pistes, clips, projets, enregistrements, effets)
...
⚡ RÈGLE D'OR AVANT DE CODER QUOI QUE CE SOIT :
Pour chaque fonctionnalité de l'app, pose-toi ces deux questions :

  1. "Est-ce que ça traite, stocke ou manipule des données ?"
     → OUI : Python FastAPI. Endpoint complet. Pas de simulation.

  2. "Est-ce que ça a besoin de < 20ms pour ne pas sembler lent ?"
     → OUI : Web API native côté React (Web Audio, Canvas, rAF)
              MAIS : sauvegarde et traitement lourd restent dans Python.
     → NON : Python FastAPI.

CORRESPONDANCES CONCRÈTES :

  Bouton "Créer une piste"   → POST /tracks/create                (Python stocke)
  Bouton "Enregistrer"       → Python reçoit les chunks audio     (MediaRecorder→/api/py)
  Bouton "Exporter"          → POST /project/export → FileResponse (Python pydub/ffmpeg)
  Bouton "Appliquer effet"   → POST /tracks/{id}/effect           (Python traite)
  Bouton "Sauvegarder"       → POST /project/save                 (Python écrit)
  Slider de volume           → WebSocket /ws/preview → Python recalcule
  Note de piano au clic      → Web Audio API React (< 5ms, zéro latence) ✅ EXCEPTION
  Canvas de dessin           → Canvas React (rendu immédiat) + POST /save pour persister
  Upload fichier             → POST /upload multipart → session RAM → traitements Python
  API externe                → service Python qui appelle vraiment l'API
  Télécharger résultat       → POST /export → FileResponse Python

⛔ INTERDICTIONS ABSOLUES dans la couche Foundation :
  ❌ useState pour stocker des données métier (pistes, clips, projets, effets...)
  ❌ Logique de traitement dans un composant React (mixage, calcul, export...)
  ❌ "TODO: implement" ou route Python vide avec pass
  ❌ Simulation de fonctionnalité côté React si Python peut la faire vraiment
...
□ RATIO CRITIQUE : nombre d'endpoints Python ≥ nombre de fonctionnalités demandées ?
...
══════════════════════════════════════════════════════════════════════
ÉTAPE 2bis — AUDIT FUITES LOGIQUES VERS LE FRONTEND (CRITIQUE)
══════════════════════════════════════════════════════════════════════

⚡ C'est l'erreur la plus fréquente. Lis les stores/*.ts et services/*.ts et vérifie :

□ Y a-t-il des useState ou des tableaux en dur dans les stores qui stockent des DONNÉES MÉTIER ?
  Données métier = pistes, clips, projets, enregistrements, effets, fichiers, entités persistantes
  → Si oui : CES DONNÉES doivent venir de Python. Génère l'endpoint manquant, connecte le store.

□ Y a-t-il des fonctions de traitement dans les stores qui devraient être dans Python ?
  Exemples suspects : mixAudio(), applyEffect(), exportProject(), processData()
  → Si oui : déplace la logique dans un endpoint Python, le store appelle /api/py/...

□ Y a-t-il des données mock en dur (tableau de constantes JS) censées être dynamiques ?
  → Si oui : génère le vrai endpoint Python GET /data → le store fetchData() remplace le mock

□ Le backend/main.py a-t-il AUTANT d'endpoints que de fonctionnalités dans l'app ?
  Règle : 1 fonctionnalité = 1 endpoint Python minimum.
  Si tu comptes 8 fonctionnalités dans la demande et 2 endpoints Python → PROBLÈME GRAVE.
  → Génère les endpoints manquants maintenant.

CATÉGORIES D'ENDPOINTS ATTENDUS selon le type de projet :

  App de montage audio/DAW :
    POST /tracks/create, DELETE /tracks/{id}
    POST /tracks/{id}/record/start, POST /tracks/{id}/record/stop
    POST /tracks/{id}/effect   → pydub/librosa applique l'effet
    POST /project/export       → pydub mixe tout → FileResponse WAV/MP3
    GET  /project/state        → retourne l'état complet du projet

  App de productivité :
    CRUD complet (GET/POST/PUT/DELETE) pour chaque entité
    POST /export → PDF/CSV via reportlab/pandas

  App de trading/finance :
    GET  /market/data → ccxt appelle vraiment l'exchange
    POST /order/place → ccxt passe vraiment l'ordre
    GET  /portfolio   → calculs côté Python

  Instrument virtuel :
    → Son immédiat : Web Audio API React ✅ (zéro latence, pas d'endpoint)
    → Enregistrement : POST /record/chunk + POST /record/stop → FileResponse
    → Sauvegarde partition : POST /score/save → Python écrit le fichier
...
⛔ INTERDICTION ABSOLUE — LOGIQUE MÉTIER DANS LES VUES :
  ❌ JAMAIS de useState pour stocker des données métier (pistes, projets, entités, fichiers)
  ❌ JAMAIS de fonctions de traitement dans un composant (calculs, mixage, export, effets)
  ❌ JAMAIS de tableaux de données en dur censés être dynamiques
  ❌ JAMAIS d'appels fetch("/api/py/...") inventés sans endpoint dans backend/main.py

  ✅ La vue IMPORTE le hook : const { tracks, addTrack, deleteTrack } = useTracksHook();
  ✅ Le hook appelle /api/py/... (généré par Checker) — la vue ne sait pas d'où viennent les données
  ✅ Les seuls useState autorisés : activeTab, isModalOpen, searchQuery, inputValue

  Si une fonctionnalité n'a pas d'endpoint Python → signale-le en commentaire, ne l'invente pas.
...
7. LOGIQUE MÉTIER DANS LES VUES (fuite frontend) :
   Cherche : useState qui stocke des données métier (pistes, clips, projets, fichiers...)
             fonctions de traitement inline (mixage, calcul, export, effets)
             tableaux de données en dur censés être dynamiques
   Fix : ces données doivent venir du store/hook qui appelle /api/py/...
         Si l'endpoint Python n'existe pas → ajoute un TODO visible MAIS ne simule pas

8. DONNÉES MOCK QUI MASQUENT DES FONCTIONNALITÉS ABSENTES :
   Cherche : const MOCK_DATA = [...] utilisées comme si elles étaient réelles
   Fix : si un endpoint Python existe → branche dessus
         si aucun endpoint n'existe pour ces données → note-le clairement
