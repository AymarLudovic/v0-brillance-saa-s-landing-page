      ...
      async start(controller) {
        ...
        try {
          ...
          if (isDesignRef) {
            ...
            // Collect design images: uploaded + reference + vibe categories
            let designImages = [...(uploadedImages || []), ...(allReferenceImages || [])].slice(0, 3);

            // Add vibe images from all categories (up to 2 per category, 6 total)
            if (vibesByCategory && Object.keys(vibesByCategory).length > 0) {
              ...
              designImages = [...designImages, ...vibeImgs].slice(0, 8);
              if (vibeCategoryNames.length > 0)
                emit(`[DESIGN:THINKING] Catégories de vibes disponibles: ${vibeCategoryNames.join(", ")}\n`);
            }
...
`;
          ...
          if (vibeCategoryNames && vibeCategoryNames.length > 0) {
            systemPrompt += `\n\nDESIGN REFERENCE CATEGORIES AVAILABLE: ${vibeCategoryNames.join(", ")}\n` +
              `Tu peux demander des images d'une catégorie via: <request_vibes category="UI" count="3"/>\n` +
              `(Ces images seront transmises au Design Agent pour le build du design anchor.)\n`;
          }
