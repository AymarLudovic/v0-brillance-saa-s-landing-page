...
const TSC_CONFIG = JSON.stringify({
  compilerOptions: {
    ...
    allowJs: true,
    skipLibCheck: false,              // ← false : on veut vérifier nos propres .d.ts de stubs
    strict: false,
    noEmit: true,
    ...
    typeRoots: ["./node_modules/@types", "./__typeroots__"],  // ← charge nos stubs comme @types
  },
  include: ["**/*.ts", "**/*.tsx"],
  exclude: ["node_modules", ".next"],  // ← BUG CORRIGÉ : __stubs__ n'est plus exclu
}, null, 2);
...
async function runTscCheck(
  ...
  try {
    ...
    onProgress(`[TSC:STUBS] ${stubCount} stubs générés dynamiquement pour les packages externes.\n`);

    await sbx.files.write("tsconfig.json", TSC_CONFIG);

    // ⚠️ MÉTHODE TYPEROOT : on écrit les stubs dans __typeroots__/index.d.ts
    // TypeScript les charge automatiquement via "typeRoots": ["./__typeroots__"]
    // C'est plus fiable que d'écrire un .d.ts à la racine qui peut être exclu
    await sbx.files.write("__typeroots__/index.d.ts", dynamicStubs);
    // On écrit AUSSI à la racine pour double couverture
    await sbx.files.write("_stubs.d.ts", dynamicStubs);

    // Installe TypeScript + @types/react + @types/react-dom + @types/node
    // ⚠️ CRITIQUE : sans @types/react, tsc ne connaît pas JSX ni React.FC ni IntrinsicAttributes
    // → laisse passer silencieusement les erreurs de props (className manquant, etc.)
    // ⚠️ CRITIQUE : sans @types/node, tsc ne connaît pas process, Buffer, etc.
    onProgress("[TSC:INSTALL] Installation de TypeScript + @types/react + @types/react-dom + @types/node...\n");
    ...
    // Écrit TOUS les fichiers .ts et .tsx
    // + les fichiers types/index.ts même s'ils sont déjà dans la liste (double sécurité)
    const tsFiles = files.filter(f => f.path.endsWith(".ts") || f.path.endsWith(".tsx"));
    onProgress(`[TSC:FILES] Écriture de ${tsFiles.length} fichiers .ts/.tsx dans le sandbox...\n`);
    await Promise.all(tsFiles.map(f => sbx!.files.write(f.path, f.content)));

    // Vérifie que les fichiers critiques ont bien été écrits
    const criticalFiles = ["types/index.ts", "lib/utils.ts"];
    const verifyResult = await sbx.commands.run(
      criticalFiles.map(f => `test -f ${f} && echo "OK:${f}" || echo "MISSING:${f}"`).join(" && "),
      { timeoutMs: 5_000 }
    );
    const missingCritical = (verifyResult.stdout ?? "").split("\n").filter(l => l.startsWith("MISSING:"));
    if (missingCritical.length > 0) {
      onProgress(`[TSC:WARN] Fichiers manquants dans le sandbox : ${missingCritical.join(", ")}\n`);
    }

    // Vérifie que les stubs sont bien chargés
    const stubsVerify = await sbx.commands.run(
      "test -f _stubs.d.ts && wc -l < _stubs.d.ts || echo '0'",
      { timeoutMs: 3_000 }
    );
    onProgress(`[TSC:STUBS] Fichier stubs présent dans le sandbox : ${(stubsVerify.stdout ?? "").trim()} lignes.\n`);
    ...
    // ── DEBUG COMPLET : affiche TOUTES les lignes d'erreur brutes pour diagnostique ──
    const rawErrorLines = rawOutput.split("\n").filter(l => l.includes("error TS"));
    if (rawErrorLines.length === 0) {
      // Pas d'erreur du tout
    } else if (rawErrorLines.length <= 10) {
      onProgress(`[TSC:RAW] Sortie brute tsc (${rawErrorLines.length} erreurs) :\n${rawErrorLines.join("\n")}\n`);
    } else {
      onProgress(
        `[TSC:RAW] ${rawErrorLines.length} erreurs brutes — premières :\n` +
        rawErrorLines.slice(0, 5).join("\n") + "\n...\n"
      );
    }

    // ── Filtre : ne garde que les vraies erreurs de code, exclut les bruit de résolution ──
    // Règle : "Cannot find module 'xyz'" est ignoré SEULEMENT pour les packages npm externes
    // "Cannot find module '@/xyz'" est GARDÉ — c'est un import interne cassé
    const realErrorLines = rawOutput
      .split("\n")
      .filter(l => {
        if (!l.includes("error TS")) return false;

        // Erreurs dans nos propres fichiers de stubs → toujours ignorer
        if (l.includes("_stubs.d.ts") || l.includes("__typeroots__")) return false;

        // "Cannot find module" ou "Could not find declaration file"
        if (l.includes("Cannot find module") || l.includes("Could not find a declaration file")) {
          ...
          if (modMatch) {
            ...
            // Import interne (@/ ou relatif) = vraie erreur, on la GARDE
            if (mod.startsWith("@/") || mod.startsWith("./") || mod.startsWith("../")) return true;
            // Package npm externe = ignoré (couvert par stubs shorthand)
            return false;
          }
          // "Could not find declaration file for module 'xyz'" = ignoré
          return false;
        }

        return true; // Toute autre error TS = vraie erreur de code
      });
...
`;
              ...
              if (fixedFiles.length > 0) {
                ...
                emit(`\n[TSC:FIXER] Aucun fichier émis par le fixer — erreurs peut-être déjà résolues.\n`);
                break;
              }
            }
          } else {
