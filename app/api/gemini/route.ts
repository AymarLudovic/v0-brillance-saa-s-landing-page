...
function buildDynamicStubs(files: { path: string; content: string }[]): string {
  ...
  for (const f of files) {
    if (!f || !f.path || !f.content || typeof f.path !== "string" || typeof f.content !== "string") continue;
    const rx = /from\s+['"](@?[^./'"@][^'"]*)['"]/g;
    let m;
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ═══════════════════════════════════════════════════════════════
          // ═══════════════════════════════════════════════════════════════
          // ÉTAPE 1 — DÉCISION 100% DÉTERMINISTE (regex, zéro LLM)
          // + STREAM INTRO PRESENTER
          // ═══════════════════════════════════════════════════════════════
          const fileCount = (currentProjectFiles ?? []).length;

          // ── Routing déterministe ─────────────────────────────────────────────
          // Ordre d'évaluation : ERREUR > NOUVEAU PROJET > QUESTION > MICRO_EDIT
          const RE_ERROR = /ReferenceError|TypeError|SyntaxError|Cannot find|is not defined|is not a function|Module not found|Cannot read prop|ENOENT|build fail|failed to compile|npm ERR|corrige (cette|l[ae']|les|ces)|fixe (cette|l[ae'])|répare|résous (cette|l[ae'])/i;
          const RE_FIX   = /\b(corrige[rz]?|corriger|fixe[rz]?|fixer|répare[rz]?|réparer|résous|résoudre|debug|débug|l['']erreur|cette erreur|bug|broken|cassé|ne (marche|fonctionne) pas|marche pas|fonctionne pas)\b/i;
          const RE_NEW   = /\b(crée|créer|construis|génère|générer|nouvelle? (app|application|projet)|from scratch|de zéro|nouveau projet)\b/i;
          const RE_CHAT  = /^(qu[e']|est-ce que|comment|pourquoi|quand|quel[le]?|que penses|explique|c['']est quoi|dis-moi|c['']est quoi)/i;

          let decision: string;
          const msgLower = lastUserMsg;
          if (fileCount === 0 || RE_NEW.test(msgLower) && fileCount < 3)  decision = "CODE_ACTION";
          else if (RE_ERROR.test(msgLower) || RE_FIX.test(msgLower))       decision = "FIX_ACTION";
          else if (RE_CHAT.test(msgLower.trim()) && fileCount > 0)         decision = "CHAT_ONLY";
          else                                                               decision = "MICRO_EDIT_ACTION";

          emit(`\n[DEBUG:DECISION] ${decision} | files=${fileCount} | error=${RE_ERROR.test(msgLower)} | fix=${RE_FIX.test(msgLower)}\n`);
          ...
          if (lastPart?.role === "user") {
            lastPart.parts.push({ text: `\n\n[CONTEXTE PROJET — ${fileCount} fichier(s)]\n${projectContext.slice(0, 600)}\n\n[DÉCISION DÉJÀ PRISE: ${decision}]\nÉcris 1-2 phrases d'intro naturelles en français. Pas de mot-clé de décision.` });
          }
          try {
            ...
            rawPresenterOutput = decision === "FIX_ACTION" ? "Je corrige ça immédiatement."
              : decision === "MICRO_EDIT_ACTION" ? "Je mets à jour ça tout de suite."
              : decision === "CODE_ACTION" ? "Je construis ça maintenant."
              : "Je suis là.";
          }
          ...
          if (rawPresenterOutput.includes("[IMAGE_IS_DESIGN_REF]") && uploadedImages && uploadedImages.length > 0) {
            ...
            // Image de design → force CODE_ACTION
            if (decision !== "CODE_ACTION") decision = "CODE_ACTION";
          }
