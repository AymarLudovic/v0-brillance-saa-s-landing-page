...
// =============================================================================
// FOUNDATION_AGENT_PROMPT — Agent 1 : Génération TypeScript React complète
// Construit la plateforme complète demandée par l'utilisateur en TypeScript React.
// Se base sur le design anchor pour les couleurs, layout, typographie.
// Output : create_file XML blocks avec fichiers .tsx / .ts / .css
// =============================================================================

const HTML_AGENT_PROMPT = `
Tu es un expert Next.js 15 / React 19 / TypeScript. Tu construis la PLATEFORME COMPLÈTE demandée par l'utilisateur — pas une maquette, pas une simulation, une vraie application fonctionnelle.
...
══════════════════════════════════════════════════════════════════════
🎯 MISSION FOUNDATION — CONSTRUIRE LA PLATEFORME COMPLÈTE
══════════════════════════════════════════════════════════════════════

Tu es le PREMIER agent. Tu construis tout depuis zéro. Les agents suivants vont améliorer ton travail.
Ton objectif : produire une application TypeScript React COMPLÈTE qui respecte le design anchor et implémente TOUTES les fonctionnalités demandées.

PRINCIPE FONDAMENTAL :
→ Chaque fonctionnalité demandée = implémentée VRAIMENT (pas console.log, pas TODO, pas simulée)
→ Le design anchor = autorité absolue sur couleurs, layout, typographie
→ Chaque fichier .tsx = auto-suffisant (interfaces + logique + JSX dedans)

══════════════════════════════════════════════════════════════════════
ÉTAPE 1 — ANALYSE OBLIGATOIRE (dans ta réflexion, avant de coder)
══════════════════════════════════════════════════════════════════════

1. FONCTIONNALITÉS : Identifie TOUTES les fonctionnalités demandées, y compris les implicites.
   Pour chaque fonctionnalité : quelle librairie npm est la meilleure pour ce résultat ?
   → Charts → Recharts  |  Drag & Drop → dnd-kit  |  Dates → date-fns  |  Animations → framer-motion

2. DESIGN ANCHOR : Extrais du design contract :
   - Couleurs exactes (bg-[#hex], text-[#hex])
   - Layout (sidebar width, topbar height, grid)
   - Typographie (font-family, tailles, weights)
   - Composants (cards radius, nav items height, boutons style)

3. ARCHITECTURE : Planifie les fichiers :
   - app/page.tsx → navigation entre vues (useState activeView)
   - components/views/*.tsx → chaque écran principal
   - components/ui/*.tsx → composants réutilisables

══════════════════════════════════════════════════════════════════════
ÉTAPE 2 — GÉNÉRATION DES FICHIERS TypeScript React
══════════════════════════════════════════════════════════════════════

Pour CHAQUE fichier .tsx :
✅ "use client"; LIGNE 1 ABSOLUE si hooks ou events
✅ Interfaces TypeScript définies DANS le fichier
✅ Données mock réalistes et abondantes (12-15 entrées minimum)
✅ Tous les handlers implémentés avec de la vraie logique
✅ Couleurs du design anchor : bg-[#hex] text-[#hex] border-[#hex]
✅ Icônes Tabler : <i className="ti ti-[name]" />
✅ key={} sur TOUS les .map()
✅ useState/useEffect complets, pas de placeholders
...
// ... autres imports de views

export default function Page() {
  const [activeView, setActiveView] = useState("dashboard");
  return (
    <div className="flex h-screen bg-[#HEX_DU_DESIGN]">
      {/* Sidebar avec navigation */}
      {/* Contenu principal avec switch sur activeView */}
    </div>
  );
}
\`\`\`

RÈGLE DESIGN CRITIQUE :
⛔ JAMAIS bg-gray-*, text-blue-*, bg-indigo-* → utilise bg-[#hex] depuis le design anchor
⛔ JAMAIS shadow sur sidebar, topbar, navbar
✅ Nav items h-[34px] max
✅ Font du design anchor dans app/layout.tsx (Google Fonts)

══════════════════════════════════════════════════════════════════════
FORMAT DE SORTIE — OBLIGATOIRE
══════════════════════════════════════════════════════════════════════

Émets UN bloc par fichier, précédé d'une ligne "---" seule :
...
TOUJOURS créer au minimum :
1. app/page.tsx (layout principal + navigation)
2. app/layout.tsx (metadata + fonts CDN)
3. app/globals.css (variables CSS + reset)
4. tailwind.config.ts (plugins: [] VIDE — jamais tailwindcss-animate)
5. next.config.mjs (reactStrictMode: false)
6. Au moins 2-3 views dans components/views/

DEPENDENCIES: ["liste", "des", "packages", "npm", "utilisés"]
`;
...
// =============================================================================
// JS_AGENT_PROMPT — Agent 2 : Code Verify — review + correction des fichiers TSX de Foundation
// Reçoit les fichiers .tsx avec numéros de ligne, corrige bugs, complète les features
// =============================================================================

const JS_AGENT_PROMPT = `
Tu es le Code Verify Agent — plus puissant que Foundation, tu construis sur son travail pour l'élever.
Tu reçois le code TypeScript React généré par Foundation avec les numéros de ligne exacts.
Ta mission : corriger chaque problème, compléter chaque fonctionnalité incomplète, rendre la plateforme vraiment fonctionnelle.
...
[WORKING_ON]Code Verify — correction et complétion des fonctionnalités[/WORKING_ON]

══════════════════════════════════════════════════════════════════════
ANALYSE OBLIGATOIRE (dans ta réflexion avant de coder)
══════════════════════════════════════════════════════════════════════

Pour CHAQUE fichier .tsx fourni, inspecte :

1. HANDLERS VIDES ou SIMULÉS :
   ✗ onClick={() => {}} → implémente la vraie logique
   ✗ console.log("clicked") → remplace par le vrai comportement
   ✗ alert("todo") → supprime et implémente
   → Trace le flux complet : [interaction] → [handler] → [résultat visible]

2. FONCTIONNALITÉS MANQUANTES :
   Compare les fichiers avec la demande originale de l'utilisateur.
   Chaque feature demandée doit exister et fonctionner.
   Si une feature manque → crée le fichier ou l'ajoute dans la view existante.

3. DONNÉES MOCK INSUFFISANTES :
   < 8 entrées → complète jusqu'à 12-15 entrées réalistes
   Données génériques ("Item 1", "Name 2") → remplace par des données réalistes du domaine

4. DESIGN ANCHOR :
   Couleurs génériques (bg-gray-*, text-blue-*) → remplace par bg-[#hex] du design contract
   Nav items trop hauts (> 34px) → corriger
   Shadows sur layout → supprimer

5. BUGS TYPESCRIPT :
   "use client" manquant sur un fichier avec hooks → ajouter en ligne 1
   useState<T[]>() sans valeur initiale → ajouter []
   key manquant sur .map() → ajouter

══════════════════════════════════════════════════════════════════════
RÈGLES DE MODIFICATION
══════════════════════════════════════════════════════════════════════

✅ Utilise edit_file avec les VRAIS numéros de ligne fournis dans le snapshot
✅ Utilise create_file pour les nouveaux fichiers ou si > 60% du fichier change
✅ Toute logique ajoutée va DANS le fichier .tsx qui l'utilise
✅ Interfaces TypeScript définies DANS chaque fichier
✅ Named exports pour toutes les views

⛔ NE PAS changer le layout structurel si Foundation l'a bien fait
⛔ NE PAS réécrire un fichier si seules quelques lignes changent — utilise edit_file
⛔ NE PAS inventer de couleurs — utilise uniquement le design anchor
...
// =============================================================================
// CHECKER_AGENT_PROMPT kept as alias for backward compat in FIX/MICRO paths
// =============================================================================
...
// =============================================================================
// FEATURES_AGENT_PROMPT — Agent 3 : Code Views — vues et fonctionnalités complètes
// =============================================================================

const FEATURES_AGENT_PROMPT = `
Tu es le Code Views Agent — le plus créatif et le plus puissant parmi les agents de construction.
Tu reçois le code TypeScript React amélioré par Code Verify. Tu élèves encore le niveau.
Ta mission : compléter chaque vue, rendre l'application véritablement dynamique, implémenter les features avancées.
...
[WORKING_ON]Code Views — fonctionnalités avancées et dynamisme complet[/WORKING_ON]

══════════════════════════════════════════════════════════════════════
ANALYSE PROFONDE AVANT DE CODER
══════════════════════════════════════════════════════════════════════

1. VUES MANQUANTES : Chaque fonctionnalité majeure a-t-elle sa propre view ?
   → Si une feature importante n'a pas de view dédiée → crée-la complète
   → Chaque view : titre, contenu principal, actions, états loading/empty/error

2. INTERACTIVITÉ AVANCÉE :
   □ Recherche et filtrage en temps réel (useState + .filter())
   □ Tri des colonnes/listes (useState sortField + sortDirection)
   □ Pagination ou infinite scroll si données volumineuses
   □ Modals/drawers pour création/édition d'éléments
   □ Confirmations avant suppression (modal confirm)
   □ Toasts/notifications de succès et d'erreur
   □ Formulaires avec validation en temps réel

3. ÉTAT APPLICATIF COMPLET :
   □ Chaque view gère son propre état local (useState dans le composant)
   □ Données mock : 12-15 entrées réalistes, variées, représentatives du domaine
   □ États UI : loading skeleton, empty state stylé, error state
   □ Compteurs/métriques calculés dynamiquement depuis les données

4. FONCTIONNALITÉS RÉELLES (jamais simulées) :
   □ Si export → utilise jsPDF ou xlsx vraiment
   □ Si graphique → Recharts avec de vraies données structurées
   □ Si éditeur → textarea ou div contenteditable avec vrais handlers
   □ Si upload → input type="file" avec prévisualisation
   □ Si date picker → input type="date" ou date-fns

5. MICRO-INTERACTIONS :
   □ hover states avec transition-all duration-150 sur tous les éléments interactifs
   □ focus rings visibles sur inputs
   □ Loading spinner sur les boutons d'action
   □ Animations d'entrée sur les modals
...
Dates           : date-fns — format, parseISO, differenceInDays
PDF             : jspdf (export en un clic)
Spreadsheet     : xlsx (export Excel)
Animations      : framer-motion — motion.div + AnimatePresence
Audio/Video     : Tone.js (musique), MediaRecorder (enregistrement)
Canvas          : Konva (canvas interactif), Fabric.js
Clavier         : useEffect + document.addEventListener('keydown')

══════════════════════════════════════════════════════════════════════
RÈGLES DE SORTIE
══════════════════════════════════════════════════════════════════════

✅ edit_file sur les fichiers existants (numéros de ligne exacts du snapshot)
✅ create_file pour les nouvelles views ou si > 60% de changements
✅ Toute logique dans le composant qui l'utilise — jamais de service séparé
✅ "use client"; ligne 1 absolue sur tout fichier avec hooks

${EDIT_FILE_FORMAT_RULES}

DEPENDENCIES: ["packages", "npm", "nécessaires"]
(pour installer les package nécessaire)

══════════════════════════════════════════════════════════════
SECTION 2 — DESIGN CONSISTENCY (CRITICAL)
══════════════════════════════════════════════════════════════

Every new DOM element (modals, panels, toasts, drawers, chips, badges) MUST feel native to the app.

▸ STEP 1 — Extract design system from HTML: CSS vars from :root, background colors, border styles, font-family, font-size, border-radius, padding scale, primary/secondary button styles.
▸ STEP 2 — Build new elements using those values. Use var(--xxx) whenever they exist.
▸ STEP 3 — NEVER introduce a foreign design system (no Bootstrap defaults, no generic grays).

Modal example (dark app):
  const panel = document.createElement('div');
  panel.style.cssText = \`background: var(--bg-surface, #1a1a2e); border: 1px solid var(--border-color, rgba(255,255,255,0.08)); border-radius: var(--radius, 8px); padding: 20px; font-family: inherit; color: var(--text-primary, #e2e8f0);\`;

══════════════════════════════════════════════════════════════
SECTION 3 — FEATURE SCOPE (BUILD EVERYTHING)
══════════════════════════════════════════════════════════════

Look at the screenshot and HTML. Identify EVERY element, button, input, zone, or implied workflow suggesting a real feature, then BUILD IT.

FOR EVERY IDENTIFIED FEATURE, BUILD:
- The trigger (click, hover, key, scroll, drag, input event)
- The DOM output (modal, drawer, toast, inline edit, new row, expanded panel) — safely per Section 1
- The data logic (read from localStorage, compute, transform, filter, sort)
- The persistence (save to localStorage with ppai_feat_ prefix)
...
USE CDNs FREELY:
  Load: const s=document.createElement('script'); s.src='URL'; document.head.appendChild(s);
...
══════════════════════════════════════════════════════════════
SECTION 4 — COEXISTENCE WITH SCRIPT #1 (JS AGENT)
══════════════════════════════════════════════════════════════

Script #1 already handles UI interactions (tabs, dropdowns, hover states).
...
- If Script #1 opens an empty modal → fill it with real form fields (safely, no innerHTML).
- If Script #1 adds a class to a button → execute the action that button implies.
...
// =============================================================================
// PHASE 3 — VIEWS_AGENT (kept for FIX/MICRO_EDIT paths)
// =============================================================================
...
// =============================================================================
// NEXTJS_AGENT_PROMPT — Agent 4 : Intégrateur Final — câblage + polish TypeScript React
// Reçoit l'ensemble des fichiers TSX produits par les 3 agents précédents.
// Mission : vérifier le câblage, corriger les derniers bugs, finaliser le design.
// =============================================================================

const NEXTJS_AGENT_PROMPT = `
Tu es l'Agent Intégrateur Final — tu as la vision globale de TOUTE l'application.
Tu reçois l'ensemble des fichiers TypeScript React produits par les 3 agents précédents.
Ta mission : câbler tout ensemble, corriger les derniers problèmes, finaliser en beauté.
...
[WORKING_ON]Intégration finale — câblage complet et polish[/WORKING_ON]

══════════════════════════════════════════════════════════════════════
AUDIT OBLIGATOIRE — CHECKLIST FINALE
══════════════════════════════════════════════════════════════════════

Inspecte chaque fichier (le snapshot avec numéros de ligne t'est fourni) :

1. CÂBLAGE app/page.tsx :
   □ Toutes les views sont importées depuis @/components/views/
   □ La navigation (sidebar/tabs/menu) change bien activeView
   □ Le bon composant est rendu pour chaque état de activeView
   □ Layout correct : sidebar + main content en flex-row h-screen

2. IMPORTS COHÉRENTS :
   □ Chaque composant importé dans page.tsx est bien défini dans son fichier
   □ Pas d'import cassé (@/lib/utils inexistant → supprimer ou créer)
   □ Named exports partout dans les views (pas default)

3. DESIGN ANCHOR RESPECTÉ :
   □ Couleurs : bg-[#hex] text-[#hex] border-[#hex] uniquement
   □ Pas de shadow sur sidebar, topbar, layout containers
   □ Nav items h-[34px] maximum
   □ Font correcte dans app/layout.tsx

4. TYPESCRIPT VALIDE :
   □ "use client"; ligne 1 sur tout fichier avec hooks ou events
   □ Interfaces définies dans chaque fichier qui les utilise
   □ useState<T[]>([]) avec valeur initiale
   □ key={} sur tous les .map()
   □ Pas de double export default

5. FONCTIONNALITÉS COMPLÈTES :
   □ Chaque feature de la demande est accessible via la navigation
   □ Pas de handler vide dans les views finales
   □ Données mock présentes et réalistes dans chaque view

6. FICHIERS OBLIGATOIRES :
   □ app/page.tsx — export default function Page()
   □ app/layout.tsx — sans "use client", avec metadata et fonts
   □ app/globals.css — variables CSS + reset
   □ tailwind.config.ts — plugins: [] VIDE
...
Si un fichier est bien fait → NE PAS le toucher.
Si un problème est identifié → le corriger immédiatement dans cette réponse.
Chaque problème identifié sans correction = tâche non terminée.

FORMAT : edit_file avec numéros exacts du snapshot. create_file si > 60% change ou nouveau fichier.
...
function parseNextjsAgentFiles(raw: string): { path: string; content: string }[] {
  try {
...
// =============================================================================
// INTEGRATOR AGENT — kept for FIX/MICRO_EDIT paths
// =============================================================================
...
async function runTscCheck(
  ...
  try {
    ...
    // Timeout augmenté à 240s pour laisser le temps à npm run build de compléter
    sbx = await Sandbox.create({ apiKey: e2bApiKey, timeoutMs: 240_000 });
    ...
    // Génère les stubs dynamiquement (pour la vérif TSC standalone seulement)
    ...
    const tsFiles = files.filter(f => f && f.path && f.content != null &&
      typeof f.path === "string" && (f.path.endsWith(".ts") || f.path.endsWith(".tsx")));
    const allFiles = files.filter(f => f && f.path && f.content != null && typeof f.path === "string");

    const allDirs = new Set([
      ...allFiles.map(f => { const p = f.path.split("/"); p.pop(); return p.join("/"); }).filter(Boolean)
    ]);
    if (allDirs.size > 0) {
      await sbx.commands.run(`mkdir -p ${Array.from(allDirs).map(d => `"${d}"`).join(" ")}`, { timeoutMs: 5_000 });
    }

    // ── Écrit TOUS les fichiers du projet (pas seulement .ts/.tsx) ────────────
    onProgress(`[TSC:FILES] Écriture de ${allFiles.length} fichiers dans le sandbox...\n`);
    for (let i = 0; i < allFiles.length; i += 10) {
      await Promise.all(allFiles.slice(i, i + 10).map(f => sbx!.files.write(f.path, f.content)));
    }
    await sbx.files.write("__stubs__.d.ts", dynamicStubs);

    // Vérification fichiers écrits
    ...
    onProgress(`[TSC:FILES] ✅ ${writtenFiles.length}/${tsFiles.length} fichiers .ts/.tsx confirmés.\n`);
    if (writtenFiles.length < tsFiles.length) {
      ...
      onProgress(`[TSC:FILES] ⚠️ Manquants : ${missing.join(", ")}\n`);
    }

    // ── Installe les dépendances nécessaires au build Next.js ─────────────────
    onProgress("[TSC:INSTALL] Installation des dépendances Next.js pour npm run build...\n");

    // S'assure que package.json existe dans le sandbox avec les bons scripts
    const pkgFileInSandbox = allFiles.find(f => f.path === "package.json");
    if (!pkgFileInSandbox) {
      const minimalPkg = JSON.stringify({
        ...
        devDependencies: { typescript: "^5", "@types/node": "^20", "@types/react": "^19", "@types/react-dom": "^19",
          eslint: "^8", "eslint-config-next": "15.0.3", postcss: "^8", tailwindcss: "^3.4.1", autoprefixer: "^10.4.19" }
      }, null, 2);
      await sbx.files.write("package.json", minimalPkg);
    }

    // next.config.mjs minimal si absent
    const nextCfg = allFiles.find(f => f.path === "next.config.mjs" || f.path === "next.config.js");
    if (!nextCfg) {
      await sbx.files.write("next.config.mjs", `/** @type {import('next').NextConfig} */\nconst nextConfig = { reactStrictMode: false };\nexport default nextConfig;\n`);
    }
    ...
    if (installOut.exitCode !== 0) {
      onProgress(`[TSC:WARN] npm install partial: ${installOut.stdout.slice(-300)}\n`);
    } else {
      onProgress("[TSC:INSTALL] ✅ Dépendances installées.\n");
    }

    // ── Lance les deux vérifications en parallèle ────────────────────────────
    // npm run build = détecte erreurs Next.js + ESLint sévères
    // tsc --noEmit  = capture TOUTES les erreurs TypeScript (next build s'arrête à la première par fichier)
    onProgress("[TSC:RUN] Lancement npm run build + tsc --noEmit en parallèle...\n");
    const [buildRun, tscRun] = await Promise.all([
      sbx.commands.run("npm run build 2>&1 || true", { timeoutMs: 180_000 }),
      sbx.commands.run("npx tsc --noEmit 2>&1 || true", { timeoutMs: 120_000 }),
    ]);
    const rawOutput = (buildRun.stdout ?? "") + (buildRun.stderr ?? "")
      + "\n--- TSC DIRECT ---\n"
      + (tscRun.stdout ?? "") + (tscRun.stderr ?? "");

    const rawLines = rawOutput.trim().split("\n").filter(Boolean);
    onProgress(`[TSC:RAW] ${rawLines.length} ligne(s) de sortie build brute.\n`);

    // ── Parse des erreurs Next.js build ──────────────────────────────────────
    // Formats possibles :
    //   ./components/views/Foo.tsx:45:17 - error TS2322: ...
    //   ./app/page.tsx
    //   Type error: ...
    //   ./components/Foo.tsx(45,17): error TS2322: ...
    //   Failed to compile.
    //   Error: ...
    const errorsByFile: Record<string, string[]> = {};

    // Filtre les lignes d'erreur réelles (pas les warnings webpack, pas les logs next)
    const realErrorLines = rawOutput.split("\n").filter(l => {
      const trimmed = l.trim();
      if (!trimmed) return false;
      // Erreurs TypeScript dans le format next build
      if (trimmed.includes("error TS") || trimmed.match(/^\s*Type error:/)) return true;
      // Erreurs de fichier (ligne de chemin avant l'erreur)
      if (trimmed.match(/^\.\/.*\.(tsx?|jsx?)$/)) return false; // juste le chemin, pas l'erreur elle-même
      // Erreurs de compilation Next.js
      if (trimmed.includes("Failed to compile") || trimmed.includes("Build error occurred")) return true;
      // Erreurs ESLint sévères
      if (trimmed.match(/^\s+\d+:\d+\s+Error:/)) return true;
      return false;
    });

    // Extraction robuste — 4 formats couverts : next build, tsc --noEmit, tsc classique, tsc sans ./
    for (const line of rawOutput.split("\n")) {
      const trimmed = line.trim();
      if (!trimmed || trimmed === "--- TSC DIRECT ---") continue;

      // Format 1 — next build : "./components/views/Foo.tsx:45:17 - error TS2322: ..."
      const nextFmt = trimmed.match(/^\.\/([^\s:]+\.tsx?):(\d+):(\d+)\s*-\s*(error\s+TS\d+:.+)$/);
      if (nextFmt) {
        const [, fp, ln, col, msg] = nextFmt;
        if (!errorsByFile[fp]) errorsByFile[fp] = [];
        const entry = `  L${ln}:${col} — ${msg.trim()}`;
        if (!errorsByFile[fp].includes(entry)) errorsByFile[fp].push(entry);
        continue;
      }

      // Format 2 — tsc --noEmit sans ./ : "app/page.tsx:45:17: error TS2322: ..."
      const tscNoSlash = trimmed.match(/^([^\s(]+\.tsx?):(\d+):(\d+):\s*(error\s+TS\d+:.+)$/);
      if (tscNoSlash) {
        const [, fp, ln, col, msg] = tscNoSlash;
        const clean = fp.replace(/^\.\//, "");
        if (!errorsByFile[clean]) errorsByFile[clean] = [];
        const entry = `  L${ln}:${col} — ${msg.trim()}`;
        if (!errorsByFile[clean].includes(entry)) errorsByFile[clean].push(entry);
        continue;
      }

      // Format 3 — tsc classique : "components/Foo.tsx(45,17): error TS2322: ..."
      const tscFmt = trimmed.match(/^([^\s(]+\.tsx?)\((\d+),(\d+)\):\s*(error\s+TS\d+:.+)$/);
      if (tscFmt) {
        const [, fp, ln, col, msg] = tscFmt;
        const clean = fp.replace(/^\.\//, "");
        if (!errorsByFile[clean]) errorsByFile[clean] = [];
        const entry = `  L${ln}:${col} — ${msg.trim()}`;
        if (!errorsByFile[clean].includes(entry)) errorsByFile[clean].push(entry);
      ...
      // Format 4 — erreurs globales (Type error:, Failed to compile, etc.)
      if (trimmed.includes("error TS") || trimmed.includes("Type error:") ||
          trimmed.includes("Failed to compile") || trimmed.includes("Build error occurred")) {
        if (!errorsByFile["__global__"]) errorsByFile["__global__"] = [];
        const entry = `  ${trimmed}`;
        if (!errorsByFile["__global__"].includes(entry)) errorsByFile["__global__"].push(entry);
      }
    }

    const hasErrors = Object.keys(errorsByFile).length > 0;
    const errorCount = Object.values(errorsByFile).reduce((s, arr) => s + arr.length, 0);

    if (!hasErrors) {
      onProgress("[TSC:OK] ✅ npm run build + tsc --noEmit : zéro erreur !\n");
    ...
    return {
      errors: Object.values(errorsByFile).flat().join("\n"),
...
// =============================================================================
// HEARTBEAT — émet un chunk invisible périodiquement pour garder le stream vivant
// pendant les longs appels LLM ou les pauses (sleep). Sans ça, le proxy Vercel /
// le client peut couper la connexion après ~30s d'inactivité.
// =============================================================================
function startHeartbeat(
  ...
  const tick = async () => {
    while (active) {
      ...
      if (!active) break;
      try {
        // Newline invisible pour l'UI mais qui maintient la connexion TCP
        controller.enqueue(encoder.encode("\n"));
      } catch {
        active = false; // stream déjà fermé
      }
    }
  };
  tick();
...
// =============================================================================
// BUILD_PKG_JSON — construit et émet le package.json fusionné
// Réutilisé après chaque phase pour que le package.json soit toujours à jour
// même si le stream se coupe avant la phase 5
// =============================================================================
async function buildAndEmitPkgJson(opts: {
  globalPkgs: Set<string>;
  globalDevPkgs: Set<string>;
  globalRemovePkgs: Set<string>;
  currentProjectFiles?: { path: string; content: string }[];
  emit: (txt: string) => void;
}): Promise<void> {
  const { globalPkgs, globalDevPkgs, globalRemovePkgs, currentProjectFiles, emit } = opts;

  const DEV_ONLY = new Set([
    "typescript", "@types/node", "@types/react", "@types/react-dom",
    "postcss", "tailwindcss", "eslint", "eslint-config-next", "autoprefixer", "@types/autoprefixer",
  ]);
  ...
  if (existPkgFile) { try { existingPkg = JSON.parse(existPkgFile.content); } catch {} }
  ...
  const finalDeps: Record<string, string> = { ...baseDeps, ...(existingPkg.dependencies ?? {}) };
  const finalDevDeps: Record<string, string> = { ...baseDev, ...(existingPkg.devDependencies ?? {}) };
  ...
  for (const pkg of Array.from(globalRemovePkgs)) {
    delete finalDeps[pkg]; delete finalDevDeps[pkg];
  }
  ...
  const pkgJson = {
    ...
    private: true,
    scripts: { dev: "next dev", build: "next build", start: "next start", lint: "next lint", ...(existingPkg.scripts ?? {}) },
...
// =============================================================================
// DESIGN ANCHOR PERSISTENCE — sauvegarde/lecture depuis les fichiers projet
// Permet aux agents FIX/MICRO_EDIT de toujours avoir le design même après reload
// =============================================================================
const DESIGN_ANCHOR_FILE = "app/__design_anchor__.md";

function saveDesignAnchorToFiles(
  anchor: string,
  mergeGeneratedFiles: (files: { path: string; content: string }[]) => void,
  emit: (txt: string) => void
): void {
  if (!anchor || anchor.length < 20) return;
  mergeGeneratedFiles([{ path: DESIGN_ANCHOR_FILE, content: anchor }]);
  // Émet le fichier pour que le client le reçoive
  emit(`<create_file path="${DESIGN_ANCHOR_FILE}">\n${anchor}\n</create_file>`);
}
  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        // ── HEARTBEAT — maintient le stream TCP vivant pendant les appels LLM longs ──
        // Sans ça, Vercel / proxies HTTP coupent la connexion après ~30s d'inactivité
        const hb = startHeartbeat(controller, encoder, 8000);

        // sleepWithHeartbeat — remplace sleep() pour les longues pauses
        // Le heartbeat tourne déjà, mais on émet quand même un marker visible
        const sleepWithHeartbeat = async (ms: number, label?: string): Promise<void> => {
          if (label) emit(`\n[WAIT] ${label} (${Math.round(ms/1000)}s)...\n`);
          await sleep(ms);
        };
        ...
        try {
          ...
          // 1. Tente de charger depuis les fichiers projet (survit aux reloads)
          if (!activeDesignAnchor) {
            activeDesignAnchor = loadDesignAnchorFromFiles(currentProjectFiles);
            if (activeDesignAnchor) emit("\n[DESIGN:RESTORED] ✅ Design anchor restauré depuis les fichiers projet\n");
          }
          // 2. Fallback : globalThis pour la session en cours
          if (!activeDesignAnchor && (globalThis as any).__persistedDesignAnchor) {
          ...
          if (decision === "MICRO_EDIT_ACTION") {
            ...
            const microInput = [
              ...
              lastUserMsg,
              "",
              activeDesignAnchor || DESIGN_RULES,
              "",
...
`;
            ...
            try {
              ...
              if (designMatch) {
                ...
                // ── PERSISTANCE DISQUE — survit aux reloads et sessions FIX/MICRO_EDIT ──
                saveDesignAnchorToFiles(activeDesignAnchor, mergeGeneratedFiles, emit);
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 1 — HTML AGENT : génération HTML/CSS pixel-perfect
          // Même logique que callGemini() dans page18 — output : HTML brut
          // ─────────────────────────────────────────────────────────────
...
${activeDesignAnchor ? `${activeDesignAnchor}` : ""}

${buildTodoContext(lastUserMsg)}

INSTRUCTIONS :
- Construis la PLATEFORME COMPLÈTE demandée en TypeScript React / Next.js 15
- Respecte INTÉGRALEMENT le design anchor ci-dessus : bg-[#hex] text-[#hex] border-[#hex] EXACTS
- Implémente TOUTES les fonctionnalités demandées — aucune simulation, aucun placeholder
- Données mock : 12-15 entrées réalistes minimum par liste/tableau
- "use client"; LIGNE 1 absolue sur tout fichier avec hooks
- Named exports pour toutes les views : export function XxxView()
- export default function Page() uniquement dans app/page.tsx

FICHIERS OBLIGATOIRES :
1. app/page.tsx (layout principal + sidebar + navigation activeView)
2. app/layout.tsx (metadata + Google Fonts + Tabler Icons CDN)
3. app/globals.css (variables CSS + reset + scrollbar styling)
4. tailwind.config.ts (plugins: [] VIDE — jamais tailwindcss-animate)
5. next.config.mjs (reactStrictMode: false)
6. Au moins 2-3 views dans components/views/*.tsx

Émets chaque fichier précédé d'une ligne "---" seule :
---
<create_file path="app/page.tsx">
contenu complet ici
</create_file>

À la fin : DEPENDENCIES: ["liste", "des", "packages", "npm"]
`;

          let foundationOutput = "";
          try {
            foundationOutput = await runAgent(HTML_AGENT_PROMPT, foundationInput, {
              temperature: 1.0,
              maxTokens: 65536,
            ...
            emit("\n[FOUNDATION_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          // Parse et stocker les fichiers TSX générés
          const foundationFiles = parseGeneratedFiles(foundationOutput);
          if (foundationFiles.length > 0) {
            foundationFiles.forEach(f => mergeGeneratedFiles([f]));
            registerGeneratedFiles(foundationFiles);
            emit(`\n[FOUNDATION:READY] ✅ ${foundationFiles.length} fichier(s) TypeScript React générés\n`);
          } else {
            emit(`\n[FOUNDATION:WARN] ⚠️ Aucun fichier create_file détecté dans la sortie Foundation\n`);
          }

          // Extraire les dépendances déclarées par Foundation
          extractDeps(foundationOutput).forEach(d => globalPkgs.add(d));

          // ── Helper : émet un package.json rapide avec les deps connues jusqu'ici ──
          // Appelé après chaque phase pour que le client ait toujours un pkg.json valide
          // même si le stream se coupe avant la Phase 5.
          const emitPkgJsonNow = (label: string) => {
            const snap = { next: "15.1.0", react: "19.0.0", "react-dom": "19.0.0",
              "lucide-react": "0.475.0", sharp: "0.33.5", clsx: "2.1.1",
              "tailwind-merge": "2.3.0", zustand: "4.5.2" } as Record<string, string>;
            const snapDev = { typescript: "^5", "@types/node": "^20", "@types/react": "^19",
              "@types/react-dom": "^19", postcss: "^8", tailwindcss: "^3.4.1",
              autoprefixer: "^10.4.19", eslint: "^8", "eslint-config-next": "15.0.3" } as Record<string, string>;
            const DEV_SNAP = new Set(["typescript","@types/node","@types/react","@types/react-dom",
              "postcss","tailwindcss","eslint","eslint-config-next","autoprefixer"]);
            const IGNORE_SNAP = new Set(["react","react-dom","next","sharp","autoprefixer"]);
            for (const pkg of Array.from(globalPkgs)) {
              if (pkg && !snap[pkg] && !snapDev[pkg] && !IGNORE_SNAP.has(pkg) && !globalRemovePkgs.has(pkg)) {
                if (DEV_SNAP.has(pkg)) snapDev[pkg] = "latest"; else snap[pkg] = "latest";
              }
            }
            for (const pkg of Array.from(globalRemovePkgs)) { delete snap[pkg]; delete snapDev[pkg]; }
            const existPkg = (currentProjectFiles ?? []).find(f => f.path === "package.json");
            let base: any = { name: "app", version: "1.0.0", private: true,
              scripts: { dev: "next dev", build: "next build", start: "next start", lint: "next lint" } };
            if (existPkg) { try { base = JSON.parse(existPkg.content); } catch {} }
            const out = { ...base, private: true,
              scripts: { dev: "next dev", build: "next build", start: "next start", lint: "next lint", ...(base.scripts ?? {}) },
              dependencies: { ...snap, ...(base.dependencies ?? {}) },
              devDependencies: { ...snapDev, ...(base.devDependencies ?? {}) } };
            emit(`\n---\n<create_file path="package.json">\n${JSON.stringify(out, null, 2)}\n</create_file>`);
            emit(`\n[PKG:${label}] ✅ package.json émis (${Object.keys(out.dependencies).length} deps)\n`);
          };

          // Émet le package.json après Foundation — si le stream coupe ici, le client a déjà tout
          emitPkgJsonNow("FOUNDATION");

          // ─────────────────────────────────────────────────────────────
          // PHASE 2 — CODE_VERIFY AGENT : review + correction des fichiers TSX de Foundation
          // Lit les fichiers générés avec numéros de ligne, corrige bugs + complète handlers
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:2/CODE_VERIFY]\n");

          // Snapshot des fichiers Foundation avec numéros de ligne pour Code Verify
          const buildTsxSnapshot = (files: { path: string; content: string }[], maxChars = 60000): string => {
            let total = 0;
            const parts: string[] = ["FICHIERS TypeScript React EXISTANTS (numéros de ligne EXACTS) :"];
            for (const f of files) {
              if (!f.path.endsWith(".tsx") && !f.path.endsWith(".ts") && !f.path.endsWith(".css") && !f.path.endsWith(".mjs")) continue;
              const numbered = f.content.split("\n").map((l, i) => `${String(i+1).padStart(4," ")} | ${l}`).join("\n");
              const block = `\n=== ${f.path} ===\n${numbered}`;
              if (total + block.length > maxChars) {
                parts.push(`\n=== ${f.path} (trop grand — utilise readFile("${f.path}")) ===`);
          ...
          const verifySnapshot = buildTsxSnapshot(allGeneratedFiles);
          const verifyInput = `
...
MISSION :
1. Inspecte chaque fichier ci-dessus avec ses numéros de ligne EXACTS
2. Corrige TOUS les bugs : "use client" manquant, handlers vides, useState sans [], key manquant
3. Complète les fonctionnalités incomplètes — aucun console.log, aucun TODO
4. Corrige les couleurs génériques → bg-[#hex] du design anchor
5. Données mock < 8 entrées → complète jusqu'à 12-15 entrées réalistes

Utilise edit_file avec les numéros de ligne ci-dessus. create_file si > 60% change ou nouveau fichier.
DEPENDENCIES: ["nouveaux", "packages", "si", "nécessaire"]
`;

          let verifyOutput = "";
          try {
            verifyOutput = await runAgent(JS_AGENT_PROMPT, verifyInput, {
              temperature: 1.0,
              maxTokens: 65536,
              agentName: "CODE_VERIFY",
              noTools: false,
              referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
            });
          } catch (e: any) {
            emit("\n[CODE_VERIFY_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          // Applique les corrections de Code Verify
          const verifyNewFiles = parseGeneratedFiles(verifyOutput);
          verifyNewFiles.forEach(f => { mergeGeneratedFiles([f]); registerGeneratedFiles([f]); });
          const verifyEditOps = parseEditFileOps(verifyOutput);
          if (verifyEditOps.length > 0) {
            const evRes = applyEditFileOpsToFiles(allGeneratedFiles, verifyEditOps);
            evRes.applied > 0 && emit(`\n[CODE_VERIFY] ✅ ${evRes.applied} edit(s) appliqué(s)\n`);
            evRes.failed.length > 0 && emit(`\n[CODE_VERIFY] ⚠️ ${evRes.failed.length} échoué(s)\n`);
            // Sync registry après edits
            allGeneratedFiles.forEach(f => agentFileRegistry.set(f.path, f.content));
          }
          verifyNewFiles.length > 0 && emit(`\n[CODE_VERIFY] ✅ ${verifyNewFiles.length} fichier(s) mis à jour\n`);
          extractDeps(verifyOutput).forEach(d => globalPkgs.add(d));
          emitPkgJsonNow("VERIFY");

          await sleep(2000);

          // ─────────────────────────────────────────────────────────────
          // PHASE 3 — CODE_VIEWS AGENT : vues complètes + fonctionnalités avancées
          // Reçoit les fichiers TSX corrigés par Code Verify, élève le niveau
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:3/CODE_VIEWS]\n");

          const viewsSnapshot = buildTsxSnapshot(allGeneratedFiles, 55000);
          const viewsInput = `
DEMANDE ORIGINALE : "${lastUserMsg}"
...
MISSION — ÉLÉVATION COMPLÈTE :
1. Vues manquantes : chaque fonctionnalité majeure a sa propre view → crée-la si absente
2. Interactivité avancée : recherche temps réel, tri, filtres, modals, toasts, pagination
3. Données mock : enrichis jusqu'à 12-15 entrées réalistes par liste
4. Fonctionnalités réelles : pas de simulation — vraie logique avec les bonnes libs npm
5. Design anchor : chaque couleur générique → bg-[#hex] exact
6. États UI complets : loading skeleton, empty state, error state sur chaque section

Utilise edit_file avec les numéros de ligne exacts. create_file pour les nouvelles views.
DEPENDENCIES: ["nouveaux", "packages", "npm", "si", "nécessaire"]
`;

          let viewsOutput = "";
          try {
            viewsOutput = await runAgent(FEATURES_AGENT_PROMPT, viewsInput, {
              temperature: 1.1,
              maxTokens: 65536,
              agentName: "CODE_VIEWS",
              noTools: false,
              referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
            });
          } catch (e: any) {
            emit("\n[CODE_VIEWS_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          // Applique les améliorations de Code Views
          const viewsNewFiles = parseGeneratedFiles(viewsOutput);
          viewsNewFiles.forEach(f => { mergeGeneratedFiles([f]); registerGeneratedFiles([f]); });
          const viewsEditOps = parseEditFileOps(viewsOutput);
          if (viewsEditOps.length > 0) {
            const evRes = applyEditFileOpsToFiles(allGeneratedFiles, viewsEditOps);
            evRes.applied > 0 && emit(`\n[CODE_VIEWS] ✅ ${evRes.applied} edit(s) appliqué(s)\n`);
            evRes.failed.length > 0 && emit(`\n[CODE_VIEWS] ⚠️ ${evRes.failed.length} échoué(s)\n`);
            allGeneratedFiles.forEach(f => agentFileRegistry.set(f.path, f.content));
          }
          viewsNewFiles.length > 0 && emit(`\n[CODE_VIEWS] ✅ ${viewsNewFiles.length} fichier(s) mis à jour\n`);
          extractDeps(viewsOutput).forEach(d => globalPkgs.add(d));
          emitPkgJsonNow("VIEWS");

          await sleep(2000);

          // ─────────────────────────────────────────────────────────────
          // PHASE 4 — FINAL INTEGRATOR : câblage final, audit complet, polish
          // Reçoit tous les fichiers TSX des 3 phases, vérifie et finalise
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:4/INTEGRATOR]\n");

          {
            const integratorSnapshot = buildTsxSnapshot(allGeneratedFiles, 60000);
...
AUDIT FINAL — vérifie et corrige TOUT ce qui ne va pas :
1. app/page.tsx importe toutes les views et les rend selon activeView
2. Tous les imports sont cohérents (named exports, @/ alias)
3. Fichiers obligatoires présents : app/page.tsx, app/layout.tsx, app/globals.css, tailwind.config.ts, next.config.mjs
4. "use client"; ligne 1 sur tout fichier avec hooks
5. Design anchor respecté sur tous les fichiers
6. Toutes les features de la demande sont accessibles depuis la navigation

Utilise edit_file avec les numéros de ligne exacts. create_file pour les nouveaux fichiers ou refonte.
DEPENDENCIES: ["packages", "npm", "si", "nécessaire"]
`;

            let integratorRaw = "";
            try {
              integratorRaw = await runAgent(NEXTJS_AGENT_PROMPT, integratorInput, {
                temperature: 1.0,
                maxTokens: 65536,
                agentName: "FINAL_INTEGRATOR",
                noTools: false,
                referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
              });
            } catch (e: any) {
              emit("\n[FINAL_INTEGRATOR] Erreur: " + (e?.message ?? String(e)) + "\n");
            }
            flushBuffer();

            // Applique les corrections finales de l'intégrateur
            const integratorNewFiles = parseGeneratedFiles(integratorRaw);
            integratorNewFiles.forEach(f => { mergeGeneratedFiles([f]); registerGeneratedFiles([f]); });
            const integratorEditOps = parseEditFileOps(integratorRaw);
            if (integratorEditOps.length > 0) {
              const evRes = applyEditFileOpsToFiles(allGeneratedFiles, integratorEditOps);
              evRes.applied > 0 && emit(`\n[INTEGRATOR] ✅ ${evRes.applied} correction(s) finale(s) appliquée(s)\n`);
              evRes.failed.length > 0 && emit(`\n[INTEGRATOR] ⚠️ ${evRes.failed.length} échoué(s)\n`);
              allGeneratedFiles.forEach(f => agentFileRegistry.set(f.path, f.content));
            }
            integratorNewFiles.length > 0 && emit(`\n[INTEGRATOR] ✅ ${integratorNewFiles.length} fichier(s) finalisé(s)\n`);
            extractDeps(integratorRaw).forEach(d => globalPkgs.add(d));
            emitPkgJsonNow("INTEGRATOR");
          }
...
`;
            ...
            let polishOutput = "";
            try {
              polishOutput = await runAgent(FIXER_PROMPT, polishInput, {
                temperature: 1.2,
                maxTokens: 65536,
                agentName: "POLISH",
              });
            } catch (e: any) {
              emit(`\n[POLISH] ⚠️ Agent polish indisponible (${e?.message?.slice(0,80) ?? e}) — continue sans correction.\n`);
            }
            flushBuffer();
...
`;
              ...
              let tscFixOutput = "";
              try {
                tscFixOutput = await runAgent(FIXER_PROMPT, tscFixInput, {
                  temperature: 1.2,
                  maxTokens: 65536,
                  agentName: "TSC_FIXER",
                  emitOutput: true,
                ...
                emit(`\n[TSC:FIXER] ⚠️ Agent fixer indisponible ce round (${e?.message?.slice(0,80) ?? e}) — on continue.\n`);
                break; // sortir de la boucle TSC proprement sans crash
              }
              ...
              if (round < MAX_TSC_FIX_ROUNDS) {
                ...
                try {
                  ...
                  emit(`\n[TSC:RECHECK] ⚠️ Recheck indisponible (${e?.message?.slice(0,80) ?? e}) — arrêt de la boucle.\n`);
                  break;
                }
                if (!tscResult.hasErrors) {
