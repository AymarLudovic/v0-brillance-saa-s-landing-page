...
function applyEditFileOpToContent(content: string, op: EditFileOp): { result: string; error?: string } {
  ...
  switch (op.action) {
    case "replace": {
      ...
      // GARDE ANTI-GASPILLAGE : si l'agent remplace ≥ 80% du fichier via edit_file
      // c'est un create_file déguisé — on applique quand même mais on log l'abus
      const coveredLines = end - start + 1;
      const coverageRatio = coveredLines / total;
      if (coverageRatio >= 0.80 && total > 30) {
        // On applique quand même (pour ne pas bloquer) mais le contenu remplace tout
        // Un vrai create_file aurait été plus propre — on note pour monitoring
        const updated = [...lines.slice(0, start), ...newLines, ...lines.slice(end + 1)];
        return { result: updated.join("\n"), error: `WARN:FULL_FILE_REPLACE — ${coveredLines}/${total} lignes remplacées (${Math.round(coverageRatio*100)}%) → utilise create_file` };
      }
      const updated = [...lines.slice(0, start), ...newLines, ...lines.slice(end + 1)];
      return { result: updated.join("\n") };
    }
...
function applyEditFileOpsToFiles(
  ...
  for (const [filePath, fileOps] of byFile.entries()) {
    ...
    for (const op of sorted) {
      ...
      if (error) {
        if (error.startsWith("WARN:FULL_FILE_REPLACE")) {
          // Appliqué avec succès mais coûteux — log sans bloquer
...
STRUCTURE app/page.tsx :
  ...
  // 5. Slot pour le blueprint dynamique généré par PRODUCT_INTELLIGENCE_AGENT (PHASE 0a)
  // Ce slot est rempli par (globalThis as any).__activeBlueprintContext AVANT buildDeepContext
  // Aucun blueprint statique — le modèle génère lui-même le plan pour N'IMPORTE quel projet
  const injectedBlueprint: string = (globalThis as any).__activeBlueprintContext ?? "";
  ...
  if (injectedBlueprint) ctx.push(injectedBlueprint);
  ctx.push('═══════════════════════');
...
Tu es un expert design system. Ta mission : analyser les images et produire un HTML/CSS pixel-perfect.
...
══════════════════════════════════════════════════════════════════════
ÉTAPE 3 — HTML/CSS PIXEL-PERFECT
══════════════════════════════════════════════════════════════════════

Après les blocs d'analyse, génère le HTML/CSS dans <design_reference>.

Règles strictes :
1. :root{} avec TOUTES les couleurs en variables nommées
2. ZÉRO couleur hardcodée hors de :root{} — tout via var(--)
3. Layout flex reproduisant EXACTEMENT la structure spatiale (sidebar fixe gauche, header en haut)
4. Nav items : height 32-36px MAX — compact toujours
5. Sidebar et header : NO box-shadow — juste border-right/border-bottom si observé
6. Cards/modals/dropdowns : shadow si observé uniquement
7. Tous les états CSS : :hover, :active, .active, :focus-visible
8. Données réalistes inventées — jamais Lorem ipsum
9. body { display: flex; height: 100vh; overflow: hidden; } — structure correcte
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=[FONT_EXACT]:wght@400;500;600;700&display=swap" rel="stylesheet">
  <style>
    :root {
      /* ── Couleurs — STRICTEMENT depuis la palette pixel fournie ── */
      --bg:                [hex full-image];
      --sidebar-bg:        [hex sidebar-left];
      --sidebar-border:    rgba([r],[g],[b],0.08);
      --sidebar-hover:     rgba([r],[g],[b],0.06);
      --sidebar-active-bg: rgba([r],[g],[b],0.12);
      --sidebar-indicator: [hex accent];
      --header-bg:         [hex header-bar];
      --header-border:     rgba([r],[g],[b],0.06);
      --content-bg:        [hex main-content];
      --card-bg:           [hex card-area];
      --card-border:       rgba([r],[g],[b],0.1);
      --card-shadow:       0 2px 8px rgba(0,0,0,0.15);  /* UNIQUEMENT sur cards */
      --text-primary:      [hex];
      --text-secondary:    [hex];
      ...
      --btn-hover:         [hex];
      --input-bg:          [hex];
      --input-border:      rgba([r],[g],[b],0.12);
      --border:            rgba([r],[g],[b],0.08);
      /* ── Typo ── */
      --font: '[FONT_EXACT]', system-ui, sans-serif;
      --text-xs: 11px; --text-sm: 13px; --text-base: 14px; --text-lg: 16px; --text-xl: 20px;
      --fw-normal: 400; --fw-medium: 500; --fw-semibold: 600; --fw-bold: 700;
      /* ── Layout ── */
      --sidebar-w: [px]px;
      --header-h:  [px]px;
      --nav-h:     34px;       /* COMPACT — jamais dépasser 36px */
    ...
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    body {
      ...
      display: flex; height: 100vh; overflow: hidden;  /* structure correcte */
    }
    /* ── SIDEBAR — NO shadow ── */
    .sidebar {
    ...
    /* ── CONTENT ── */
    .content { flex: 1; overflow-y: auto; background: var(--content-bg); padding: var(--gap); }
    /* ── NAV ITEMS — COMPACT ── */
    .nav-item {
      height: var(--nav-h);
    ...
    .nav-item.active {
      background: var(--sidebar-active-bg); color: var(--text-primary); font-weight: var(--fw-medium);
    }
    ...
    /* ── CARDS — shadow ici UNIQUEMENT ── */
    .card {
      background: var(--card-bg); border: 1px solid var(--card-border);
      border-radius: var(--radius-md); padding: 20px;
      box-shadow: var(--card-shadow);  /* AUTORISÉ sur cards */
    }
    /* ── BUTTONS ── */
    .btn-primary {
    ...
    /* ... autres composants observés ... */
  </style>
</head>
<body>
  <aside class="sidebar">
    <!-- Reproduction fidèle de la sidebar avec ses éléments exacts -->
  </aside>
  <div class="main-wrap">
    <header class="topbar">
      <!-- Reproduction fidèle du header -->
    </header>
    <main class="content">
      <!-- Reproduction fidèle du contenu avec données réalistes -->
    </main>
  </div>
</body>
</html>
...
⛔ INTERDIT — GASPILLAGE DE TOKENS CRITIQUE :
  • edit_file avec start_line=1 et end_line=DERNIÈRE_LIGNE → c'est un create_file déguisé
    → Utilise TOUJOURS create_file si tu modifies ≥ 60% du fichier
    → Un edit_file qui couvre 80-100% du fichier consomme 3× plus de tokens qu'un create_file
  • Plusieurs edit_file consécutifs sur le même fichier quand un seul create_file suffirait

RÈGLE DE CHOIX :
• edit_file  → fichier existant avec < 60% de changements, modifications ciblées (1-50 lignes)
• create_file → nouveau fichier OU modifications > 60% du fichier OU refonte complète
...
// =============================================================================
// DESIGN ENFORCER — Convertit le HTML/CSS du design anchor en DesignSystem.tsx
// S'exécute AVANT Foundation, génère components/ui/DesignSystem.tsx
// Ce fichier est LA source de vérité visuelle pour tous les agents suivants
// =============================================================================

const DESIGN_ENFORCER_PROMPT = `
Tu es le Design Enforcer. Ta mission unique : lire le HTML/CSS du design anchor et le convertir
PIXEL PAR PIXEL en un fichier TypeScript de tokens de design React/Tailwind.
...
⛔ INTERDICTIONS ABSOLUES
══════════════════════════════════════════════════════════════════════
- ZÉRO couleur inventée — tout vient du HTML/CSS fourni
- ZÉRO classe Tailwind générique (bg-gray-*, text-blue-*) si le HTML/CSS a des hex
- ZÉRO interprétation — copie les valeurs EXACTEMENT comme elles sont dans le HTML/CSS
- Ne génère QUE le fichier components/ui/DesignSystem.tsx — rien d'autre

══════════════════════════════════════════════════════════════════════
MISSION : CONVERSION DIRECTE HTML/CSS → REACT TOKENS
══════════════════════════════════════════════════════════════════════

Étape 1 — Lis le HTML/CSS et extrais CHAQUE valeur :
- Chaque variable CSS --nom: valeur → devient un token nommé
- Chaque classe CSS .sidebar { background: #hex } → devient sidebarBg: "bg-[#hex]"
- Chaque media query, hover state, transition → documenté dans les tokens

Étape 2 — Génère components/ui/DesignSystem.tsx :
Ce fichier contient :
1. Les tokens de design (constantes exportées avec les hex EXACTS)
2. Les composants de base (Layout, Sidebar, Topbar, Card, Button, Input, Badge, NavItem)
3. Chaque composant utilise UNIQUEMENT les tokens — jamais de valeur hardcodée ailleurs

FORMAT DE SORTIE :

<create_file path="components/ui/DesignSystem.tsx">
"use client";
import React from "react";

// ═══════════════════════════════════════════════════════
// DESIGN TOKENS — Extraits pixel par pixel du design anchor
// Source de vérité pour TOUS les agents
// ═══════════════════════════════════════════════════════
export const DS = {
  // Backgrounds (hex exacts du design anchor)
  bg:          "#[hex exact depuis --bg ou body background]",
  sidebarBg:   "#[hex exact depuis --sidebar-bg ou .sidebar background]",
  headerBg:    "#[hex exact depuis --header-bg ou .topbar background]",
  cardBg:      "#[hex exact depuis --card-bg ou .card background]",
  contentBg:   "#[hex exact depuis --content-bg ou main background]",
  inputBg:     "#[hex exact depuis --input-bg ou input background]",

  // Borders (rgba exacts)
  sidebarBorder: "[rgba exact depuis --sidebar-border ou border-right de sidebar]",
  headerBorder:  "[rgba exact depuis --header-border ou border-bottom de topbar]",
  cardBorder:    "[rgba exact depuis --card-border ou border de .card]",
  border:        "[rgba exact depuis --border]",

  // Text (hex exacts)
  textPrimary:   "#[hex exact]",
  textSecondary: "#[hex exact]",
  textMuted:     "[rgba exact]",

  // Accent & interactive
  accent:        "#[hex exact depuis --accent]",
  accentHover:   "#[hex légèrement ajusté]",
  accentMuted:   "[rgba accent à 12% d'opacité]",

  // Buttons
  btnBg:         "#[hex exact]",
  btnText:       "#[hex exact]",
  btnHoverBg:    "#[hex exact]",

  // Nav states
  navHover:      "[rgba exact depuis --sidebar-hover]",
  navActiveBg:   "[rgba exact depuis --sidebar-active-bg]",
  navIndicator:  "#[hex exact depuis --sidebar-indicator]",

  // Shadows (UNIQUEMENT sur cards/modals — JAMAIS sur sidebar/topbar/layout)
  cardShadow:    "[valeur exacte depuis --card-shadow ou box-shadow de .card]",

  // Typography
  font:          "'[nom exact depuis --font ou font-family]', system-ui, sans-serif",

  // Layout dimensions (px exacts)
  sidebarW:      "[px exact depuis --sidebar-w ou width de .sidebar]",
  headerH:       "[px exact depuis --header-h ou height de .topbar]",

  // Border radius
  radiusSm:      "[px exact]",
  radiusMd:      "[px exact]",
  radiusLg:      "[px exact]",
  radiusInput:   "[px exact depuis la règle: ≥38px→25px, sinon 10-12px]",
  radiusBtn:     "[px exact depuis la règle: ≤32px→8-10px, >32px→25px]",

  // Tailwind classes prêtes à l'emploi (bg-[#hex] avec hex exacts)
  tw: {
    bg:          "bg-[#hex]",
    sidebarBg:   "bg-[#hex]",
    headerBg:    "bg-[#hex]",
    cardBg:      "bg-[#hex]",
    contentBg:   "bg-[#hex]",
    textPrimary: "text-[#hex]",
    textSecondary:"text-[#hex]",
    textMuted:   "text-[rgba(r,g,b,0.5)]",
    accent:      "bg-[#hex]",
    accentText:  "text-[#hex]",
    accentBorder:"border-[#hex]",
    border:      "border-[rgba(r,g,b,0.08)]",
    cardBorder:  "border-[rgba(r,g,b,0.1)]",
    navHover:    "hover:bg-[rgba(r,g,b,0.06)]",
    navActiveBg: "bg-[rgba(r,g,b,0.12)]",
  }
} as const;

// ═══════════════════════════════════════════════════════
// COMPOSANTS DE BASE — Utilisent UNIQUEMENT DS tokens
// ═══════════════════════════════════════════════════════

// Layout principal — structure exacte du design anchor
export function AppLayout({ sidebar, topbar, children }: {
  ...
  return (
    <div className="flex h-screen overflow-hidden" style={{ background: DS.bg, color: DS.textPrimary, fontFamily: DS.font }}>
      <aside
        className="flex-shrink-0 flex flex-col"
        style={{ width: DS.sidebarW, background: DS.sidebarBg, borderRight: \`1px solid \${DS.sidebarBorder}\` }}
      >
        {sidebar}
      </aside>
      <div className="flex-1 flex flex-col overflow-hidden">
        {topbar && (
          <header
            ...
            style={{ height: DS.headerH, background: DS.headerBg, borderBottom: \`1px solid \${DS.headerBorder}\` }}
          >
            {topbar}
          </header>
        )}
        <main className="flex-1 overflow-y-auto p-6" style={{ background: DS.contentBg }}>
          {children}
        </main>
      </div>
    </div>
...
// NavItem — hauteur compacte, indicateur actif, hover state
export function NavItem({ icon, label, active, onClick }: {
  ...
  return (
    <div
      onClick={onClick}
      className="relative flex items-center gap-2 cursor-pointer transition-colors duration-150 rounded-[6px]"
      style={{
        height: "34px", padding: "0 10px",
        background: active ? DS.navActiveBg : "transparent",
        color: active ? DS.textPrimary : DS.textSecondary,
        fontWeight: active ? 500 : 400,
        fontSize: "13px",
      }}
      onMouseEnter={e => { if (!active) (e.currentTarget as HTMLElement).style.background = DS.navHover; }}
      onMouseLeave={e => { if (!active) (e.currentTarget as HTMLElement).style.background = "transparent"; }}
    >
      {active && (
        <span className="absolute left-0 top-1/2 -translate-y-1/2 rounded-r"
          style={{ width: "3px", height: "16px", background: DS.navIndicator }} />
      )}
      {icon && <i className={\`ti ti-\${icon} text-[16px]\`} />}
      <span className="whitespace-nowrap">{label}</span>
    </div>
  );
}

// Card — shadow UNIQUEMENT ici, jamais sur layout
...
export function Button({ children, onClick, variant = "primary", size = "md" }: {
  ...
  const radius = size === "sm" ? DS.radiusSm : DS.radiusBtn;
  const styles: React.CSSProperties = variant === "primary"
    ? { background: DS.btnBg, color: DS.btnText, border: "none" }
    : variant === "secondary"
    ? { background: "transparent", color: DS.accent, border: \`1px solid \${DS.accent}\` }
    : { background: "transparent", color: DS.textSecondary, border: "none" };
  return (
    <button
      onClick={onClick}
      className="font-semibold text-sm cursor-pointer transition-all duration-150 active:scale-[0.98]"
      style={{ height: h, padding: \`0 \${px}\`, borderRadius: radius, ...styles }}
      onMouseEnter={e => { if (variant === "primary") (e.currentTarget as HTMLButtonElement).style.background = DS.btnHoverBg; }}
      onMouseLeave={e => { if (variant === "primary") (e.currentTarget as HTMLButtonElement).style.background = DS.btnBg; }}
    >
      {children}
    </button>
  );
...
export function Badge({ children, type = "neutral" }: {
  ...
  const colors = {
    ...
    neutral: { bg: DS.accentMuted, text: DS.textPrimary },
  };
  const c = colors[type];
  return (
    <span className="inline-flex items-center font-medium whitespace-nowrap"
      style={{ background: c.bg, color: c.text, borderRadius: "9999px", padding: "2px 8px", fontSize: "12px" }}>
      {children}
    </span>
  );
}
...
// Input / SearchBox
export function Input({ placeholder, icon, value, onChange }: {
  placeholder?: string; icon?: string; value?: string; onChange?: (v: string) => void;
}) {
  return (
    <div className="relative flex items-center">
      {icon && <i className={\`ti ti-\${icon} absolute left-3 text-[15px]\`} style={{ color: DS.textMuted }} />}
      <input
        value={value}
        ...
        style={{
          background: DS.inputBg, color: DS.textPrimary,
          border: \`1px solid \${DS.border}\`,
          borderRadius: DS.radiusInput,
          height: "36px",
          padding: icon ? "0 12px 0 34px" : "0 12px",
        }}
        onFocus={e => { e.currentTarget.style.boxShadow = \`0 0 0 2px \${DS.accentMuted}\`; }}
...
Tu es l'Agent Intégrateur. Tu critiques ET tu codes immédiatement ce qui manque.
Tu ne laisses AUCUN problème sans correction — identifier sans coder = mission échouée.

${ERROR_PREVENTION_BIBLE}

[WORKING_ON]Intégration & Correction complète — code tout ce qui manque[/WORKING_ON]

══════════════════════════════════════════════════════════════════════
${DESIGN_RULES}

ÉTAPE 1 — AUDIT EXHAUSTIF (readFile sur chaque fichier)
══════════════════════════════════════════════════════════════════════

Commence par readFile() sur app/page.tsx ET chaque fichier view généré.
Pour chaque fichier, vérifie :

FONCTIONNALITÉS :
  • Chaque bouton déclenche une action réelle ? (pas onClick vide, pas console.log)
  • Chaque formulaire a un onSubmit qui fait quelque chose ?
  • Chaque modal a un état d'ouverture/fermeture câblé ?
  • Chaque liste affiche de vraies données (>= 8 entrées mock) ?
  • Chaque filtre/recherche filtre réellement ?
  • Chaque navigation change de vue ?
  • Chaque feature demandée par l'utilisateur est complète de bout en bout ?

QUALITÉ :
  • Implémentation naïve à remplacer ? (opacity pour luminosité → getImageData, divs% → Recharts)
  • Données mock réalistes et abondantes ?
  • États loading/error/empty gérés visuellement ?
...
DESIGN (si designAnchor reçu) :
  • Les couleurs du code correspondent-elles à celles du design de référence ?
  • Les composants ressemblent-ils visuellement à la référence ?
  • Les variables CSS :root sont-elles utilisées ou les couleurs sont hardcodées ?

══════════════════════════════════════════════════════════════════════
ÉTAPE 2 — CODE TOUT CE QUI MANQUE (immédiatement, sans attendre)
══════════════════════════════════════════════════════════════════════

Pour CHAQUE problème identifié à l'étape 1 → corrige-le MAINTENANT avec edit_file.
Aucun problème ne reste sans correction dans ta réponse.

Règles :
  • readFile() EN PREMIER → edit_file avec numéros exacts
  • Si handler vide → implémente la vraie logique complète
  • Si données insuffisantes → ajoute les entrées réalistes
  • Si feature absente → implémente-la complètement dans le fichier concerné
  • Si > 40% change → create_file complet plutôt que 10 edit_file
  • Si couleurs du design non respectées → corrige les className Tailwind/variables CSS

Après tes corrections, résume ce que tu as corrigé en une liste courte.
...
// =============================================================================
// HARD_CONSTRAINTS_BLOCK — 10 règles courtes injectées JUSTE AVANT chaque génération
// Positionnées en fin de userMessage pour rester dans la fenêtre d'attention immédiate
// Flash retient le début ET la fin — pas le milieu. Ces contraintes vont en FIN.
// =============================================================================

// =============================================================================
// PRODUCT_INTELLIGENCE_PROMPT — Agent Phase 0a
// Génère dynamiquement un blueprint produit complet pour N'IMPORTE quel projet
// Appelé UNE FOIS avant Foundation avec la demande utilisateur brute
// Son output devient la LOI pour tous les agents suivants
// temperature: 0.3 — précision maximale, pas de créativité hasardeuse
// =============================================================================

const PRODUCT_INTELLIGENCE_PROMPT = `
Tu es un Product Intelligence Agent. Ta seule mission : analyser la demande et produire un blueprint produit exhaustif qui servira de loi absolue pour tous les agents de développement.

RÈGLE FONDAMENTALE : ton output doit permettre à un développeur (ou un agent IA) de construire une version COMPLÈTE et RECONNAISABLE du produit demandé — pas une maquette, pas un prototype, une vraie application avec toutes ses fonctionnalités clés.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PROCESSUS D'ANALYSE (obligatoire avant de répondre)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

1. IDENTIFIER LE TYPE DE PROJET
   • Est-ce un clone d'une app existante ? (Spotify, Notion, Figma, Slack, TradingView, Shopify, etc.)
   • Est-ce un type d'app générique ? (CRM, DAW, e-commerce, dashboard analytics, LMS, etc.)
   • Est-ce un projet custom/original ? (décrire ses domaines fonctionnels)

2. DÉCOMPOSER EN VUES EXHAUSTIVES
   Pour chaque vue, décrire :
   - Son nom et rôle dans l'app
   - Ses composants UI principaux (listes, charts, forms, modals, panels)
   - Ses interactions clés (filtres, drag, actions CRUD, navigation)
   - Les données qu'elle affiche
   Règle : minimum 5 vues, maximum 12 — toujours les vues qu'un vrai utilisateur utiliserait au quotidien

3. CHOISIR LES MEILLEURES LIBRAIRIES
   Pour chaque domaine fonctionnel :
   - Audio/DAW → tone.js + wavesurfer.js (pas Web Audio API brut)
   - Charts financiers → lightweight-charts (pas recharts pour des bougies)
   - Charts analytics → recharts ou chart.js
   - Canvas/dessin → konva (pas fabric.js pour des UIs complexes)
   - Drag & drop → @dnd-kit/core (pas react-beautiful-dnd obsolète)
   - Rich text → react-contenteditable ou tiptap
   - Markdown → react-markdown + remark-gfm
   - État global → zustand (jamais redux pour du nouveau code)
   - Dates → date-fns (pas moment.js)
   - Animations → framer-motion
   - 3D → three.js + @react-three/fiber
   Règle : choisir la lib qui donne le MEILLEUR résultat, pas la plus simple

4. DÉFINIR LA LOGIQUE RÉELLE
   Pas de "données mockées statiques" — définir :
   - Les stores Zustand nécessaires et leur état
   - Les calculs/algorithmes à implémenter (ex: prix simulé avec random walk, indicateurs techniques)
   - Les interactions en temps réel (setInterval, requestAnimationFrame)
   - Les volumes de données mock (jamais < 20 entrées pour les listes principales)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FORMAT DE RÉPONSE — OBLIGATOIRE, AUCUNE DÉVIATION
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Réponds UNIQUEMENT avec ce bloc structuré, rien avant, rien après :

<product_blueprint>
╔══════════════════════════════════════════════════════════════════╗
║  PRODUCT BLUEPRINT — [NOM DU PROJET] — LOI ABSOLUE              ║
║  Ces views ET cette logique DOIVENT être présentes dans l'app   ║
╚══════════════════════════════════════════════════════════════════╝

TYPE : [clone d'app connue / app métier / projet custom]
DOMAINE : [audio, finance, e-commerce, productivité, créatif, social, etc.]

VUES OBLIGATOIRES (toutes — aucune stub, aucune omission) :
  1. [NomView] — [description détaillée: composants, interactions, données affichées]
  2. [NomView] — [...]
  ... (toutes les vues, min 5)

LIBRAIRIES REQUISES (utilise ces libs exactes — pas d'alternatives inférieures) :
  • [lib] — [raison précise et usage dans cette app]
  • [lib] — [...]

LOGIQUE RÉELLE OBLIGATOIRE (pas de mockdata passif — vraie logique fonctionnelle) :
  → [store / algorithme / calcul / simulation à implémenter]
  → [...]

DONNÉES MOCK REQUISES (volume et cohérence) :
  → [entité]: [N entrées minimum], [champs requis]
  → [...]

⚡ CRITÈRE DE SUCCÈS : un utilisateur qui ouvre cette app doit reconnaître immédiatement
   le produit et pouvoir utiliser TOUTES ses fonctionnalités sans trouver de stub.
</product_blueprint>
`;

const HARD_CONSTRAINTS_BLOCK = `
⛔ HARD CONSTRAINTS — VIOLATIONS INTERDITES SANS EXCEPTION :
[C1] bg-gray-*, text-blue-*, text-green-* et toute classe Tailwind générique couleur → INTERDIT — utilise bg-[#hex] avec les hex du design contract
[C2] Navbar/topbar height > 50px → INTERDIT — max 48px, backdrop-blur-md si sticky
[C3] Shadow sur sidebar ou entre sidebar et main content → INTERDIT — border-right 1px semi-transparent uniquement
[C4] Main content plus sombre que sidebar → INTERDIT — main toujours 1 cran plus clair
[C5] Bouton height > 40px → INTERDIT — primary: rounded-full px généreux, secondary: border 1px outline
[C6] Input/searchbox sans background différent du parent → INTERDIT — bg légèrement différent toujours
[C7] onClick vide ou console.log → INTERDIT — chaque handler fait une vraie action visible
[C8] Données mock < 12 entrées → INTERDIT — données réalistes et abondantes obligatoires
[C9] View stub (div vide ou placeholder) → INTERDIT — chaque view est complète et navigable
[C10] Classes Tailwind génériques couleur après avoir reçu un design contract → VIOLATION CRITIQUE
`;

// =============================================================================
// VIEW_BUILDER_PROMPT — Construit UNE seule vue complète avec tout son budget tokens
// Appelé N fois (une fois par vue) pour éviter le token starvation sur Foundation
// =============================================================================

const VIEW_BUILDER_PROMPT = `
Tu es un expert React/TypeScript. Ta mission : construire UNE seule view de façon COMPLÈTE et PROFESSIONNELLE.
Tu as TOUT le budget de tokens pour cette unique view — utilise-le entièrement.

${ERROR_PREVENTION_BIBLE}

[WORKING_ON]Construction d'une view complète[/WORKING_ON]

${DESIGN_RULES}

RÈGLES ABSOLUES :
✅ "use client"; ligne 1 obligatoire
✅ Toutes les interfaces TypeScript définies EN HAUT du fichier
✅ Toute la logique de cette view DANS ce fichier (useState, handlers, data)
✅ Named export : export function NomView()
✅ Données mock réalistes et abondantes (15+ entrées minimum)
✅ CHAQUE bouton a un handler qui fait une vraie action visible (pas console.log)
✅ États loading/error/empty gérés visuellement
✅ Interactions complètes : filtres qui filtrent, modals qui s'ouvrent, forms qui soumettent
✅ La view doit être AUTONOME — elle fonctionne seule sans dépendances externes non définies

INTERDIT :
⛔ View stub ou placeholder
⛔ "// TODO: implement"
⛔ handlers vides
⛔ données mockées avec < 8 entrées
⛔ importer depuis des fichiers inexistants

${EDIT_FILE_FORMAT_RULES}
`;

// =============================================================================
// DESIGN_LINTER_PROMPT — Agent post-génération qui scanne les violations design
// Temperature: 0.2 — travail chirurgical, pas créatif
// =============================================================================

const DESIGN_LINTER_PROMPT = `
Tu es un Design Linter. Tu lis le code généré et tu corriges UNIQUEMENT les violations de règles design.
Tu ne touches PAS à la logique, pas aux données, pas à la structure JSX — UNIQUEMENT les violations CSS/Tailwind.

VIOLATIONS À DÉTECTER ET CORRIGER IMMÉDIATEMENT :

1. COULEURS TAILWIND GÉNÉRIQUES (si un design contract est fourni) :
   Patterns interdits : bg-gray-*, bg-slate-*, bg-zinc-*, bg-neutral-*, text-gray-*, text-slate-*, text-blue-*, text-green-*, text-red-*, border-gray-*
   Action : remplace par bg-[#hex] avec la couleur correspondante du design contract

2. NAVBAR/TOPBAR TROP HAUTE :
   Pattern : h-16, h-20, h-24, py-4 ou plus sur un nav/header → max h-12 ou h-[48px]
   Action : remplace par h-12 ou py-2

3. SHADOW SUR SIDEBAR :
   Pattern : shadow-* sur un élément sidebar/aside → INTERDIT
   Action : supprime le shadow, ajoute border-r border-white/10 si séparation nécessaire

4. BOUTON TROP GRAND :
   Pattern : h-12, h-14, h-16, py-4 ou plus sur un bouton → max h-10
   Action : remplace par h-10 py-2

5. INPUT SANS BACKGROUND DIFFÉRENT :
   Pattern : input/textarea sans bg défini ou avec le même bg que le parent
   Action : ajoute bg-black/20 ou bg-white/10 selon le thème

FORMAT DE RÉPONSE :
Pour chaque violation trouvée → un edit_file chirurgical.
Si aucune violation → réponds uniquement : "DESIGN_LINT_OK — aucune violation détectée."
Ne génère PAS de nouveaux fichiers. Ne réécris PAS entièrement. Corrections ciblées uniquement.
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const runAgent = async (
          ...
          // thinkingLevel dynamique selon l'agent — HIGH uniquement pour DESIGN et FOUNDATION
          // MEDIUM pour les agents de correction/vérification (économie tokens + vitesse)
          // LOW pour les agents purement chirurgicaux (FIXER, CHECKER, POLISH)
          const thinkingLevelForAgent = (() => {
            if (agentName === 'DESIGN_AGENT' || agentName === 'FOUNDATION') return 'HIGH' as const;
            if (agentName === 'VIEWS' || agentName === 'INTEGRATOR' || agentName === 'VIEW_BUILDER') return 'MEDIUM' as const;
            return 'LOW' as const; // CHECKER, FIXER, POLISH, TSC_FIXER, MICRO_EDIT
          })();
          const thinkingConfig = { thinkingLevel: thinkingLevelForAgent, includeThoughts: true };
        ...
        try {
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 0a — PRODUCT INTELLIGENCE
          // Analyse la demande et génère un blueprint produit exhaustif
          // Fonctionne pour N'IMPORTE quel projet — pas de liste hardcodée
          // Son output est stocké dans globalThis.__activeBlueprintContext
          // et injecté dans TOUS les agents via buildDeepContext()
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:0a/PRODUCT_INTELLIGENCE]\n");
          (globalThis as any).__activeBlueprintContext = "";

          try {
            const piInput = `Demande utilisateur : "${lastUserMsg}"

Analyse cette demande et génère le blueprint produit complet selon tes instructions.
Sois exhaustif — liste toutes les vues qu'un vrai utilisateur attendrait, les meilleures librairies, et la logique réelle nécessaire.
`;
            const piOutput = await callWithRetry(
              () => createStream(
                MODEL_ID,
                `${BASE_SYSTEM_PROMPT}\n\n${PRODUCT_INTELLIGENCE_PROMPT}`,
                [{ role: "user", parts: [{ text: piInput }] }],
                { temperature: 0.3, maxTokens: 8192, thinkingConfig: { thinkingLevel: "MEDIUM" as const, includeThoughts: false } }
              ),
              () => {},
              { maxAttempts: 2, baseDelay: 5000 }
            );

            // Extraire le bloc <product_blueprint>
            const bpMatch = piOutput.match(/<product_blueprint>([\s\S]*?)<\/product_blueprint>/);
            if (bpMatch) {
              (globalThis as any).__activeBlueprintContext = bpMatch[1].trim();
              const lineCount = bpMatch[1].trim().split("\n").length;
              emit(`\n[PRODUCT_INTELLIGENCE] ✅ Blueprint généré (${lineCount} lignes) — injecté dans tous les agents\n`);
            } else {
              // Fallback: utiliser l'output brut s'il contient des infos utiles
              const fallback = piOutput.trim().substring(0, 3000);
              if (fallback.length > 200) {
                (globalThis as any).__activeBlueprintContext = `\n═══ ANALYSE PRODUIT ═══\n${fallback}\n═══════════════════════`;
                emit(`\n[PRODUCT_INTELLIGENCE] ⚠️ Balise absente — analyse brute utilisée\n`);
              } else {
                emit(`\n[PRODUCT_INTELLIGENCE] ⚠️ Output insuffisant — agents procèdent sans blueprint\n`);
              }
            }
          } catch (e: any) {
            emit(`\n[PRODUCT_INTELLIGENCE] ⚠️ Skip (${e?.message?.slice(0, 60)}) — agents procèdent sans blueprint\n`);
          }
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 0b — DESIGN ENFORCER
          // Convertit le design anchor HTML/CSS → DesignSystem.tsx React
          // Ce fichier devient la source de vérité pour tous les agents
          // ─────────────────────────────────────────────────────────────
          if (activeDesignAnchor) {
            emit("\n[PHASE:0b/DESIGN_ENFORCER]\n");
            const enforcerInput = `
Tu vas convertir ce design anchor HTML/CSS en un fichier DesignSystem.tsx React.
Copie les valeurs PIXEL PAR PIXEL — aucune invention.

${activeDesignAnchor}

Génère UNIQUEMENT <create_file path="components/ui/DesignSystem.tsx">...</create_file>.
`;
            try {
              const enforcerOutput = await runAgent(DESIGN_ENFORCER_PROMPT, enforcerInput, {
                temperature: 0.3, // low temp = strict literal conversion
                maxTokens: 65536,
                agentName: "DESIGN_ENFORCER",
                noTools: false,
              });
              flushBuffer();
              const enforcerFiles = parseGeneratedFiles(enforcerOutput);
              if (enforcerFiles.length > 0) {
                mergeGeneratedFiles(enforcerFiles);
                emit("\n[DESIGN_ENFORCER] ✅ DesignSystem.tsx généré — source de vérité design prêt\n");
              } else {
                emit("\n[DESIGN_ENFORCER] ⚠️ Pas de fichier généré — les agents utiliseront le design anchor directement\n");
              }
            } catch (e: any) {
              emit("\n[DESIGN_ENFORCER] Skip: " + (e?.message?.slice(0, 60) ?? "") + "\n");
            }
          }

          // ─────────────────────────────────────────────────────────────
          // PHASE 0c — PACKAGE.JSON ANTICIPÉ
          // Émis IMMÉDIATEMENT avant tout le code — survit à N'IMPORTE quel crash
          // Si Foundation, INTEGRATOR, TSC ou n'importe quelle phase crash,
          // le package.json est déjà livré au client avec les deps de base.
          // Sera mis à jour en PHASE 5b avec les vraies versions résolues.
          // ─────────────────────────────────────────────────────────────
          {
            ...
            emit("\n[PKG:EARLY] ✅ package.json anticipé émis — survit à tout crash ultérieur\n");
          }
          ...
          const designSystemExists = allGeneratedFiles.some(f => f.path === "components/ui/DesignSystem.tsx");
...
${designSystemExists ? `⚡ DESIGN SYSTEM DISPONIBLE : importe et utilise DS depuis "components/ui/DesignSystem" — c'est la source de vérité des couleurs et composants.
  import { DS, AppLayout, NavItem, Card, Button, Badge, Input } from "@/components/ui/DesignSystem";
  TOUTES les couleurs viennent de DS.tw.* ou DS.* — JAMAIS de valeurs inventées.` : ""}
...
`;          const foundationOutput = await runAgent(FOUNDATION_PROMPT, foundationInput, {
            temperature: 0.7, // Flash: 1.4 = hallucinations CSS — 0.7 max pour génération code
            maxTokens: 65536,
...
CURRENT FILES — index (readFile("chemin") pour voir le contenu complet) :
${buildAllFilesSnapshot(12000)}

Utilise readFile() pour lire chaque fichier avant d'éditer — numéros de ligne exacts obligatoires.
...
`;
          ...
          const uiOutput = await runAgent(CHECKER_AGENT_PROMPT, uiInput, {
            temperature: 0.4, // Flash: agent correcteur = température basse obligatoire
            maxTokens: 65536,
            agentName: "CHECKER",
            referenceImages: effectiveReferenceImages.length > 0 ? effectiveReferenceImages.slice(0, 2) : undefined,
          });
...
${designSystemExists ? `⚡ DESIGN SYSTEM : utilise DS et les composants de "@/components/ui/DesignSystem" pour toutes les couleurs et composants de base.` : ""}
...
Utilise readFile() avant tout edit_file — numéros de ligne exacts obligatoires.
...
`;
          const viewsOutput = await runAgent(VIEWS_AGENT_PROMPT, viewsInput, {
            temperature: 0.7, // Flash: design final — créatif mais pas hallucinatoire
            maxTokens: 65536,
            agentName: "VIEWS",
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 3b — VIEW_BUILDER : Complète les views manquantes du blueprint
          // Si un Product Blueprint a été détecté, on vérifie que TOUTES les views
          // listées ont été générées — sinon on les construit une par une
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:3b/VIEW_BUILDER]\n");

          {
            // Extraire les views existantes depuis les fichiers générés
            const existingViewPaths = allGeneratedFiles
              .filter(f => f.path.includes("views/"))
              .map(f => f.path.toLowerCase());

            // Lire le blueprint dynamique généré par PRODUCT_INTELLIGENCE (Phase 0a)
            const activeBlueprint: string = (globalThis as any).__activeBlueprintContext ?? "";

            // Parser les VUES OBLIGATOIRES du blueprint dynamique
            const viewsSection = activeBlueprint.match(/VUES OBLIGATOIRES[\s\S]*?\n([\s\S]*?)\n(?:LIBRAIRIES|LOGIQUE|DONNÉES|⚡)/);
            const viewLines = viewsSection
              ? viewsSection[1]
                  .split("\n")
                  .filter(l => l.trim().match(/^\d+\./))
                  .map(l => {
                    const raw = l.replace(/^\s*\d+\.\s*/, "").trim();
                    // Extraire le nom de la vue (avant le " — " de description)
            ...
            if (viewLines.length > 0) {
              const missingViews = viewLines.filter(viewName => {
                const slug = viewName.toLowerCase()
                  .replace(/[^a-z0-9]/g, "")
                  .substring(0, 12);
                return !existingViewPaths.some(p => p.replace(/[^a-z0-9]/g, "").includes(slug));
              });

              if (missingViews.length > 0) {
                emit(`\n[VIEW_BUILDER] 🔍 ${missingViews.length} view(s) manquante(s) : ${missingViews.join(", ")}\n`);

                for (const missingView of missingViews.slice(0, 5)) {
                  emit(`\n[VIEW_BUILDER] 🔨 Construction : ${missingView}...\n`);

                  const existingViewsContext = allGeneratedFiles
                    .filter(f => f.path.includes("views/") || f.path === "app/page.tsx")
                    .map(f => `=== ${f.path} ===\n${f.content.substring(0, 800)}...`)
                    .join("\n---\n");

                  const viewBuilderInput = `
DEMANDE ORIGINALE : "${lastUserMsg}"

${activeBlueprint ? `BLUEPRINT PRODUIT (extrait du plan global) :\n${activeBlueprint.substring(0, 2500)}` : ""}

${activeDesignAnchor ? `DESIGN CONTRACT (respecte ces couleurs EXACTEMENT) :\n${activeDesignAnchor.substring(0, 2000)}` : ""}

VIEW À CONSTRUIRE : ${missingView}

CONTEXTE — fichiers déjà générés (pour cohérence des imports et du design) :
${existingViewsContext}

${HARD_CONSTRAINTS_BLOCK}

Génère UNIQUEMENT le fichier components/views/${missingView.replace(/[^a-zA-Z0-9]/g, "")}View.tsx
Ce fichier doit être COMPLET, fonctionnel, avec toutes les données mock et toute la logique.
Named export : export function ${missingView.replace(/[^a-zA-Z0-9]/g, "")}View()
`;

                  try {
                    const viewBuilderOutput = await runAgent(VIEW_BUILDER_PROMPT, viewBuilderInput, {
                      temperature: 0.7,
                      maxTokens: 65536,
                      agentName: "VIEW_BUILDER",
                      noTools: true,
                    });
                    mergeGeneratedFiles(parseGeneratedFiles(viewBuilderOutput));
                    extractDeps(viewBuilderOutput, "DEPENDENCIES").forEach(d => globalPkgs.add(d));
                    extractDeps(viewBuilderOutput, "DEVDEPENDENCIES").forEach(d => globalDevPkgs.add(d));
                    emit(`\n[VIEW_BUILDER] ✅ ${missingView} générée\n`);
                  } catch (e: any) {
                    emit(`\n[VIEW_BUILDER] ⚠️ Erreur sur ${missingView}: ${e?.message?.slice(0, 60)}\n`);
                  }
                }

                // Mettre à jour app/page.tsx pour importer les nouvelles views
                const newViewFiles = allGeneratedFiles.filter(f =>
                  f.path.includes("views/") &&
                  !existingViewPaths.includes(f.path.toLowerCase())
                );

                if (newViewFiles.length > 0) {
                  emit(`\n[VIEW_BUILDER] 🔗 Mise à jour app/page.tsx avec ${newViewFiles.length} nouvelle(s) view(s)...\n`);
                  const pageTsx = allGeneratedFiles.find(f => f.path === "app/page.tsx");
                  if (pageTsx) {
                    const updatePageInput = `
Voici app/page.tsx actuel :

${pageTsx.content}

Ces nouvelles views ont été générées et doivent être importées et rendues :
${newViewFiles.map(f => `- ${f.path} → export function ${f.path.split("/").pop()?.replace(".tsx", "")}`).join("\n")}

Ajoute les imports manquants et les cases de rendu conditionnel dans le switch/conditional de activeView.
Retourne le fichier COMPLET via create_file.
`;
                    try {
                      const pageUpdateOutput = await runAgent(FIXER_PROMPT, updatePageInput, {
                        temperature: 0.3,
                        maxTokens: 32768,
                        agentName: "PAGE_UPDATER",
                        noTools: true,
                      });
                      mergeGeneratedFiles(parseGeneratedFiles(pageUpdateOutput));
                    } catch (e: any) {
                      emit(`\n[VIEW_BUILDER] ⚠️ Erreur mise à jour page.tsx: ${e?.message?.slice(0, 60)}\n`);
                    }
                  }
                }

              } else {
                emit(`\n[VIEW_BUILDER] ✅ Toutes les views du blueprint sont présentes\n`);
              }
            } else {
              emit(`\n[VIEW_BUILDER] ℹ️ Aucun blueprint ou aucune vue parseable — agents ont généré librement\n`);
            }
          }
...

          {
            ...
            // INTEGRATOR reçoit uniquement l'index des fichiers (noms + nb lignes + 15 premières lignes)
            // Il utilise readFile() pour lire en détail les fichiers qu'il veut corriger
            // Cela évite le crash de stream par dépassement de budget token
            const integratorFileIndex = allGeneratedFiles
              .filter(f => f.path.endsWith(".tsx") || f.path.endsWith(".ts") || f.path.endsWith(".css"))
              .map(f => {
                const lines = f.content.split("\n");
                const preview = lines.slice(0, 15).map((l, i) => `${String(i+1).padStart(3," ")} | ${l}`).join("\n");
                return `=== ${f.path} (${lines.length} lignes) ===\n${preview}${lines.length > 15 ? `\n... (${lines.length - 15} lignes supplémentaires — utilise readFile("${f.path}") pour voir tout)` : ""}`;
              })
              .join("\n\n");
            ...
            const integratorInput = [
              ...
              "",
              "MÉTHODE : utilise readFile(\"chemin\") sur chaque fichier avant de le corriger.",
              "Ne devine pas les numéros de ligne — lis le fichier d'abord, edite ensuite.",
              "",
              activeDesignAnchor ? "DESIGN DE RÉFÉRENCE :\n" + activeDesignAnchor.substring(0, 1500) : "",
              "",
              "INDEX DES FICHIERS (preview 15 lignes — readFile pour voir complet) :",
              integratorFileIndex || "(aucun fichier généré)",
              "",
              HARD_CONSTRAINTS_BLOCK,
              "",
              "RÈGLE ABSOLUE : pour chaque problème → readFile() → corrige avec edit_file ciblé (pas de replace ligne 1 à fin).",
              "Si un fichier nécessite > 60% de changements → create_file complet.",
            ].filter(Boolean).join("\n");
            ...
            try {
              integratorOutput = await runAgent(INTEGRATOR_PROMPT, integratorInput, {
                temperature: 0.4,
                maxTokens: 32768, // réduit : l'input est léger, output ciblé suffit
                agentName: "INTEGRATOR",
                noTools: false, // readFile() actif — l'agent lit ce dont il a besoin
              });
            } catch (e: any) {
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 4a — DESIGN LINTER : scan des violations design post-génération
          // Agent léger à 0.2 — lit tous les fichiers .tsx et corrige UNIQUEMENT
          // les violations CSS/Tailwind (couleurs génériques, hauteurs, shadows...)
          // Ne touche pas à la logique, seulement au design
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:4a/DESIGN_LINTER]\n");

          {
            const linterFileContents = allGeneratedFiles
            ...
            const linterInput = `
${activeDesignAnchor ? `DESIGN CONTRACT — couleurs de référence :\n${activeDesignAnchor.substring(0, 1500)}\n` : "Pas de design contract fourni — vérifie uniquement les violations structurelles (hauteurs, shadows, boutons)."}

CODE À AUDITER :
${linterFileContents.substring(0, 60000)}

${HARD_CONSTRAINTS_BLOCK}

Scanne chaque fichier pour les violations listées dans ton prompt.
Pour chaque violation → edit_file chirurgical immédiat.
Si aucune violation → réponds : DESIGN_LINT_OK
`;

            try {
              const linterOutput = await runAgent(DESIGN_LINTER_PROMPT, linterInput, {
                temperature: 0.2,  // chirurgical — zéro créativité
                maxTokens: 32768,  // budget réduit — tâche ciblée
                agentName: "DESIGN_LINTER",
                noTools: true,     // tout le contexte est fourni directement
              });

              if (!linterOutput.includes("DESIGN_LINT_OK")) {
                mergeGeneratedFiles(parseGeneratedFiles(linterOutput));
                // Appliquer les edit_file du linter
                const lintEditOps = parseEditFileOps(linterOutput);
                if (lintEditOps.length > 0) {
                  const lintEditResult = applyEditFileOps(lintEditOps, allGeneratedFiles);
                  emit(`\n[DESIGN_LINTER] ✅ ${lintEditResult.applied} violation(s) corrigée(s)\n`);
                  if (lintEditResult.failed.length > 0) {
                    emit(`\n[DESIGN_LINTER] ⚠️ ${lintEditResult.failed.length} correction(s) échouée(s)\n`);
                  }
                } else {
                  emit(`\n[DESIGN_LINTER] ✅ Corrections appliquées\n`);
                }
              } else {
                emit(`\n[DESIGN_LINTER] ✅ Aucune violation design détectée\n`);
              }
            } catch (e: any) {
              emit(`\n[DESIGN_LINTER] ⚠️ Skip: ${e?.message?.slice(0, 60)}\n`);
            }
          }
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 4c — COLLECTE DEPS FINALE
          // Le package.json a déjà été émis en PHASE 0c.
          // Ici on scanne les imports pour capturer les packages non déclarés
          // avant que PHASE 5b émette la version définitive avec vraies versions.
          // ─────────────────────────────────────────────────────────────
          {
            allGeneratedFiles.forEach(f => {
              for (const m of f.content.matchAll(/from\s+['"]([^@./][^'"]*)['"]/g)) {
                const pkg = m[1].split("/")[0];
                if (pkg && !pkg.startsWith(".")) globalPkgs.add(pkg);
              }
            });
            emit("\n[PKG:DEPS_COLLECTED] ✅ " + globalPkgs.size + " package(s) collectés — version définitive en PHASE 5b\n");
          }
