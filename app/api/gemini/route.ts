...
const FIX_RULES: FixRule[] = [
  ...
  // ── globals.css : @apply border-border et autres classes shadcn undefined ────
  // Erreur : The `border-border` class does not exist
  // Cause : LLM génère @apply avec des classes qui ne sont définies que si
  //         tailwind.config.ts étend les couleurs avec les CSS variables
  {
    name: "globals-css-apply-undefined",
    detect: (p, c) => (p.endsWith("globals.css") || p === "app/globals.css")
      && /@apply\s+(?:border-border|bg-background|text-foreground|bg-card|text-card-foreground|bg-primary|text-primary-foreground|bg-secondary|bg-muted|text-muted|bg-accent|ring-ring|outline-ring)/.test(c),
    fix: (p, c) => fixGlobalsCss(c),
  },
  // ── tailwind.config.ts manquant ou sans les couleurs CSS variables ──────────
  // Erreur : "border-border" class does not exist car tailwind.config ne mappe pas --border
  {
    name: "tailwind-config-missing-colors",
    detect: (p, c) => p === "tailwind.config.ts"
      && c.includes("extend")
      && !c.includes("hsl(var(--border))") && !c.includes('"border"')
      // seulement si on a un globals.css qui utilise ces vars (vérification légère)
      && c.includes("colors"),
    fix: (_, c) => {
      // Injecte le mapping des couleurs CSS dans le extend.colors existant
      const colorEntries = [
      ...
      // Trouve le bloc colors: { ... } dans extend et injecte si pas déjà là
      if (/colors\s*:\s*\{/.test(c)) {
        return c.replace(/colors\s*:\s*\{/, `colors: {\n        ${colorEntries},`);
      }
      // Sinon ajoute colors dans extend
      return c.replace(/extend\s*:\s*\{/, `extend: {\n      colors: {\n        ${colorEntries},\n      },`);
    },
  },

  // ── Assure que tailwind.config.ts existe quand globals.css utilise @tailwind ─
  // (ce fixer est en fallback — l'ensureTailwindConfig s'en charge en priorité)
...
// =============================================================================
// GLOBALS.CSS FIXER — supprime les classes Tailwind qui n'existent pas
// Cause principale de l'erreur : "The `border-border` class does not exist"
// Ces classes (border-border, bg-background, text-foreground...) sont des
// conventions shadcn/ui qui nécessitent un tailwind.config.ts avec extend.colors.
// On les remplace par du CSS pur ou on les supprime.
// =============================================================================

function fixGlobalsCss(content: string): string {
  if (!content.includes("@tailwind")) return content;

  // Map des classes @apply problématiques → leur équivalent CSS pur
  const APPLY_REPLACEMENTS: Record<string, string> = {
    "border-border":            "border-color: hsl(var(--border, 214.3 31.8% 91.4%))",
    "bg-background":            "background-color: hsl(var(--background, 0 0% 100%))",
    "text-foreground":          "color: hsl(var(--foreground, 222.2 84% 4.9%))",
    "bg-card":                  "background-color: hsl(var(--card, 0 0% 100%))",
    "text-card-foreground":     "color: hsl(var(--card-foreground, 222.2 84% 4.9%))",
    "bg-popover":               "background-color: hsl(var(--popover, 0 0% 100%))",
    "text-popover-foreground":  "color: hsl(var(--popover-foreground, 222.2 84% 4.9%))",
    "bg-primary":               "background-color: hsl(var(--primary, 222.2 47.4% 11.2%))",
    "text-primary-foreground":  "color: hsl(var(--primary-foreground, 210 40% 98%))",
    "bg-secondary":             "background-color: hsl(var(--secondary, 210 40% 96.1%))",
    "text-secondary-foreground":"color: hsl(var(--secondary-foreground, 222.2 47.4% 11.2%))",
    "bg-muted":                 "background-color: hsl(var(--muted, 210 40% 96.1%))",
    "text-muted-foreground":    "color: hsl(var(--muted-foreground, 215.4 16.3% 46.9%))",
    "bg-accent":                "background-color: hsl(var(--accent, 210 40% 96.1%))",
    "text-accent-foreground":   "color: hsl(var(--accent-foreground, 222.2 47.4% 11.2%))",
    "bg-destructive":           "background-color: hsl(var(--destructive, 0 84.2% 60.2%))",
    "text-destructive-foreground": "color: hsl(var(--destructive-foreground, 210 40% 98%))",
    "ring-ring":                "outline-color: hsl(var(--ring, 222.2 84% 4.9%))",
    "outline-ring":             "outline-color: hsl(var(--ring, 222.2 84% 4.9%))",
  };
  ...
  for (const line of lines) {
    const applyMatch = line.match(/^(\s*)@apply\s+(.+?)\s*;?\s*$/);
    if (applyMatch) {
      const indent = applyMatch[1];
      const classes = applyMatch[2].trim().split(/\s+/);
      const converted: string[] = [];
      const kept: string[] = [];

      for (const cls of classes) {
        if (APPLY_REPLACEMENTS[cls]) {
          converted.push(`${indent}${APPLY_REPLACEMENTS[cls]};`);
        } else if (
          // Classes qui référencent des variables CSS non définies dans tailwind config par défaut
          /^(?:border|bg|text|ring|outline)-[a-z]/.test(cls) &&
          !cls.match(/^(?:border|bg|text|ring|outline)-(?:white|black|gray|red|blue|green|yellow|purple|pink|indigo|teal|cyan|orange|transparent|current|inherit|none|auto|[0-9])/)
        ) {
          // Classe custom inconnue → supprimer l'@apply, juste ignorer
          // (ne pas casser le CSS)
        } else {
          kept.push(cls);
        }
      }

      if (converted.length > 0) result.push(...converted);
      if (kept.length > 0) result.push(`${indent}@apply ${kept.join(" ")};`);
      // Si les deux sont vides, la ligne est supprimée (classe non reconnue)
    } else {
      result.push(line);
    }
  }
...
// Génère/met à jour tailwind.config.ts en se basant sur les CSS vars de globals.css
// Si globals.css contient --border, --background etc., tailwind.config.ts doit
// les mapper sinon les @apply échouent
// =============================================================================

function ensureTailwindConfig(files: GeneratedFile[]): GeneratedFile[] {
  const globalsCss = files.find(f => f.path === "app/globals.css" || f.path === "globals.css");
  if (!globalsCss) return files;

  // Extrait les variables CSS déclarées dans :root {}
  const rootVarsMatch = globalsCss.content.match(/:root\s*\{([^}]*)\}/);
  if (!rootVarsMatch) return files;

  const cssVarBlock = rootVarsMatch[1];
  const cssVars: Record<string, string> = {};
  const varRx = /--(\w[\w-]*)\s*:\s*([^;]+);/g;
  let m;
  while ((m = varRx.exec(cssVarBlock)) !== null) {
    cssVars[m[1]] = m[2].trim();
  }

  if (Object.keys(cssVars).length === 0) return files;

  // Noms de couleurs standard qui mappent vers des variables CSS
  const COLOR_NAMES = [
  ...
  const definedColorVars = COLOR_NAMES.filter(name => cssVars[name]);

  if (definedColorVars.length === 0) return files;

  // Génère le bloc colors pour tailwind.config.ts
  const colorsBlock = definedColorVars
    .map(name => {
      const key = name.includes("-") ? `"${name}"` : name;
      return `        ${key}: "hsl(var(--${name}))",`;
    })
    .join("\n");
...
`;
  ...
  if (existingIdx >= 0) {
    // Met à jour seulement si le config existant ne définit pas déjà les couleurs
    const existing = files[existingIdx].content;
    if (!existing.includes("hsl(var(--border))") && !existing.includes('"border"')) {
      const updated = [...files];
      updated[existingIdx] = { path: "tailwind.config.ts", content: tailwindConfig };
      return updated;
    }
    return files;
  }
  ...
  // Ajoute le fichier
  return [...files, { path: "tailwind.config.ts", content: tailwindConfig }];
}
...
6. tailwind.config.ts — OBLIGATOIRE avec les mappings de couleurs CSS variables

NE PAS generer : hooks, composants UI, vues, layout, page.tsx.

════════════════════════════════════════════════════════════
REGLE TAILWIND CONFIG — OBLIGATOIRE
════════════════════════════════════════════════════════════

Tu DOIS generer tailwind.config.ts. Il DOIT contenir dans extend.colors
les mappings des variables CSS qui seront utilisees dans globals.css.

EXEMPLE OBLIGATOIRE — tailwind.config.ts :
...
SANS CE FICHIER ou SANS CES MAPPINGS : le build echoue avec "border-border class does not exist".
...
les definisse dans extend.colors.

DEUX OPTIONS — CHOISIS UNE SEULE ET SUIS-LA :

OPTION A (recommandee) — N'utilise PAS @apply pour les classes qui referencent des CSS vars.
Ecris directement le CSS pur :
  A EVITER : @apply border-border bg-background text-foreground;
  CORRECT :  border-color: hsl(var(--border)); background-color: hsl(var(--background)); color: hsl(var(--foreground));

OPTION B — Si tu veux utiliser @apply border-border etc., tu DOIS generer tailwind.config.ts avec :
  extend: {
    colors: {
      border: "hsl(var(--border))",
      background: "hsl(var(--background))",
...
REGLE : globals.css + tailwind.config.ts doivent etre COHERENTS.
Si globals.css a des CSS vars --border etc. → tailwind.config.ts DOIT les mapper.
Si tu n'es pas sur → utilise l'Option A (CSS pur, pas de @apply avec classes CSS-var).

Tu dois generer AUSSI tailwind.config.ts si globals.css utilise des CSS variables.
...
□ GLOBALS.CSS + TAILWIND.CONFIG — ERREUR FATALE LA PLUS COURANTE :
  ...
  FIX OBLIGATOIRE SI globals.css contient @apply border-border (ou bg-background, text-foreground...) :
  
  Option A (recommandée) — Convertir @apply en CSS pur dans globals.css :
    Avant : @apply border-border;
    Apres : border-color: hsl(var(--border));
    
    Avant : @apply bg-background text-foreground;
    Apres : background-color: hsl(var(--background)); color: hsl(var(--foreground));
  
  Option B — S'assurer que tailwind.config.ts définit les couleurs :
    extend: {
      colors: {
        border: "hsl(var(--border))",
        background: "hsl(var(--background))",
  ...
  VÉRIFIE : si globals.css a @apply border-border → tailwind.config.ts DOIT avoir border dans colors.
  Si tailwind.config.ts n'existe pas → le créer avec Option B.
  Si globals.css a @apply avec ces classes → les convertir en CSS pur (Option A, plus sûre).
...
ERREUR "The `border-border` class does not exist" ou similaire (bg-background, text-foreground...) :
  ...
  Fix 2 (tailwind config) : dans tailwind.config.ts, ajoute dans extend.colors :
    ...
    card: { DEFAULT: "hsl(var(--card))", foreground: "hsl(var(--card-foreground))" },
    ring: "hsl(var(--ring))",
  Si tailwind.config.ts n'existe pas → le creer avec ces mappings.
...
`;
          ...
          function applyProgrammaticFixers() {
            // 0. globals.css — supprime les @apply avec classes undefined (border-border etc.)
            const cssIdx = allGeneratedFiles.findIndex(f =>
              f.path === "app/globals.css" || f.path === "globals.css"
            );
            if (cssIdx >= 0) {
              const fixed = fixGlobalsCss(allGeneratedFiles[cssIdx].content);
              if (fixed !== allGeneratedFiles[cssIdx].content) {
                allGeneratedFiles[cssIdx] = { ...allGeneratedFiles[cssIdx], content: fixed };
              }
            }

            // 0b. tailwind.config.ts — génère/met à jour en lisant les CSS vars de globals.css
            const withTailwind = ensureTailwindConfig(allGeneratedFiles);
            if (withTailwind.length !== allGeneratedFiles.length ||
                withTailwind.some((f, i) => f.content !== allGeneratedFiles[i]?.content)) {
              for (let i = 0; i < withTailwind.length; i++) {
                allGeneratedFiles[i] = withTailwind[i];
              }
              if (withTailwind.length > allGeneratedFiles.length) {
                for (let i = allGeneratedFiles.length; i < withTailwind.length; i++) {
                  allGeneratedFiles.push(withTailwind[i]);
                }
              }
            }

            // 1. Type safety fixer (store/interface coherence)
