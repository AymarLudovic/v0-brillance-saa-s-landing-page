...
`;
          ...
          // Inject design.md TOUJOURS si présent — complète le design anchor ou le remplace
          // (avant : injecté seulement quand !activeDesignAnchor — c'était l'erreur)
          ...
          // Inject app/__design_anchor__.md raw HTML/CSS si présent dans les fichiers projet
          // (double-sécurité : l'agent voit le fichier ET la directive DESIGN CONTRACT)
          ...
          if (designAnchorFile && designAnchorFile.content.length > 100) {
            systemPrompt +=
              `\n\n╔══════════════════════════════════════════════════════════════╗\n` +
              `║  HTML/CSS DU DESIGN AGENT — AUTORITÉ ABSOLUE — NE PAS DÉROGER  ║\n` +
              `╚══════════════════════════════════════════════════════════════════╝\n` +
              `Le Design Agent a produit ce HTML/CSS pixel-perfect. TU DOIS extraire ses variables\n` +
              `:root{} et les utiliser EXACTEMENT dans tous tes composants. INTERDIT de les remplacer\n` +
              `par des valeurs Tailwind génériques (gray-*, blue-*, etc.).\n\n` +
              `=== ${DESIGN_ANCHOR_FILE} ===\n` +
              `${designAnchorFile.content.slice(0, 12000)}\n` +
              `=== END ===\n`;
          }
