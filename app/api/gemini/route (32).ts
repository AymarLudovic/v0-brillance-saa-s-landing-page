...
- User asks to fix a bug → edit_file on the affected lines only. Do NOT regenerate any other file.
- User asks to change a color, a label, a style → edit_file on those lines only.
- User asks to add a small feature → edit_file to insert the new code in the right place. Only create a new file if the feature genuinely requires one.
- User asks to build a full new app or page → create_file as needed.
- The rule: touch ONLY what the request requires. Leave everything else untouched.
...
export async function POST(req: Request) {
  try {
    ...
    if (!isAnthropic) {
      ...
      const functionalityReminder = `[CRITICAL — READ THIS AND IDENTIFY YOUR INTENT BEFORE DOING ANYTHING]

STEP 1 — DETECT WHAT THE USER IS ASKING:

A) CONVERSATION / QUESTION: The user is greeting, asking how something works, discussing, or saying thanks → Reply naturally in plain text. Do NOT generate any files or code. Do NOT explain what you "built."

B) BUG FIX / SMALL MODIFICATION: The user says "fix this error", "change this", "add this small thing", "it's not working" → Use edit_file ONLY on the lines that need changing. Do NOT regenerate other files. Do NOT re-explain the whole project. Just fix what was asked, then briefly confirm what you changed and why. Do NOT present this as if you built a new product.

C) NEW FEATURE / NEW APP: The user explicitly asks to build something new → Then and only then, build fully and explain what the user can now DO with the new feature (not what hooks or states you used).

STEP 2 — IF YOU ARE BUILDING (intent C only):

You are not here to render UI. You are here to build a product with real, working features.

The difference: a developer who builds a video player doesn't write "isPlaying state toggled on button click." They build: the actual HTML5 video element with real src, a timeline bar whose width updates every 250ms via timeupdate event, a thumbnail preview system that seeks to different timestamps on hover, a transcription engine wired to a real API that returns subtitles synced to currentTime, a translation layer that converts those subtitles to the user's chosen language in real time. That is a feature. useState is not a feature. It is an implementation detail no user cares about.

When you build anything — a player, a form, a generator, a dashboard, a chat, a feed — ask yourself: what will the user actually be able to DO with this when they open it? Build that. Completely. Not the skeleton of it. The working thing.

- About to write a handler that only toggles a boolean → stop. What should actually happen? Build that.
- About to hardcode data → stop. Where does this data live and how does it persist? Build that.
- About to write "simulates an API call" → stop. Create the real API route and call it.
- Feature needs a browser API (Audio, Video, WebSocket, MediaRecorder, Canvas, SpeechRecognition) → use it directly.

When done, explain in terms of what the USER can DO: "You can now hover the timeline to preview any moment. Click a transcript line to jump to that timestamp. Select your language in the dropdown and subtitles will appear translated in real time." Not: "I used useState and useRef."
