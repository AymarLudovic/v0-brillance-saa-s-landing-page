...
// =============================================================================
// FEATURE_PLANNER — Analyse la demande et produit un contrat d'implémentation
// S'exécute AVANT Foundation. Garantit que toutes les features sont implémentées.
// =============================================================================

const FEATURE_PLANNER_PROMPT = `
Tu es l'Agent Planificateur. Tu analyses la demande utilisateur et produis un contrat
d'implémentation COMPLET et DÉTAILLÉ que tous les agents suivants DEVRONT respecter.

[WORKING_ON]Planification — contrat d'implémentation des features[/WORKING_ON]

Tu NE génères PAS de code. Tu génères un plan en texte structuré.

══════════════════════════════════════════════════════════════════════
ANALYSE LA DEMANDE ET PRODUIS CE PLAN :
══════════════════════════════════════════════════════════════════════

<feature_contract>
VIEWS À CRÉER :
- [NomView] : [description courte de ce que cette view affiche et fait]
  - URL/route dans app/page.tsx : activeView === "[nom]"
  ...

FONCTIONNALITÉS REQUISES (pour chaque feature, détaille l'implémentation) :

FEATURE 1 : [Nom exact]
  State requis : const [name, setName] = useState<[Type]>([valeur initiale])
  Handler requis : function handle[Action]([params]: [types]) { /* logique complète */ }
  Données mock (si nécessaire) : const MOCK_[DATA]: [Type][] = [ /* 12+ entrées */ ]
  JSX requis : [décris le composant qui rend cette feature]
  Librairies si nécessaire : [npm package si requis]
...
[continuer pour CHAQUE feature demandée]

NAVIGATION REQUISE :
  app/page.tsx doit gérer : const [activeView, setActiveView] = useState("[view initiale]")
  Sidebar items : [liste exacte des items avec leur view cible]

TYPES TYPESCRIPT REQUIS :
  interface [Nom] { [champs typés] }
  ...

LIBRAIRIES NPM REQUISES :
  DEPENDENCIES: ["lib1", "lib2"]

DONNÉES MOCK REQUISES :
  [Pour chaque feature qui affiche des données : type et structure des données mock]
</feature_contract>

RÈGLES :
- Liste ABSOLUMENT TOUTES les features demandées — rien ne doit être oublié
- Pour chaque handler, décris ce qu'il fait réellement (pas "gère le clic" — mais "filtre le tableau selon searchQuery")
- Si une feature nécessite une vraie librairie (graphique → recharts, son → tone.js), dis-le
- Les données mock doivent être réalistes et abondantes (12+ entrées minimum)
`;
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          // PHASE 0c — FEATURE PLANNER
          // Analyse la demande et produit un contrat d'implémentation complet
          // Foundation doit suivre ce contrat à la lettre
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:0c/FEATURE_PLANNER]\n");
          let featureContract = "";
          try {
            const plannerInput = `
Demande utilisateur : "${lastUserMsg}"

${projectContext ? "Contexte projet existant :\n" + projectContext : "Nouveau projet — pas de fichiers existants."}

Analyse cette demande et produis le contrat d'implémentation complet.
Chaque feature demandée DOIT apparaître dans le contrat avec son état, ses handlers et ses données.
`;
            const plannerOutput = await runAgent(FEATURE_PLANNER_PROMPT, plannerInput, {
              temperature: 0.7, // focused planning
              maxTokens: 16384,
              agentName: "FEATURE_PLANNER",
              noTools: true,
            });
            flushBuffer();
            const contractMatch = plannerOutput.match(/<feature_contract>([\s\S]*?)<\/feature_contract>/);
            if (contractMatch) {
              featureContract = contractMatch[1].trim();
              emit("\n[PLANNER] ✅ Contrat de " + featureContract.split("\n").length + " lignes généré\n");
            } else {
              featureContract = plannerOutput.slice(0, 4000);
              emit("\n[PLANNER] ⚠️ Format libre — contrat extrait\n");
            }
          } catch (e: any) {
            emit("\n[PLANNER] Skip: " + (e?.message?.slice(0, 60) ?? "") + "\n");
          }
...
${featureContract ? `
═══════════════════════════════════════════════════════════════════
CONTRAT D'IMPLÉMENTATION — TU DOIS IMPLÉMENTER CHAQUE LIGNE
Chaque feature listée ici doit avoir son état, ses handlers et son JSX
AUCUNE feature ne peut être omise ou simulée
═══════════════════════════════════════════════════════════════════
${featureContract}
` : ""}
