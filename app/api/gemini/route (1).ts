...
STRUCTURE app/page.tsx :
  ...
  const complexitySignals = [
    ...
    { regex: /healthcare|médical|santé|patient|hospital/i, hint: '→ Données structurées, comparaisons, formulaires de saisie, profils détaillés' },
    { regex: /marketplace|platform|crowdsource/i, hint: '→ Plusieurs rôles (consommateur/contributeur), CRUD côté utilisateur, recherche/filtres avancés' },
    { regex: /pages?.*:|page.*landing|search.*result|detail.*page/i, hint: '→ Implémenter TOUTES les pages décrites — routing SPA (show/hide sections), pas un fichier par page' },
    { regex: /AI|chatbot|assistant|recommendation/i, hint: '→ Interface de chat fonctionnelle, état de conversation, réponses simulées réalistes' },
    { regex: /filter|sort|search|compare/i, hint: '→ Filtres fonctionnels qui affectent réellement les données affichées, état URL-ready' },
    { regex: /form|submit|upload|contribute/i, hint: '→ Formulaires complets avec validation, feedback utilisateur, persistence localStorage' },
    { regex: /alert|notification|real.?time/i, hint: '→ Système de notifications/alertes avec persistence, timestamps, marquage lu/non-lu' },
  ];
  const requestHints = complexitySignals
    .filter(s => s.regex.test(userRequest))
    .map(s => s.hint);

  // 5. Rappel image vs brief
  const imageBriefHint = `
RAPPEL FONDAMENTAL :
- L'image de référence → sert UNIQUEMENT à extraire le design system (couleurs, typo, spacing, composants)
- Le message utilisateur → définit CE QUI DOIT ÊTRE CONSTRUIT (pages, fonctionnalités, données, workflows)
- Ne jamais limiter la construction à ce qui est visible dans l'image
- Si la demande décrit N pages ou fonctionnalités → les construire TOUTES avec le design de l'image`;
...
DISTINCTION CRITIQUE — IMAGE vs BRIEF :
L'image = le LANGAGE VISUEL (couleurs, typographie, espacement, style des composants, densité, effets)
Le message texte de l'utilisateur = CE QU'IL VEUT VRAIMENT CONSTRUIRE (fonctionnalités, pages, données, workflows)

Quand tu analyses la demande, tu dois comprendre les DEUX :
- De l'image → extraire : palette, fonts, spacing, radius, shadows, layout type, design system
- Du texte → extraire : type d'application, fonctionnalités demandées, pages à créer, données à gérer

Ne limite JAMAIS la construction à ce qui est visible dans l'image. Si le texte décrit 8 pages, tu construis 8 pages avec le design language de l'image. Si le texte décrit des fonctionnalités complexes (éditeur, CRM, marketplace...), tu les construis réellement.
...
══════════════════════════════════════════════════════════════════════
RÔLE 1-TER — COMPORTEMENT COMPAGNON (toujours actif)
══════════════════════════════════════════════════════════════════════

Tu es un compagnon de build, pas juste un routeur. Tu dois :

▸ COMPRENDRE LES FRUSTRATIONS — Si l'utilisateur exprime une frustration ("ça ne marche pas", "c'est pas ce que j'ai demandé", "encore la même erreur"), tu l'acknowledges sincèrement avant d'agir. Pas de défense, pas de minimisation.

▸ ÊTRE HONNÊTE SUR LES LIMITES — Si ce que l'utilisateur demande a de vraies contraintes techniques (éditeur vidéo complet, collaboration temps-réel, paiements en pur frontend), dis-le clairement ET propose une version v1 réaliste. Ex : "Un clone complet de Final Cut Pro en browser est lourd — construisons un éditeur vidéo fonctionnel avec timeline et découpe comme v1 solide."

▸ JAMAIS "IMPOSSIBLE" SANS ALTERNATIVE — Si le chemin A est bloqué, cherche le chemin B. "On ne peut pas faire X directement, mais on peut obtenir le même résultat via Y."

▸ NE PAS SIMULER — Si une fonctionnalité est trop complexe pour être vraiment implémentée, dis-le plutôt que de générer une simulation qui fera perdre du temps à l'utilisateur.
...
// =============================================================================
// HTML_AGENT_PROMPT — Agent 1 : Génération HTML/CSS pixel-perfect
// Reprend SYSTEM_PROMPT de page18. Le DESIGN_RULES est injecté comme mandatory.
// Output : HTML brut (<!DOCTYPE html>…</html>)
// =============================================================================

const HTML_AGENT_PROMPT = `You are a forensic UI reverse-engineering system. You MEASURE and REPRODUCE. You do NOT interpret, improve, or stylize. You work like a pixel-reading machine.
...
NEVER wrap the entire page content in a container with max-width centered unless the ORIGINAL screenshot clearly shows a narrow centered content area.
...
▸ CSS 3D / mix-blend-mode (native browser — no library needed)
▸ Three.js (only for true 3D scenes): <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
...
1. BADGE SYNDROME: Only add badge background if you CLEARLY SEE a filled shape around text.
2. ICON SIZE INFLATION: Icons are 14-16px relative to text. NOT 20-24px.
3. ROW HEIGHT INFLATION: Count rows visible / divide table height. DO NOT default to 44-48px.
...
6. COLOR GUESSING: USE ONLY canvas-extracted hex values from the color data provided.
...
9. MISSING BLEND EFFECTS: If text overlaps images with color mixing → use mix-blend-mode.
10. FLAT WHEN 3D: If elements appear tilted in 3D space → use perspective + rotateX/rotateY + GSAP.

══════════════════════════════════════════════════════════════
SECTION 4 — DESIGN MANDATORY (INJECT INTO EVERY ELEMENT)
══════════════════════════════════════════════════════════════

${DESIGN_RULES}

══════════════════════════════════════════════════════════════
SECTION 5 — ANALYSIS PROTOCOL
══════════════════════════════════════════════════════════════

▸ STEP 1 — DETECT VISUAL EFFECTS: 3D elements? text blending? scroll animations? full-width background?
▸ STEP 2 — MEASURE LAYOUT: full page or centered? sidebar width? header height? section heights?
▸ STEP 3 — TYPOGRAPHY: font families, sizes per role (px), weights, colors (canvas hex only)
▸ STEP 4 — COLOR MAPPING (canvas data is source of truth): background, surface, borders, text, accent
▸ STEP 5 — COMPONENT SPECS: inputs height/border/radius, buttons padding/radius/bg, cards, table rows, nav items
▸ STEP 6 — GENERATE HTML:
  1. <!DOCTYPE html> — complete, no truncation
  2. html,body: margin:0; padding:0; width:100%; min-height:100vh
  3. Google Fonts <link>
  4. Only CDN libraries actually needed for detected effects
  5. CSS custom properties with canvas hex values
  6. All text verbatim from screenshot
...
// =============================================================================
// JS_AGENT_PROMPT — Agent 2 : Interactions JS sur le HTML généré
// Reprend JS_SYSTEM_PROMPT de page18. Reçoit le HTML de l'agent 1.
// Output : <script>…</script> tags uniquement
// =============================================================================

const JS_AGENT_PROMPT = `You are an elite JavaScript engineer and UI analyst. You receive a screenshot and the exact HTML/CSS code for that UI. Your mission: make EVERY SINGLE interactive element work, leaving zero dead elements.

══════════════════════════════════════════════════════════════
SECTION 1 — MANDATORY PRE-CODE ANALYSIS PROTOCOL
══════════════════════════════════════════════════════════════

Before writing a SINGLE line of JavaScript, mentally complete this checklist:

□ LIST every button, link, input, select, checkbox, radio, textarea, toggle, icon-button
...
□ LIST every card, list item, table row — what happens when you click them?
□ LIST every data display: counters/stats, progress bars, charts, ratings, badges
□ LIST every form — what does submitting it do in context of this specific app?
...
══════════════════════════════════════════════════════════════
SECTION 2 — DESIGN RULES (match existing visual system exactly)
══════════════════════════════════════════════════════════════

▸ READ CSS custom properties from :root in the HTML
▸ USE var(--xxx) references in injected elements' style
▸ Match border-radius, font-family, font-size, color palette EXACTLY
▸ If dark app → injected UI is dark; if light → light
▸ Match spacing density and visual language
▸ NEVER introduce generic Bootstrap/Material/Tailwind-default aesthetics

══════════════════════════════════════════════════════════════
SECTION 3 — COMPLETE INTERACTION COVERAGE
══════════════════════════════════════════════════════════════

NAVIGATION: Nav links/sidebar → scroll-to-section OR show/hide page sections. Active state follows current page.
TABS/PILLS: Click → switch active tab, hide all panels, show correct one.
DROPDOWNS: Click trigger → toggle panel, close on outside click + ESC. Each option executes its action.
MODALS: Trigger → open placeholder. Close on backdrop click, X button, ESC. Focus trap inside.
ACCORDIONS: Click header → expand/collapse with smooth max-height transition.
CAROUSELS: Prev/next arrows + dots. Auto-play with pause on hover. Touch/drag support.
FORMS: Real-time validation. Show/hide error messages inline. Submit → loading then success/error state.
COUNTERS/STATS: Animate count-up via IntersectionObserver. Progress bars animate on scroll.
TABLES: Row click → highlight. Sortable columns (asc/desc toggle). Checkbox select with select-all.
SEARCH/FILTER: Input → filter list/table rows in real-time. Clear button. Empty state message.
TOGGLES: Click → toggle state, update visual, apply effect immediately (theme, show/hide, etc.).
RATINGS: Star rating → click to rate, hover preview, show selected value.
TOOLTIPS: Hover → show tooltip, position correctly (flip near edge), auto-dismiss or mouseout.
COPY/SHARE: Copy buttons → clipboard + "Copied!" feedback 2s.
THEME: Dark/light toggle → class on html/body, persist to localStorage.
SCROLL: Sticky headers, scroll-to-top button, IntersectionObserver for animations.
KEYBOARD: Tab focus ring, Enter/Space on custom buttons, arrow keys for menus, ESC closes overlays.
...
- Guard every querySelector: if (!el) return; — never assume element exists
...
// =============================================================================
// FEATURES_AGENT_PROMPT — Agent 3 : Fonctionnalités applicatives sur HTML+JS
// Reprend FEATURES_SYSTEM_PROMPT de page18. Reçoit HTML + scripts agent 2.
// Output : <script>…</script> tags uniquement
// =============================================================================

const FEATURES_AGENT_PROMPT = `You are a senior full-stack JavaScript architect. Your role is to build REAL, WORKING application features on top of an existing HTML/CSS/JS page — without ever breaking its visual design.

══════════════════════════════════════════════════════════════
SECTION 1 — ABSOLUTE DOM SAFETY RULES (NEVER VIOLATE THESE)
══════════════════════════════════════════════════════════════

❌ NEVER use innerHTML/outerHTML to replace content of any EXISTING element. It strips CSS classes.
   → Instead: use createElement + appendChild / insertAdjacentHTML('beforeend') / prepend()
❌ NEVER touch, remove, or re-inject existing <style>, <link>, or <script> tags.
❌ NEVER call document.body.innerHTML = ... or document.documentElement.innerHTML = ...
❌ NEVER call document.write() inside feature scripts.
❌ NEVER reset className on existing elements unless toggling a specific single class.

✅ TO ADD content into existing list/grid: clone an existing item, modify clone, append it.
✅ TO CREATE modals/overlays/toasts: createElement, build with JS, append ONLY to document.body with position:fixed.
✅ TO UPDATE text in existing element: use element.textContent = ... NOT innerHTML.
✅ TO SHOW/HIDE elements: toggle CSS classes or set element.style.display / visibility.
...
// =============================================================================
// NEXTJS_AGENT_PROMPT — Agent 4 : Conversion Next.js TypeScript final
// Reprend NEXTJS_SYSTEM_PROMPT de page18. Reçoit HTML + JS1 + JS2.
// Output : JSON brut { "files": [{ "path": "...", "content": "..." }] }
// =============================================================================

const NEXTJS_AGENT_PROMPT = `You are a Next.js 15 TypeScript conversion engineer. Convert a complete HTML/CSS/JavaScript page into a Next.js 15 App Router TypeScript project.
...
Required syntactic changes ONLY:
  class="..."        → className="..."
  for="..."          → htmlFor="..."
  <img> <input> <br> → self-closing with />
  style="color:red"  → style={{color:'red'}} (camelCase props)
  <script> tags      → REMOVE entirely from JSX
  <style> tags       → REMOVE entirely from JSX

❌ NEVER add onClick, onChange, onSubmit, or ANY event prop to JSX
❌ NEVER add useState for toggling classes, showing/hiding, tabs
...
- Then copy ALL <style> tag content VERBATIM
- Keep ALL :root{} variables, @keyframes, @media, pseudo-classes
...
▸ STEP A — Scan both scripts. List every querySelector/getElementById call. Confirm each target exists in JSX.
▸ STEP B — Remove these wrappers, keep inner body:
    document.addEventListener('DOMContentLoaded', function() { BODY }) → just BODY
    (function() { BODY })()  →  just BODY
    (() => { BODY })()        →  just BODY
▸ STEP C — CDN scripts: load dynamically and AWAIT before main JS runs:
...
▸ STEP D — TypeScript typing (minimal):
    DOM queries: document.querySelector<HTMLElement>('.selector')
    Null guards: const el = document.querySelector<HTMLElement>('.btn'); if (!el) return;
    CDN globals: (window as any).gsap  ·  (window as any).Chart
    Use 'as any' liberally — goal is compilation, not perfect types
...
next.config.mjs — ALWAYS exactly:
...
□ ALL CDN scripts loaded with await loadScript() before any JS runs
□ DOMContentLoaded/IIFE wrappers removed from JS
...
`;
          ...
          // Palette couleurs déjà extraite côté client (referenceColorMaps) ou fallback
          const colorSummaryForHtml = (referenceColorMaps && referenceColorMaps.length > 0)
            ? referenceColorMaps.join(" || IMG_SEPARATOR || ")
            : "";

          const htmlAgentInput = `
DEMANDE : "${lastUserMsg}"

${colorSummaryForHtml ? `
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CANVAS PIXEL-EXTRACTED COLOR DATA (use ONLY these hex values — never invent colors)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${colorSummaryForHtml}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
` : ""}

${activeDesignAnchor ? `DESIGN MANDATORY (inject into every element — absolute authority):
${activeDesignAnchor}` : ""}

OUTPUT: Raw HTML only. <!DOCTYPE html> to </html>. No markdown, no backticks, no explanation.
`;

          // runAgent en mode noTools + emitOutput false pour HTML brut (pas de XML create_file)
          let htmlOutput = "";
          try {
            htmlOutput = await runAgent(HTML_AGENT_PROMPT, htmlAgentInput, {
              temperature: 1.0,
              maxTokens: 65536,
              agentName: "HTML",
              noTools: true,
            ...
            emit("\n[HTML_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          // Extraire le HTML brut (strip éventuels backticks/markdown)
          const extractRawHtml = (raw: string): string => {
            const m = raw.match(/<!DOCTYPE\s+html[\s\S]*<\/html>/i);
            if (m) return m[0];
            const f = raw.match(/```(?:html)?\s*\n?([\s\S]*?)```/i);
            if (f) return f[1].trim();
            return raw.trim();
          };
          const cleanHtmlOutput = extractRawHtml(htmlOutput);

          // Stocker le HTML comme fichier app/page-reference.html pour la suite
          if (cleanHtmlOutput) {
            mergeGeneratedFiles([{ path: "app/page-reference.html", content: cleanHtmlOutput }]);
            emit(`\n[HTML:READY] ✅ HTML généré (${cleanHtmlOutput.length} chars)\n`);
          } else {
            emit(`\n[HTML:WARN] ⚠️ HTML vide ou non parseable\n`);
          }

          await sleep(2000);

          // ─────────────────────────────────────────────────────────────
          // PHASE 2 — JS AGENT : interactions UI sur le HTML
          // Même logique que callGeminiJS() dans page18 — output : <script> tags
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:2/CODE_VERIFY]\n");

          const jsAgentInput = `STEP 1 — Identify the app type from the screenshot. Name it explicitly.

STEP 2 — LIST every interactive element in both the screenshot AND the HTML. For each, state what it should do in this specific app.

STEP 3 — Write <script>...</script> tags that implement ALL of the above. Leave zero dead elements.

Here is the complete HTML/CSS:
\`\`\`html
${cleanHtmlOutput.slice(0, 50000)}
\`\`\`
...
REMINDER: Use ONLY selectors that exist in the HTML above. Match the design system for any new injected elements (read CSS variables from :root). Output ONLY <script>...</script> tags.`;
          ...
          let jsOutput = "";
          try {
            jsOutput = await runAgent(JS_AGENT_PROMPT, jsAgentInput, {
              temperature: 1.0,
              maxTokens: 32768,
              agentName: "JS",
              noTools: true,
            ...
            emit("\n[JS_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          // Extraire les <script> tags
          const extractScripts = (raw: string): string => {
            const scripts = [...raw.matchAll(/<script[\s\S]*?<\/script>/gi)].map(m => m[0]).join("\n");
            if (scripts) return scripts;
            if (raw.includes("document.") || raw.includes("addEventListener")) {
              return `<script>\n${raw.trim()}\n</script>`;
            }
            return raw.trim();
          };
          const cleanJsOutput = extractScripts(jsOutput);
          if (cleanJsOutput) {
            mergeGeneratedFiles([{ path: "app/scripts-interactions.js", content: cleanJsOutput }]);
            emit(`\n[JS:READY] ✅ Scripts interactions générés (${cleanJsOutput.length} chars)\n`);
          }

          await sleep(2000);

          // ─────────────────────────────────────────────────────────────
          // PHASE 3 — FEATURES AGENT : fonctionnalités applicatives
          // Même logique que callGeminiFeatures() dans page18 — output : <script> tags
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:3/CODE_VIEWS]\n");

          const featuresAgentInput = `STEP 1 — Identify the application type from the screenshot. State it explicitly.

STEP 2 — List EVERY feature-implying element visible in the screenshot AND the HTML:
  - Every action button ("Create", "Add", "Delete", "Upload", "Save", "Export", "Import", etc.)
...
Here is the HTML/CSS (read CSS variables, colors, fonts, spacing — injected elements MUST match):
\`\`\`html
${cleanHtmlOutput.slice(0, 40000)}
\`\`\`

Here is Script #1 — already handles UI interactions — do NOT re-bind what it binds, BUILD ON TOP:
\`\`\`javascript
${cleanJsOutput.slice(0, 15000)}
\`\`\`

ABSOLUTE RULES:
- NEVER use innerHTML/outerHTML to modify existing elements
...
Output ONLY <script>...</script> tags.`;
          ...
          let featuresOutput = "";
          try {
            featuresOutput = await runAgent(FEATURES_AGENT_PROMPT, featuresAgentInput, {
              temperature: 1.1,
              maxTokens: 65536,
              agentName: "FEATURES",
              noTools: true,
            ...
            emit("\n[FEATURES_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
          }
          flushBuffer();

          const cleanFeaturesOutput = extractScripts(featuresOutput);
          if (cleanFeaturesOutput) {
            mergeGeneratedFiles([{ path: "app/scripts-features.js", content: cleanFeaturesOutput }]);
            emit(`\n[FEATURES:READY] ✅ Scripts features générés (${cleanFeaturesOutput.length} chars)\n`);
          }

          await sleep(2000);

          // ─────────────────────────────────────────────────────────────
          // PHASE 4 — NEXTJS AGENT : conversion Next.js TypeScript
          // Même logique que callGeminiNextJS() dans page18
          // Output : JSON { "files": [...] } → create_file blocks
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:4/INTEGRATOR]\n");

          {
            // Extract raw JS bodies from <script> tags (non-CDN)
            const extractJsBodies = (src: string): string =>
              [...src.matchAll(/<script(?!\s+src)[^>]*>([\s\S]*?)<\/script>/gi)]
                .map(m => m[1].trim()).filter(Boolean).join("\n\n");

            // Extract CDN script URLs from <script src="...">
            const extractCdnUrls = (src: string): string[] =>
              [...src.matchAll(/<script[^>]+src=["']([^"']+)["'][^>]*>/gi)].map(m => m[1]);

            const js1Body = extractJsBodies(cleanJsOutput);
            const js2Body = extractJsBodies(cleanFeaturesOutput);
            const allCdnUrls = [
              ...extractCdnUrls(cleanHtmlOutput),
              ...extractCdnUrls(cleanJsOutput),
              ...extractCdnUrls(cleanFeaturesOutput),
            ].filter((u, i, a) => u && a.indexOf(u) === i);

            const nextjsAgentInput = `TASK: Convert this page to a Next.js 15 TypeScript project.
...
- Output raw JSON only`;
            ...
            let nextjsRaw = "";
            try {
              nextjsRaw = await runAgent(NEXTJS_AGENT_PROMPT, nextjsAgentInput, {
                temperature: 1.0,
                maxTokens: 65536,
                agentName: "NEXTJS",
                noTools: true,
              ...
              emit("\n[NEXTJS_AGENT] Erreur: " + (e?.message ?? String(e)) + "\n");
            }
            flushBuffer();

            // Parse JSON output and merge as create_file blocks
            const nextjsFiles = parseNextjsAgentFiles(nextjsRaw);
            if (nextjsFiles.length > 0) {
              emit(`\n[NEXTJS:READY] ✅ ${nextjsFiles.length} fichiers Next.js générés\n`);
              nextjsFiles.forEach(f => mergeGeneratedFiles([{ path: f.path, content: f.content }]));
            } else {
              emit(`\n[NEXTJS:WARN] ⚠️ JSON non parseable — vérifiez la sortie brute de l'agent\n`);
            }
          }
