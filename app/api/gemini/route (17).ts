    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ─────────────────────────────────────────────────────────────────
          // Étape 0a — heuristique rapide (µs, zéro appel IA)
          // Si le message est clairement une question/discussion → CHAT_ONLY
          // direct, sans consommer de tokens Presenter.
          // ─────────────────────────────────────────────────────────────────
          const fastChatOnly = (() => {
            const m = lastUserText.trim();
            if (!m) return false;
            if (currentProjectFiles.length === 0) return false; // premier message → probablement CODE
            const isErr = /ReferenceError|TypeError|SyntaxError|Cannot find|is not defined|Module not found|build fail|failed to compile/i.test(m);
            if (isErr) return false;
            const isFix = /\b(corrige|fixe|répare|debug|erreur|bug|crash)\b/i.test(m.toLowerCase());
            if (isFix) return false;
            // Phrases interrogatives courtes sans intention de construction
            return /^(qu[e']|est-ce que|comment|pourquoi|quand|quel|qu'est|explique|c'est quoi|dis-moi|c'est quoi|keskon|keskel|kessé|t'as|t as|tu as|qu'as|tu peux expliquer|peux-tu expliquer)/i.test(m);
          })();

          let rawPresenterOutput = "";

          if (fastChatOnly) {
            rawPresenterOutput = "CHAT_ONLY";
          } else {
          // Étape 0b — Presenter (classifieur IA, silent)
          try {
            if (isAnthropic) {
          ...
          const isDesignRef = rawPresenterOutput.includes("[IMAGE_IS_DESIGN_REF]") && uploadedImages && uploadedImages.length > 0;
          ...
          if (isDesignRef) {
            ...
            const designImages = [...(uploadedImages || []), ...(allReferenceImages || [])].slice(0, 3);
