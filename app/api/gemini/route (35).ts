        ...
        if (parts && parts.length > 0) {
          for (const part of parts) {
            ...
            if ((part as any).functionCall) {
              collectedFunctionCalls.push((part as any).functionCall);
              continue;
            }
        ...
        const runAgent = async (
          ...
          try {
            while (toolRound < MAX_TOOL_ROUNDS) {
              ...
              // Ajoute la réponse du modèle (avec les functionCalls) puis les résultats outils
              contents.push({
                role: "model" as const,
                parts: pendingFunctionCalls.map((fc: any) => ({ functionCall: fc })),
              });
              contents.push({
                role: "user" as const,
                parts: toolResults,
