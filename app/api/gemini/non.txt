...
// écriture des fichiers → npm install types → tsc --noEmit → parse des erreurs
// =============================================================================

const TSC_DECLARATIONS = `
// Stubs pour les packages npm externes : on veut vérifier nos types internes
// sans avoir besoin d'un npm install complet (trop lent).
// Avec ces déclarations, tsc check les types internes et ignore les modules externes.
declare module "zustand" {
  export function create<T>(): (fn: (set: any, get: any) => T) => (() => T) & { getState: () => T; setState: (s: Partial<T>) => void };
  export function create<T>(fn: (set: any, get: any) => T): (() => T) & { getState: () => T };
}
declare module "zustand/middleware" { export function persist(fn: any, opts: any): any; export function devtools(fn: any, opts?: any): any; }
declare module "clsx" { export function clsx(...args: any[]): string; export default clsx; }
...
declare module "recharts" { export const [key: string]: any; }
declare module "axios" { const axios: any; export default axios; export const { get, post, put, delete: del, patch }: any; }
declare module "date-fns" { export const [key: string]: any; }
declare module "react-hook-form" { export function useForm(opts?: any): any; export const Controller: any; }
declare module "zod" { export const z: any; }
declare module "@hookform/resolvers/zod" { export function zodResolver(s: any): any; }
declare module "lightweight-charts" { export const [key: string]: any; }
declare module "react-hot-toast" { const toast: any; export default toast; export const Toaster: any; }
declare module "sonner" { export const Toaster: any; const toast: any; export default toast; }
...
declare module "next/navigation" { export function useRouter(): any; export function usePathname(): string; export function useSearchParams(): any; export function useParams(): any; }
declare module "next/server" { export class NextResponse { static json(data: any, init?: any): any; static redirect(url: string): any; } export type NextRequest = Request; }
declare module "*.css" {}
`;
...
async function runTscCheck(
  ...
  try {
    onProgress("\n[TSC:START] Initialisation du sandbox TypeScript...\n");
    sbx = await Sandbox.create({ apiKey: e2bApiKey, timeoutMs: 120_000 });

    // Écrit le tsconfig et les stubs de déclarations
    await sbx.files.write("tsconfig.json", TSC_CONFIG);
    await sbx.files.write("declarations.d.ts", TSC_DECLARATIONS);

    // Installe uniquement TypeScript (rapide ~5s, pas besoin des vraies deps)
    onProgress("[TSC:INSTALL] Installation de TypeScript...\n");
    const installResult = await sbx.commands.run(
      "npm install --save-dev typescript --no-package-lock --prefer-offline 2>&1 | tail -3",
      { timeoutMs: 45_000 }
    );
    if (installResult.exitCode !== 0 && !installResult.stdout.includes("added")) {
      onProgress("[TSC:WARN] npm install a eu un souci, tentative directe...\n");
    }

    // Écrit tous les fichiers .ts et .tsx générés
    const tsFiles = files.filter(f => f.path.endsWith(".ts") || f.path.endsWith(".tsx"));
    onProgress(`[TSC:FILES] Écriture de ${tsFiles.length} fichiers TypeScript...\n`);

    await Promise.all(tsFiles.map(async (f) => {
      // files.write() crée automatiquement les dossiers parents (doc officielle E2B)
      await sbx!.files.write(f.path, f.content);
    }));

    // Lance tsc --noEmit
    onProgress("[TSC:RUN] Vérification TypeScript en cours...\n");
    const tscResult = await sbx.commands.run(
      "npx tsc --noEmit 2>&1 || true",
      { timeoutMs: 60_000 }
    );

    const rawOutput = tscResult.stdout ?? "";
    const hasErrors = rawOutput.includes("error TS") || rawOutput.includes(": error");

    if (!hasErrors) {
      onProgress("[TSC:OK] ✅ Aucune erreur TypeScript détectée !\n");
      return { errors: "", hasErrors: false };
    }

    // Parse et nettoie les erreurs
    const errorLines = rawOutput
      .split("\n")
      .filter(l => l.includes("error TS") || l.includes(": error"))
      .slice(0, 40) // max 40 erreurs
      .join("\n");

    const errorCount = (rawOutput.match(/error TS/g) ?? []).length;
    onProgress(`[TSC:ERRORS] ⚠️ ${errorCount} erreur(s) TypeScript détectée(s). Correction automatique...\n`);

    return { errors: rawOutput, hasErrors: true };
  } catch (err: any) {
    onProgress(`[TSC:SKIP] Sandbox indisponible (${err.message?.slice(0, 60) ?? "?"}), on continue sans vérification.\n`);
    return { errors: "", hasErrors: false };
  } finally {
    if (sbx) {
...
`;
          ...
          if (e2bApiKey) {
            ...
            for (let round = 0; round < MAX_TSC_FIX_ROUNDS; round++) {
              ...
              // Passe les erreurs au FIXER agent
              emit(`\n[TSC:FIXER] Round ${round + 1}/${MAX_TSC_FIX_ROUNDS} — Correction des erreurs TypeScript...\n`);

              // Identifie les fichiers cassés à partir des messages d'erreur
              const brokenPaths = new Set<string>();
              for (const m of tscResult.errors.matchAll(/([^\s(]+\.tsx?)\(?\d+/g)) {
                const p = m[1].replace(/^\.\//, "");
                if (allGeneratedFiles.some(f => f.path === p)) brokenPaths.add(p);
              }
              ...
              // Fichiers supplémentaires dont les types sont référencés
              const typesFile = allGeneratedFiles.find(f => f.path === "types/index.ts");
              const typesContext = typesFile ? `\n=== types/index.ts ===\n${typesFile.content}` : "";

              const tscFixInput = `
ERREURS TYPESCRIPT RÉELLES (sortie de tsc --noEmit) :
\`\`\`
${tscResult.errors.slice(0, 8000)}
\`\`\`

FICHIERS CASSÉS :
${brokenFilesContext || "(tous les fichiers générés — cherche l'erreur dans les imports/types)"}
${typesContext}

INSTRUCTIONS :
- Corrige EXACTEMENT les erreurs signalées par tsc, rien de plus.
- Si l'erreur est "Property X does not exist on type Y" → vérifie l'interface dans types/index.ts.
- Si l'erreur est "Module ... has no exported member X" → corrige l'export (named vs default).
- Si "use client must be the first expression" → déplace-le en ligne 1.
- Si "() => void" dans un objet Zustand → remplace par l'implémentation réelle.
- Émets le fichier COMPLET corrigé pour chaque fichier touché.
`;

              const tscFixOutput = await runAgent(FIXER_PROMPT, tscFixInput, {
                temperature: 0.2,
                maxTokens: 32768,
                emitOutput: false, // silencieux
              });

              // Merge les corrections
