...
// Fluid Compute permet jusqu'à 300s sur Hobby (800s sur Pro) — l'attente réseau
// (poll vers Upstash) ne compte presque pas dans le CPU facturé. On l'utilise
// pour faire du vrai long-polling au lieu d'un faux "blocking" de 25s.
// ⚠️ Vérifie que "Fluid Compute" est activé sur ton projet Vercel (Settings → Functions).
...
async function upstash(commands: unknown[][]): Promise<unknown[]> {
  // Garde-fou : un seul appel réseau ne doit JAMAIS pouvoir bloquer la boucle
  // de listen_workspace indéfiniment. Si Upstash ne répond pas en 8s, on
  // abandonne cet appel et la boucle retente au tick suivant.
...
const K = {
  ...
  cursor:    (ws: string, ag: string)  => `ws:${ws}:cursor:${ag}`,
...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "Blocks up to ~270 seconds (default 20s — safe with zero configuration on any MCP " +
      "client, since the server can never know a client's own cutoff in advance). " +
      "If you know your specific client tolerates longer waits, you may pass a higher timeout. " +
      "Returns the INSTANT another agent does something — no event is ever missed, " +
      "even across multiple calls, because your position in the event stream is saved server-side. " +
      "Even on timeout (no new event), it returns a full status snapshot: tasks currently claimed " +
      "and for how long, which agents are present, and the last known project context — so you can " +
      "tell 'still working, normal for complex code' apart from 'actually stuck'. " +
      "CRITICAL BEHAVIOR: as soon as this tool returns (event or timeout), call it again immediately, " +
      "in a loop, as long as you have nothing else to actively do. " +
      "Only stop looping if the user gives you an explicit new instruction.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        timeout:      { type: "number", description: "Seconds to wait. Max 270. Default 20 (safe everywhere with no setup)." },
      },
      required: ["workspace_id"],
    },
  },
  ...
  switch (name) {
    ...
    case "register_agent": {
      ...
      const briefing = [
        ...
        ``,
        `🔴 LIVE MODE: call listen_workspace now, then immediately call it again every time it returns ` +
        `(event found or timeout) — keep looping whenever you're idle. This is the only way other agents' ` +
        `actions reach you in real time. Stop only when the user gives you something else to actively do.`,
      ].join("\n");
    ...
    // ── listen_workspace ──────────────────────────────────────────────────
    // Long-polling avec curseur persistant : aucun event n'est jamais perdu,
    // même si l'agent rappelle l'outil avec du délai entre deux appels.
    // Tient jusqu'à ~maxDuration (280s) grâce à Fluid Compute.
    case "listen_workspace": {
      // Défaut sûr à 20s, SANS configuration nécessaire chez l'utilisateur.
      // Le serveur ne peut jamais connaître la limite du client (Cursor, Claude
      // Code, Codex, MCP Jam...) — invisible et différente partout. 20s passe
      // sous la limite la plus basse connue (MCP Jam = 30s par défaut), donc ça
      // marche pour tout le monde sans rien régler. Un utilisateur avancé qui
      // sait que son client tolère plus peut forcer un timeout plus long lui-même.
      const timeout    = Math.min(Number(args.timeout ?? 20), 270);
      const deadline   = Date.now() + timeout * 1000;
      const streamKey  = `stream:${ws}`;
      const cursorKey  = K.cursor(ws, ag);

      // Reprend exactement où cet agent s'est arrêté. Premier appel jamais
      // fait → on saute au bout du stream (on ignore l'historique passé,
      // get_history/get_context servent à ça).
      let lastId = await rGet(cursorKey) as string | null;
      if (!lastId) {
        const [tail] = await upstash([["XREVRANGE", streamKey, "+", "-", "COUNT", "1"]]);
        lastId = (Array.isArray(tail) && Array.isArray(tail[0])) ? (tail[0] as [string, string[]])[0] : "0";
        await rSet(cursorKey, lastId, 86400 * 7);
      }

      while (Date.now() < deadline) {
        try {
          const [entries] = await upstash([["XREAD", "COUNT", "20", "STREAMS", streamKey, lastId]]);

          if (entries && Array.isArray(entries) && entries.length > 0) {
            const records = (entries[0] as [string, [string, string[]][]])[1];
            const events: { agent: string; action: string; detail: string; id: string }[] = [];

            for (const [id, fields] of records) {
              lastId = id; // avance le curseur même pour ses propres events
              const obj: Record<string, string> = {};
              for (let i = 0; i < fields.length; i += 2) obj[fields[i]] = fields[i + 1];
              if (obj.agent && obj.agent !== ag) {
                events.push({ agent: obj.agent, action: obj.action ?? "", detail: obj.detail ?? "", id });
              }
            }

            // Persiste le curseur AVANT de retourner — même si le process est
            // tué juste après, rien ne sera re-livré ni perdu au prochain appel.
            await rSet(cursorKey, lastId, 86400 * 7);

            if (events.length > 0) {
              const lines = events.map(e => `🔔 **${e.agent}** → ${e.action}: ${e.detail}`).join("\n");
              return { content: [{ type: "text", text:
                `Updates reçus en live sur "${ws}" :\n\n${lines}\n\n` +
                `Tu es à jour à la seconde. Rappelle listen_workspace MAINTENANT pour rester branché — ` +
                `ne t'arrête de boucler que si l'utilisateur te demande explicitement autre chose.`
              }]};
            }
          }
      ...
      // Timeout atteint sans nouvel event — mais au lieu de dire juste "rien",
      // on donne un vrai état des lieux : tâches en cours (et depuis quand),
      // contexte actuel, agents actifs. Ça permet de distinguer "calme parce
      // que tout est fini" de "calme parce qu'un agent travaille encore".
      const [taskIds, agentIds, ctx] = await Promise.all([
        rGet(K.tasks(ws)),
        rGet(K.agents(ws)),
        rGet(K.context(ws)),
      ]) as [string[] | null, string[] | null, WorkspaceContext | null];
      ...
      const liveTasks = taskRecords.filter(Boolean) as (TaskRecord & { id: string })[];

      const taskLines = liveTasks.length > 0
        ? liveTasks.map(t => {
            const age = Math.round((Date.now() - t.claimed_at) / 60000);
            const flag = age >= 5 ? " ⚠️ ça commence à être long, peut-être bloqué" : " — probablement toujours en cours";
            return `• "${t.id}" tenue par **${t.claimed_by}** depuis ${age}min${flag}`;
          }).join("\n")
        : "Aucune tâche en cours — rien n'est revendiqué en ce moment.";

      const agentLines = (agentIds ?? []).filter(id => id !== ag);

      return { content: [{ type: "text", text:
        `⏱️ Pas de nouvel événement pendant ces ${timeout}s, mais voici l'état actuel de "${ws}" (rien n'est perdu, curseur sauvegardé) :\n\n` +
        `🔒 Tâches en cours :\n${taskLines}\n\n` +
        `🤖 Autres agents présents : ${agentLines.length > 0 ? agentLines.join(", ") : "aucun"}\n\n` +
        (ctx ? `📋 Dernier contexte connu : ${ctx.summary} (prochaine étape : ${ctx.progress})\n\n` : "") +
        `→ Si une tâche est marquée ⚠️, l'autre agent met du temps — c'est normal pour du code complexe, ` +
        `pas forcément bloqué. Rappelle listen_workspace pour continuer à surveiller.`
      }]};
    }
