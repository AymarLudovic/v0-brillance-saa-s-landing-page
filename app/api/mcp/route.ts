...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "Automatically chains up to 4 rounds (~90s total). " +
      "IMPORTANT: if the response ends with 'Calling listen_workspace again (round N/4)...', " +
      "you MUST immediately call listen_workspace again with round=N to keep watching.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        round:        { type: "number", description: "Current round number (1-4). Default 1. Pass the next round number when re-calling." },
      },
      required: ["workspace_id"],
    },
  },
