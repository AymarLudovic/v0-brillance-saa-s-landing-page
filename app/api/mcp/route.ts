...
// ── Redis key helpers ─────────────────────────────────────────────────────────
// ws inclut déjà le namespace utilisateur quand fourni
// Ex: "abc123:mon-projet" → toutes les clés préfixées par cet identifiant
const K = {
  ...
  global:    (uid: string)             => uid ? `u:${uid}:workspaces` : "global:workspaces",
  vision:    (ws: string)              => `ws:${ws}:vision`,
};
...
async function trackWorkspace(ws: string, uid = "") {
  const key  = K.global(uid);
  const list = (await rGet(key) as string[] | null) ?? [];
  // Stocke le ws sans le préfixe uid pour l'affichage
  const displayWs = uid ? ws.replace(`${uid}:`, "") : ws;
  if (!list.includes(displayWs)) { list.push(displayWs); await rSet(key, list, 86400 * 90); }
}
...
const TOOLS = [
  {
    name: "save_context",
    description:
      "Saves the current conversation context to a shared workspace so any AI model or agent " +
      "can instantly understand what happened without re-explanation. " +
      "CRITICAL FORMAT RULES for the summary field — write in plain language like this: " +
      "'The user asked me to [exact request]. I [what you did step by step]. " +
      "I created/generated [what exactly] which [what it does] at [file path or location]. " +
      "The result is [concrete outcome].' " +
      "NEVER put raw code in summary. NEVER be vague. Be specific enough that a different AI model " +
      "reading this can immediately continue the work without asking questions. " +
      "Call this after every significant action, not just at the end.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string", description: "Project identifier. Ex: my-app-v2" },
        user_id:      { type: "string", description: "Optional. Firebase UID from your dashboard connection instructions. Isolates your data from other users." },
        agent_id:     { type: "string", description: "Your unique instance ID. If you were not given one, use your platform name + a number. Ex: claude-01, cursor-01, gpt-01" },
        summary:      { type: "string", description: "Plain language explanation of what the user asked and what you did. No code. Other AI models must understand this." },
        progress:     { type: "string", description: "Exactly what is done and what the next concrete step is. Ex: Homepage HTML done. Next: add CSS animations." },
        decisions:    { type: "array",  items: { type: "string" }, description: "Key decisions made and why. Ex: Chose Tailwind over CSS modules because user prefers utility classes." },
        files:        { type: "array",  items: { type: "string" }, description: "Files created or modified with their purpose. Ex: src/components/Hero.tsx — main hero section" },
      },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        user_id:      { type: "string", description: "Optional. Firebase UID from your dashboard." },
      },
  ...
  },
  {
    ...
    description:
      ...
      "Use a clear human-readable task_id like 'build-login-page' or 'write-api-docs'. " +
      "The description must explain in plain English what you will do and the expected result.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        user_id:      { type: "string", description: "Optional. Firebase UID from your dashboard." },
        agent_id:     { type: "string", description: "Your agent ID. If not given one, use platform+number. Ex: claude-01, cursor-01" },
        task_id:      { type: "string", description: "Clear readable name. Ex: build-homepage, write-api-docs, fix-login-bug. No technical strings." },
        description:  { type: "string", description: "Plain English: what you will do and expected result. Ex: Build the hero section with title and CTA button using Tailwind." },
      },
  ...
  {
    name: "set_workspace_vision",
    description:
      "Sets or updates the global vision/objective of the workspace. " +
      "All agents will see this vision in every get_context call. " +
      "Use this to define what the project is trying to achieve at a high level. " +
      "When updated, all listening agents are notified immediately.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        user_id:      { type: "string", description: "Optional. Firebase UID from your dashboard." },
        agent_id:     { type: "string" },
        vision:       { type: "string", description: "The global goal of this workspace. Ex: Build a weather app for mobile users in West Africa. The app must be fast, offline-capable, and use French." },
      },
      required: ["workspace_id", "agent_id", "vision"],
    },
  },
  {
    name: "wait_for_event",
  ...
  },
  {
    ...
    description:
      "Watches the workspace for real-time changes from other agents — task claims, releases, " +
      "context updates, broadcasts, reviews. Returns instantly when something happens, " +
      "or gives a full workspace snapshot after 23s. " +
      "Automatically chains up to 10 rounds (~230s total). " +
      "IMPORTANT: if the response ends with 'Calling listen_workspace again (round N/10)...', " +
      "you MUST immediately call listen_workspace again with round=N to keep watching.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        user_id:      { type: "string", description: "Optional. Firebase UID from your dashboard." },
        round:        { type: "number", description: "Current round number (1-10). Default 1. Pass the next round number when re-calling." },
      },
      required: ["workspace_id"],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>) {
  const uid   = String(args.user_id      ?? ""); // Firebase UID optionnel
  const rawWs = String(args.workspace_id ?? "");
  // Namespace: si user_id fourni, préfixe le ws pour isoler les données
  const ws  = uid && rawWs ? `${uid}:${rawWs}` : rawWs;
  const ag  = String(args.agent_id     ?? "auto");

  // ── Auto-registration ─────────────────────────────────────────────────────
  // Si l'agent appelle un tool sans s'être enregistré, on le fait automatiquement.
  // Ça permet à un modèle de juste "lire le contexte et continuer" sans formalités.
  if (ws && ag && name !== "register_agent") {
    const existing = await rGet(K.agent(ws, ag));
    if (!existing) {
      const sessionClient = await rGet("session:last_client") as { name: string } | null;
      const autoPlatform  = sessionClient?.name ?? "";
      const autoRecord = {
        agent_id:     ag,
      ...
      await rSet(K.agent(ws, ag), autoRecord, 86400 * 7);
      const agents = (await rGet(K.agents(ws)) as string[] | null) ?? [];
      if (!agents.includes(ag)) { agents.push(ag); await rSet(K.agents(ws), agents); }
      await trackWorkspace(ws);
      await logAction(ws, "auto_register", ag, `Auto-registered via ${autoPlatform || "unknown platform"}`);
    } else {
      // Met à jour last_seen à chaque appel
      const rec = existing as Record<string, unknown>;
      rec.last_seen = Date.now();
      await rSet(K.agent(ws, ag), rec, 86400 * 7);
    }
  }
  ...
  switch (name) {
    ...
    case "get_context": {
      ...
      const vision = await rGet(K.vision(ws)) as { vision: string; set_by: string } | null;

      if (!ctx && !vision) return { content: [{ type: "text", text:
        `⚠️ No context found for workspace "${ws}". Start a new project or check the workspace ID.`
      }]};
      const age = ctx ? Math.round((Date.now() - ctx.updated_at) / 60000) : null;
      return { content: [{ type: "text", text:
        ...
        (ctx ? (
          ...
          `**Key decisions:**\n${ctx.decisions.map((d: string) => `• ${d}`).join("\n") || "None"}\n\n` +
          `**Files touched:**\n${ctx.files.map((f: string) => `• ${f}`).join("\n") || "None"}`
        ) : "No detailed context saved yet — you can start working and call save_context.")
      }]};
    }
    ...
    // ── set_workspace_vision ──────────────────────────────────────────────
    case "set_workspace_vision": {
      const vision = String(args.vision ?? "");
      if (!vision) return { content: [{ type: "text", text: "⚠️ Vision cannot be empty." }]};

      const existing = await rGet(K.vision(ws)) as { vision: string } | null;
      const isUpdate = !!existing;

      await rSet(K.vision(ws), { vision, set_by: ag, updated_at: Date.now() }, 86400 * 90);
      await trackWorkspace(ws, uid);
      await logAction(ws, "set_vision", ag, vision.slice(0, 100));
      await publish(ws, { action: "vision_updated", agent: ag, vision: vision.slice(0, 200), is_update: isUpdate });

      // Notifie les agents qui écoutent
      if (isUpdate) {
        await rSet(`event:${ws}:vision_changed`, { by: ag, vision, previous: existing?.vision }, 3600);
      }

      return { content: [{ type: "text", text:
        `🎯 Workspace vision ${isUpdate ? "updated" : "set"} for "${ws}":\n\n` +
        `"${vision}"\n\n` +
        `All agents will see this vision in every get_context call.` +
        (isUpdate ? `\n\nAll listening agents have been notified of the update.` : "")
      }]};
    }
    ...
    case "listen_workspace": {
      // Rounds de 23s (limite Vercel). Détecte : events, tâches, contexte, historique.
      // Retourne immédiatement si changement détecté, sinon rapport complet au timeout.
      ...
      // Snapshot initial
      const [initCtx, initTasks, initHistory] = await Promise.all([
        rGet(K.context(ws)) as Promise<{updated_at:number;updated_by:string;summary:string;progress:string}|null>,
        rGet(K.tasks(ws))   as Promise<string[]|null>,
        rGet(K.history(ws)) as Promise<{action:string;agent:string;detail:string;timestamp:number}[]|null>,
      ]);
      const initCtxTs     = initCtx?.updated_at    ?? 0;
      const initTaskCount = (initTasks ?? []).length;
      const initHistCount = (initHistory ?? []).length;
      const initTaskSet   = new Set(initTasks ?? []);

      // Event keys à surveiller
      const eventKeys = [
        `event:${ws}:broadcast`,
        `event:${ws}:review:${ag}:pending`,
        `event:${ws}:review:${ag}:result`,
      ];
      const initEvents: Record<string, string> = {};
      for (const k of eventKeys) {
        const v = await rGet(k);
        initEvents[k] = v ? JSON.stringify(v) : "";
      }

      const changes: string[] = [];

      // Boucle polling 600ms
      while (Date.now() < deadline && changes.length === 0) {
        await new Promise(r => setTimeout(r, 600));

        // 1. Event keys
        for (const k of eventKeys) {
          const cur = await rGet(k);
          const curStr = cur ? JSON.stringify(cur) : "";
          if (curStr && curStr !== initEvents[k]) {
            const ev = cur as Record<string,unknown>;
            const type = k.includes("broadcast") ? "broadcast"
              : k.includes("pending") ? "review request" : "review response";
            changes.push(`📢 **${ev.from ?? ev.by ?? "Agent"}** sent a ${type}: "${String(ev.message ?? ev.task_id ?? "").slice(0,80)}"`);
          }
        }

        // 2. Tâches
        const curTasks = (await rGet(K.tasks(ws)) as string[]|null) ?? [];
        if (curTasks.length < initTaskCount) {
          changes.push(`✅ A task was just released — an agent finished their work`);
        } else {
          const newTasks = curTasks.filter(t => !initTaskSet.has(t));
          for (const t of newTasks) {
            const td = await rGet(K.task(ws, t)) as {claimed_by:string;description:string}|null;
            changes.push(`🔒 **${td?.claimed_by ?? "Agent"}** just claimed task: **${t}**${td?.description ? ` — ${td.description.slice(0,60)}` : ""}`);
          }
        }

        // 3. Contexte
        const curCtx = await rGet(K.context(ws)) as {updated_at:number;updated_by:string;summary:string;progress:string}|null;
        if (curCtx && curCtx.updated_at > initCtxTs) {
          changes.push(`📋 **${curCtx.updated_by}** updated the project context\nNew progress: ${curCtx.progress}`);
        }

        // 4. Historique
        const curHist = (await rGet(K.history(ws)) as {action:string;agent:string;detail:string}[]|null) ?? [];
        if (curHist.length > initHistCount) {
          const newEntries = curHist.slice(0, curHist.length - initHistCount);
          for (const e of newEntries.filter(e => e.agent !== ag).slice(0, 3)) {
            changes.push(`⚡ **${e.agent}** → ${e.action.replace(/_/g," ")}${e.detail ? `: ${e.detail.slice(0,70)}` : ""}`);
          }
        }
      }

      // Changements détectés → retour immédiat avec contexte frais
      if (changes.length > 0) {
        const [freshCtx, freshTasks] = await Promise.all([
          rGet(K.context(ws)) as Promise<{summary:string;progress:string}|null>,
          rGet(K.tasks(ws))   as Promise<string[]|null>,
        ]);
        return { content: [{ type: "text", text:
          `🔔 **New activity on workspace "${ws}"** (after ${ROUND * (round-1)}s)\n\n` +
          changes.join("\n") + "\n\n" +
          `**Current state:**\n` +
          (freshCtx ? `• Progress: ${freshCtx.progress}\n` : "") +
          `• Active tasks: ${(freshTasks ?? []).join(", ") || "none"}\n\n` +
          `React to what happened above, then call listen_workspace again if needed.`
        }]};
      }

      // Timeout — rapport complet
      const [tCtx, tTasks, tAgents, tHist] = await Promise.all([
        rGet(K.context(ws)) as Promise<{summary:string;progress:string;updated_by:string;updated_at:number}|null>,
        rGet(K.tasks(ws))   as Promise<string[]|null>,
        rGet(K.agents(ws))  as Promise<string[]|null>,
        rGet(K.history(ws)) as Promise<{action:string;agent:string}[]|null>,
      ]);

      const taskDetails = await Promise.all(
        (tTasks ?? []).map(async tid => {
          const t = await rGet(K.task(ws, tid)) as {claimed_by:string;description:string}|null;
          return t ? `  • **${tid}** → ${t.claimed_by}: ${t.description?.slice(0,60) ?? "in progress"}` : null;
        })
      );

      const nextRound    = round + 1;
      const totalElapsed = ROUND * round;
      const shouldContinue = nextRound <= maxRounds;
      const lastAction = (tHist ?? [])[0];

      return { content: [{ type: "text", text:
        `⏳ **Round ${round}/${maxRounds}** — nothing new in the last ${ROUND}s (${totalElapsed}s total)\n\n` +
        `**Workspace "${ws}" snapshot:**\n` +
        (tCtx
          ? `• Context: ${tCtx.summary?.slice(0,100)}\n• Progress: ${tCtx.progress}\n• Saved ${Math.round((Date.now()-tCtx.updated_at)/60000)}min ago by ${tCtx.updated_by}\n`
          : "• No context yet\n") +
        `• Active agents: ${(tAgents ?? []).join(", ") || "none"}\n` +
        ((tTasks ?? []).length > 0
          ? `• Tasks in progress:\n${taskDetails.filter(Boolean).join("\n")}\n`
          : "• No tasks claimed right now\n") +
        (lastAction ? `• Last action: **${lastAction.agent}** → ${lastAction.action.replace(/_/g," ")}\n` : "") +
        (shouldContinue
          ? `\nOther agents may still be working. Calling listen_workspace again (round ${nextRound}/${maxRounds})...`
          : `\n✅ Watched for ~90s total. Workspace seems idle now.`)
      }]};
    }
