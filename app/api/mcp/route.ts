...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      ...
      "Blocks up to ~270 seconds (default 25s, raise via timeout param only if your " +
      "MCP client's own request timeout allows it — e.g. MCP Jam defaults to 30s client-side) " +
      "and returns the INSTANT another agent does something " +
      "(save context, claim a task, broadcast, release, etc.) — no event is ever missed, " +
      "even across multiple calls, because your position in the event stream is saved server-side. " +
      "CRITICAL BEHAVIOR: as soon as this tool returns (whether it found an event or timed out), " +
      "call it again immediately, in a loop, as long as you have nothing else to actively do. " +
      "This is how you stay 'live' with other agents — there is no other mechanism. " +
      "Only stop looping if the user gives you an explicit new instruction.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: { type: "string" },
        timeout:      { type: "number", description: "Seconds to wait. Max 270. Default 270." },
      },
      required: ["workspace_id"],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>) {
  ...
  switch (name) {
    ...
    case "listen_workspace": {
      // Défaut prudent à 25s : la plupart des clients MCP (MCP Jam = 30s par
      // défaut, beaucoup d'autres aussi) coupent la requête côté client avant
      // la fin si on attend plus longtemps — ça afficherait une erreur de
      // timeout au lieu d'une réponse propre. Si ton client a un timeout plus
      // long (ex: Claude Code), passe explicitement timeout=270 pour en profiter.
      const timeout    = Math.min(Number(args.timeout ?? 25), 270);
      const deadline   = Date.now() + timeout * 1000;
      ...
      return { content: [{ type: "text", text:
        `Aucune activité sur "${ws}" depuis ${timeout}s — workspace calme, pas d'event perdu (curseur sauvegardé).\n` +
        `Rappelle listen_workspace immédiatement pour continuer à écouter en live, sans trou.`
      }]};
    }
