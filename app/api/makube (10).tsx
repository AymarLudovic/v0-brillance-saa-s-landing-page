...
hooks/use*.ts — PATTERN OBLIGATOIRE pour toute donnée venant de Python
  ✅ Custom hooks simples : useState + useEffect + fetch
  ✅ PAS de Zustand pour les données serveur — trop d'erreurs de syntaxe
  ✅ Un hook par entité métier (useTracks, useProject, useClips, useOrders...)
  ✅ Chaque action dans le hook appelle /api/py/... et met à jour le state local après
  ...
  export function useEntityName() {
    ...
    const fetchItems = useCallback(async () => {
      setLoading(true);
      try {
        const r = await fetch("/api/py/entity");
    ...
    const createItem = async (data: Partial<EntityType>) => {
      const r = await fetch("/api/py/entity/create", {
        method: "POST", headers: {"Content-Type":"application/json"},
    ...
    const updateItem = async (id: string, data: Partial<EntityType>) => {
      const r = await fetch(\`/api/py/entity/\${id}\`, {
    ...
    const deleteItem = async (id: string) => {
      await fetch(\`/api/py/entity/\${id}\`, { method: "DELETE" });
...
stores/*.ts — Zustand UNIQUEMENT pour état UI global pur (jamais pour données serveur)
  \`\`\`ts
  // SEUL usage autorisé de Zustand :
  interface UIState {
    sidebarOpen: boolean; setSidebarOpen: (v: boolean) => void;
    activeModal: string | null; setActiveModal: (v: string | null) => void;
...
Pour les fichiers EXISTANTS : utilise edit_file (par numéros de ligne), jamais create_file complet.

À la fin :
DEPENDENCIES: ["package1", "package2"]  ← npm packages nécessaires
DEVDEPENDENCIES: ["package-dev"]         ← npm dev packages
PYPACKAGES: ["pydub", "librosa"]         ← libs Python SPÉCIFIQUES (pas fastapi/uvicorn/python-dotenv)
...
══════════════════════════════════════════════════════════════════════
ÉTAPE 4 — GÉNÈRE LES HOOKS POUR CHAQUE ENTITÉ PYTHON
══════════════════════════════════════════════════════════════════════

⚡ RÈGLE : Un hook par groupe d'endpoints Python. PAS de Zustand store pour les données serveur.
Zustand = uniquement useUIStore (sidebarOpen, theme, activeModal). Rien d'autre.

Pour chaque entité métier dans backend/main.py → génère un hook complet :

hooks/use*.ts
  ✅ "use client"; LIGNE 1 ABSOLUMENT
  ✅ useState + useEffect + useCallback — pattern simple, zéro Zustand
  ✅ Toutes les actions CRUD : create, read, update, delete
  ✅ Appels vers /api/py/... (JAMAIS localhost:8000)
  ✅ loading + error gérés

  Pattern OBLIGATOIRE — hook complet pour une entité :
  \`\`\`ts
  "use client";
  import { useState, useEffect, useCallback } from "react";
  ...
  export function useTracks() {
    ...
    const refresh = useCallback(async () => {
      setLoading(true);
      try { setTracks(await fetch("/api/py/tracks").then(r => r.json())); }
      catch(e) { setError(String(e)); }
      finally { setLoading(false); }
    }, []);

    const createTrack = async (data: Partial<Track>) => {
      const r = await fetch("/api/py/tracks/create", {
        method: "POST", headers: {"Content-Type":"application/json"},
        body: JSON.stringify(data)
      });
    ...
    const updateTrack = async (id: string, data: Partial<Track>) => {
      const r = await fetch(\`/api/py/tracks/\${id}\`, {
        method: "PUT", headers: {"Content-Type":"application/json"},
        body: JSON.stringify(data)
      });
      const updated = await r.json();
    ...
    const deleteTrack = async (id: string) => {
      await fetch(\`/api/py/tracks/\${id}\`, { method: "DELETE" });
      setTracks(prev => prev.filter(t => t.id !== id));
    };

    useEffect(() => { refresh(); }, [refresh]);
    return { tracks, loading, error, refresh, createTrack, updateTrack, deleteTrack };
  }
  \`\`\`
...
À la fin :
DEPENDENCIES: ["package1", "package2"]  ← npm packages
DEVDEPENDENCIES: ["package-dev"]
PYPACKAGES: ["pydub", "librosa"]        ← libs Python spécifiques

RAPPEL FORMAT FICHIERS :
...
Pour les fichiers EXISTANTS (moins de 60% de changements) : edit_file uniquement.
Pour les nouveaux fichiers : "---" puis <create_file path="...">contenu</create_file>
...
async function runTscCheck(
  ...
  try {
    ...
    // ── ÉTAPE 1 : tsc --noEmit (erreurs TypeScript) ─────────────────────────
    ...
    const tscOutput = (tscRun.stdout ?? "") + (tscRun.stderr ?? "");

    // ── ÉTAPE 2 : next build (erreurs SWC/webpack = overlay rouge navigateur) ─
    // next build attrape les erreurs que tsc manque :
    // - "Unterminated string constant" (chaîne JS non fermée)
    // - "Expected ','" (virgule manquante dans tableau/objet)
    // - "Module parse failed" (erreur SWC/webpack)
    // - Toutes les erreurs qui apparaissent dans l'overlay rouge du navigateur
    onProgress("[NEXT_BUILD:RUN] next build en cours (attrape les erreurs SWC/overlay)...\n");
    // Installation minimale de next pour le build check
    const nextInstall = await sbx.commands.run(
      "npm install --save-dev next@15.1.0 react@19.0.0 react-dom@19.0.0 --no-package-lock --prefer-offline 2>&1 | tail -2",
      { timeoutMs: 60_000 }
    );
    if (nextInstall.exitCode !== 0) {
      onProgress("[NEXT_BUILD:WARN] Installation next partielle — build check dégradé\n");
    }
    const nextBuildRun = await sbx.commands.run(
      "npx next build --no-lint 2>&1 || true",
      { timeoutMs: 120_000 }
    );
    const nextBuildOutput = (nextBuildRun.stdout ?? "") + (nextBuildRun.stderr ?? "");

    // Extraire seulement les erreurs de next build (pas le bruit de compilation normale)
    const nextBuildErrors = nextBuildOutput
      .split("\n")
      .filter(l =>
        l.includes("Error:") || l.includes("error:") ||
        l.includes("Failed to compile") || l.includes("SyntaxError") ||
        l.includes("Module parse failed") || l.includes("Unterminated") ||
        l.includes("Expected ','") || l.includes("Expected '}'") ||
        l.includes("Unexpected token") || l.includes("Build failed")
      )
      .join("\n");

    if (nextBuildErrors) {
      onProgress("[NEXT_BUILD:ERRORS] Erreurs SWC/webpack détectées :\n" + nextBuildErrors + "\n");
    } else if (nextBuildRun.exitCode === 0) {
      onProgress("[NEXT_BUILD:OK] ✅ next build réussi — zéro erreur overlay navigateur\n");
    }

    // Fusionner les deux sorties pour le filtre d'erreurs
    const rawOutput = tscOutput + "\n" + nextBuildErrors;

    // Affiche le bilan des deux checks
    const rawLines = rawOutput.trim().split("\n").filter(Boolean);
    onProgress(`[CHECK:RAW] ${rawLines.length} ligne(s) d'erreurs combinées (TSC + next build).\n`);
...
`;
          ...
          // ─────────────────────────────────────────────────────────────
          emit("\n[PHASE:5/BUILD_CHECK]\n"); // TSC + next build — attrape erreurs TypeScript ET erreurs overlay navigateur
...
PATTERNS FRÉQUENTS — ERREURS TSC :
...
PATTERNS FRÉQUENTS — ERREURS RUNTIME (overlay rouge dans le navigateur) :
- "Cannot read properties of undefined (reading 'x')" → variable non initialisée avant .map()/.find()
  Fix : useState([]) avec valeur initiale tableau vide, ou optional chaining data?.items ?? []
- "TypeError: X is not a function" → import manquant ou mauvais export (default vs named)
  Fix : vérifie import { X } vs import X et que le fichier source exporte bien X
- "Objects are not valid as a React child" → tu rends un objet JS directement dans JSX
  Fix : accède à la propriété string/number : {item.name} pas {item}
- "Each child in a list should have a unique key" → .map() sans key={item.id}
  Fix : ajoute key={item.id ?? index} sur chaque élément du .map()
- "Hydration failed" → différence serveur/client au premier rendu
  Fix : entoure le rendu dynamique d'un useEffect ou ajoute suppressHydrationWarning
- "Unterminated string constant" → guillemet ouvert non fermé dans du JSX ou un tableau JS
  Fix : vérifie que chaque " ouvre et ferme sur la même ligne dans les tableaux JS
- "Expected ',' got 'X'" → ligne précédente dans un tableau JS n'a pas de virgule finale
  Fix : ajoute , à la fin de chaque élément du tableau
`;
