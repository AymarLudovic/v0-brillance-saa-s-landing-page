...
`;
          ...
          // ── VERIFY : feedback loop sur la sortie Foundation (Claude Code phase 3) ──
          {
            const _verify = verifyAgentOutput(allGeneratedFiles, "FOUNDATION");
            if (_verify.hasIssues) {
              emit(`\n[VERIFY:FOUNDATION] ${_verify.severity.toUpperCase()} — ${_verify.issues.length} problème(s) détecté(s):\n${_verify.issues.map(i => "  · " + i).join("\n")}\n`);
              // Injecter les issues dans le contexte du prochain agent (feedback loop)
              (globalThis as any)._foundationIssues = _verify.issues;
            } else {
              emit(`\n[VERIFY:FOUNDATION] ✅ Aucun problème critique détecté\n`);
...
`;
          ...
          // ── VERIFY : feedback loop après Checker ──────────────────────
          {
            const _v = verifyAgentOutput(allGeneratedFiles, "CHECKER");
            emit(`\n[VERIFY:CHECKER] ${_v.severity === "ok" ? "✅ OK" : _v.severity.toUpperCase() + " — " + _v.issues.length + " problème(s):\n" + _v.issues.map(i => "  · " + i).join("\n")}\n`);
...
`;
          ...
          // ── VERIFY : feedback loop après Views ─────────────────────────
          {
            const _vViews = verifyAgentOutput(allGeneratedFiles, "VIEWS");
            emit(`\n[VERIFY:VIEWS] ${_vViews.severity === "ok" ? "✅ OK" : _vViews.severity.toUpperCase() + " — " + _vViews.issues.length + " problème(s):\n" + _vViews.issues.map(i => "  · " + i).join("\n")}\n`);
          }
