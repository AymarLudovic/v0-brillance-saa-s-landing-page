...
await sandbox.files.write("/home/user/package.json", JSON.stringify(defaultPackageJson, null, 2))
        ...
        // next.config.ts — simple, sans proxy Python
        await sandbox.files.write(
...
export default nextConfig;`
        ...
        // page.tsx — placeholder minimal, sera remplacé par l'IA
        await sandbox.files.write(
  ...
  return (
    <main style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", fontFamily: "sans-serif" }}>
      <p style={{ color: "#888" }}>Ready. Describe what to build.</p>
    </main>
  );
}`
        )
...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', fontFamily:'sans-serif', color:'#666' }}>
      ...
      case "runCommand": {
        const { command } = body
        if (!command?.trim()) throw new Error("command manquant")

        const sandbox = await e2b.Sandbox.create({ apiKey, timeoutMs: 60000 })
        try {
          const result = await sandbox.commands.run(command.trim(), {
            cwd: "/home/user",
            timeoutMs: 30000,
          })
          await sandbox.kill()
          ...
          async start(controller) {
            ...
            try {
              ...
              const defaultPkg = {
                ...
                dependencies: { next: "15.1.9", react: "19.0.0", "react-dom": "19.0.0", "lucide-react": "0.561.0" },
