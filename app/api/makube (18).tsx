...
RÈGLE C — ORDRE DES ROUTES : statiques AVANT paramétrées
  FastAPI matche les routes dans l'ordre de déclaration.
  /expenses/stats déclaré APRÈS /expenses/{id} → FastAPI capture "stats" comme un ID.
  TOUJOURS : routes fixes en premier, routes avec {paramètre} en dernier.

  ❌ FAUX (bug silencieux) :       ✅ CORRECT :
  @app.get("/items/{id}")          @app.get("/items/stats")   ← fixe en premier
  @app.get("/items/stats")         @app.get("/items/export")  ← fixe en premier
                                   @app.get("/items/{id}")    ← paramètre en dernier
...
AUDIT BACKEND PYTHON (PRIORITÉ ABSOLUE) :
  ...
  □ Routes statiques déclarées AVANT les routes paramétrées ?
    Route statique = /items/stats, /items/export, /items/count
    Route paramétrée = /items/{id}, /items/{item_id}
    → Mauvais ordre : RÉORDONNE — FastAPI capture "/stats" comme un {id} si déclaré après
