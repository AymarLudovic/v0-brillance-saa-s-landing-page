...
# /health ne depend que de fastapi — repond toujours
@app.get("/health")
def health():
    return {"status": "ok"}
        ...
        try {
          ...
          try {
            ...
            if (pyCheck.stdout.trim() === "YES") {
              ...
              // Garantir packages installés avant de lancer uvicorn
              try {
                await sandbox.commands.run(
                ...
                await sandbox.commands.run(
                  "pip install fastapi uvicorn[standard] pydantic pillow numpy pydub --quiet",
                  { cwd: "/home/user", timeoutMs: 120000 }
                )
              } catch(e: any) { console.log("[v0] pre-install warning:", e.message) }
              // Kill existing uvicorn
              await sandbox.commands.run("pkill -f uvicorn || true", { timeoutMs: 5000 }).catch(() => {})
              await new Promise(r => setTimeout(r, 1000))
              // Start uvicorn sans --reload (evite redemarrages intempestifs)
              await sandbox.commands.run(
                "nohup python3 -m uvicorn backend.main:app --host 0.0.0.0 --port 8000 > /home/user/backend.log 2>&1 &",
