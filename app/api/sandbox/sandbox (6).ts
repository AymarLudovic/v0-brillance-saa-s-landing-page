...
async def audio_process(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            cmd = ["ffmpeg", "-y", "-i", in_path, "-af", af, "-q:a", "4", "-ac", "1", out_path]
...
async def video_process(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Limiter à 720p pour la rapidité
...
`
        ...
        try {
          ...
          // Installer ffmpeg système (requis par pydub pour audio/vidéo)
          try {
            await sandbox.commands.run(
              "apt-get install -y ffmpeg 2>/dev/null || true",
              { cwd: "/home/user", timeoutMs: 120000 }
            )
            console.log("[v0] ffmpeg installed")
          } catch {}
          ...
          if (action === "install") {
            ...
            try {
              ...
              if (reqCheck.stdout.trim() === "YES") {
                ...
                // ffmpeg système requis par pydub
                try {
                  await sandbox.commands.run("which ffmpeg || apt-get install -y ffmpeg 2>/dev/null || true", { timeoutMs: 120000 })
                } catch {}
