...
# /health ne depend d aucune lib externe — uvicorn demarre toujours
@app.get("/health")
def health():
    return {"status": "ok"}
...
def photo_process(req: PhotoReq):
    # Imports lazy : si Pillow/numpy manque, seulement cette route echoue
    try:
        import numpy as np
        from PIL import Image, ImageEnhance
    except ImportError as e:
        return {"success": False, "error": "Pillow/numpy non installe : " + str(e)}
    try:
        ...
        if name == "bw":
            ...
            img = Image.fromarray(np.stack([nr, ng, nb], 2).astype(np.uint8))
        elif name == "warm":
            ...
            img = Image.fromarray(np.clip(arr*0.68+55, 0, 255).astype(np.uint8))
        elif name == "cinema":
...
async def audio_analyze(file: UploadFile = File(...)):
    try:
        ...
        return {"success": False, "error": "pydub non installe : " + str(e)}
    try:
        ...
        for i in range(0, len(seg), chunk_ms):
            points.append(seg[i:i+chunk_ms].rms)
        if not points:
            points = [0]
...
async def audio_process(
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            filters = []
            if bass != 0:   filters.append(f"equalizer=f=80:t=h:width=200:g={bass:.1f}")
            if mid != 0:    filters.append(f"equalizer=f=1000:t=h:width=1000:g={mid:.1f}")
            if treble != 0: filters.append(f"equalizer=f=8000:t=h:width=4000:g={treble:.1f}")
            if speed != 1.0: filters.append(f"atempo={max(0.5, min(2.0, speed)):.2f}")
            af = ",".join(filters) if filters else "anull"
            subprocess.run(["ffmpeg","-y","-i",in_path,"-af",af,"-q:a","4","-ac","1",out_path],
                           capture_output=True, timeout=60)
            with open(out_path, "rb") as f:
                out_bytes = f.read()
        return {"success": True, "audio": b64_encode(out_bytes), "mime": "audio/mpeg"}
    except Exception as e:
        return {"success": False, "error": str(e)}

# ─── VIDEO ──────────────────────────────────────────────────────
VIDEO_FILTER_MAP = {
    "none": "null", "vivid": "eq=saturation=2:contrast=1.1",
    "bw": "hue=s=0",
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(in_path, "wb") as f:
                f.write(data)
            vf_parts = []
            base_f = VIDEO_FILTER_MAP.get(filter_name, "null")
            if base_f != "null": vf_parts.append(base_f)
            if brightness != 1.0 or contrast != 1.0:
                vf_parts.append(f"eq=brightness={brightness-1.0:.2f}:contrast={contrast:.2f}")
            if speed != 1.0: vf_parts.append(f"setpts={round(1.0/speed,4)}*PTS")
            ...
            with open(out_path, "rb") as f:
                out_bytes = f.read()
        return {"success": True, "video": b64_encode(out_bytes), "mime": "video/mp4", "size_kb": round(len(out_bytes)/1024,1)}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
export default config;`
        ...
        // page.tsx — Creator Studio : Python ready check + erreurs visibles
        await sandbox.files.write(
...
function usePython() {
  ...
  const check = useCallback(async () => {
    ...
    setError(null);
    try {
      const r = await fetch("/api/py/health", { signal: AbortSignal.timeout(4000) });
      if (r.ok) { setReady(true); setChecking(false); return; }
      throw new Error("HTTP " + r.status);
    } catch (e: any) {
      setReady(false);
      setError("Python pas encore prêt — " + (e.message || "réseau"));
      setChecking(false);
      ...
      <div className="text-center max-w-xs">
        {checking ? (
          <>
            ...
            <p className="text-xs text-gray-600 mt-1">Vérifie /api/py/health toutes les 3s</p>
          </>
        ) : (
          <>
...
`
        ...
        try {
          ...
          // ffmpeg requis par pydub (audio/video processing)
          await sandbox.commands.run(
            "which ffmpeg || (apt-get update -qq && apt-get install -y ffmpeg 2>/dev/null) || true",
