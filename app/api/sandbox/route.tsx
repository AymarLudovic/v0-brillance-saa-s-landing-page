  ...
  try {
    ...
    switch (action) {
      case "create": {
        ...
        // ─── next.config.ts ─────────────────────────────────────────────
        // IMPORTANT : le rewrite /api/py/* reste pour les vraies requêtes métier
        // (photo/process, audio/process, video/process).
        // Le health-check utilise désormais /api/health (route Next.js interne)
        // pour éviter le HTTP 500 quand uvicorn n'est pas encore démarré.
        await sandbox.files.write(
...
const nextConfig: NextConfig = {
  ...
  async rewrites() {
    return [
      {
        // Proxy métier : photo / audio / video uniquement
        source: "/api/py/:path*",
...
export default nextConfig;`,
        ...
        // FIX PRINCIPAL : cette route Next.js fait le health-check côté serveur.
        // Si uvicorn est down, elle retourne { ready: false } avec HTTP 200
        // (JAMAIS de 500). Le hook usePython() ne verra donc plus d'erreur 500.
        await sandbox.files.write(
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ ready: false, error: "Python HTTP " + res.status });
  } catch (e: any) {
    // uvicorn pas encore démarré — on répond proprement au lieu de laisser Next.js
    // retourner un HTTP 500 qui embrouille le hook côté client.
    return NextResponse.json({ ready: false, error: e.message ?? "unreachable" });
  }
}`,
        )
...
# /health ne dépend que de fastapi — répond toujours immédiatement
@app.get("/health")
def health():
    return {"status": "ok"}
...
def photo_process(req: PhotoReq):
    ...
    try:
        ...
        if name == "bw": img = img.convert("L").convert("RGB")
            ...
            arr[:,:,0]=np.clip(arr[:,:,0]*1.15,0,255); arr[:,:,2]=np.clip(arr[:,:,2]*.85,0,255)
            img = Image.fromarray(arr.astype(np.uint8))
        elif name == "cool":
            arr[:,:,0]=np.clip(arr[:,:,0]*.85,0,255); arr[:,:,2]=np.clip(arr[:,:,2]*1.2,0,255)
            img = Image.fromarray(arr.astype(np.uint8))
        elif name == "fade": img = Image.fromarray(np.clip(arr*.68+55,0,255).astype(np.uint8))
        elif name == "cinema":
            arr[:,:,0]=np.clip(arr[:,:,0]*.9,0,255); arr[:,:,1]=np.clip(arr[:,:,1]*.88,0,255); arr[:,:,2]=np.clip(arr[:,:,2]*1.1,0,255)
            img = Image.fromarray(arr.astype(np.uint8))
...
export const metadata: Metadata = {
  ...
  description: "Audio · Photo · Vidéo — Python FastAPI + Next.js",
};

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="fr">
      <body className="antialiased bg-gray-950 text-white m-0 p-0">{children}</body>
    </html>
  );
}`,
        )
...
export default config;`,
        ...
        // ─── app/page.tsx ───────────────────────────────────────────────
        // FIX : usePython() appelle maintenant /api/health (route Next.js interne)
        //       au lieu de /api/py/health (rewrite → uvicorn).
        //       Résultat : plus jamais de HTTP 500 pendant le démarrage.
        await sandbox.files.write(
...
// Appelle /api/health (route Next.js interne) au lieu de /api/py/health (rewrite).
// /api/health retourne TOUJOURS { ready: bool } en HTTP 200, même si uvicorn est
// down → plus de HTTP 500 qui faisait croire à une erreur serveur Next.js.
function usePython() {
  ...
  const check = useCallback(async () => {
    ...
    setError(null);
    try {
      // /api/health est une route Next.js — jamais de 500 même si Python est down
      const r = await fetch("/api/health", {
        signal: AbortSignal.timeout(4000),
        cache: "no-store",
      });
      const data: { ready: boolean; error?: string } = await r.json();
      if (data.ready) {
        setReady(true);
        setChecking(false);
        return;
      }
      throw new Error(data.error ?? "uvicorn pas encore prêt");
    } catch (e: any) {
      setReady(false);
      setError(e.message ?? "réseau");
      setChecking(false);
      // Retry toutes les 3s jusqu'à ce que Python soit prêt
      ...
      <div className="text-center max-w-xs">
        {checking ? (
          <>
            ...
            <p className="text-xs text-gray-600 mt-1">Vérifie /api/health toutes les 3s</p>
          </>
        ) : (
          <>
            <div className="text-4xl mb-3">🐍</div>
            <p className="text-sm text-gray-400 font-medium mb-1">Python pas encore prêt</p>
            <p className="text-xs text-gray-600 mb-3 break-all">{error}</p>
...
`,
        ...
        // ─── Installation Python pendant le create ────────────────────────
        // On installe TOUT ici une seule fois (ffmpeg inclus).
        // Le case "start" n'a plus besoin de réinstaller quoi que ce soit.
        try {
          console.log("[v0] Setting up Python environment (one-time)...")
          // ffmpeg
          await sandbox.commands.run(
            "which ffmpeg || apt-get install -y ffmpeg -qq 2>/dev/null || true",
      ...
      case "build": {
        ...
        try {
          if (action === "install") {
            ...
            // pip install si requirements.txt présent (évite re-install si déjà fait dans create)
            try {
          ...
          commandResult = { stdout: e.stdout || "", stderr: e.stderr || e.message || "", exitCode: e.exitCode || 1 }
      ...
      case "start": {
        ...
        // ── 2. Démarrer uvicorn (PAS de pip/apt ici — déjà fait dans create) ──
        ...
        try {
          ...
          if (pyCheck.stdout.trim() === "YES") {
            ...
            // Tuer uvicorn existant
            await sandbox.commands.run("pkill -f uvicorn || true", { timeoutMs: 5000 }).catch(() => {})
            await new Promise((r) => setTimeout(r, 1000))
            // Lancer uvicorn — sans --reload pour éviter les redémarrages parasites
            await sandbox.commands.run(
            ...
            if (!pythonStarted) {
              const blogResult = await sandbox.commands
                .run("tail -20 /home/user/backend.log 2>/dev/null || echo 'no logs'", { timeoutMs: 3000 })
        ...
        // ── 4. Attendre que Next.js soit prêt ───────────────────────────
        ...
        let serverLogs = ""
        try {
          const logsResult = await sandbox.commands.run("tail -100 /home/user/server.log", { timeoutMs: 5000 })
