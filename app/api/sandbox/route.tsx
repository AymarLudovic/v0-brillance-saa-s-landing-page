...
await sandbox.files.write("/home/user/package.json", JSON.stringify(defaultPackageJson, null, 2))
        ...
        // next.config.ts — avec proxy vers FastAPI Python (port 8000)
        await sandbox.files.write(
...
export default nextConfig;`,
        ...
        // backend/ — FastAPI Python par défaut
        await sandbox.files.write(
  ...
  return (
    <div style={{ 
      ...
      case "build": {
        ...
        try {
          if (action === "install") {
            ...
            // 2. Python pip install si requirements.txt existe
            try {
              const pyCheck = await sandbox.commands.run(
                "test -f /home/user/backend/requirements.txt && echo EXISTS || echo NONE",
                { cwd: "/home/user", timeoutMs: 5000 }
              )
              if (pyCheck.stdout.trim() === "EXISTS") {
                ...
                console.log("[v0] pip install done:", pipResult.exitCode)
                // Merge stderr into commandResult for visibility
                if (pipResult.exitCode !== 0) {
                  commandResult = { ...commandResult, stderr: (commandResult.stderr || "") + "\n[Python] " + pipResult.stderr }
                }
              }
            } catch (pipErr: any) {
      ...
      case "start": {
        ...
        try {
          // Start FastAPI Python backend si disponible (port 8000)
          try {
            const pyCheck = await sandbox.commands.run(
              "test -f /home/user/backend/main.py && echo EXISTS || echo NONE",
              { cwd: "/home/user", timeoutMs: 5000 }
            )
            if (pyCheck.stdout.trim() === "EXISTS") {
              console.log("[v0] Starting Python FastAPI backend on port 8000...")
              await sandbox.commands.run(
                "pkill -f uvicorn || true && sleep 1 && nohup uvicorn backend.main:app --host 0.0.0.0 --port 8000 --reload > /home/user/backend.log 2>&1 &",
          ...
          const pythonUrl = `https://${sandbox.getHost(8000)}`
          return NextResponse.json({
            success: serverReady,
            action,
            url,
      ...
      case "stop": {
        ...
        try {
          await sandbox.commands.run("pkill -f 'next' || true && pkill -f 'uvicorn' || true", {
            cwd: "/home/user",
            timeoutMs: 10000,
          })
          await sandbox.commands.run("fuser -k 3000/tcp || true", {
