...
`
      ...
      case "writeFiles": {
        ...
        for (const f of requestFiles) {
          ...
          try {
            let writeContent = contentValue;
            if (filePathValue === "package.json" || filePathValue.endsWith("/package.json")) {
              try {
                const sanitized = writeContent.replace(/,(\s*[}\]])/g, "$1");
                writeContent = JSON.stringify(JSON.parse(sanitized), null, 2);
              } catch { /* keep as-is */ }
            }
            await sandbox.files.write(`/home/user/${filePathValue}`, writeContent)
      ...
      case "addFile": {
        ...
        // ── Sanitize package.json — strip trailing commas so JSON.parse never fails ──
        ...
        if (filePath === "package.json" || filePath.endsWith("/package.json")) {
          try {
            ...
            // If still invalid, keep original and let npm give a clear error
            finalContent = content;
          }
        }

        await sandbox.files.write(`/home/user/${filePath}`, finalContent)
      ...
      case "addFiles": {
        ...
        for (const f of requestFiles) {
          ...
          if (filePathValue === "package.json" || filePathValue.endsWith("/package.json")) {
            try {
              const sanitized = fileContent.replace(/,(\s*[}\]])/g, "$1");
              fileContent = JSON.stringify(JSON.parse(sanitized), null, 2);
            } catch { /* keep as-is */ }
          }
          await sandbox.files.write(`/home/user/${filePathValue}`, fileContent)
          console.log(`[v0] Fichier ${filePathValue} écrit dans le sandbox ${bodySandboxId}`)
        }
