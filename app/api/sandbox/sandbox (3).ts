...
function PhotoTab() {
  ...
  const loadFile = (f: File) => {
    ...
    // Redimensionner à max 1200px avant envoi — évite 15MB inutiles
    const img = new Image();
      ...
      <div className="w-64 border-r border-gray-800 p-4 overflow-y-auto shrink-0">
        ...
        {original && <>
          ...
          <button onClick={process} disabled={busy}
            ...
            {busy ? "⏳ Traitement (~2-4s)…" : "▶ Traiter via Python"}
          </button>
...
function AudioTab() {
  ...
  const loadFile = async (f: File) => {
    if (f.size > 8 * 1024 * 1024) {
      alert("Fichier trop lourd (max 8MB). Utilise un fichier plus court.");
      return;
    }
...
function VideoTab() {
  ...
  const loadFile = (f: File) => {
    if (f.size > 15 * 1024 * 1024) {
      alert("Vidéo trop lourde (max 15MB). Utilise un court extrait.");
      return;
    }
      ...
      <div className="w-64 border-r border-gray-800 p-4 overflow-y-auto shrink-0">
        ...
        {file && <>
          ...
          <button onClick={process} disabled={processing}
            className="w-full py-2.5 rounded-xl font-semibold text-sm"
            style={{background: processing ? "#1f2937" : "#4f46e5", color: processing ? "#4b5563" : "white"}}>
            {processing ? "⏳ Encodage en cours… (~10-30s)" : "▶ Encoder via Python"}
          </button>
