  ...
  return (
    <div className="min-h-screen bg-black text-white p-6 font-mono">
          ...
          <svg viewBox="0 0 300 60" className="w-full" preserveAspectRatio="none" style={{ height: 60 }}>
            ...
            {history.length > 1 && (
              <polygon
                ...
                points={[
                  ...history.map((v, i) => `${(i / 29) * 300},${60 - (v / 100) * 56}`),
                  `${((history.length - 1) / 29) * 300},60`,
                  "0,60"
                ].join(" ")}
              />
            )}
      ...
      <div className="mt-4 text-center text-xs text-gray-700">
        Données lues par Python psutil · streamées via WebSocket ws://localhost:8000/ws/monitor · affichées par React
      </div>
