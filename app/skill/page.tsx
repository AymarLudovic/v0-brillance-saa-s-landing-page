...
export default function SiteInspector() {
  ...
  // Design export modal (Figma / Framer)
  const [showDesignExportModal, setShowDesignExportModal] = useState(false)
  const [designExportTab, setDesignExportTab] = useState<"figma" | "framer">("figma")
  const [designExportStatus, setDesignExportStatus] = useState<string | null>(null)
  ...
  // ── Design export (Figma / Framer) ────────────────────────────────────────

  /** Method 1 — Figma Capture Mode (free, no API key)
   *  Injects Figma's own capture.js into the extracted HTML,
   *  opens it in a new tab. The floating toolbar lets the user
   *  click "Copy to clipboard", then paste (Ctrl+V) in Figma.
   */
  const exportToFigmaCaptureMode = async () => {
    setDesignExportStatus("Preparing…")
    try {
      ...
      if (!html) { setDesignExportStatus("❌ Could not extract HTML. Try clicking Download Prompt first."); return }
  ...
  s.onload = function() {
    setTimeout(function() {
      ...
      const injected = html.includes("</body>")
        ? html.replace("</body>", captureInject + "</body>")
        : html + captureInject

      const blob = new Blob([injected], { type: "text/html" })
      const blobUrl = URL.createObjectURL(blob)
      const win = window.open(blobUrl, "_blank")
      if (!win) { setDesignExportStatus("❌ Pop-up blocked. Please allow pop-ups for this site."); return }
      setTimeout(() => URL.revokeObjectURL(blobUrl), 15000)
      setDesignExportStatus("✅ Opened! Click the Figma toolbar → 'Copy to clipboard', then paste in Figma (Ctrl+V / ⌘V).")
    } catch (e) {
      setDesignExportStatus(`❌ Error: ${e instanceof Error ? e.message : String(e)}`)
    }
  }

  /** Method 2 — Copy HTML for html.to.design plugin
   *  Copies the extracted HTML so the user can paste it
   *  in the html.to.design plugin's "HTML" tab in Figma.
   */
  const copyHtmlForFigmaPlugin = async () => {
    setDesignExportStatus("Extracting…")
    try {
      let html = extractedHTML
      if (!html) {
        await extractInlinedHTML()
      ...
      if (!html) { setDesignExportStatus("❌ Could not extract HTML."); return }
      await navigator.clipboard.writeText(html)
      setDesignExportStatus("✅ Copied! In Figma: run html.to.design → click 'HTML' tab → paste.")
    } catch {
      setDesignExportStatus("❌ Clipboard write failed. Try the 'Capture mode' button instead.")
    }
  }

  /** Framer — same clipboard copy, user uses HTML to Framer extension */
  const copyHtmlForFramer = async () => {
    setDesignExportStatus("Extracting…")
    try {
      let html = extractedHTML
      if (!html) {
      ...
      if (!html) { setDesignExportStatus("❌ Could not extract HTML."); return }
      await navigator.clipboard.writeText(html)
      setDesignExportStatus("✅ Copied! Install the 'HTML to Framer' Chrome extension, open your site, then paste in Framer (Ctrl+V / ⌘V).")
    } catch {
      setDesignExportStatus("❌ Clipboard write failed.")
    }
  }
...
💡 **3 opportunités de contenu** — sujets à cibler pour ce domaine`
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
      ...
      {result && (
        <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t border-[#eee] px-4 py-3 z-30">
          <div className="max-w-4xl mx-auto flex items-center gap-2 overflow-x-auto no-scrollbar">
            ...
            {/* ── Export to Figma / Framer ── */}
            <button
              onClick={() => { setDesignExportStatus(null); setDesignExportTab("figma"); setShowDesignExportModal(true) }}
              className="flex-shrink-0 flex items-center gap-1.5 h-[36px] px-4 rounded-[10px] border border-[#e5e5e5] bg-white text-sm font-medium text-black hover:bg-gray-50 transition-colors whitespace-nowrap"
              title="Export to Figma or Framer"
            >
              {/* Figma-style icon */}
              <svg width="13" height="13" viewBox="0 0 38 57" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <path d="M8 47.5C8 41.7 12.7 37 18.5 37H29v10.5C29 53.3 24.3 58 18.5 58S8 53.3 8 47.5z" fill="#0ACF83"/>
                <path d="M8 9.5C8 3.7 12.7-1 18.5-1H29v21H18.5C12.7 20 8 15.3 8 9.5z" fill="#FF7262"/>
                <path d="M8 28.5C8 22.7 12.7 18 18.5 18H29v21H18.5C12.7 39 8 34.3 8 28.5z" fill="#F24E1E"/>
                <path d="M29 -1h10.5C45.3-1 50 3.7 50 9.5S45.3 20 39.5 20H29V-1z" fill="#FF7262"/>
              </svg>
              <span>Export</span>
            </button>
      ...
      {/* ── Design Export modal (Figma / Framer) ── */}
      <Dialog open={showDesignExportModal} onOpenChange={(v) => { setShowDesignExportModal(v); if (!v) setDesignExportStatus(null) }}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>Export to design tool</DialogTitle>
            <DialogDescription>Import the extracted design into Figma or Framer as editable layers.</DialogDescription>
          </DialogHeader>

          {/* Tabs */}
          <div className="flex gap-1 bg-gray-100 rounded-xl p-1 w-fit mb-5">
            {(["figma", "framer"] as const).map((tab) => (
              <button
                key={tab}
                onClick={() => { setDesignExportTab(tab); setDesignExportStatus(null) }}
                className={`flex items-center gap-2 px-4 py-1.5 text-sm font-medium rounded-lg transition-all capitalize ${
                  designExportTab === tab ? "bg-white shadow-sm text-black" : "text-gray-500 hover:text-gray-700"
                }`}
              >
                {tab === "figma" ? (
                  <>
                    <svg width="11" height="16" viewBox="0 0 38 57" fill="none">
                      <ellipse cx="29.5" cy="28.5" rx="10.5" ry="10.5" fill="#1ABCFE"/>
                      <path d="M8 47.5C8 41.7 12.7 37 18.5 37H29v10.5C29 53.3 24.3 58 18.5 58S8 53.3 8 47.5z" fill="#0ACF83"/>
                      <path d="M8 9.5C8 3.7 12.7-1 18.5-1H29v21H18.5C12.7 20 8 15.3 8 9.5z" fill="#FF7262"/>
                      <path d="M8 28.5C8 22.7 12.7 18 18.5 18H29v21H18.5C12.7 39 8 34.3 8 28.5z" fill="#F24E1E"/>
                      <path d="M29-1h10.5C45.3-1 50 3.7 50 9.5S45.3 20 39.5 20H29V-1z" fill="#FF7262"/>
                  ...
                  <>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none">
                      <rect x="2" y="2" width="9" height="9" rx="1.5" fill="#0055FF"/>
                      <rect x="13" y="2" width="9" height="9" rx="1.5" fill="#0055FF" opacity=".5"/>
                      <rect x="2" y="13" width="9" height="9" rx="1.5" fill="#0055FF" opacity=".3"/>
                    </svg>
                    Framer
                  </>
                )}
          ...
          {/* ── Figma tab ── */}
          {designExportTab === "figma" && (
            <div className="space-y-3">

              {/* Method 1 — Capture mode */}
              <div className="border border-gray-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[11px] font-bold bg-black text-white px-2 py-0.5 rounded-full">Recommended</span>
                  <span className="text-[11px] text-gray-400">No plugin needed</span>
                </div>
                <h3 className="text-sm font-semibold text-black mb-1">Figma Capture Mode</h3>
                <p className="text-xs text-gray-500 mb-3 leading-relaxed">
                  Opens the extracted site in a new tab with Figma's own capture toolbar. 
                  Click <strong>"Copy to clipboard"</strong> in the floating toolbar, then switch to Figma and paste <kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+V</kbd>.
                </p>
                <Button onClick={exportToFigmaCaptureMode} disabled={extracting || !result}>
                  Open in Figma capture mode →
                </Button>
              </div>

              {/* Method 2 — html.to.design plugin */}
              <div className="border border-gray-200 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-black mb-1">html.to.design plugin</h3>
                <ol className="text-xs text-gray-500 space-y-1 mb-3 list-decimal list-inside leading-relaxed">
                  <li>Install the <a href="https://figma.com/community/plugin/1159123024924461424" target="_blank" rel="noopener noreferrer" className="text-blue-500 underline hover:text-blue-700">html.to.design</a> plugin in Figma</li>
                  <li>Click the button below to copy the extracted HTML</li>
                  <li>In Figma: run the plugin → click the <strong>HTML</strong> tab → paste</li>
                </ol>
                <Button variant="outline" onClick={copyHtmlForFigmaPlugin} disabled={extracting || !result}>
                  <Copy size={13} className="mr-2" /> Copy HTML for plugin
                </Button>
              </div>
            </div>
          )}
          ...
          {/* ── Framer tab ── */}
          {designExportTab === "framer" && (
            <div className="space-y-3">

              {/* Method 1 — Via Figma (best fidelity) */}
              <div className="border border-gray-200 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[11px] font-bold bg-black text-white px-2 py-0.5 rounded-full">Best fidelity</span>
                </div>
                <h3 className="text-sm font-semibold text-black mb-1">Import via Figma</h3>
                <p className="text-xs text-gray-500 mb-3 leading-relaxed">
                  Export to Figma first using the Capture Mode, then in Framer use 
                  <strong> File → Import → Figma</strong>. This gives the highest quality result with fully editable layers.
                </p>
                <Button onClick={() => { setDesignExportTab("figma"); setDesignExportStatus(null) }}>
                  Go to Figma export →
                </Button>
              </div>

              {/* Method 2 — HTML to Framer extension */}
              <div className="border border-gray-200 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-black mb-1">HTML to Framer extension</h3>
                <ol className="text-xs text-gray-500 space-y-1 mb-3 list-decimal list-inside leading-relaxed">
                  <li>Install the <a href="https://chromewebstore.google.com/detail/html-to-framer/haijifigpgpndcnbbjooffflaceedhdp" target="_blank" rel="noopener noreferrer" className="text-blue-500 underline hover:text-blue-700">HTML to Framer</a> Chrome extension</li>
                  <li>Click the button below to copy the extracted HTML</li>
                  <li>Open Framer canvas → paste <kbd className="px-1 py-0.5 bg-gray-100 rounded text-[10px]">Ctrl+V / ⌘V</kbd></li>
                </ol>
                <Button variant="outline" onClick={copyHtmlForFramer} disabled={extracting || !result}>
                  <Copy size={13} className="mr-2" /> Copy HTML for Framer
                </Button>
              </div>
            </div>
          )}
          ...
          <DialogFooter className="mt-2">
            <Button variant="ghost" onClick={() => { setShowDesignExportModal(false); setDesignExportStatus(null) }}>Close</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
