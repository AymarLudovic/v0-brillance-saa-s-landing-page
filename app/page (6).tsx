...
══════════════════════════════════════════════════════════════
SECTION 5 — DESIGN SYSTEM MANIFEST FOR DOWNSTREAM JS AGENTS
══════════════════════════════════════════════════════════════

At the very END of your <style> block, after all other CSS, you MUST append a clearly labeled
comment block that documents the design system for the JavaScript agents that will run after you.
This is mandatory — without it, JS-injected elements will look foreign and break the design.

The comment block MUST follow this exact format:

/* ============================================================
   DESIGN SYSTEM MANIFEST — FOR JS AGENTS
   Read these values before injecting ANY new DOM element.
   Every modal, dropdown, tooltip, panel, toast you create
   MUST use these values to stay visually native to this app.
   ============================================================

   APP_TYPE: [the type of application: dashboard / e-commerce / CRM / kanban / landing / etc.]
   COLOR_THEME: [dark / light / glass / colorful]

   BACKGROUNDS:
     --ppai-bg-page:    [canvas hex — the outermost page background]
     --ppai-bg-surface: [canvas hex — cards, panels, sidebars]
     --ppai-bg-hover:   [canvas hex — hover state on interactive rows/items]
     --ppai-bg-input:   [canvas hex — input fields, textareas]

   BORDERS:
     --ppai-border:        [canvas hex — standard border color]
     --ppai-border-width:  [e.g. 1px]
     --ppai-radius-sm:     [e.g. 0px / 4px / 6px — small elements like badges, tags]
     --ppai-radius-md:     [e.g. 4px / 8px / 12px — cards, panels, dropdowns]
     --ppai-radius-lg:     [e.g. 8px / 16px / 20px — modals, large surfaces]

   TYPOGRAPHY:
     --ppai-font-family:   [exact font-family string as in body/main elements]
     --ppai-text-primary:  [canvas hex — main text]
     --ppai-text-secondary:[canvas hex — muted/secondary text]
     --ppai-text-muted:    [canvas hex — placeholder, disabled text]
     --ppai-font-size-sm:  [e.g. 11px / 12px]
     --ppai-font-size-md:  [e.g. 13px / 14px]
     --ppai-font-size-lg:  [e.g. 15px / 16px]

   INTERACTIVE:
     --ppai-accent:        [canvas hex — primary action / highlight color]
     --ppai-accent-text:   [canvas hex — text on accent background]
     --ppai-btn-bg:        [canvas hex — primary button background]
     --ppai-btn-text:      [canvas hex — primary button text]
     --ppai-btn-padding:   [e.g. 6px 14px / 8px 20px]
     --ppai-btn-radius:    [mirrors --ppai-radius-sm or -md]
     --ppai-btn-font-size: [e.g. 12px / 13px]
     --ppai-btn-font-weight: [e.g. 500 / 600]

   SHADOWS:
     --ppai-shadow:        [e.g. none / 0 2px 8px rgba(0,0,0,0.3)]

   SPACING:
     --ppai-density: [compact / normal / spacious]
     --ppai-gap-sm:  [e.g. 4px / 6px — tight spacing]
     --ppai-gap-md:  [e.g. 8px / 12px — standard spacing]
     --ppai-gap-lg:  [e.g. 16px / 24px — section spacing]

   MODAL RECIPE (copy this exactly for any modal you create):
     backdrop: position:fixed; inset:0; z-index:9000; background:rgba(0,0,0,[0.4-0.8 based on theme])
     panel:    background:var(--ppai-bg-surface); border:var(--ppai-border-width) solid var(--ppai-border);
               border-radius:var(--ppai-radius-lg); padding:var(--ppai-gap-lg);
               font-family:var(--ppai-font-family); color:var(--ppai-text-primary);
               box-shadow:var(--ppai-shadow)

   TOAST/NOTIFICATION RECIPE:
     position:fixed; bottom:20px; right:20px; z-index:9999;
     background:var(--ppai-bg-surface); border:1px solid var(--ppai-border);
     border-radius:var(--ppai-radius-md); padding:10px 16px;
     font-family:var(--ppai-font-family); font-size:var(--ppai-font-size-md);
     color:var(--ppai-text-primary); box-shadow:var(--ppai-shadow)

   DROPDOWN RECIPE:
     position:absolute; z-index:8000; min-width:160px;
     background:var(--ppai-bg-surface); border:1px solid var(--ppai-border);
     border-radius:var(--ppai-radius-md); overflow:hidden; box-shadow:var(--ppai-shadow)
     — Each option: padding:var(--ppai-gap-sm) var(--ppai-gap-md);
       font-size:var(--ppai-font-size-md); color:var(--ppai-text-primary);
       cursor:pointer; hover → background:var(--ppai-bg-hover)

   IMAGE_REFERENCE: [brief description of the visual mood from the screenshot:
     e.g. "dark navy sidebar with cyan accents, dense compact layout, sharp corners"
     or "light white glass cards, purple gradient accents, rounded 12px radius, airy spacing"]
   ============================================================ */

Fill in every value from the canvas hex data and your visual analysis of the screenshot.
These CSS custom properties (--ppai-*) are NOT used in the page styles — they exist ONLY as
a machine-readable manifest for the JS agents. Do not reference them in any selector.`;
...
══════════════════════════════════════════════════════════════
SECTION 2 — DESIGN FIDELITY (MANDATORY — READ BEFORE WRITING ANY CODE)
══════════════════════════════════════════════════════════════

The HTML you receive contains a "DESIGN SYSTEM MANIFEST — FOR JS AGENTS" comment block inside the <style> tag.
THIS IS YOUR DESIGN BIBLE. Read it before writing a single line of code.

▸ STEP 1 — FIND AND READ THE MANIFEST:
  Locate the comment block starting with "DESIGN SYSTEM MANIFEST — FOR JS AGENTS" in the <style> section.
  Extract every --ppai-* value defined there. These give you the exact colors, radii, fonts, spacing,
  and recipes for modals/toasts/dropdowns that match this specific app's visual language.

▸ STEP 2 — USE THE MANIFEST VALUES IN ALL INJECTED ELEMENTS:
  Every DOM element you create MUST use those values. Reference them as CSS custom properties:
    el.style.cssText = `
      background: var(--ppai-bg-surface);
      border: var(--ppai-border-width) solid var(--ppai-border);
      border-radius: var(--ppai-radius-md);
      font-family: var(--ppai-font-family);
      color: var(--ppai-text-primary);
      padding: var(--ppai-gap-md);
    `

▸ STEP 3 — IF NO MANIFEST EXISTS, extract values manually:
  - Read CSS custom properties from :root in the HTML (--color-*, --bg-*, --border-*, etc.)
  - If no CSS vars: read from existing elements:
      const refCard = document.querySelector('[class*="card"],[class*="panel"],[class*="surface"]')
      const cs = getComputedStyle(refCard || document.body)
      // reuse cs.backgroundColor, cs.borderRadius, cs.fontFamily, cs.color, etc.

▸ STEP 4 — ALSO use the screenshot image as design reference:
  The screenshot shows you the exact visual mood of the app (dark/light, dense/airy, sharp/rounded,
  accent colors, visual hierarchy). Every element you inject must feel visually native to that mood.
  Ask yourself: "Would a user be able to tell this was injected, or does it look built-in?" → It must look built-in.

HARD RULES FOR ALL INJECTED ELEMENTS:
▸ Match border-radius EXACTLY (0px? 4px? 8px? 12px? pill? — use --ppai-radius-* or read from existing)
▸ Match font-family, font-size scale, font-weight from the manifest or existing text elements
▸ Match the color palette: dark app → dark injected UI; light app → light injected UI
▸ Match spacing density: compact app → compact elements; spacious app → spacious elements
▸ Match button style exactly: padding, border, radius, background, font-weight, hover state
▸ NEVER introduce generic Bootstrap/Material/Tailwind-default looks if they clash with the app
▸ NEVER use hardcoded generic colors (#fff, #000, #007bff, #f5f5f5) — always derive from manifest/existing
▸ ALL overlays (modals, drawers, toasts, context menus) use position:fixed and float above everything
▸ The user must NOT be able to tell which elements were original vs injected by you

══════════════════════════════════════════════════════════════
SECTION 3 — THE ZERO-SIMPLIFICATION RULE (READ THIS FIRST)
══════════════════════════════════════════════════════════════

THIS IS THE MOST IMPORTANT RULE IN THIS PROMPT. READ IT CAREFULLY.

❌ NEVER simplify an interaction to a toast/alert/console.log when the element demands real behavior.
❌ NEVER write placeholder code like: el.addEventListener('click', () => alert('Coming soon'))
❌ NEVER leave any interactive element with no handler or a trivial fake handler.
❌ NEVER stop mid-feature. If a menu opens → it must show real items. If a form submits → it must
   process the data. If a button says "Add" → it must actually add something to the UI.
❌ NEVER write partial implementations. Either fully build the feature or skip it — no half-measures.

✅ ALWAYS implement the COMPLETE behavior an element implies, given the app type.
✅ ALWAYS think: "What would a real user expect to happen when they click this?" → Build that.
✅ ALWAYS follow through to the end: trigger → logic → DOM result → feedback → persistence if needed.
✅ IF an element triggers a sub-workflow (e.g. a menu item opens a sub-panel), build the ENTIRE
   sub-workflow, not just the toggle.
✅ IF multiple elements of the same type exist (e.g. 5 menu items, 3 action buttons, 8 list rows),
   EVERY SINGLE ONE must have its own correct behavior wired — not just the first one.

MINIMUM COMPLETENESS BAR FOR EACH ELEMENT TYPE:
  Button "Delete" or trash icon → must actually remove the target element from the DOM
  Button "Edit" or pencil icon → must open an inline edit field or edit panel with Save/Cancel
  Button "Add" / "New" / "Create" → must inject a new real item/row into the list/grid
  Menu item → must execute its stated action (filter, navigate, change view, sort, etc.)
  Navigation link/sidebar item → must switch the active section/view, update active state visually
  Tab → must show the correct panel, hide others, update active indicator
  Form submit → must validate all fields, show inline errors, show success state, persist data
  Search/filter input → must filter the visible list/table in real time
  Dropdown option → must apply its value and update the relevant UI
  Checkbox/toggle → must apply its effect immediately (show/hide, enable/disable, filter)
  Star rating → must persist the selection and update all stars visually
  Upload zone → must handle the File object, show preview or filename, give feedback
  Sortable column header → must actually sort the rows, toggle asc/desc, show arrow indicator

══════════════════════════════════════════════════════════════
SECTION 4 — COMPLETE INTERACTION COVERAGE
══════════════════════════════════════════════════════════════

Your job is to wire ALL of these in a way that makes sense for the specific app detected.
For EACH element below, build the FULL behavior — not a simplified version:

NAVIGATION & ROUTING
- Nav links / sidebar items → scroll-to-section OR show/hide page sections OR hash routing
- Active state highlights follow the current page/section (update ALL nav items)
- Mobile hamburger → sidebar slide-in/out with overlay, close on outside click
- Breadcrumbs → navigate back to parent section
- Back/forward buttons → history.back() or equivalent

TABS, PILLS & SEGMENTED CONTROLS
- Click → switch active tab (update active class on ALL tabs), hide ALL panels, show ONLY correct one
- URL hash or data attribute maps tab to panel
- Tab with counter badge → badge updates on relevant action

DROPDOWNS & MENUS
- Click trigger → toggle panel, close on outside click (document listener), close on ESC
- EVERY option item → executes its specific action (filter, sort, select value, navigate, etc.)
- Multi-level menus → hover or click for sub-menu, position correctly
- Selecting an option → update the trigger label/value to reflect current selection

MODALS & DIALOGS
- Trigger → open modal with correct content for THIS specific trigger
- Each distinct trigger → its own modal content (not all triggers opening the same empty shell)
...
- Arrow/chevron icon must rotate on open/close

CAROUSELS & SLIDERS
- Prev/next arrows → slide items, disable arrow at boundaries
- Dot indicators → click to jump to slide, update active dot
- Auto-play with pause on hover
- Touch/drag support with pointer events

FORMS & INPUTS
- Real-time validation (required fields, email format, min length, etc.)
- Show/hide error messages inline under each field with correct styling
- Submit → validate ALL fields first, show loading state on button, then success state with real feedback
- On success → update the relevant list/table/UI with the new data
...
- Countdown timers → live countdown with setInterval if present

INTERACTIVE TABLES & LISTS
- Row click → highlight/select with correct active styles
- Sortable column headers → sort ALL rows by that column (asc/desc toggle, show arrow indicator)
- Row checkbox → select/deselect individual rows, update "select all" state, enable bulk action buttons
- Inline action buttons (edit, delete, duplicate, etc.) → each performs its full action on that row

SEARCH & FILTER
- Input → filter visible list/table rows in real-time (case-insensitive, multi-field match)
- Clear button → reset filter, restore all rows
- Filter chips/tags → each toggles its filter criterion, update results count
- Empty state message when no results match

TOGGLES & SWITCHES
- Click → toggle boolean state, update visual indicator AND apply the effect immediately
- Dark/light mode toggle → apply to <html>, persist to localStorage

RATINGS & REVIEWS
- Star rating → click to rate, highlight all stars up to selected, show numeric value
- Hover preview → temporarily show hover state, revert on mouseout if not clicked

TOOLTIPS
- Hover (desktop) / click (touch) → show tooltip near element with correct positioning
- Flip tooltip if near viewport edge
- Auto-dismiss on mouseout

DRAG & DROP (UI layer)
- Draggable items → cursor:grab, visual feedback (opacity, shadow) while dragging
- Drop zones → highlight on drag-over
- Reorder within list → update DOM order, persist new order

COPY & SHARE
- Copy buttons → copy correct target text to clipboard, show "Copied!" for 2s then revert
...
- When iterating over similar elements (buttons, rows, items), use querySelectorAll + forEach —
  wire EVERY instance, not just the first one
- Persist interaction state to localStorage where it makes sense (active tab, selected filters, etc.)
...
async function callGeminiJS(
  ...
  const userMsg = `STEP 1 — IDENTIFY THE APP TYPE from the screenshot. Name it explicitly (e.g. "SaaS dashboard", "e-commerce admin", "kanban board", "CRM", etc.).

STEP 2 — EXHAUSTIVE ELEMENT AUDIT. List EVERY interactive element you find in BOTH the screenshot AND the HTML code. For each element, state:
  - What it is (button, nav item, tab, input, dropdown, toggle, icon-button, etc.)
  - Its selector (class or id from the HTML)
  - What it must do in this SPECIFIC app type — the full real behavior, not a simplified version

Do NOT skip any element. If you find 12 nav items, list all 12. If you find 5 action buttons, list all 5.

STEP 3 — COMPLETE IMPLEMENTATION. Write <script>...</script> that fully implements EVERY item from Step 2.

CRITICAL COMPLETENESS RULES FOR STEP 3:
  ▸ NO toasts/alerts as a substitute for real behavior. A toast may ACCOMPANY an action but never REPLACE it.
  ▸ EVERY nav/menu item must navigate or filter — not just log or toast.
  ▸ EVERY button that says Add/Create/New must inject a real new element into the DOM.
  ▸ EVERY button that says Delete/Remove must actually remove the target from the DOM.
  ▸ EVERY button that says Edit must open an editable state for that specific item.
  ▸ EVERY tab must show its correct panel and hide all others.
  ▸ EVERY dropdown option must apply its specific effect.
  ▸ ALL instances of a repeated element (e.g. all rows, all cards) must be wired — not just the first.
  ▸ If an action involves multiple steps, implement ALL steps end-to-end.
  ▸ When in doubt: ask "what would a real user expect?" → build exactly that.
...
REMINDER: Use ONLY selectors that exist in the HTML above. Match the design system for any new injected elements (read CSS variables from :root). Output ONLY <script>...</script> tags.`;
  ...
  return withGeminiRetry(async () => {
    const ai = getAI(apiKey);
    const parts: any[] = [];
    if (imageB64 && mimeType) parts.push({ inlineData: { mimeType, data: imageB64 } });
...
${userBrief ? `\nAPP BRIEF (what this app is meant to do — use this to ensure the conversion preserves all real functionality):\n${userBrief}\n` : ''}
...
async function fixNextJSBuildErrors(
  ...
  const historyBlock = previousErrors && previousErrors.length > 0
    ? `\nPREVIOUS FIX ATTEMPTS THAT DID NOT WORK (do NOT repeat these fixes):\n${previousErrors.map((e,i)=>`Attempt ${i+1}:\n${e.slice(0,2000)}`).join('\n---\n')}\n`
    : '';

  const briefBlock = userBrief
    ? `\nAPP BRIEF (what this app is supposed to do — use to understand intentional patterns in the code):\n${userBrief}\n`
    : '';

  const userMsg = `Fix the TypeScript/build errors in this Next.js project.
${briefBlock}
BUILD ERRORS (current attempt):
\`\`\`
...
9. READ THE ERROR CAREFULLY — fix the root cause, not just the symptom. If a selector targets an element that doesn't exist in JSX, add the element to JSX (preserving all existing classes/ids). Don't delete the JS code.
10. If previous attempts failed, try a fundamentally different approach — don't repeat what didn't work.
...
/* ── Manager Agent ── */
const MANAGER_PROMPT = `You are an intent detection agent for a UI generation platform. Your job is to understand what the user ACTUALLY wants before doing anything.
...
"chat"        → DEFAULT route for anything that is NOT a clear build/feature request.
  Use for: greetings, questions, feedback, thanks, asking "what can you do", general conversation,
  unclear intent, ambiguous messages, short vague messages, anything that reads as a question or statement.
  IMPORTANT: When in doubt → always pick "chat". Never trigger generation on ambiguous input.
  Examples: "hello", "nice work", "what is this?", "can you explain X", "looks good", "I love it"

"generate"    → ONLY when user explicitly wants to build a brand-new app from scratch.
  Requires: a CLEAR description of an app/UI to build AND an image is attached.
  Keywords that confirm intent: "build", "create", "generate", "make me", "reproduce this", "convert this".
  NEVER pick "generate" if: no image is attached, or the message is vague, or they're just describing a screenshot without asking to build it.

"html_js"     → User wants to ADD a client-side feature to an existing project that already exists.
  Requires: an existing project already generated. The feature can be done purely in the browser.
  Examples: dark mode toggle, modal, chart, filter, animation, form validation, carousel, tab, search, counter, localStorage state.

"html_nextjs" → User wants to ADD a feature that needs server-side logic to an existing project.
  Requires: an existing project. The feature needs a server (email sending, real API fetch, file upload, auth, database).

"nextjs"      → User wants pure server infrastructure on an existing project. No new UI needed.
  Examples: new API route, server action, middleware, authentication flow, database migration, env config.

══════════════════════════════════════════════════════════════
DECISION RULES (apply in order)
══════════════════════════════════════════════════════════════

1. If the message is conversational, a question, or intent is unclear → "chat"
2. If image is attached AND user explicitly says to build/create/reproduce AND no project exists → "generate"
3. If a project exists AND user describes a new feature → "html_js" or "html_nextjs" or "nextjs"
4. If image is attached but no clear build intent → "chat" (ask what they want)
5. NEVER auto-trigger "generate" just because an image was attached

IMPORTANT: When in doubt between html_js and html_nextjs, pick html_js. Most UI features work client-side.
IMPORTANT: "generate" requires BOTH an image AND an explicit build request. One without the other → "chat".`;
...
async function callGeminiManager(
  ...
  const ctx = [
    `User has ${hasImage ? 'an image attached' : 'NO image attached'}.`,
    hasExistingProject ? 'A project has already been generated (app exists).' : 'No app generated yet (first time).',
    `User message: "${userMessage}"`,
    hasImage && !hasExistingProject
      ? 'NOTE: Image is attached but check if user explicitly asks to build/create — if not, route to "chat".'
      : '',
  ].filter(Boolean).join('\n');
...
/* ── Smart Chat Agent (routing + response + brief extraction in one call) ── */
async function callGeminiSmartChat(
  ...
  hasProject: boolean,
  hasImage: boolean,
): Promise<ManagerDecision & { reply: string; brief?: string }> {
  const systemMsg = `You are the AI assistant and build companion of Pixel Perfect AI — a vibe-coding platform that builds full web apps (HTML, JavaScript, and Next.js TypeScript) from UI screenshots.

Your personality: warm, direct, honest, enthusiastic about design and code. You are a BUILD COMPANION, not just a router.

══════════════════════════════════════════════════════════════
YOUR TRIPLE ROLE — CRITICAL
══════════════════════════════════════════════════════════════

You do THREE things simultaneously:
1. RESPOND to the user naturally and helpfully
2. DECIDE whether to trigger a development pipeline
3. When triggering dev: EXTRACT the real app brief (what they want to build)

Always append at the end:
<ROUTE>{"route":"...","reasoning":"one sentence"}</ROUTE>

When route is NOT "chat", also append:
<BRIEF>
App type: [what kind of app this is — e.g. video editor, CRM dashboard, e-commerce admin]
Core purpose: [the real problem it solves / what the user actually wants to achieve]
Key features required: [explicit feature list — be very specific about what was asked, not just what is visible in the image]
Technical expectations: [any specific tech, integrations, real data flows mentioned by the user]
What the image provides: [design reference / layout template / color palette / UI inspiration]
</BRIEF>

Route values:
  "chat"        → You answered fully. No pipeline needed. DEFAULT for anything conversational.
  "generate"    → Build new app from scratch. Only if image attached AND user explicitly says build/create/reproduce.
  "html_js"     → Add client-side feature to existing project.
  "html_nextjs" → Add server-side feature to existing project.
  "nextjs"      → Pure server infrastructure on existing project.

══════════════════════════════════════════════════════════════
ROUTING RULES
══════════════════════════════════════════════════════════════

1. Conversational, question, greeting, unclear intent → "chat"
2. Image attached AND explicit build/create/reproduce request AND no project → "generate"
3. Project exists AND client-side feature described → "html_js"
4. Project exists AND server-side feature described → "html_nextjs"
5. Project exists AND pure server infra → "nextjs"
WHEN IN DOUBT → "chat". "generate" requires BOTH image AND explicit build request.

══════════════════════════════════════════════════════════════
COMPANION BEHAVIOR — NON-NEGOTIABLE
══════════════════════════════════════════════════════════════

Current context:
- ${hasProject ? 'User has a generated app. They can add features, modify it, or just chat.' : 'No app generated yet.'}
- ${hasImage ? 'An image is currently attached.' : 'No image attached.'}
${hasImage && !hasProject ? '\nImage attached — suggest describing what they want to build from it.' : ''}

▸ UNDERSTAND FRUSTRATIONS — if user expresses frustration ("it doesn't work", "that's not what I asked", "same error again"), acknowledge it genuinely first. Don't be defensive.

▸ BE HONEST ABOUT LIMITS — if what they ask has real technical constraints (full video editor, real-time sync, payment processing in pure frontend), say so clearly AND propose a realistic first version. Example: "A full timeline video editor is heavy — let's build a functional trimmer + cuts as v1, then iterate."

▸ NEVER SAY "IMPOSSIBLE" WITHOUT AN ALTERNATIVE — if path A is blocked, find path B. "Can't do X directly, but we can achieve it via Y."

▸ CLARIFY AMBIGUITY — if the request is vague ("make it better", "add features"), ask ONE specific clarifying question rather than guessing.

▸ DISTINGUISH DESIGN FROM FUNCTION — the image is used as a design reference, not a spec. The user wants a working app, not just a screenshot clone. The BRIEF tag captures this distinction explicitly.

For "chat": respond fully (2-4 sentences max unless complex), append <ROUTE>.
For dev routes: one honest acknowledgment sentence, then <ROUTE> + <BRIEF>.

Be concise. Never preachy. Never fake enthusiasm about things that won't work.`;
  ...
  const response = await ai.models.generateContent({
    ...
    config: { systemInstruction: systemMsg, temperature: 0.75, maxOutputTokens: 1500 },
  });
  const raw = extractText(response);
  ...
  // Extract the BRIEF tag (only present for dev routes)
  const briefMatch = raw.match(/<BRIEF>([\s\S]*?)<\/BRIEF>/);
  const brief = briefMatch ? briefMatch[1].trim() : undefined;

  // Reply is everything before the tags
  const reply = raw
    .replace(/<ROUTE>[\s\S]*?<\/ROUTE>/g, '')
    .replace(/<BRIEF>[\s\S]*?<\/BRIEF>/g, '')
    .trim();

  return { route, reasoning, reply, brief };
}
...
async function callGeminiFeatureSnippet(
  ...
  const briefSection = userBrief
    ? `\nAPP CONTEXT (what this app is — use to ensure the new snippet fits the application's purpose):\n${userBrief}\n`
    : '';
...
Generate ONLY the HTML snippet for this feature. Match the existing design exactly. The snippet must serve the real application purpose described in the app context above — not a generic placeholder.`;
...
export default function PixelPerfectAI() {
  ...
  /* ── Project brief & error context ── */
  const [userBrief, setUserBrief]             = useState(''); // what the user actually wants to build
  const [buildErrorHistory, setBuildErrorHistory] = useState<string[]>([]); // accumulated build errors for smarter fixing
  ...
  const [messages, setMessages]           = useState<ChatMessage[]>([
    { id: '0', role: 'ai', type: 'text', content: 'Bonjour! Je suis Pixel Perfect AI 👋\n\nJe peux t\'aider à:\n• **Construire** une app à partir d\'une capture d\'écran\n• **Ajouter des features** à un projet existant\n• **Discuter** de design, code, ou architecture\n\nParle-moi librement — je détecte l\'intention automatiquement.' }
  ]);
  ...
  const runSandboxBuild = async (sid: string, buildAttempts: number) => {
    ...
    if (!res.success) {
      ...
      if (buildAttempts < 2 && apiKey) {
        ...
        try {
          ...
          // Accumulate error history so each attempt knows what the previous one tried
          const currentErrorHistory = [...buildErrorHistory, errors];
          setBuildErrorHistory(currentErrorHistory);
          const fixed = await fixNextJSBuildErrors(
            apiKey, currentFiles, errors, msg => sbLog(msg),
            userBrief || undefined,
            buildAttempts > 0 ? buildErrorHistory : undefined,
          );
    ...
    // Reset error history on success
    setBuildErrorHistory([]);
...
══════════════════════════════════════════════════════════════`
    ...
    try {
      ...
      const nxFiles = await callGeminiNextJS(apiKey, imgB64, imgMime, html, scripts, featMerged, progress, activeBrief || undefined);
  ...
  const smartPatch = async (userRequest: string) => {
    ...
    try {
      // Single agent call: decides route, responds, AND extracts brief
      ...
      // Capture & store brief if present (enriches all subsequent dev calls)
      const activeBrief = decision.brief
        ? (userBrief ? `${userBrief}\n\n--- Updated request ---\n${decision.brief}` : decision.brief)
        : userBrief;
      if (decision.brief) setUserBrief(activeBrief);
      ...
      if (decision.route === 'html_js' || decision.route === 'html_nextjs') {
        ...
        if (decision.route === 'html_js') {
          ...
          const allFiles = await callGeminiNextJS(apiKey, imgB64, imgMime, enrichedHtml, newJs1, enrichedWithJs, progress, activeBrief || undefined);
  ...
  const sendMessage = async () => {
    ...
    try {
      if (!apiKey) {
        addMsg({ role:'ai', type:'text', content:'Please set your Gemini API key first (top right corner).' });
        return;
      }

      // If no text at all — just acknowledge the image upload, don't auto-generate
      if (!text) {
        addMsg({ role:'ai', type:'text', content:'Image uploaded! 📸 Describe what you want to build from it, or ask me anything.' });
        return;
      }

      // Always route through the manager — it decides intent
      await smartPatch(text);
    } finally {
      setIsSending(false);
    }
  };
