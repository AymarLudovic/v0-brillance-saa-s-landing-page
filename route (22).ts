    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // Déclenché si: Presenter le demande ET (images uploadées OU vibes disponibles)
          ...
          const isDesignRef = rawPresenterOutput.includes("[IMAGE_IS_DESIGN_REF]") &&
            ((uploadedImages && uploadedImages.length > 0) || hasVibes);
          ...
          if (isDesignRef) {
            ...
            const requestedVibes: { category: string; count: number }[] = [];
            const rvRegex = /<request_vibes\s+category="([^"]+)"\s+count="?(\d+)"?\s*\/>/g;
            let rvMatch;
            while ((rvMatch = rvRegex.exec(rawPresenterOutput)) !== null) {
              requestedVibes.push({ category: rvMatch[1], count: parseInt(rvMatch[2], 10) });
            }
            ...
            if (vibesByCategory && Object.keys(vibesByCategory).length > 0) {
              ...
              if (requestedVibes.length > 0) {
                ...
                for (const req of requestedVibes) {
                  const catImgs = (vibesByCategory[req.category] ?? []) as string[];
                  vibeImgs.push(...catImgs.slice(0, req.count));
                }
                emit(`[DESIGN:THINKING] Agent a demandé: ${requestedVibes.map(r => `${r.count}x ${r.category}`).join(", ")}\n`);
              } else {
