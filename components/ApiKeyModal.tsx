...
export default function ApiKeyModal() {
  ...
  if (!isOpen) {
    return (
      <button 
        onClick={openModal}
        className="fixed bottom-4 right-4 z-40 px-4 py-2 bg-[#1a1a1a] border border-white/10 rounded-lg text-xs text-gray-400 hover:text-white transition-colors flex items-center gap-2"
      >
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="relative w-[350px] h-[350px] bg-[#0f0f0f] rounded-3xl border border-white/10 overflow-hidden shadow-2xl flex flex-col">
        ...
        <div className="flex-1 p-6 flex flex-col relative">
          ...
          <div className="flex justify-between items-start mb-6">
            <div>
              <div className="inline-flex items-center justify-center px-2 py-1 rounded-md bg-[#1a1a1a] border border-white/5 text-[10px] font-medium text-gray-300 mb-2">
                NEW
              </div>
          ...
          <div className="mt-4">
            {!isInputMode ? (
              <button 
                ...
                className="w-full h-10 bg-white text-black rounded-full text-sm font-semibold hover:bg-gray-200 transition-colors"
              >
                Set your API key
              </button>
            ) : (
              <div className="flex items-center gap-2 w-full h-10">
                <div className="flex-1 h-full bg-[#1a1a1a] rounded-full border border-white/10 flex items-center px-3 gap-2">
                    <Lock1 size={16} variant="Bold" className="text-gray-500 shrink-0"/>
                    <input 
                        type="password"
                        value={inputValue}
                ...
                <button 
                    onClick={handleSave}
                    className="h-full px-5 bg-white text-black rounded-full text-xs font-bold hover:bg-gray-200 transition-colors shrink-0"
                >
                    SET
                </button>
              </div>
...
function FeatureItem({ icon, title, desc }: { icon: React.ReactNode, title: string, desc: string }) {
  return (
    <div className="flex items-start gap-3">
      ...
      <div>
        <h3 className="text-[11px] font-medium text-gray-200 leading-none mb-0.5">{title}</h3>
