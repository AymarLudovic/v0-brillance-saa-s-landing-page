  ...
  useEffect(() => {
    const checkKey = async () => {
      try {
        ...
        if (key) { setHasKey(true); setIsOpen(false); setInputValue(key); }
        else { setHasKey(false); setIsOpen(true); }
      } catch (e) { console.error("Erreur init Key:", e); setIsOpen(true); }
    };
    checkKey();
  }, [provider]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (verify === inputValue.trim()) {
        ...
        setHasKey(true);
        onKeySaved?.();
        setTimeout(() => { setIsOpen(false); window.location.reload(); }, 1500);
