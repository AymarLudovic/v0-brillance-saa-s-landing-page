...
export default function VercelDeployModal({ currentProject, isOpen, onClose }: VercelDeployModalProps) {
    ...
    // Ajout d'un état pour le nom du projet éditable
        ...
        try {
            ...
            if (Array.isArray(sdkLogs)) {
                sdkLogs.forEach((event: any) => {
                    ...
                    // --- MODIFICATION 1 : Détection plus intelligente de la fin ---
                    // On scanne le texte pour "Deployment completed" ou "Build Completed"
                    if (text && (
                        ...
                        stopPolling();
                        setIsDeploying(false);
                        // On force l'URL ici car on sait que ça a marché
                    ...
                    // Ignorer les warnings npm pour la couleur rouge
                    if (event.type === 'stderr') {
                        if (text.toLowerCase().includes('warn')) {
                            type = 'stdout'; 
                        } else if (text.toLowerCase().includes('error') || text.toLowerCase().includes('err!')) {
                            type = 'stderr';
                            // Si c'est une vraie erreur critique, on arrête le spinner
