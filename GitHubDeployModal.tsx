...
const handlePushToGitHub = async () => {
        ...
        const safety = setTimeout(() => {
            ...
            setLogs(prev => [...prev, {
                ...
                message: '⏱ Timeout — opération trop longue, arrêt forcé.',
                type: 'error'
            }]);
        }, 2 * 60 * 1000);
        ...
        try {
            ...
            while (true) {
                ...
                if (done) {
                    // Stream terminé — si pas encore fini proprement, on arrête le spinner
                    if (!finished) setIsDeploying(false);
                    break;
                }
                ...
                for (let i = 0; i < lines.length - 1; i++) {
                    ...
                    try {
                        ...
                        if (data.type === 'DONE') {
                            finished = true;
                            setRepoUrl(data.url || null);
                            setIsDeploying(false);
                            clearTimeout(safety);
                        }

                        // Succès détecté dans le message (fallback)
                        if (data.message && (data.message.includes('🚀') || data.message.includes('success'))) {
                            finished = true;
                            setIsDeploying(false);
                            clearTimeout(safety);
