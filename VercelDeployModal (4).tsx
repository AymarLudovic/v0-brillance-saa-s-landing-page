...
export default function VercelDeployModal({ currentProject, isOpen, onClose, sandboxId, onDeployError }: VercelDeployModalProps) {
  ...
  const realDeployUrl = useRef<string>('');
  const [productionUrl, setProductionUrl] = useState<string | null>(null);

  // ── Récupérer l'URL de production depuis l'API Vercel ────────────────────
  const fetchProductionUrl = async (projName: string) => {
    try {
      ...
      // alias de production = premier domaine lié au projet
      const alias = data.alias?.[0]?.domain || data.targets?.production?.alias?.[0] || null;
      if (alias) setProductionUrl(`https://${alias}`);
    } catch {}
  };
  ...
  const finishDeploy = (success: boolean, url?: string) => {
    ...
    if (success) {
      setDeployFailed(false);
      const finalUrl = url || realDeployUrl.current;
      if (finalUrl) setDeployUrl(finalUrl.startsWith('http') ? finalUrl : `https://${finalUrl}`);
      addLog('✅ Déploiement réussi !', 'success');
      // Récupérer l'URL de production
      fetchProductionUrl(projectName);
    } else {
      setDeployFailed(true);
      const stderr = stderrAccumulator.current.join('\n').trim();
      if (stderr && onDeployError) onDeployError(stderr);
    }
  };
      ...
      sdkLogs.forEach((event: any) => {
        ...
        if (event.type === 'deployment-state') {
          ...
          if (state) {
            ...
            if (state === 'READY') { finishDeploy(true); return; }
  ...
  const handleDeploy = async () => {
    ...
    try {
      // ── 1. Récupérer TOUS les fichiers du sandbox (inclut components/, globals.css, etc.)
      let allFiles: { filePath: string; content: string }[] = [];
      if (sandboxId) {
        try {
          addLog('📂 Récupération des fichiers sandbox...', 'system');
          ...
          if (sandboxData.success && sandboxData.files) {
            // sandbox renvoie Record<string, string>
            allFiles = Object.entries(sandboxData.files as Record<string, string>).map(
              ([filePath, content]) => ({ filePath, content })
            );
            addLog(`✅ ${allFiles.length} fichiers récupérés depuis le sandbox`, 'system');
          }
        } catch {
          addLog('⚠️ Sandbox inaccessible, fallback sur fichiers locaux', 'system');
        }
      }

      // ── 2. Fallback : fusionner avec currentProject.files si sandbox vide/mort
      if (allFiles.length === 0 && currentProject?.files?.length) {
        allFiles = currentProject.files.map((f: any) => ({
          filePath: f.filePath.startsWith('/') ? f.filePath.substring(1) : f.filePath,
          content: f.content,
        }));
      }

      // ── 3. Envoyer au route Vercel
      const response = await fetch('/api/deploy/vercel', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ projectName, files: allFiles, token })
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Déploiement échoué');
      // Stocker la vraie URL Vercel dès maintenant
      if (data.url) realDeployUrl.current = data.url.startsWith('http') ? data.url : `https://${data.url}`;
    ...
    <div className="fixed top-5 right-12 z-[9999] w-[390px] h-auto bg-[#fbfbf9] rounded-[16px] border border-[rgba(55,50,47,0.08)] overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
      ...
      <div className="flex-1 p-1 flex flex-col overflow-y-auto custom-scrollbar">
        ...
        <div className="mt-4 flex flex-col items-center gap-1 shrink-0 pb-2">
          {(deployUrl || productionUrl) ? (
            <div className="flex flex-col gap-1.5 w-full px-2">
              {productionUrl && (
                <a href={productionUrl} target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-1.5 w-full h-[30px] rounded-[8px] bg-[#1e52f1] text-white text-xs font-bold hover:bg-blue-700 transition-colors">
                  🌐 Open live site <ArrowUp size={10} className="rotate-45" />
                </a>
              )}
              {deployUrl && (
                <a href={`https://vercel.com/deployments/${deployUrl.split('/').pop()}`} target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-center gap-1.5 w-full h-[28px] rounded-[8px] bg-[#f7f4ed] text-[#212121] text-xs hover:bg-[#eeebe4] transition-colors">
                  View deployment logs <ArrowUp size={9} className="rotate-45" />
                </a>
              )}
            </div>
          ) : (
            <div className="flex w-full pl-2 pr-2 pt-2 justify-between border-t border-[rgba(55,50,47,0.08)] items-center gap-1">
              ...
              <a href="https://vercel.com/account/tokens" target="_blank" rel="noopener noreferrer" className="text-[12px] text-[#888] hover:text-black transition-colors flex items-center gap-1 underline decoration-dotted underline-offset-4">
