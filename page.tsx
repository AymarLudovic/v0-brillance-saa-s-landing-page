    ...
    <div className="py-6">
      ...
      {activeTab === 'insights' && (
        <div className="border border-gray-100 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="bg-gray-50 rounded-xl p-3 text-xs text-gray-700 leading-relaxed min-h-[80px] flex items-start">
            {aiInsightLoading
      ...
      {activeTab === 'inspect' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="border border-gray-100 rounded-2xl p-4 flex flex-col gap-3">
            ...
            {/* Result */}
            <div className="bg-gray-50 rounded-xl p-3 text-xs text-gray-700 leading-relaxed flex-1 min-h-[100px]">
...
Be direct. Talk like a founder to a founder. No bullet padding, no fluff.`;
      ...
      setAiInsight(data.candidates?.[0]?.content?.parts?.[0]?.text ?? 'No insight returned.');
    } catch {
      setAiInsight('Analysis failed. Check NEXT_PUBLIC_GEMINI_API_KEY in your .env.local');
    } finally {
      setAiInsightLoading(false);
    }
  }, [current, previous, period, siteId, siteInfo]);
...
Max 180 words. Direct, founder-to-founder tone. No fluff.`,
      ...
      const data = await res.json();
      const report = data.candidates?.[0]?.content?.parts?.[0]?.text ?? 'Could not analyze the site.';
