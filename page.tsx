...
export default function SiteInspector() {
  ...
  const handleAnalyzeClick = () => {
    if (!user) { setError("Connectez-vous pour analyser un site."); return }
    if (trialExpired) { setError("Votre période d'essai est expirée. Abonnez-vous pour continuer."); return }
    analyzeSite()
  }
    ...
    <div className="min-h-screen bg-white overflow-x-hidden">
          ...
          <div className="p-4 border-t border-[#f0f0f0] flex-shrink-0">
            {trialExpired ? (
              <div className="rounded-xl bg-red-50 border border-red-200 p-3 text-center">
                <p className="text-xs font-semibold text-red-700 mb-1">Essai expiré</p>
                <p className="text-[11px] text-red-500 mb-2">Abonnez-vous pour continuer à analyser des sites.</p>
                <div className="inline-flex items-center gap-1 bg-[#003087] text-white text-[11px] font-semibold px-3 py-1.5 rounded-lg">
                  <span>PayPal</span>
                  <span className="opacity-80">·</span>
                  <span>1.99$ / mois</span>
                </div>
                <p className="text-[10px] text-red-400 mt-1.5">Page d'abonnement bientôt disponible</p>
              </div>
            ) : trialDaysLeft !== null ? (
              <div className="rounded-xl bg-amber-50 border border-amber-200 p-3">
                <div className="flex items-center justify-between mb-1.5">
                  <p className="text-xs font-semibold text-amber-700">Période d'essai</p>
                ...
                <p className="text-[10px] text-amber-600">
                  Après l'essai : <span className="font-semibold">1.99$ / mois</span> via PayPal
                </p>
              </div>
            ) : null}
          </div>
      ...
      <div className="p-4 sm:p-8">
        ...
        <div className={`max-w-4xl mx-auto ${result ? "pb-24" : "pb-36"}`}>
            ...
            <div className="h-[48px] w-full ring-2 ring-[#eee] rounded-[14px] flex items-center p-1">
              <div className="h-full w-full bg-white ring-2 ring-[#FAFAFA] rounded-[12px] flex items-center px-2 gap-1">
                ...
                <button
                  ...
                  title={!user ? "Connectez-vous pour analyser" : trialExpired ? "Essai expiré — abonnez-vous" : ""}
          ...
          {/* Auth nudge — visible uniquement si non connecté */}
          {!authLoading && !user && (
            <p className="text-center text-xs text-gray-400 mt-2 mb-4">
              <button onClick={() => setSidebarOpen(true)} className="underline hover:text-black transition-colors">
                Connectez-vous
              </button>{" "}pour analyser un site.
            </p>
          )}
          {!authLoading && user && trialExpired && (
            <p className="text-center text-xs text-red-500 mt-2 mb-4">
              Votre essai est expiré — abonnez-vous pour continuer (1.99$/mois via PayPal).
            </p>
          )}
          ...
          {/* ── 2ème iframe : preview du HTML statique extrait ── */}
          {showExtracted && extractedHTML && (
            <div className="mt-6">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                    ...
                    {extractedSize}KB{" "}
                    {extractedSize < 200 ? "✓ Claude-ready" : extractedSize < 500 ? "⚠ large" : "✗ trop lourd"}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    onClick={copyExtracted}
                    className="flex items-center gap-1.5 h-[30px] px-3 rounded-[8px] border border-[#eee] bg-white text-xs font-medium text-black hover:bg-gray-50"
                  >
                    <Copy size={12} /> Copy
                  </button>
                  <button
                    onClick={downloadExtracted}
                    className="flex items-center gap-1.5 h-[30px] px-3 rounded-[8px] bg-black text-white text-xs font-medium hover:bg-gray-800"
                  >
                    <Download size={12} /> Download
                  </button>
                  <button
                    onClick={() => setShowExtracted(false)}
                    className="flex items-center h-[30px] w-[30px] justify-center rounded-[8px] border border-[#eee] text-gray-400 hover:text-black hover:bg-gray-50"
                  >
                    ✕
                  </button>
                </div>
              </div>
              {/* Wrapper avec même ratio que le 1er iframe */}
              <div
              ...
              <p className="text-xs text-gray-400 mt-2 text-center">
                Compare avec la preview originale ci-dessus — ce que Claude / Lovable / v0 recevra
              </p>
            </div>
          )}
