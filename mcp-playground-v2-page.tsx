    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      {blocks.map((block, i) => {
        if (block.type === "image" && block.data) {
          return (
            <div key={i} style={{ borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.08)" }}>
              <img
                src={`data:${block.mimeType ?? "image/svg+xml"};base64,${block.data}`}
                alt="MCP visual result"
...
export default function MCPPlayground() {
  ...
  const [lastResult, setLastResult] = useState<ContentBlock[]>([]);
  ...
  const handleCall = useCallback(async () => {
    ...
    try {
      ...
      // Store rich blocks for visual rendering
      setLastResult(content);

      // Log text blocks, skip image blobs from log
      for (const c of content) {
        if (c.type === "text")  addLog("received", c.text ?? "");
        if (c.type === "image") addLog("received", `[image: ${c.mimeType ?? "image"} — rendered below ↓]`);
      }

      // If no content array, fallback
      if (!content.length) addLog("received", JSON.stringify(result, null, 2));

      addLog("success", `✓ ${selected.name} returned`);

      // Switch to result view if image returned
      if (content.some(c => c.type === "image")) setMobileView("call");
                ...
                {lastResult.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    <div style={{
                      ...
                      <span style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", color: "rgba(240,239,244,0.25)", textTransform: "uppercase" }}>
                        Result
                      </span>
                      <div style={{ height: 1, flex: 1, background: "rgba(255,255,255,0.06)" }}/>
                    </div>
                    <ContentRenderer blocks={lastResult} />
                  </div>
                )}
              </div>
            </>
