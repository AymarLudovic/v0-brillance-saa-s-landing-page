...
// FlagImg — utilise flagcdn.com (fiable, gratuit, CDN mondial)
...
// Garder flag() pour la compatibilité des BRow prefix
...
function BRow({ name, count, total, prefix, code }: {
  name: string; count: number; total: number; prefix?: string; code?: string;
}) {
  ...
  return (
    <div className="flex items-center gap-2 py-1.5">
      {code ? (
        ...
        <span className="text-sm shrink-0 w-5 text-center">{prefix}</span>
      ) : null}
...
export default function Dashboard({ params }: { params: { siteId: string } }) {
  ...
  const countries = useMemo(() => topN(current.map((p) => p.country || 'Unknown'), 5)
    .map(({ name, count }) => ({
      name, count,
      prefix: flag(codeFor(name)),       // emoji fallback
      code: codeFor(name),               // pour FlagImg CDN
    })), [current]);
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900">
      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        {/* ── 3 stat cards ── */}
        <div className="grid grid-cols-3 border-b border-gray-100 divide-x divide-gray-100 py-4 overflow-x-auto">
          {[
        ...
        {/* ── Pages + Referrers ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 border-b border-gray-100 sm:divide-x divide-gray-100 py-2">
          {/* Pages */}
          <div className="pr-4">
        ...
        {/* ── 4-col breakdown ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-0 sm:divide-x divide-gray-100 border-b border-gray-100 py-4">
          {/* Countries */}
