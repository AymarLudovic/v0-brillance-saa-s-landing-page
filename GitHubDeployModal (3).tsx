...
export default function GitHubDeployModal({ currentProject, isOpen, onClose }: GitHubDeployModalProps) {
  ...
  const handlePushToGitHub = async () => {
    ...
    setLogs([]);
    setRepoUrl(null);
    // Variables locales mutables — évitent la closure stale du React state
    ...
    try {
      ...
      streamLoop: while (true) {
        ...
        for (let i = 0; i < lines.length - 1; i++) {
          ...
          try {
            ...
            // ── Fin erreur ──────────────────────────────────────────────────
            if (data.type === 'error') {
              addLog(`❌ ${data.message || 'Erreur inconnue'}`, 'error');
              streamDone = true;
              break streamLoop;
            }
            ...
            if (data.message) {
              ...
              // Ignorer les messages d'authentification intermédiaires (non-bloquants)
              if (data.message.toLowerCase().includes('authenticating')) {
                addLog(data.message, 'info');
                continue; // on continue à lire — ce n'est pas une fin
              }
      ...
      finishPush(localSuccess, localUrl);
