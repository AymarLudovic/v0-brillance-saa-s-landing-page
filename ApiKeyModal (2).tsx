...
export default function ApiKeyModal({ provider = 'gemini', onKeySaved }: ApiKeyModalProps) {
  // Pas d'état isOpen — c'est page.tsx qui monte/démonte ce composant via pendingApiKeyProvider
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      ...
      <div className="relative w-[380px] h-[450px] bg-[#0a0a0a] rounded-3xl border border-white/10 overflow-hidden shadow-2xl flex flex-col">
        {/* ✕ ferme en appelant onKeySaved → page.tsx set pendingApiKeyProvider(null) → démonte */}
        <button onClick={() => onKeySaved?.()} className="absolute top-3 right-3 p-1 rounded-full bg-black/20 hover:bg-white/10 text-gray-400 hover:text-white transition-colors z-10"><X size={16} /></button>
