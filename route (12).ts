...
const REASONING_CACHE_FILE = "app/__reasoning_cache__.md"; // conservé pour rétro-compatibilité (fichiers existants ignorés)
...
▸ CSS 3D / mix-blend-mode (NO library needed — native browser):
  Use for: overlapping text over images (mix-blend-mode), 3D card tilts (perspective + rotateY/rotateX), text clipping (background-clip: text), layered visual compositions
...
2. ICON SIZE INFLATION: Icons 14-16px max. NOT 20-24px.
3. ROW HEIGHT INFLATION: Count rows / divide height. 12 rows in 400px = ~33px/row. Do NOT default to 44-48px.
4. BORDER-RADIUS CREEP: Professional UIs often have 0-4px radius on inputs/cells. Only round things that LOOK visually round.
5. PADDING INFLATION: If text is close to its container edge → padding is 4-8px. Do not inflate to 12-16px.
...
9. MISSING BLEND EFFECTS: If text overlaps images with color mixing → use mix-blend-mode. Do not skip.
10. FLAT WHEN 3D: If elements appear tilted/rotated in 3D space → use perspective + rotateX/rotateY, optionally GSAP.
...
▸ STEP 2 — MEASURE LAYOUT
  ...
  - Sidebar width if present; Header height
...
▸ STEP 6 — GENERATE HTML
  ...
  8. FEATURE HOOKS (CRITICAL): Add explicit id and class attributes to all elements requiring JS interactivity
...
<body><!-- Pixel-perfect reproduction — every measurement applied --></body>
</html>
</design_reference>

⛔ AFTER </design_reference>: Write NOTHING. No TSX files. Stay focused on HTML only.
`;
...
async function runReasoningPipeline(opts: {
  ...
  const { userMessage, systemContext, projectMemory, isAnthropic, anthropic, ai, modelId, emit } = opts;

  let answerA = "", answerB = "", answerC = "";

  // ── A+B : toujours calculés frais, streamés directement au client ─────────────
  ...
  // ── C : dépend du message utilisateur courant ─────────────────────────────────
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          // ═════════════════════════════════════════════════════════════════
          // PHASE 0.75 — REASONING (pré-calculé côté client)
          //
          // Le client exécute les 3 questions A+B+C AVANT cet appel et nous
          // transmet le résultat dans `precomputedReasoning`. On l'injecte
          // directement en tête de l'historique — aucun appel IA ici.
          // ═════════════════════════════════════════════════════════════════
          emit("\n[REASONING:START]\n");
          ...
          if (precomputedReasoning?.userTurn && precomputedReasoning?.assistantTurn) {
            reasoningTurns = precomputedReasoning;
            emit("[REASONING:DONE] ✅ Conscience activée (pré-calculée côté client)\n");
          } else {
            // Fallback : si le client ne fournit pas le reasoning, on l'ignore
            // et on stream directement — le modèle reste capable sans ce contexte.
            emit("[REASONING:SKIP] Pas de reasoning pré-calculé — stream direct.\n");
